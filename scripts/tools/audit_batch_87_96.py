...
def main():
    ...
    print(f"Auditing {len(plans)} plans:\n")
    ...
    for path in sorted(plans):
        ...
        status = "PASS" if (char_count >= 250000 and has_polish and test_count == 100 and has_authority and has_sim and has_checklist) else "FAIL"
        if status == "FAIL":
            all_ok = False
    ...
    for r in results:
        ...
        q = r["checklist"]
        st = r["status"]
        print(f"{fname:<40} | Chars: {c:>10} | Polish: {p} | Tests: {t:>3} | Auth: {a} | Sim: {s} | QA: {q} => {st}")
