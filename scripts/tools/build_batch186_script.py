#!/usr/bin/env python3
"""
Builder for Batch 186 expansion script.
Reads scripts/tools/batch186_candidates.json and synthesizes expand_oldest_485_plans_batch186.py.
Adds Section XX precision expansion adding +19k to 26k characters per plan over Batch 185.
"""
import json, os, re

BASE = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
CANDIDATES_FILE = os.path.join(BASE, "scripts/tools/batch186_candidates.json")
OUTPUT_SCRIPT = os.path.join(BASE, "scripts/tools/expand_oldest_485_plans_batch186.py")
...
for i, c in enumerate(cands):
    ...
    if not short_slug:
        short_slug = "PLAN"
    pid = f"PLAN-B186-{i+1:03d}-{short_slug[:12]}"
    plans.append({
        "id": pid,
        "path": p,
        "domain": domain,
...
# Read template from batch 185
with open(os.path.join(BASE, "scripts/tools/expand_oldest_485_plans_batch185.py"), "r", encoding="utf-8") as f:
    batch185_text = f.read()

# Locate where AUTHORITY_SNIPPET begins
auth_pos = batch185_text.find("AUTHORITY_SNIPPET =")
if auth_pos == -1:
    raise ValueError("Could not find AUTHORITY_SNIPPET in batch 185 script")

tail_text = batch185_text[auth_pos:]
tail_text = tail_text.replace("BATCH-185", "BATCH-186")
tail_text = tail_text.replace("Batch 185", "Batch 186")
tail_text = tail_text.replace("ALL 485 BATCH-185", "ALL 485 BATCH-186")

# Section XX implementation to deliver +19k to 26k boost over Batch 185
section_xx = '''
...
# Inject Section XX right before `return "".join(s)` in core_expansion
...
tail_text = tail_text.replace(injection_marker, section_xx + "\n" + injection_marker)
