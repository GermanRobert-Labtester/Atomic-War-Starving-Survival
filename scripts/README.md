...
| [`generate-architecture-map.sh`](ci/generate-architecture-map.sh)<br>[`generate-architecture-map.py`](ci/generate-architecture-map.py) | 🟢 `ACTIVE` | CI Gate (`architecture_map_drift`) | Regenerates [`ARCHITECTURE_TEST_MAP.md`](../docs/architecture/ARCHITECTURE_TEST_MAP.md). |
| [`generate-plan-integration-audit.py`](ci/generate-plan-integration-audit.py) | 🟢 `ACTIVE` | CI Gate (`plan_integration_audit_drift`) | Regenerates [`RECENT_PLAN_INTEGRATIONS_AUDIT.md`](../docs/plans/RECENT_PLAN_INTEGRATIONS_AUDIT.md) — programmatic per-plan integration verification. |
| [`generate-ui-design-map.py`](ci/generate-ui-design-map.py) | 🟢 `ACTIVE` | CI Gate (`ui_design_map_drift`) | Regenerates [`UI_DESIGN_MAP.md`](../docs/ui/UI_DESIGN_MAP.md) — programmatic UI design system map (tokens, IA, invariants). |
