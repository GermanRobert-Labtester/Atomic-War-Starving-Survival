...
namespace AtomicWar.GodotApp
{
    ...
    public static string TryCaptureDirect(WaterTreatmentState state)
    {
        return TryCapture(state);
    }

    /// <summary>Direct aggregate restore: deserialize state from envelope JSON.</summary>
    public static WaterTreatmentState? TryRestoreDirect(string json)
    {
        return TryRestore(json);
    }

    /// <summary>Capture state to JSON without writing to disk.</summary>
    public static string TryCapture(WaterTreatmentState state)
    {
        try
        {
        ...
        {
            GD.PrintErr("[WaterTreatmentSaveStore] capture failed: " + e.Message);
            return string.Empty;
        }
    }

    /// <summary>Restore state from JSON without reading from disk.</summary>
    public static WaterTreatmentState? TryRestore(string json)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(json)) return null;
            return s_json.Deserialize<WaterTreatmentState>(json);
        }
        catch (Exception e)
        {
            GD.PrintErr("[WaterTreatmentSaveStore] restore failed: " + e.Message);
            return null;
        }
    }
