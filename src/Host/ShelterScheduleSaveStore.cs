...
namespace AtomicWar.GodotApp
{
    ...
    public static string TryCaptureDirect(ShelterScheduleState state)
    {
        return TryCapture(state);
    }

    /// <summary>Direct aggregate restore: deserialize state from envelope JSON.</summary>
    public static ShelterScheduleState? TryRestoreDirect(string json)
    {
        return TryRestore(json);
    }

    /// <summary>Capture state to JSON without writing to disk.</summary>
    public static string TryCapture(ShelterScheduleState state)
    {
        try
        {
        ...
        {
            GD.PrintErr("[ShelterScheduleSaveStore] capture failed: " + e.Message);
            return string.Empty;
        }
    }

    /// <summary>Restore state from JSON without reading from disk.</summary>
    public static ShelterScheduleState? TryRestore(string json)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(json)) return null;
            return s_json.Deserialize<ShelterScheduleState>(json);
        }
        catch (Exception e)
        {
            GD.PrintErr("[ShelterScheduleSaveStore] restore failed: " + e.Message);
            return null;
        }
    }
