...
namespace AtomicWar.GodotApp.UI
{
    public partial class FungiCultivationBedPanel : Control, IBindablePanel
    {
        ...
        public void Bind(FungiCultivationSystem system) { _system = system; RefreshView(); }
        public void Unbind() { _system = null; }

        public override void _Ready()
        {
            SetAnchorsPreset(LayoutPreset.FullRect);
            _shell = new AshfallDashboardShell("BIO-HYDROPONICS // FUNGI CULTIVATION", minWidth: 1000, minHeight: 650);
            AddChild(_shell);
            _shell.SetContent(new Label { Text = "Mushroom beds monitoring..." });
