...
namespace AtomicWar.GodotApp.UI
{
    public partial class SurvivorDowntimePanel : Control, IBindablePanel
    {
        ...
        public void Bind(SurvivorDowntimeSystem system) { _system = system; RefreshView(); }
        public void Unbind() { _system = null; }

        public override void _Ready()
        {
            SetAnchorsPreset(LayoutPreset.FullRect);
            _shell = new AshfallDashboardShell("SHELTER REC // HOBBIES & DOWNTIME", minWidth: 1000, minHeight: 650);
            AddChild(_shell);
            _shell.SetContent(new Label { Text = "Recreation center." });
