...
namespace AtomicWar.GodotApp.UI
{
    public partial class WinterFreezePanel : Control, IBindablePanel
    {
        ...
        public void Bind(YearOfAshDeepFreezeSystem system) { _system = system; RefreshView(); }
        public void Unbind() { _system = null; }

        public override void _Ready()
        {
            SetAnchorsPreset(LayoutPreset.FullRect);
            _shell = new AshfallDashboardShell("DEEP FREEZE // CRYOGENIC WINTER", minWidth: 1000, minHeight: 650);
            AddChild(_shell);
            _shell.SetContent(new Label { Text = "Winter tracking offline." });
