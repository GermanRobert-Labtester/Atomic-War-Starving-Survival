...
namespace AtomicWar.GodotApp.UI
{
    public partial class MechanicalProstheticsLathePanel : Control, IBindablePanel
    {
        ...
        private void BuildInterface()
        {
            ...
            {
                Text = "SURGICAL MACHINING // PROSTHETICS & CYBERNETIC LATHE [PROSTH-01]",
            ...
            {
                Text = "STATUS: MILLING ACTIVE - TITANIUM-LEAD PNEUMATIC ARM / VANE"
            };
            ...
            var leftPanel = CreatePanelFrame("NEURAL IMPEDANCE & AMPUTATION ROSTER");
            ...
            _telemetryContainer.AddChild(CreateTelemetryRow("PATIENT IMPEDANCE", "4.8 kOhm [NEURAL MATCH]", AshfallUiHelpers.ToColor(DesignTheme.Warm)));
            _telemetryContainer.AddChild(CreateTelemetryRow("PHANTOM PAIN INDEX", "38% [TOLERABLE THRESHOLD]", AshfallUiHelpers.ToColor(DesignTheme.Dim)));
            _telemetryContainer.AddChild(CreateTelemetryRow("BIOCOMPATIBILITY RISK", "14.5% [TITANIUM-BONE COUPLING]", AshfallUiHelpers.ToColor(DesignTheme.Dim)));
            _telemetryContainer.AddChild(CreateTelemetryRow("CURRENT PATIENT", "SCOUT VANE (RIGHT ARM AMP)", AshfallUiHelpers.ToColor(DesignTheme.Warm)));
            _telemetryContainer.AddChild(CreateTelemetryRow("NEXT SURGERY QUEUE", "FOUNDRY CARVER (LEG CRUSH)", AshfallUiHelpers.ToColor(DesignTheme.Dim)));

            // Center Column (Interactive Controls)
            var centerPanel = CreatePanelFrame("PRECISION CNC LATHE & ARTICULATION");
            ...
            var rightPanel = CreatePanelFrame("ALLOY INVENTORY & ASSEMBLED LIMBS");
            ...
            _dataContainer.AddChild(CreateTelemetryRow("MACHINED TITANIUM RODS", "8 UNITS IN TOOLING RACK", AshfallUiHelpers.ToColor(DesignTheme.Dim)));
            _dataContainer.AddChild(CreateTelemetryRow("SURGICAL STEEL SCREWS", "45 UNITS PURIFIED", AshfallUiHelpers.ToColor(DesignTheme.Dim)));
            _dataContainer.AddChild(CreateTelemetryRow("HEAVY MINING ARM", "1 ASSEMBLED AND CALIBRATED", AshfallUiHelpers.ToColor(DesignTheme.Warm)));
            _dataContainer.AddChild(CreateTelemetryRow("SPRING-PISTON RUNNER LEG", "1 ASSEMBLED AND CALIBRATED", AshfallUiHelpers.ToColor(DesignTheme.Warm)));
