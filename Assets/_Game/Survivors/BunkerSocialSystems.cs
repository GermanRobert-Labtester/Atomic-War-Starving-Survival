...
namespace AtomicWar._Game.Survivors
{
    ...
    public class RomanceSystem
    {
        ...
        public void ApplyAuras(float gameHours, IReadOnlyList<Survivor> survivors)
        {
            ...
            {
                lookup.TryGetValue(key.A, out var a);
                lookup.TryGetValue(key.B, out var b);
                if (a == null || b == null || !a.IsAlive || !b.IsAlive) continue;
        ...
        private static Dictionary<string, Survivor> BuildLookup(IReadOnlyList<Survivor> survivors)
        {
            if (survivors == null) return new Dictionary<string, Survivor>(0, StringComparer.Ordinal);
            var dict = new Dictionary<string, Survivor>(survivors.Count, StringComparer.Ordinal);
            for (int i = 0; i < survivors.Count; i++)
            {
                var s = survivors[i];
    ...
    public class FeudSystem
    {
        ...
        {
            if (gameHours <= 0f || survivors == null || survivors.Count == 0 || rng == null || _feuds.Count == 0) return 0;
            var lookup = BuildLookup(survivors);
            int landed = 0;
            foreach (var feh in _feuds)
            {
                lookup.TryGetValue(feh.A, out var a);
                lookup.TryGetValue(feh.B, out var b);
                if (a == null || b == null || !a.IsAlive || !b.IsAlive) continue;
    ...
    public class PregnancySystem
    {
        ...
        {
            if (survivors == null || survivors.Count == 0 || _active.Count == 0) return;
            var lookup = BuildLookup(survivors);
            var toRemove = new List<string>();
            foreach (var kv in _active)
            {
                var rec = kv.Value;
                lookup.TryGetValue(rec.PatientId, out var patient);
    ...
    public class BlackMarketSystem
    {
        ...
        {
            if (survivors == null || survivors.Count == 0 || rng == null || _alliances.Count == 0) return 0;
            var lookup = BuildLookup(survivors);
            int smuggled = 0;
            var drop = new List<SocialPairKey>();
            foreach (var kv in _alliances)
            {
                lookup.TryGetValue(kv.Value.A, out var a);
                lookup.TryGetValue(kv.Value.B, out var b);
