...
{
    /// <summary>
    /// Loads dynamic_questlines.json (standalone, non-arc quests) into
    /// DynamicQuestlineSystem. Branch rewards/costs are authored for a
    /// future branch-resolution feature and are not consumed here — only
    /// stage descriptions and objective items, which StartQuest accepts.
    /// </summary>
    public static class DynamicQuestlineCatalogLoader
    {
        ...
        public static void LoadInto(DynamicQuestlineSystem system)
        {
            ...
            string path = Path.Combine(Application.streamingAssetsPath, "Data/dynamic_questlines.json");
            ...
            for (int i = 0; i < container.entries.Length; i++)
            {
                ...
                if (entry.stages.Length != FixedStageCount)
                    Debug.LogWarning($"[DynamicQuestlineCatalogLoader] '{entry.quest_id}' has " +
