...
namespace AtomicWar._Game.Core
{
    ...
    /// Stage text comes from holdfast_quests.json (creative pack). Flags match Exp 2/3.
    /// </summary>
    [Serializable]
    ...
    public class HoldfastQuestSystem
    {
        ...
        {
            if (day >= SheetMinDay && !IsStarted(Sheet))
                TryStart(Sheet, day);
            if ((IsStarted(Sheet) || day >= ClerkFallbackDay) && !IsStarted(Clerk))
                TryStart(Clerk, day);
            ...
            if (IsCompleted(Drawer) && !IsStarted(Levy))
                TryStart(Levy, day);
            if ((IsCompleted(Levy) || IsStarted(Levy)) && !IsStarted(Membrane) && day >= 0)
            {
                // Membrane is crisis-gated by BrineWaterSystem; host starts it on steam trip.
            }
        ...
        private bool PrereqsMet(string questId, int day)
        {
            switch (questId)
            {
                ...
                case Hatch: return IsStarted(SecondList) || _state.endingId != null;
                default: return true;
            }
        }
