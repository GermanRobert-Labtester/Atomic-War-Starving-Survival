using System;
using System.Collections.Generic;
using UnityEngine;

namespace AtomicWar._Game.Factions
{
    [Serializable]
    public class Fauna_RatSwarmState
    {
        public string id = "fauna_rat_swarm";
        public string displayName = "RatSwarm";
        public bool isPresent = false;
        public int count = 0;
    }

    /// <summary>
    /// "The Ash Gets Deeper" content batch (Prompts #326–#330) — Fauna
    /// archetype `Fauna_RatSwarm`. Behaviour is host-injected; the data
    /// (presence + count) is persisted here.
    /// </summary>
    public class Fauna_RatSwarm
    {
        private Fauna_RatSwarmState _state = new Fauna_RatSwarmState();

        public event Action<Fauna_RatSwarmState> OnSighted;
        public event Action<Fauna_RatSwarmState> OnCountChanged;

        public Fauna_RatSwarmState State => _state;

        public void Sighted(int count) { _state.isPresent = true; _state.count = count; OnSighted?.Invoke(_state); }
        public void SetCount(int count) { _state.count = count; OnCountChanged?.Invoke(_state); }
        public void Clear() { _state.isPresent = false; _state.count = 0; }

        public Fauna_RatSwarmState CaptureState() => _state;
        public void RestoreState(Fauna_RatSwarmState saved) { _state = saved ?? new Fauna_RatSwarmState(); }
    }
}
