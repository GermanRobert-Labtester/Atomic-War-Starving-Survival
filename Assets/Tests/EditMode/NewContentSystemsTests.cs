...
namespace AtomicWar.Tests.EditMode
{
    ...
    public class NoiseDisciplineSystemTests
    {
        ...
        [Test]
        public void RegisterSourceUpdatesExistingOrAddsNew()
        {
            var sys = new NoiseDisciplineSystem();
            sys.RegisterSource("generator", 15f);
            Assert.AreEqual(15f, sys.CurrentLevel);

            // Update existing source
            sys.RegisterSource("generator", 25f);
            Assert.AreEqual(25f, sys.CurrentLevel);
            Assert.AreEqual(1, sys.Current.Sources.Count);

            // Add second source
            sys.RegisterSource("radio", 10f);
            Assert.AreEqual(35f, sys.CurrentLevel);
            Assert.AreEqual(2, sys.Current.Sources.Count);
        }

        [Test]
        public void RestoreStateRebuildsDictionaryLookup()
        {
            var sys = new NoiseDisciplineSystem();
            sys.RegisterSource("generator", 15f);
            sys.RegisterSource("hammering", 20f);
            ...
            var state = sys.CaptureState();
            var restoredSys = new NoiseDisciplineSystem();
            restoredSys.RestoreState(state);

            Assert.AreEqual(2, restoredSys.Current.Sources.Count);
            Assert.AreEqual(35f, restoredSys.CurrentLevel);

            // Verify lookup works after restore
            restoredSys.SetSourceActive("generator", false);
            Assert.AreEqual(20f, restoredSys.CurrentLevel);
        }
    }
