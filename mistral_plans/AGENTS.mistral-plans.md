...
## MISTRAL PLAN PERSISTENCE — NON-NEGOTIABLE

This repository has one canonical location for AI-generated implementation plans, regardless of which Mistral-powered agent/client is being used.

**Canonical plan root:** `./.mistral/plans/`

Treat that path as the value of `MISTRAL_PLAN_ROOT`. If a task creates, revises, expands, or materially updates a development plan, the plan **must be persisted there before the agent claims the planning task is complete**.

### Applies to every client

This rule is client-agnostic and applies when working through:

- Mistral Vibe / Vibe CLI;
- goose using a Mistral model/provider;
- Pi / Pi Agent using a Mistral model/provider;
- Mistral subagents or delegated planning agents;
- any future CLI, IDE agent, ACP client, MCP host, or wrapper that operates on this repository.

The client does **not** get to choose another plan directory.

### Mandatory plan-save contract

1. **Every actionable project plan is a file.** Do not leave the only copy of a plan in chat/session history.
2. **Save only under `./.mistral/plans/`.** Do not create competing plan folders under `docs/`, `.vibe/`, `.goose/`, `.pi/`, temporary directories, home directories, or the repository root.
3. **Persist before finalizing.** When a planning task is complete, write/update the plan file first, then report its exact repository-relative path.
4. **Create the directory if absent.** `mkdir -p .mistral/plans` is allowed for this documentation-only purpose unless the user explicitly requested a completely read-only task.
5. **Never silently fall back.** If the agent cannot write to `MISTRAL_PLAN_ROOT`, it must report the write restriction. It must not save the plan somewhere else.
6. **Read existing plans first.** Before creating a new plan, inspect `.mistral/plans/` for a plan covering the same milestone/system. Update the authoritative existing plan when appropriate instead of spawning duplicates.
7. **Preserve history.** Do not delete older plans merely because they are superseded. Mark them `SUPERSEDED` and link to the replacement unless the user explicitly requests deletion.
8. **No implementation inside plan mode unless requested.** A plan describes intended work. Creating/updating the Markdown plan file is the only expected project mutation during a plan-only task.
9. **Subagents obey the same root.** A parent agent delegating planning must tell the subagent to read/write under `.mistral/plans/`; subagent scratch plans are not authoritative until persisted there.
10. **Plan persistence survives client switching.** When continuing work in another agent/client, resume from the files in `.mistral/plans/`, not from assumptions about another client's chat/session memory.

### What counts as a plan

Persist a plan when the user requests or the agent produces a substantive:
...
- production roadmap or milestone plan;
- multi-step QA/test plan tied to code changes;
- phased execution strategy intended to be continued later.

Tiny one-off action lists that are immediately executed in the same turn do not need their own plan file unless the user explicitly asks for a saved plan.

### File naming

Use deterministic, human-readable Markdown filenames:

`YYYY-MM-DD_<scope>_<purpose>.md`

Examples:

- `2026-08-18_holdfast_2d_viewport_integration.md`
- `2026-08-18_main_cs_modularization.md`
- `2026-08-18_alpha_0_8_execution_plan.md`

Rules:

- lowercase `snake_case` after the date;
- `.md` only;
- no spaces;
- never overwrite an unrelated plan;
- if two genuinely separate plans would have the same name, append `_02`, `_03`, etc.

### Required plan header

Every new plan begins with:

```markdown
# <Plan Title>

- **Status:** DRAFT | READY | IN_PROGRESS | BLOCKED | COMPLETE | SUPERSEDED
- **Created:** YYYY-MM-DD
- **Updated:** YYYY-MM-DD
- **Plan root:** `.mistral/plans/`
- **Scope:** <systems/files/feature>
- **Execution client:** <mistral-vibe | goose | pi | other>
```

Do not encode API keys, secrets, access tokens, private MCP credentials, or machine-specific absolute paths in plan files.

### Plan structure

Prefer this compact execution structure:

1. **Goal** — exact outcome.
2. **Current evidence** — relevant existing code/data/tests; verify before planning replacements.
3. **Constraints / invariants** — especially ASHFALL Core/Data/Godot rules.
4. **Execution phases** — ordered dependencies and concrete tasks.
5. **Files/systems affected** — expected touch surface.
6. **Verification** — exact tests/gates required.
7. **Risks / rollback** — material failure modes only.
8. **Completion criteria** — objective definition of done.
9. **Next action** — first executable step.

### Plan update behavior

When executing an existing plan:

- read the plan before modifying related systems;
- keep its `Updated` date and `Status` current;
- mark completed steps in the plan when useful for handoff continuity;
- record meaningful scope changes or blockers;
- do not rewrite completed history to make execution look cleaner than it was;
- when finished, set `Status: COMPLETE` only after required verification passes.

### Cross-client loading

`AGENTS.md` is the canonical project instruction source for this rule.

- **Mistral Vibe:** project `AGENTS.md` is loaded from the working-directory hierarchy. Do not create a Vibe-only alternate plan location.
- **Pi Agent:** project `AGENTS.md` is loaded as project context. Do not create a Pi-only alternate plan location.
- **goose:** ensure project context includes `AGENTS.md` (for example through `CONTEXT_FILE_NAMES`) or keep the companion `.goosehints` bridge. The plan location remains `.mistral/plans/` either way.

Recommended goose context setting:

```bash
export CONTEXT_FILE_NAMES='["AGENTS.md", ".goosehints"]'
```

If an agent discovers a client-specific default such as `.vibe/plans/`, `.goose/plans/`, or `.pi/plans/`, **ignore that default for ASHFALL** unless the user explicitly changes this policy. The repository-level `MISTRAL_PLAN_ROOT` wins.

### Plan-mode write restriction

Some clients expose a read-only planning profile. If the selected profile/tool policy blocks all writes, the agent cannot truthfully satisfy the persistence contract from that mode alone. In that case:

1. finish the plan content;
2. do not save it to a fallback location;
3. switch/exit to a write-capable documentation mode when supported, or report that the write policy prevented persistence;
4. persist the plan to `.mistral/plans/` before claiming it has been saved.
