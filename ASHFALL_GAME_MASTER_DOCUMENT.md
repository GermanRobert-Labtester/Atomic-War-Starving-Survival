...
# Section XIV — The Ash Gets Deeper (Prompts #326–#330)

## Tone
Cold. Exhausted. Human. Restrained. No heroes. No magic. No mutants with glowing eyes. Just people trying not to die in a world that stopped caring whether they do.

## XIV.1 — Items (80 new)
Authored as `Assets/_Game/Inventory/AshGetsDeeperItemsCatalog.cs` (518 LoC, pure-data builder). Five categories: Medical & Pathology (14), Survival & Shelter (12), Weapons & Combat (8, grounded no fantasy), Comfort / Morale / Narrative (10), Scavenged Tech & Components (7), Food & Consumables (8), Clothing & Protective Gear (6). Materialise through `MaterialiseAll(lookup)` and merge into the existing `ItemCatalogSO.items` at boot.

All ids follow the brief exactly and are stable snake_case. Full id registry in `AshGetsDeeperItemsCatalog.Ids`. Examples: `morphine_ampoule`, `surgical_thread`, `tourniquet`, `bone_splint_kit`, `prussian_blue_capsules`, `ash_tarp`, `lead_sheet`, `candle_tallow`, `pipe_shotgun_single`, `crossbow_salvaged`, `bolt_steel_pack_10`, `nail_bat`, `improvised_molotov`, `hatchet_rusted`, `bear_trap_salvaged`, `harmonica_dented`, `child_shoe_pair`, `seed_envelope_tomato`, `chess_set_carved`, `pocket_watch_stopped`, `cigarette_pack_sealed`, `vial_of_soil`, `geiger_tube_salvaged`, `capacitor_bank_small`, `copper_tubing_1m`, `welding_rod_pack_20`, `circuit_breaker_30a`, `bearing_set_industrial`, `rat_meat_skewer`, `insect_paste_brick`, `ash_bread_flat`, `canned_milk_condensed`, `vinegar_bottle`, `dried_mushroom_string`, `salt_pork_strip`, `wild_garlic_bulb`, `patched_raincoat`, `improvised_lead_vest`, `welding_leathers`, `thermal_underlayer`, `improvised_gaiters`.

## XIV.2 — Locations (10 new)
Authored as `Assets/_Game/Data/AshGetsDeeperLocationsCatalog.cs` (121 LoC). Maps directly to `LocationDefinitionSO` (danger / travel / baseRads). New ids: `collapsed_school`, `irradiated_greenhouse`, `flooded_subway_station`, `pharmacy_chain_ruin`, `military_checkpoint_wreck`, `water_treatment_plant`, `highway_pileup`, `church_of_st_maren`, `radio_tower_base`, `pet_cemetery`. The 6th entry (`highway_pileup`) was already referenced by `quest_mechanic_highway_heart` (turn 1); this is the data side finally landing.

## XIV.3 — NPC Archetypes & Mutated Fauna (12 new)
10 small per-class files in `Assets/_Game/Factions/` (NPC + Fauna), each with a serializable State class, `CaptureState/RestoreState`, and an `OnEncounterStarted/Resolved` event pair. Id registry in `AshGetsDeeperNpcIds.cs`. Hostile/ambiguous: `npc_ash_widows`, `npc_the_tollman`, `npc_burned_patrol`, `npc_the_collector`, `npc_feral_children`, `npc_the_surgeon_caravan`. Fauna: `fauna_irradiated_dogs`, `fauna_crows_ash`, `fauna_bloated_cattle`, `fauna_rat_swarm`. (Note: `npc_burned_patrol` was already a const in `Quest_GarrisonLastOrder` (turn 1); this is the data side landing.)

## XIV.4 — Encounters (15 new)
Authored as `Assets/_Game/Events/AshGetsDeeperEncountersCatalog.cs` (191 LoC, maps to `GameEvent`). Six shelter: `enc_power_surge`, `enc_oxygen_thin`, `enc_pipe_burst`, `enc_condensation_mold`, `enc_silent_death`, `enc_rat_king`. Five surface/expedition: `enc_ash_quicksand`, `enc_dead_radio_operator`, `enc_frozen_family`, `enc_minefield_remnant`, `enc_black_rain_pocket`. Four moral/social: `enc_who_eats`, `enc_the_child_asks`, `enc_exile_vote`, `enc_mercy_request`. The brief's free-form `Trigger` column is stashed in `bodyText`'s tail (`[Trigger: ...]`) so designers can read it at runtime without cross-referencing the catalog file.

## XIV.5 — Economy Tuning (Hardcore Survival Mode)
Authored as `Assets/_Game/Economy/HardcoreEconomyTuning.cs` (146 LoC, pure-data). Four scarcity tiers (Critical / High / Moderate / Low) with day ranges × 2.5 / × 2.0 / × 1.5 / × 0.5 multipliers. Five faction trade preferences (Central Garrison / Upland Militia / Cultists of the Glow / Scavenger Warlords / Safe Haven Rebuilders) with comma-separated `buysAtPremium` / `refuses` / `tradeCurrency` strings. Four dynamic price-shock events (Plume Passing × 1.8 for 3 days, Convoy Ambush × 2.0 for 7 days, Faction War × 3.0 for 14 days, Winter Deepens × 2.2 for 30 days) ready to feed the existing `DynamicEconomySystem` opt-in.

## XIV.6 — Lore Expansion
Appended to the master document. The town of Tessarat is deepened (4,200 people, weekly livestock market, sub-pen built 1962, reactivated Day -14, 14th-century Church of St. Maren, Broadcast Tower 7). The fracture timeline is expanded to 15 days (Day -30 through Day 60) including the Day 21 cult formation and the Day 45 first faction war.

10 new survivor-backstory echoes authored as `Assets/_Game/Events/AshGetsDeeperEchoesCatalog.cs` (98 LoC) and materialisable into `DiaryFragmentSO` for the existing `JournalSystem`: `echo_shopping_list` (the blue-ink receipt), `echo_bus_timetable` (K. ALDRIC waited for everyone), `echo_voicemail_transcript` (Dad on the napkin, folded eight times), `echo_garden_marker` (Suki's tomatoes, still watered), `echo_military_gravestone` (the name scratched out by hand), `echo_bunker_graffiti` (WE WERE HERE. WE WERE SCARED. WE STAYED.), `echo_pharmacy_board` (FLU SHOTS TUESDAY / IODINE RATIONED / GONE DON'T WAIT), `echo_child_toy` (Tomas's wooden train), `echo_ration_card` (M. Vasquez, eleven punches), `echo_last_newspaper` (the market closed for road maintenance).

## XIV.7 — Wire-up status
All files compile. None are wired into `GameBootstrap` yet. The host must:
1. Call `AshGetsDeeperItemsCatalog.MaterialiseAll(_itemCatalog.GetById)` and merge into `_itemCatalog.items` (mirror of `NewRecipesCatalog.MaterialiseAll` from turn 2).
2. Call `AshGetsDeeperLocationsCatalog.MaterialiseAll()` and merge into `_locationCatalog`.
3. Call `AshGetsDeeperEncountersCatalog.MaterialiseAll()` and merge into the event catalog.
4. Call `AshGetsDeeperEchoesCatalog.MaterialiseAll()` and register the resulting `DiaryFragmentSO[]` with `JournalSystem`.
5. Construct + wire each NPC / Fauna class through its own boot method (mirror of `WeatherAcidSnow` from turn 3).
6. Read `HardcoreEconomyTuning.ScarcityMultipliers` / `PriceShockEvents` from `DynamicEconomySystem` at boot.
