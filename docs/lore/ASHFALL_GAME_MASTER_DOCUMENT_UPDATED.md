...
## CHAPTER 9: EXPANSIONS & MASTER CATALOG ADDITIONS (v2.5 EXPANSION RELEASE)

### 9.1 Expansion Batch: "The Ash Gets Deeper"
- **Authored Items Catalog (`AshGetsDeeperItemsCatalog`):** 80 data-driven items spanning survival tools, dosimeter spares, iodine chelation compounds, field rations, gas mask filters, improvised structural struts, and radiation shielding materials.
- **Exploration Locations Catalog (`AshGetsDeeperLocationsCatalog`):** 10 dangerous post-exchange locations with dynamic radiation threat levels and resource yields.
- **Narrative Encounters Catalog (`AshGetsDeeperEncountersCatalog`):** 15 moral dilemma encounters detailing hatch raids, refugee negotiations, contaminated storehouse discoveries, and radio intercept leads.
- **Journal Echoes Catalog (`AshGetsDeeperEchoesCatalog`):** 10 tragic personal logs detailing pre-war civilian final messages.
- **NPC & Fauna Archetypes (`AshGetsDeeperNpcIds`):** 12 custom archetypes including `FaunaAshCrows`, `FaunaBloatedCattle`, `FaunaIrradiatedDogs`, `FaunaRatSwarm`, `NPCAshWidows`, `NPCBurnedPatrol`, `NPCFeralChildren`, `NPCSurgeonsCaravan`, `NPCTheCollector`, `NPCTheTollman`.

### 9.2 Expansion Batch: "Into the Ash" (Parts II & III)
- **Data-Driven Items Catalog (`IntoTheAshItemsCatalog`):** 31 items spanning Quest Items, Lore Artefacts, Improvised Weapons, Specialized Field Tools, and Medical Synthetics.
- **8 Authored Strategic Locations (`IntoTheAshLocationsCatalog`):**
  1. `district_coordination_office`: Bureaucratic record hub, compliance ledger, regional census archives.
  2. `checkpoint_kilo_memorial`: Post-exchange mass grave, soldier journals, Protocol Nine order.
  3. `militia_grain_exchange`: Agricultural stockpile, barter hub, Marta's grain receipt.
  4. `the_glow_chapel`: Irradiated chapel, holy rad water, Brother Orin's journal.
  5. `st_maren_hospital_annex`: Contaminated triage center, medical archives, surgical supplies.
  6. `radio_tower_seven_bunker`: Military broadcast bunker, long-range transmitter, backup generator.
  7. `marta_farmhouse`: Isolated homestead, soil samples, seed stocks.
  8. `toll_house`: River bridge checkpoint, tax ledger, extortion records.
- **6 Multi-Stage Narrative Quest Chains (`IntoTheAshQuestsCatalog`):**
  1. `Quest_DeepWell`: "Water From Below" — 5-stage shelter aquifer drilling project.
  2. `Quest_MechanicHighwayHeart`: "Highway Heart" — 4-stage vehicle engine extraction and restoration.
  3. `Quest_ChildSoldierRifle`: "Child With a Rifle" — 3-stage morale and disarmament narrative.
  4. `Quest_CultGlowCommunion`: "Glow Communion" — 4-stage rad-worship cult confrontation/infiltration.
  5. `Quest_GarrisonLastOrder`: "Last Order of Protocol Nine" — 5-stage central military order investigation.
  6. `Quest_MilitiaGrainWar`: "The Grain War" — 4-stage economic warfare between Militia and Garrison.

### 9.3 Section X New Weather Systems
- **5 Dynamic Weather Event Handlers (`GameBootstrap.Weather.NewContent`):**
  1. `weather_ash_lightning`: Static discharge in ash plumes; blocks surface travel; risks electrical fires.
  2. `weather_fog_of_particulate`: Suspended radioactive particulates; 2m visibility; 5 mSv/h unmasked; 2x filter load.
  3. `weather_thermal_inversion`: Cold surface air trap; acoustic amplification 3x; surface rads 2x.
  4. `weather_ice_storm`: Freezing rain; hatch frozen shut; solar generation disabled; heating fuel +50%.
  5. `weather_silence`: Zero-wind high-rad clear sky — deceptive and lethal.

### 9.4 System Hardening & Reliability Metrics
- **Zero-Allocation Simulation Loops:** Optimized `NeedsSystem`, `LocationScavengingSystem`, `RadiationSystem`, `TemperatureSystem`, and `WeatherSystem` to execute with zero heap allocations in tick passes.
- **SubscriptionBag Event Protection:** Integrated `SubscriptionBag` across all `GameBootstrap` extensions, guaranteeing 100% event unbinding on scene unload or object destruction.
- **Atomic Iron Man Save Security:** Hardened `Mode_IronMan` to delete `.json`, `.bak`, and `.tmp` files atomically upon survivor wipeout.
- **100% EditMode Test Suite Pass Rate:** Verified across **2,145 EditMode test cases** (0 failures).
