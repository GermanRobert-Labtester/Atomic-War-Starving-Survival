...
| `instant_pain_relief_hp`, `trauma_suppression_duration_days` (morphine) | health | Pain-relief flows through item `healthEffect`; `morphine` exists only in `chemical_dependency_items.json` (dependency catalog), **not** as an items.json item. | **DEFERRED.** Requires a canonical morphine item first; effects must then be that item's fields, applied via the use pipeline. |
| `chemical_dependency_risk` (morphine) | dependency | `ChemicalDependencySystem.OnSubstanceConsumed(survivorId, itemId, kind)` with `DependencyIncreasePerDose=0.15` and catalog severity. The **system's** probability contract is authoritative; the JSON `0.35` may not override it. | **LIVE-ROUTE EXISTS but DEFERRED** until the morphine item identity exists. When mapped, risk is owned by the system + dependency catalog row (`morphine`), never by this JSON value. Exactly one consumption event per authorized use. |
