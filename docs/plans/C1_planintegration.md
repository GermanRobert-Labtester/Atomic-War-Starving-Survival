...
- **Panel presentation DONE 2026-09-17 (Wave 8 B1):** the economy presentation wave landed — `economy_detail` now renders the embargo banner (active weather, affected regions/categories, blocked-route notice, neutral state when clear) from `TradeEmbargoSystem.GetEmbargoSummary`, and the regional heat map (5 regions × authored goods, CHEAP/NEUTRAL/EXPENSIVE text labels, keyboard-focusable cells, per-cell decomposition from typed `ExplainPrice(itemId, side, region)` factor rows only — no panel-side math); `traveling_caravan` renders ORIGIN REGION, SPECIALTY CARGO, ROUTE STATE (NOMINAL/SLOWED/BLOCKED + weather reason) and the route progress multiplier, with the `OnCaravanEmbargoed`/`OnCaravanResumed` clearing edge wired (stale blocked presentation clears on the resume event, never a poll/reopen). Refresh rides the session `StateChanged` event only. Gates: panel lifecycle PASS, caravan/bridge selftests PASS, a11y 5/5, PanelRouteGate 20/20, UiPanelContract scene-node contracts unchanged, build 0/0. The combined Phase 1 economy gate is now complete end-to-end (Core + presentation).
- Caravan stock quotes remain ration-denominated (existing `priceRations` model); regional composition of caravan trade-screen pricing remains as previously scoped (not part of the B1 acceptance set).
...
### Remaining blockers
- None for the Core + presentation phases. Next: C1.4 (crisis prediction).
