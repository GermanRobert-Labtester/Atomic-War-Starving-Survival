...
# recursive data index
data_pairs=[]
for base,_,fs in os.walk('Assets/StreamingAssets/Data'):
    for f in fs:
        if f.endswith('.json'): data_pairs.append((f,os.path.join(base,f)))
data_files=[f for f,_ in data_pairs]
idpat=re.compile(r'\b((?:item|loc|faction|quest|enc|echo|radio|recipe|survivor|event|perk|trait)_[a-z0-9_]{3,})\b')
idset=set()
for f,p in data_pairs:
    idset|=set(idpat.findall(rd(p)))
print("recursive json files:",len(data_pairs),"ids:",len(idset))
# W regenerate
...
against **{len(idset)} ids found anywhere under `Data/` (recursive)**.
...
"""
body_w=''; withids=0; unresolved=0
for r in orph:
    a=r['auth'][0]; t=rd(r['file'])
    lits=sorted(set(idpat.findall(t)))
    if not lits: continue
    withids+=1
    body_w+=f"### `{a}`\n\n| Id literal | Resolves |\n|---|:---:|\n"
    for l in lits[:10]:
        ok=l in idset
        if not ok: unresolved+=1
        body_w+=f"| `{l}` | {'yes' if ok else '**no**'} |\n"
    body_w+='\n'
open(dirp+'PLAN-ORPHAN-SEAL-01_APPENDIX-W_DATA_IDS.md','w',encoding='utf-8').write(hdr_w+body_w)
# X regenerate: fields-only, proper classes
...
for r in orph:
    a=r['auth'][0]; t=rd(r['file'])
    fields=re.findall(r'(?:public|private|internal|protected)\s+static\s+(readonly\s+)?([\w<>,\[\]\.\?]+)\s+(\w+)\s*(?:=|;)', t)
    props=re.findall(r'(?:public|private|internal|protected)\s+static\s+([\w<>,\[\]\.\?]+)\s+(\w+)\s*\{\s*get', t)
    names=[n for _,_,n in fields]+[n for _,n in props]
    single=[n for n in names if re.search(r'Instance|Singleton|Shared|Default$',n,re.I)]
    mut=[n for ro,_,n in fields if not ro]
    coll=[n for ro,ty,n in fields if ro and re.match(r'(List|Dictionary|HashSet|IReadOnlyList|IReadOnlyDictionary|ConcurrentDictionary)<',ty)]
    fl=[]
    if single: fl.append('S:'+','.join(single[:3]))
    if mut: fl.append('M:'+','.join(mut[:3]))
    if coll: fl.append('C:'+','.join(coll[:3]))
    if fl: flagged+=1
    body_x+=f"| `{a}` | {', '.join('`'+n+'`' for n in names[:8]) or '—'} | {' · '.join(fl) or '—'} |\n"
hdr_x+=f"\n**{flagged} of {len(orph)} orphans carry at least one flagged static.**\n\n"
open(dirp+'PLAN-ORPHAN-SEAL-01_APPENDIX-X_STATIC_HAZARDS.md','w',encoding='utf-8').write(hdr_x+body_x)
# V regenerate with recursive catalog matching
save_txt=rd('Assets/Ashfall.Core/Save/SaveSectionRegistry.cs')
def propose(name):
    s=re.sub(r'(System|Engine|Coordinator|Manager)$','',name); return re.sub(r'([a-z0-9])([A-Z])',r'\1_\2',s).lower()
def tokens(name):
...
def catalogs(a):
    tk=tokens(a); return [f for f in data_files if tk and sum(1 for w in tk if w in f.lower())>=max(1,len(tk)-1)][:3]
...
print('orphans with id literals:',withids,'unresolved literal rows:',unresolved,'static-flagged:',flagged)
