...
**Suites:**
- `Ashfall.Core.Tests/FoundryAccordExpansionTests.cs` (11 tests)
...
- `godot --headless --data-integrity-selftest` (298 catalogs)
...
|---|---|---|---|
| `FoundryAccordExpansionTests` | `Catalog_LoadsAllAccordsWithoutErrors` | Asserts 18 regional records load, including exactly 10 exact-signatory Foundry accords, schema version 1, and a non-empty collection ID. | **PASS** |
...
| `FoundryAccordExpansionTests` | `FoundryRoster_ContainsExactlyTheSixNewDependencyReadyAccords` | Confirms the Foundry roster is exactly 10, names all six additions, and keeps them absent from the live Plan 103 policy catalog. | **PASS** |
...
| `FoundryAccordExpansionTests` | `Tags_FollowNormalizedVocabularyWithoutSynonymSplits` | Asserts all tags conform to the 52-keyword normalized vocabulary without synonym drift. | **PASS** |
...
- `godot --headless --path . -- --silent-foundry-selftest`: 27/27 PASS (exactly 10 Foundry-signatory accords verified).
- `godot --headless --path . -- --data-integrity-selftest`: 298 catalogs PASS (0 errors).
