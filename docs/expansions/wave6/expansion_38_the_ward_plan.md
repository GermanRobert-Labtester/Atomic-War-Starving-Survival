...
**Lyophilization** is a planning subject in the master's C2 and C3 cluster. The source of expansion ideas is `docs/newest-ashfall-master-expansion-authority-v2-0-complete-compiled-edition-volumes-1-57.md`: it asks for data-backed narrative depth, one existing Core authority, a host route, persistence where state changes, and player-observable outcomes. This document is a reviewable integration plan, not a production claim or a declaration of runtime completeness.

**Current premise:** The Ward design bible is pre-integration, but the base LyophilizationSystem already has a host, catalog, panel and save. The named LyophilizationEngine subclass has no direct host reference. Ward integration should consume the base owner rather than create a second batch ledger.

**First bounded outcome:** one existing recipe from inventory input to drying, expiry, and ward protocol use. This is the first package to promote after the current premise is rechecked. Other cases in the register are candidate acceptance probes and narrative directions; they are not simultaneous build requirements.

**One-authority boundary:** Inventory owns physical items; power owns supply; medical pipeline owns patient care; LyophilizationSystem owns batch viability. Core source: `LyophilizationSystem; LyophilizationEngine is a compatibility subclass`. Candidate host: `LyophilizationHostSession and Main.Plans130_133.cs`. Persistence: `lyophilization`. Authored data: `lyophilization_catalog.json`. Any proposed additional field must be justified by a real save-round-trip failure, and any proposed new catalog must first pass a live ID collision and consumer audit.

## Integration architecture

```text
Current authored JSON / real owner inputs
    -> LyophilizationSystem; LyophilizationEngine is a compatibility subclass
    -> LyophilizationHostSession and Main.Plans130_133.cs
    -> existing save and day owner (lyophilization)
...
|---|---|
| Core owner | `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` |
| Public methods | `LyophilizationSystem.StartBatch, TickDay, CanUseBatch, TryUseBatch, RegisterMedicalProtocol, CaptureState` |
| Host | `LyophilizationHostSession and Main.Plans130_133.cs` |
| Save | `lyophilization` |
| Catalog or input | `lyophilization_catalog.json` |
...
The master expansion document's register is precise, restrained, and grounded in physical evidence. A dated glass vial may anchor a line, but the line must never imply an outcome the owning system did not report. Technical records should name a measurement or reason code; an intimate record should name what its speaker can know. One event should not grant the same reward through both a quest flag and an independent host callback. For every proposed paragraph, write its trigger, consumer, fallback, and continuity constraint before the prose itself. After acceptance, move only the approved text into the JSON authority; do not load Markdown at runtime.
...
|---|---|---|
| `recipe_lyophilize_blood_plasma` | `recipe_id`, `display_name`, `input_item_id`, `input_amount`, `container_item_id`, `container_amount`, `output_item_id`, `output_amount` | Check exact loader, availability, ending or output consumer, and save round trip. |
| `recipe_lyophilize_organic_residue` | `recipe_id`, `display_name`, `input_item_id`, `input_amount`, `container_item_id`, `container_amount`, `output_item_id`, `output_amount` | Check exact loader, availability, ending or output consumer, and save round trip. |

### Lyophilization case 001 — recipe selection / first encounter

**Scene and prose.** The player reaches the recipe selection before the shift bell. First encounter matters here: the owner has no prior record for this subject. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial is set beside the record. No one calls the entry final.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is default state and catalog bootstrap. The preview and commit must use the same canonical snapshot or revalidate just before mutation. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 002 — recipe selection / legacy restore

**Scene and prose.** The player reaches the recipe selection after a long wait. Legacy restore matters here: the save predates an additive field. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is old-save default and checksum boundary. A failed transition must preserve the previous owner state and retain a visible reason code. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 003 — recipe selection / duplicate request

**Scene and prose.** The player reaches the recipe selection when the light changes. Duplicate request matters here: the command is submitted twice before a day tick. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A hand pauses over a dated glass vial; the reason is written before the name.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is idempotent transition or typed refusal. A consumer may project this fact but cannot infer an additional reward from the prose. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 004 — recipe selection / late day

**Scene and prose.** The player reaches the recipe selection while the ledger is still open. Late day matters here: the action arrives at the boundary between two campaign days. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial marks the place. It offers no answer by itself.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is day source and tick order. The save capture must follow the successful event and restore without publishing it again. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 005 — recipe selection / unknown authored ID

**Scene and prose.** The player reaches the recipe selection at the edge of the next day. Unknown authored id matters here: a referenced catalog ID is absent. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial stays where both sides can see it. The count remains unchanged.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is loader validation and safe fallback. The preview and commit must use the same canonical snapshot or revalidate just before mutation. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 006 — recipe selection / missing participant

**Scene and prose.** The player reaches the recipe selection before the shift bell. Missing participant matters here: the actor or target is no longer available. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial is set beside the record. No one calls the entry final.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is living roster or canonical entity lookup. A failed transition must preserve the previous owner state and retain a visible reason code. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 007 — recipe selection / scarce input

**Scene and prose.** The player reaches the recipe selection after a long wait. Scarce input matters here: the canonical owner reports less stock than the preview. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is atomic cost validation. A consumer may project this fact but cannot infer an additional reward from the prose. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 008 — recipe selection / interrupted power

**Scene and prose.** The player reaches the recipe selection when the light changes. Interrupted power matters here: the operating facility loses available power. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A hand pauses over a dated glass vial; the reason is written before the name.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is power owner projection. The save capture must follow the successful event and restore without publishing it again. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 009 — recipe selection / route not opened

**Scene and prose.** The player reaches the recipe selection while the ledger is still open. Route not opened matters here: the player has not unlocked the required route. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial marks the place. It offers no answer by itself.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is reachability and authorization. The preview and commit must use the same canonical snapshot or revalidate just before mutation. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 010 — recipe selection / panel reopened

**Scene and prose.** The player reaches the recipe selection at the edge of the next day. Panel reopened matters here: the player closes and reopens the relevant surface. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial stays where both sides can see it. The count remains unchanged.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is binding, refresh, and disposal. A failed transition must preserve the previous owner state and retain a visible reason code. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For panel reopened, inspect binding, refresh, and disposal in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 011 — recipe selection / same seed replay

**Scene and prose.** The player reaches the recipe selection before the shift bell. Same seed replay matters here: two campaigns use identical seed and input order. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial is set beside the record. No one calls the entry final.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is deterministic output and event order. A consumer may project this fact but cannot infer an additional reward from the prose. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For same seed replay, inspect deterministic output and event order in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 012 — recipe selection / neighboring event

**Scene and prose.** The player reaches the recipe selection after a long wait. Neighboring event matters here: another owner publishes a fact in the same day phase. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is one-way adapter ordering. The save capture must follow the successful event and restore without publishing it again. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 013 — recipe selection / past story window

**Scene and prose.** The player reaches the recipe selection when the light changes. Past story window matters here: the player reaches the scene after its intended campaign band. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A hand pauses over a dated glass vial; the reason is written before the name.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is explicit availability rule. The preview and commit must use the same canonical snapshot or revalidate just before mutation. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 014 — recipe selection / empty roster

**Scene and prose.** The player reaches the recipe selection while the ledger is still open. Empty roster matters here: no living eligible participant is present. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial marks the place. It offers no answer by itself.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is zero-count behavior. A failed transition must preserve the previous owner state and retain a visible reason code. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 015 — recipe selection / dense roster

**Scene and prose.** The player reaches the recipe selection at the edge of the next day. Dense roster matters here: many valid participants expose iteration order. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial stays where both sides can see it. The count remains unchanged.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is stable sorted processing. A consumer may project this fact but cannot infer an additional reward from the prose. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 016 — recipe selection / conflicting command

**Scene and prose.** The player reaches the recipe selection before the shift bell. Conflicting command matters here: another command requests an incompatible transition. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial is set beside the record. No one calls the entry final.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is single authority and refusal reason. The save capture must follow the successful event and restore without publishing it again. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 017 — recipe selection / bad identifier

**Scene and prose.** The player reaches the recipe selection after a long wait. Bad identifier matters here: the request contains an empty or wrong-domain ID. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is input guard and no-op. The preview and commit must use the same canonical snapshot or revalidate just before mutation. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 018 — recipe selection / failed capture

**Scene and prose.** The player reaches the recipe selection when the light changes. Failed capture matters here: save capture fails after a successful Core action. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A hand pauses over a dated glass vial; the reason is written before the name.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is dirty state and retry semantics. A failed transition must preserve the previous owner state and retain a visible reason code. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 019 — recipe selection / restore after fact

**Scene and prose.** The player reaches the recipe selection while the ledger is still open. Restore after fact matters here: the campaign reloads just after publishing an event. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial marks the place. It offers no answer by itself.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is exactly-once consumer behavior. A consumer may project this fact but cannot infer an additional reward from the prose. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 020 — recipe selection / controller path

**Scene and prose.** The player reaches the recipe selection at the edge of the next day. Controller path matters here: the same operation uses controller focus and back. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial stays where both sides can see it. The count remains unchanged.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is accessible route parity. The save capture must follow the successful event and restore without publishing it again. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.
...
### Lyophilization case 021 — input issue / first encounter

**Scene and prose.** The player reaches the input issue when the light changes. First encounter matters here: the owner has no prior record for this subject. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A hand pauses over a dated glass vial; the reason is written before the name.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is default state and catalog bootstrap. A failed transition must preserve the previous owner state and retain a visible reason code. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For first encounter, inspect default state and catalog bootstrap in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 022 — input issue / legacy restore

**Scene and prose.** The player reaches the input issue while the ledger is still open. Legacy restore matters here: the save predates an additive field. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial marks the place. It offers no answer by itself.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 023 — input issue / duplicate request

**Scene and prose.** The player reaches the input issue at the edge of the next day. Duplicate request matters here: the command is submitted twice before a day tick. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial stays where both sides can see it. The count remains unchanged.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 024 — input issue / late day

**Scene and prose.** The player reaches the input issue before the shift bell. Late day matters here: the action arrives at the boundary between two campaign days. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial is set beside the record. No one calls the entry final.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 025 — input issue / unknown authored ID

**Scene and prose.** The player reaches the input issue after a long wait. Unknown authored id matters here: a referenced catalog ID is absent. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is loader validation and safe fallback. A failed transition must preserve the previous owner state and retain a visible reason code. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For unknown authored ID, inspect loader validation and safe fallback in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 026 — input issue / missing participant

**Scene and prose.** The player reaches the input issue when the light changes. Missing participant matters here: the actor or target is no longer available. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A hand pauses over a dated glass vial; the reason is written before the name.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 027 — input issue / scarce input

**Scene and prose.** The player reaches the input issue while the ledger is still open. Scarce input matters here: the canonical owner reports less stock than the preview. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial marks the place. It offers no answer by itself.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 028 — input issue / interrupted power

**Scene and prose.** The player reaches the input issue at the edge of the next day. Interrupted power matters here: the operating facility loses available power. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial stays where both sides can see it. The count remains unchanged.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is power owner projection. The preview and commit must use the same canonical snapshot or revalidate just before mutation. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For interrupted power, inspect power owner projection in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 029 — input issue / route not opened

**Scene and prose.** The player reaches the input issue before the shift bell. Route not opened matters here: the player has not unlocked the required route. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial is set beside the record. No one calls the entry final.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is reachability and authorization. A failed transition must preserve the previous owner state and retain a visible reason code. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For route not opened, inspect reachability and authorization in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 030 — input issue / panel reopened

**Scene and prose.** The player reaches the input issue after a long wait. Panel reopened matters here: the player closes and reopens the relevant surface. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 031 — input issue / same seed replay

**Scene and prose.** The player reaches the input issue when the light changes. Same seed replay matters here: two campaigns use identical seed and input order. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A hand pauses over a dated glass vial; the reason is written before the name.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is deterministic output and event order. The save capture must follow the successful event and restore without publishing it again. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For same seed replay, inspect deterministic output and event order in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 032 — input issue / neighboring event

**Scene and prose.** The player reaches the input issue while the ledger is still open. Neighboring event matters here: another owner publishes a fact in the same day phase. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial marks the place. It offers no answer by itself.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is one-way adapter ordering. The preview and commit must use the same canonical snapshot or revalidate just before mutation. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For neighboring event, inspect one-way adapter ordering in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 033 — input issue / past story window

**Scene and prose.** The player reaches the input issue at the edge of the next day. Past story window matters here: the player reaches the scene after its intended campaign band. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial stays where both sides can see it. The count remains unchanged.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 034 — input issue / empty roster

**Scene and prose.** The player reaches the input issue before the shift bell. Empty roster matters here: no living eligible participant is present. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial is set beside the record. No one calls the entry final.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is zero-count behavior. A consumer may project this fact but cannot infer an additional reward from the prose. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For empty roster, inspect zero-count behavior in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 035 — input issue / dense roster

**Scene and prose.** The player reaches the input issue after a long wait. Dense roster matters here: many valid participants expose iteration order. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is stable sorted processing. The save capture must follow the successful event and restore without publishing it again. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For dense roster, inspect stable sorted processing in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 036 — input issue / conflicting command

**Scene and prose.** The player reaches the input issue when the light changes. Conflicting command matters here: another command requests an incompatible transition. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A hand pauses over a dated glass vial; the reason is written before the name.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is single authority and refusal reason. The preview and commit must use the same canonical snapshot or revalidate just before mutation. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For conflicting command, inspect single authority and refusal reason in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 037 — input issue / bad identifier

**Scene and prose.** The player reaches the input issue while the ledger is still open. Bad identifier matters here: the request contains an empty or wrong-domain ID. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial marks the place. It offers no answer by itself.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is input guard and no-op. A failed transition must preserve the previous owner state and retain a visible reason code. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For bad identifier, inspect input guard and no-op in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 038 — input issue / failed capture

**Scene and prose.** The player reaches the input issue at the edge of the next day. Failed capture matters here: save capture fails after a successful Core action. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial stays where both sides can see it. The count remains unchanged.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is dirty state and retry semantics. A consumer may project this fact but cannot infer an additional reward from the prose. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For failed capture, inspect dirty state and retry semantics in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 039 — input issue / restore after fact

**Scene and prose.** The player reaches the input issue before the shift bell. Restore after fact matters here: the campaign reloads just after publishing an event. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial is set beside the record. No one calls the entry final.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is exactly-once consumer behavior. The save capture must follow the successful event and restore without publishing it again. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For restore after fact, inspect exactly-once consumer behavior in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 040 — input issue / controller path

**Scene and prose.** The player reaches the input issue after a long wait. Controller path matters here: the same operation uses controller focus and back. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is accessible route parity. The preview and commit must use the same canonical snapshot or revalidate just before mutation. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For controller path, inspect accessible route parity in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 041 — container check / first encounter

**Scene and prose.** The player reaches the container check at the edge of the next day. First encounter matters here: the owner has no prior record for this subject. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial stays where both sides can see it. The count remains unchanged.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is default state and catalog bootstrap. A consumer may project this fact but cannot infer an additional reward from the prose. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For first encounter, inspect default state and catalog bootstrap in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 042 — container check / legacy restore

**Scene and prose.** The player reaches the container check before the shift bell. Legacy restore matters here: the save predates an additive field. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial is set beside the record. No one calls the entry final.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 043 — container check / duplicate request

**Scene and prose.** The player reaches the container check after a long wait. Duplicate request matters here: the command is submitted twice before a day tick. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 044 — container check / late day

**Scene and prose.** The player reaches the container check when the light changes. Late day matters here: the action arrives at the boundary between two campaign days. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A hand pauses over a dated glass vial; the reason is written before the name.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 045 — container check / unknown authored ID

**Scene and prose.** The player reaches the container check while the ledger is still open. Unknown authored id matters here: a referenced catalog ID is absent. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial marks the place. It offers no answer by itself.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is loader validation and safe fallback. A consumer may project this fact but cannot infer an additional reward from the prose. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For unknown authored ID, inspect loader validation and safe fallback in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 046 — container check / missing participant

**Scene and prose.** The player reaches the container check at the edge of the next day. Missing participant matters here: the actor or target is no longer available. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial stays where both sides can see it. The count remains unchanged.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 047 — container check / scarce input

**Scene and prose.** The player reaches the container check before the shift bell. Scarce input matters here: the canonical owner reports less stock than the preview. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial is set beside the record. No one calls the entry final.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 048 — container check / interrupted power

**Scene and prose.** The player reaches the container check after a long wait. Interrupted power matters here: the operating facility loses available power. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is power owner projection. A failed transition must preserve the previous owner state and retain a visible reason code. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For interrupted power, inspect power owner projection in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 049 — container check / route not opened

**Scene and prose.** The player reaches the container check when the light changes. Route not opened matters here: the player has not unlocked the required route. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A hand pauses over a dated glass vial; the reason is written before the name.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is reachability and authorization. A consumer may project this fact but cannot infer an additional reward from the prose. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For route not opened, inspect reachability and authorization in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 050 — container check / panel reopened

**Scene and prose.** The player reaches the container check while the ledger is still open. Panel reopened matters here: the player closes and reopens the relevant surface. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial marks the place. It offers no answer by itself.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 051 — container check / same seed replay

**Scene and prose.** The player reaches the container check at the edge of the next day. Same seed replay matters here: two campaigns use identical seed and input order. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial stays where both sides can see it. The count remains unchanged.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is deterministic output and event order. The preview and commit must use the same canonical snapshot or revalidate just before mutation. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For same seed replay, inspect deterministic output and event order in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 052 — container check / neighboring event

**Scene and prose.** The player reaches the container check before the shift bell. Neighboring event matters here: another owner publishes a fact in the same day phase. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial is set beside the record. No one calls the entry final.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is one-way adapter ordering. A failed transition must preserve the previous owner state and retain a visible reason code. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For neighboring event, inspect one-way adapter ordering in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 053 — container check / past story window

**Scene and prose.** The player reaches the container check after a long wait. Past story window matters here: the player reaches the scene after its intended campaign band. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 054 — container check / empty roster

**Scene and prose.** The player reaches the container check when the light changes. Empty roster matters here: no living eligible participant is present. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A hand pauses over a dated glass vial; the reason is written before the name.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is zero-count behavior. The save capture must follow the successful event and restore without publishing it again. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For empty roster, inspect zero-count behavior in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 055 — container check / dense roster

**Scene and prose.** The player reaches the container check while the ledger is still open. Dense roster matters here: many valid participants expose iteration order. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial marks the place. It offers no answer by itself.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is stable sorted processing. The preview and commit must use the same canonical snapshot or revalidate just before mutation. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For dense roster, inspect stable sorted processing in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 056 — container check / conflicting command

**Scene and prose.** The player reaches the container check at the edge of the next day. Conflicting command matters here: another command requests an incompatible transition. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial stays where both sides can see it. The count remains unchanged.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is single authority and refusal reason. A failed transition must preserve the previous owner state and retain a visible reason code. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For conflicting command, inspect single authority and refusal reason in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 057 — container check / bad identifier

**Scene and prose.** The player reaches the container check before the shift bell. Bad identifier matters here: the request contains an empty or wrong-domain ID. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial is set beside the record. No one calls the entry final.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is input guard and no-op. A consumer may project this fact but cannot infer an additional reward from the prose. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For bad identifier, inspect input guard and no-op in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 058 — container check / failed capture

**Scene and prose.** The player reaches the container check after a long wait. Failed capture matters here: save capture fails after a successful Core action. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is dirty state and retry semantics. The save capture must follow the successful event and restore without publishing it again. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For failed capture, inspect dirty state and retry semantics in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 059 — container check / restore after fact

**Scene and prose.** The player reaches the container check when the light changes. Restore after fact matters here: the campaign reloads just after publishing an event. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A hand pauses over a dated glass vial; the reason is written before the name.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is exactly-once consumer behavior. The preview and commit must use the same canonical snapshot or revalidate just before mutation. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For restore after fact, inspect exactly-once consumer behavior in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 060 — container check / controller path

**Scene and prose.** The player reaches the container check while the ledger is still open. Controller path matters here: the same operation uses controller focus and back. The record under review is `recipe_lyophilize_blood_plasma`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial marks the place. It offers no answer by itself.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is accessible route parity. A failed transition must preserve the previous owner state and retain a visible reason code. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For controller path, inspect accessible route parity in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 061 — vacuum start / first encounter

**Scene and prose.** The player reaches the vacuum start after a long wait. First encounter matters here: the owner has no prior record for this subject. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is default state and catalog bootstrap. The save capture must follow the successful event and restore without publishing it again. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For first encounter, inspect default state and catalog bootstrap in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 062 — vacuum start / legacy restore

**Scene and prose.** The player reaches the vacuum start when the light changes. Legacy restore matters here: the save predates an additive field. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A hand pauses over a dated glass vial; the reason is written before the name.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 063 — vacuum start / duplicate request

**Scene and prose.** The player reaches the vacuum start while the ledger is still open. Duplicate request matters here: the command is submitted twice before a day tick. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial marks the place. It offers no answer by itself.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 064 — vacuum start / late day

**Scene and prose.** The player reaches the vacuum start at the edge of the next day. Late day matters here: the action arrives at the boundary between two campaign days. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial stays where both sides can see it. The count remains unchanged.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
### Lyophilization case 065 — vacuum start / unknown authored ID

**Scene and prose.** The player reaches the vacuum start before the shift bell. Unknown authored id matters here: a referenced catalog ID is absent. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “A dated glass vial is set beside the record. No one calls the entry final.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.

**Owner trace.** The query begins with `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` and its actual input owners. The player route, if one exists, passes through `LyophilizationHostSession and Main.Plans130_133.cs`. The boundary question for this card is loader validation and safe fallback. The save capture must follow the successful event and restore without publishing it again. The recorded state belongs in `lyophilization` only where the current owner is stateful. Authored identity comes from `lyophilization_catalog.json` when applicable; otherwise this case uses current Core values and no new JSON authority. Never add a field, store, route, or mechanic because this card happens to name one.

**Integration decision.** Promote this case only after the implementer shows a real scene trigger, the exact live API call, the before/after owner state, and the visible destination. If the condition is false, show a typed refusal and preserve every neighboring owner. If it is true, publish the current Core fact once and let the authorized adapter carry any consequence. For unknown authored ID, inspect loader validation and safe fallback in both a fresh campaign and a restored campaign. The content editor then checks that the line's speaker could know the detail and that it does not duplicate a nearby catalog entry.

**Acceptance record.** Record (a) the input snapshot and exact ID, (b) eligibility or refusal code, (c) one owner transition or a proven no-op, (d) save behavior appropriate to `lyophilization`, (e) any deterministic ordering requirement, and (f) panel/journal focus and refresh. Include file:line evidence and a focused command in the promoted package. A code compilation result alone does not prove this route; a catalog row alone does not prove reachability. Classify this card as accepted, deferred, or rejected rather than silently broadening the package.

### Lyophilization case 066 — vacuum start / missing participant

**Scene and prose.** The player reaches the vacuum start after a long wait. Missing participant matters here: the actor or target is no longer available. The record under review is `recipe_lyophilize_organic_residue`. This is a candidate moment for a short physical observation, not a guaranteed quest beat: “Someone turns a dated glass vial over. The blank side is easier to read.” If the source state cannot establish that observation, keep the sentence in the proposal layer. The player must see the actual current state and a reason to act or wait; the text cannot turn a failed request into a narrative success.
...
## Finished architecture handoff for Lyophilization

The documentary integration architecture is complete at the planning level: `lyophilization_catalog.json` or current owner inputs → `LyophilizationSystem; LyophilizationEngine is a compatibility subclass` → `LyophilizationHostSession and Main.Plans130_133.cs` → `lyophilization` where mutable → the current player surface. The first package is **one existing recipe from inventory input to drying, expiry, and ward protocol use**. The 66 cases above form a candidate review register; they do not establish that the user can already perform them. Historical status claims remain dated above this marker. Production integration requires an authorized package, exact path claims, current call-site checks, and focused verification.
