...
- Concern: **Muster**. Current owner: `MusterSystem and its internal MusterWarfareEngine`.
- Host candidate: `MusterHostSession and Main.Muster.cs`. This is the location to audit, not a pre-authorization to edit.
- Persistence seam: `muster`. Capture and restore must be proven through the live save registry before changing any field.
- Authored data family: `muster_camp_scenes.json, muster_epilogues.json, muster_faction_actions.json, muster_faction_culture.json, muster_witnesses.json`. Every new ID must pass schema, reference, consumer, and reachability checks.
- Current source finding: The host and section are live; welfare, faction standing, and ending enrollment still require per-route proof.
- Outcome sought: a public call that commits real people and counted supplies; the actionable route is to select an available muster action through the existing host; the observable result is that camp, witness, and endgame projections agree on the consequence.
...
The first promoted slice should join one currently loaded muster camp action to a witness observation and a later epilogue trace. `MusterSystem` already owns an internal `MusterWarfareEngine`; `MusterHostSession` and `Main.Muster.cs` already exist. Its warfare contract forbids ghost soldiers and requires unique living survivor IDs, and supply reservation must conserve issued, remaining, lost, and returned quantities. Check whether the five live muster catalogs are all consumed before authoring another. A faction effect belongs to the standing authority; an ending effect belongs to existing Reckoning enrollment. The Muster panel may explain why an action cannot start, but cannot invent a soldier, a ration, or a branch commitment. Mark the old bible's named NPC and location claims against current catalog IDs before converting any one of its story beats into JSON.

## Current API checkpoints for the first slice

| Role | Current source | Symbol or audit action |
|---|---|---|
| Core campaign | `Assets/Ashfall.Core/Muster/MusterSystem.cs` | `RecordWitnessResult, SelectApproachFor, ResolveEndingKey` |
| Core warfare | `Assets/Ashfall.Core/Muster/MusterWarfareEngine.cs` | `CanMobilize, Mobilize, ResolveEngagement, Withdraw` |
| Host | `src/Main.Muster.cs` | `SetupMuster, SaveMuster` |
...
public readonly record struct MusterActionRequest(
...
### Muster probe 001 — camp arrival / fresh campaign

**Situation.** A player reaches camp arrival before the next watch. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, there is no prior feature state and no optional catalog row has been unlocked. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The record keeps the names; it cannot make the choice for them.” This is candidate presentation copy, contingent on the state actually supporting it.

**Authority trace.** Read the live projection from `MusterSystem and its internal MusterWarfareEngine`; do not infer the result from the panel's previous label. Send a single request through `MusterHostSession and Main.Muster.cs` only after the route exists. The Core transition must validate the identifier, eligibility, and current day against its own state. The host then applies only authorized neighboring effects, in documented order, and captures `muster` through its current store. Content comes from `muster_camp_scenes.json, muster_epilogues.json, muster_faction_actions.json, muster_faction_culture.json, muster_witnesses.json` by stable ID. If that data family cannot represent this scene, classify it as an authoring proposal; do not add a shadow table.

**Failure and recovery.** For fresh campaign, the expected no-op is “No entry made. The previous record remains in force.” Retrying after the underlying condition changes must re-query the owner. Reloading must reproduce the same resolved state and must not replay the first successful event. Compare reserved, issued, lost, returned, and remaining supplies through the Muster warfare invariant; a refused mobilization uses no stores and creates no ghost soldier. A withdrawal must return only what the existing reservation actually holds. If a survivor, faction, object, or location is missing, use the existing fallback presentation and preserve the save envelope.
...
### Muster probe 002 — camp arrival / legacy restore

**Situation.** A player reaches camp arrival while the corridor is quiet. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, an older save lacks the newest additive field while retaining its original checksum contract. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “Someone has left room on the page for what happens next.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 003 — camp arrival / repeated command

**Situation.** A player reaches camp arrival at the marked change of shift. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the same player action is submitted twice before the next day advances. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “No one in the room calls this a victory.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 004 — camp arrival / day rollover

**Situation.** A player reaches camp arrival after the shelter lights dip. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the action is requested on the boundary between campaign days. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The count is exact. The reason for it is harder to file.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 006 — camp arrival / survivor death

**Situation.** A player reaches camp arrival before the next watch. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, a referenced survivor dies between preview and commitment. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The record keeps the names; it cannot make the choice for them.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 007 — camp arrival / resource shortage

**Situation.** A player reaches camp arrival while the corridor is quiet. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the owner ledger has less stock than the preview advertised. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “Someone has left room on the page for what happens next.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 010 — camp arrival / reopened panel

**Situation.** A player reaches camp arrival as the ledger is closed. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the panel is disposed and reopened after a state change. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The object is small enough to miss, and too specific to replace.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 012 — camp arrival / cross-system signal

**Situation.** A player reaches camp arrival while the corridor is quiet. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, a neighboring owner publishes a fact during the same day phase. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “Someone has left room on the page for what happens next.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 013 — camp arrival / late campaign

**Situation.** A player reaches camp arrival at the marked change of shift. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the relevant branch is encountered after the nominal story window. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “No one in the room calls this a victory.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 014 — camp arrival / zero participants

**Situation.** A player reaches camp arrival after the shelter lights dip. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the roster projection contains no eligible living survivor. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The count is exact. The reason for it is harder to file.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 015 — camp arrival / many participants

**Situation.** A player reaches camp arrival as the ledger is closed. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, a large roster and dense catalog make iteration order visible. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The object is small enough to miss, and too specific to replace.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 016 — camp arrival / conflicting command

**Situation.** A player reaches camp arrival before the next watch. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, a second command requests an incompatible state transition. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The record keeps the names; it cannot make the choice for them.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 017 — camp arrival / malformed input

**Situation.** A player reaches camp arrival while the corridor is quiet. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the input has an empty, unknown, or wrong-domain identifier. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “Someone has left room on the page for what happens next.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 019 — camp arrival / restore after event

**Situation.** A player reaches camp arrival after the shelter lights dip. A counted canteen gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the game restores immediately after event publication. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The count is exact. The reason for it is harder to file.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 021 — faction appeal / fresh campaign

**Situation.** A player reaches faction appeal after the shelter lights dip. A empty bedroll gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, there is no prior feature state and no optional catalog row has been unlocked. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The count is exact. The reason for it is harder to file.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 022 — faction appeal / legacy restore

**Situation.** A player reaches faction appeal as the ledger is closed. A empty bedroll gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, an older save lacks the newest additive field while retaining its original checksum contract. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The object is small enough to miss, and too specific to replace.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 023 — faction appeal / repeated command

**Situation.** A player reaches faction appeal before the next watch. A empty bedroll gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the same player action is submitted twice before the next day advances. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The record keeps the names; it cannot make the choice for them.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 024 — faction appeal / day rollover

**Situation.** A player reaches faction appeal while the corridor is quiet. A empty bedroll gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the action is requested on the boundary between campaign days. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “Someone has left room on the page for what happens next.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 025 — faction appeal / missing catalog reference

**Situation.** A player reaches faction appeal at the marked change of shift. A empty bedroll gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, an authored ID cannot be resolved by the current loader. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “No one in the room calls this a victory.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 026 — faction appeal / survivor death
...
### Muster probe 027 — faction appeal / resource shortage
...
### Muster probe 028 — faction appeal / power interruption
...
### Muster probe 029 — faction appeal / blocked route
...
### Muster probe 030 — faction appeal / reopened panel
...
### Muster probe 031 — faction appeal / same seed replay
...
### Muster probe 032 — faction appeal / cross-system signal
...
### Muster probe 033 — faction appeal / late campaign
...
### Muster probe 034 — faction appeal / zero participants
...
### Muster probe 035 — faction appeal / many participants
...
### Muster probe 036 — faction appeal / conflicting command
...
### Muster probe 037 — faction appeal / malformed input
...
### Muster probe 038 — faction appeal / failed save write
...
### Muster probe 039 — faction appeal / restore after event
...
### Muster probe 040 — faction appeal / accessibility route
...
### Muster probe 041 — supply reservation / fresh campaign

**Situation.** A player reaches supply reservation while the corridor is quiet. A sealed manifest gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, there is no prior feature state and no optional catalog row has been unlocked. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “Someone has left room on the page for what happens next.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 042 — supply reservation / legacy restore

**Situation.** A player reaches supply reservation at the marked change of shift. A sealed manifest gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, an older save lacks the newest additive field while retaining its original checksum contract. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “No one in the room calls this a victory.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 043 — supply reservation / repeated command

**Situation.** A player reaches supply reservation after the shelter lights dip. A sealed manifest gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the same player action is submitted twice before the next day advances. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The count is exact. The reason for it is harder to file.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 044 — supply reservation / day rollover

**Situation.** A player reaches supply reservation as the ledger is closed. A sealed manifest gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the action is requested on the boundary between campaign days. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The object is small enough to miss, and too specific to replace.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 045 — supply reservation / missing catalog reference

**Situation.** A player reaches supply reservation before the next watch. A sealed manifest gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, an authored ID cannot be resolved by the current loader. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The record keeps the names; it cannot make the choice for them.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 046 — supply reservation / survivor death
...
### Muster probe 047 — supply reservation / resource shortage
...
### Muster probe 048 — supply reservation / power interruption
...
### Muster probe 049 — supply reservation / blocked route
...
### Muster probe 050 — supply reservation / reopened panel
...
### Muster probe 051 — supply reservation / same seed replay
...
### Muster probe 052 — supply reservation / cross-system signal
...
### Muster probe 053 — supply reservation / late campaign
...
### Muster probe 054 — supply reservation / zero participants
...
### Muster probe 055 — supply reservation / many participants
...
### Muster probe 056 — supply reservation / conflicting command
...
### Muster probe 057 — supply reservation / malformed input
...
### Muster probe 058 — supply reservation / failed save write
...
### Muster probe 059 — supply reservation / restore after event
...
### Muster probe 060 — supply reservation / accessibility route
...
### Muster probe 061 — soldier nomination / fresh campaign

**Situation.** A player reaches soldier nomination as the ledger is closed. A field bandage gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, there is no prior feature state and no optional catalog row has been unlocked. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The object is small enough to miss, and too specific to replace.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 062 — soldier nomination / legacy restore

**Situation.** A player reaches soldier nomination before the next watch. A field bandage gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, an older save lacks the newest additive field while retaining its original checksum contract. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The record keeps the names; it cannot make the choice for them.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 063 — soldier nomination / repeated command

**Situation.** A player reaches soldier nomination while the corridor is quiet. A field bandage gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the same player action is submitted twice before the next day advances. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “Someone has left room on the page for what happens next.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 064 — soldier nomination / day rollover

**Situation.** A player reaches soldier nomination at the marked change of shift. A field bandage gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, the action is requested on the boundary between campaign days. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “No one in the room calls this a victory.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Muster probe 065 — soldier nomination / missing catalog reference

**Situation.** A player reaches soldier nomination after the shelter lights dip. A field bandage gives the scene a physical anchor. The request concerns a public call that commits real people and counted supplies. In this variant, an authored ID cannot be resolved by the current loader. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The count is exact. The reason for it is harder to file.” This is candidate presentation copy, contingent on the state actually supporting it.
...
The integration architecture for **Muster** is therefore: authored JSON `muster_camp_scenes.json, muster_epilogues.json, muster_faction_actions.json, muster_faction_culture.json, muster_witnesses.json` → `MusterSystem and its internal MusterWarfareEngine` → `MusterHostSession and Main.Muster.cs` → existing save owner `muster` → a read-only Godot projection. This is the entire intended direction of dependency. A consumer may ask the owner for a fact; it may not mutate the owner's private copy or introduce a parallel clock, resource, outcome, or ledger. The phase-0 audit must replace the illustrative framework with exact current signatures and record any contrary source evidence.
