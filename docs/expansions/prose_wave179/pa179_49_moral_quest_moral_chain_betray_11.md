# Subject Plan 179.49 — The Crisis Gambit: branching-quest discovery prose
...
Review one field—`discovery`—for selector `quest_moral_chain_betray_11` in `Assets/StreamingAssets/Data/moral_choice_quests_branching.json`. Current depth: 68 words / 362 characters. The aim is to clarify the authored encounter that players see when this moral-choice quest is offered. The plan does not ask for a longer passage; retain the current text if a revision would add no useful information.
...
This is the Betray branch record **The Crisis Gambit**. The branch identity and actual situation are source facts; do not use this pass to rewrite the larger quest arc.

## Verified premise and source evidence

- **VERIFIED (2026-09-24):** `quest_moral_chain_betray_11` is present exactly once in the current `schema_version: 1` `quests` array, which contains 100 entries.
...
**Trigger:** The water pump fails. People are desperate. You know how to fix it — but timing the fix is worth more than the fix itself.

**Place:** `loc_water_station` · **category:** `trust` · **day window:** 88–0 (0 max means unbounded in the current Core contract).

**Existing discovery prose (68 words / 362 characters):**

> The main water pump dies at noon. By evening, the tanks are below a quarter. People are rationing spit. You know what's wrong — the impeller gasket. You've been meaning to mention it for weeks. You could fix it in an hour. But an hour of crisis is also an hour of leverage. The shelter committee is already arguing about who's to blame. Nobody has asked you yet.

**Existing choices, frozen for context:** 1. Fix it immediately and ask for nothing; 2. Wait until the crisis peaks, then fix it — and name your price; 3. Tell the committee you know the problem but need resources; 4. Share what you know so anyone can fix it

Adjacent chain records: predecessor `quest_moral_chain_betray_10`; successor `quest_moral_chain_betray_12`. Read them in the live catalog before a revision; do not borrow a neighboring record’s reveal, character knowledge, or outcome.
...
```json
{
  ...
  "choices": [
    {
      "empathy_delta": 2,
      "epitaph": "Fixed it in an hour. Asked for nothing. The debt is invisible and infinite.",
      "label": "Fix it immediately and ask for nothing",
      "moral_delta": 6,
      "outcome_text": "You walk to the pump house, pull the gasket, replace it with the spare you've been carrying. Water flows within the hour. People cheer. You wave it off. The committee never knows how close the gasket was to failing completely. You've saved them and asked for nothing. The debt is invisible and infinite.",
      "set_flag": "flag_repaired_infrastructure"
    },
    {
      "empathy_delta": 0,
      "epitaph": "Waited until the tanks hit ten percent. Named my price. They agreed. The silence while I waited was the loudest thing I've ever heard.",
      "label": "Wait until the crisis peaks, then fix it — and name your price",
      "moral_delta": -15,
      "outcome_text": "You wait. The tanks drop to ten percent. Fights break out at the tap. At the darkest hour, you stand up and say: 'I can fix it. But I want the supply committee chair.' They look at you. They look at their children. They agree. You fix the pump in forty minutes. The silence while you worked was the loudest thing you've ever heard."
    },
    {
      "empathy_delta": 0,
      "epitaph": "Described the problem. Fixed it with an audience. A transaction, not a tragedy.",
      "label": "Tell the committee you know the problem but need resources",
      "moral_delta": -3,
      "outcome_text": "You describe the gasket issue and request two people and four hours. The committee grants it. You fix the pump with an audience. Everyone sees the work. Everyone knows your name. It's not as clean as doing it quietly, but it's not as cruel as waiting. A transaction, not a tragedy."
    },
    {
      "empathy_delta": 1,
      "epitaph": "Wrote down the repair steps. Made myself unnecessary. The water runs.",
      "label": "Share what you know so anyone can fix it",
      "moral_delta": 4,
      "outcome_text": "You write down the repair procedure — gasket location, replacement steps, torque specifications — and post it at the pump house. The shelter's mechanic follows your instructions and has the water running by morning. You've made yourself unnecessary. There's a cost to that, but the water runs."
    }
  ],
  "display_name": "The Crisis Gambit",
  "id": "quest_moral_chain_betray_11",
...
- Compare before/after JSON, proving only `quest_moral_chain_betray_11.discovery` changed; assert all IDs, array order, gates, trigger/category/place, choice payloads, flags, outcomes, and score deltas are unchanged.
