# Subject Plan 179.43 — The Broker's Game: branching-quest discovery prose
...
Review one field—`discovery`—for selector `quest_moral_chain_betray_05` in `Assets/StreamingAssets/Data/moral_choice_quests_branching.json`. Current depth: 69 words / 414 characters. The aim is to clarify the authored encounter that players see when this moral-choice quest is offered. The plan does not ask for a longer passage; retain the current text if a revision would add no useful information.
...
This is the Betray branch record **The Broker's Game**. The branch identity and actual situation are source facts; do not use this pass to rewrite the larger quest arc.

## Verified premise and source evidence

- **VERIFIED (2026-09-24):** `quest_moral_chain_betray_05` is present exactly once in the current `schema_version: 1` `quests` array, which contains 100 entries.
...
**Trigger:** Two groups both trust you as a neutral mediator. You can sell each side's position to the other.

**Place:** `loc_water_station` · **category:** `trust` · **day window:** 42–0 (0 max means unbounded in the current Core contract).

**Existing discovery prose (69 words / 414 characters):**

> The water station and the knowledge keeper's camp both want the same thing: the seed bank in the abandoned homesteads to the south. Both send you as their intermediary — both because you're trusted, both because they think you favor them. The water station offers trade goods for intelligence on the camp's negotiation limits. The knowledge keepers offer the same. You could play both sides, or play them straight.

**Existing choices, frozen for context:** 1. Negotiate an honest split between both groups; 2. Sell each side's limits to the other and profit from both; 3. Sabotage the talks so you can take the seed bank alone; 4. Withdraw from mediation entirely

Adjacent chain records: predecessor `quest_moral_chain_betray_04`; successor `quest_moral_chain_betray_06`. Read them in the live catalog before a revision; do not borrow a neighboring record’s reveal, character knowledge, or outcome.
...
```json
{
  ...
  "choices": [
    {
      "empathy_delta": 2,
      "epitaph": "Negotiated the split. Both sides grumbled. No blood. Fairness opens doors.",
      "label": "Negotiate an honest split between both groups",
      "moral_delta": 8,
      "outcome_text": "You lay the maps on the table. The water station takes the upper fields; the knowledge keepers take the root cellars. Both sides grumble. Neither side is satisfied. But the seed bank is divided without blood, and you walk away with a reputation for fairness that will open doors for months."
    },
    {
      "empathy_delta": 0,
      "epitaph": "Sold both sides to each other. Took a cut from each. They both left feeling cheated.",
      "label": "Sell each side's limits to the other and profit from both",
      "moral_delta": -12,
      "outcome_text": "You tell the water station the keepers will accept less. You tell the keepers the station is desperate. Both deals close in your favor — you take a mediator's cut from each side that nobody else would have approved. The seed bank is divided, but both groups leave feeling cheated. They don't know it was you. Not yet."
    },
    {
      "empathy_delta": 0,
      "epitaph": "Burned the talks. Took the seed bank alone. A king's ransom in calories. Two alliances in ashes.",
      "label": "Sabotage the talks so you can take the seed bank alone",
      "moral_delta": -18,
      "outcome_text": "You arrange for the talks to collapse — a leaked insult, a missed deadline, a forged message. Both groups retreat, furious at each other. You go to the abandoned homesteads alone at dawn and take the entire seed bank. It's a king's ransom in calories. The two groups don't speak to each other for months. They don't speak to you either.",
      "set_flag": "flag_broke_treaty"
    },
    {
      "empathy_delta": 0,
      "epitaph": "Walked away from both. The talks dragged on. My name wasn't on any of it.",
      "label": "Withdraw from mediation entirely",
      "moral_delta": 0,
      "outcome_text": "You tell both groups you can't serve two masters. They find other intermediaries. The talks drag on for weeks and produce a worse deal for everyone. But your name isn't on any of it. Sometimes the smartest play is not playing."
    }
  ],
  "display_name": "The Broker's Game",
  "id": "quest_moral_chain_betray_05",
...
- Compare before/after JSON, proving only `quest_moral_chain_betray_05.discovery` changed; assert all IDs, array order, gates, trigger/category/place, choice payloads, flags, outcomes, and score deltas are unchanged.
