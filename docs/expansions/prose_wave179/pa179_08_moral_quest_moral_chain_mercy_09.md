# Subject Plan 179.08 — The Cost Collector: branching-quest discovery prose
...
This is the Mercy branch record **The Cost Collector**. The branch identity and actual situation are source facts; do not use this pass to rewrite the larger quest arc.

## Verified premise and source evidence

- **VERIFIED (2026-09-24):** `quest_moral_chain_mercy_09` is present exactly once in the current `schema_version: 1` `quests` array, which contains 100 entries.
...
**Trigger:** Someone you helped months ago returns with a group — and a demand.

**Place:** `loc_shelter_gate` · **category:** `trust` · **day window:** 95–0 (0 max means unbounded in the current Core contract).

**Existing discovery prose (56 words / 324 characters):**

> The man whose leg you splinted on the east road stands at your gate with twelve armed followers. He's not threatening — not exactly. 'You built something here,' he says. 'Something good. I want in. Not as a guest. As a partner. Equal say, equal share.' His twelve are capable, well-fed, and watching your response carefully.

**Existing choices, frozen for context:** 1. Welcome them as partners — strength in numbers; 2. Offer residency but not equal governance; 3. Decline — ask them to build their own camp under your patrol umbrella; 4. Turn them away — twelve armed strangers is too many

Adjacent chain records: predecessor `quest_moral_chain_mercy_08`; successor `quest_moral_chain_mercy_10`. Read them in the live catalog before a revision; do not borrow a neighboring record’s reveal, character knowledge, or outcome.
...
```json
{
  ...
  "choices": [
    {
      "empathy_delta": 1,
      "epitaph": "Shared the table with twelve new partners. Doubled the capability. Doubled the politics.",
      "label": "Welcome them as partners — strength in numbers",
      "moral_delta": 8,
      "outcome_text": "You share the table. His twelve integrate over the following month — slowly, carefully, watching each other learn the rhythms. The shelter doubles in capability. The politics double too."
    },
    {
      "empathy_delta": 0,
      "epitaph": "Offered residency without governance. Six stayed. Six moved on. Six is plenty.",
      "label": "Offer residency but not equal governance",
      "moral_delta": 5,
      "outcome_text": "You make the terms clear: shelter, food, work — but governance stays as it is. He considers it. His twelve discuss it. Half stay. Half move on. Six capable people is still six capable people."
    },
    {
      "empathy_delta": 0,
      "epitaph": "Pointed them south to build their own. Under my patrol, their own rules. A new camp in the corridor.",
      "label": "Decline — ask them to build their own camp under your patrol umbrella",
      "moral_delta": 3,
      "outcome_text": "You point them to cleared ground a kilometer south. Under your patrol. Separate governance. He nods — it's fair, and he knows it. Within a month there's a new camp in the corridor. Allied, not absorbed."
    },
    {
      "empathy_delta": 0,
      "epitaph": "Closed the gate on twelve capable people. He looked at me like a broken promise. They went east.",
      "label": "Turn them away — twelve armed strangers is too many",
      "moral_delta": -5,
      "outcome_text": "You close the gate. He doesn't argue. He looks at you the way someone looks at a promise that wasn't made but was understood. His twelve turn east. Another camp gets what you refused."
    }
  ],
  "display_name": "The Cost Collector",
  "id": "quest_moral_chain_mercy_09",
...
- Compare before/after JSON, proving only `quest_moral_chain_mercy_09.discovery` changed; assert all IDs, array order, gates, trigger/category/place, choice payloads, flags, outcomes, and score deltas are unchanged.
