# Subject Plan 179.39 — The Empty Promise: branching-quest discovery prose
...
Review one field—`discovery`—for selector `quest_moral_chain_betray_01` in `Assets/StreamingAssets/Data/moral_choice_quests_branching.json`. Current depth: 50 words / 280 characters. The aim is to clarify the authored encounter that players see when this moral-choice quest is offered. The plan does not ask for a longer passage; retain the current text if a revision would add no useful information.
...
This is the Betray branch record **The Empty Promise**. The branch identity and actual situation are source facts; do not use this pass to rewrite the larger quest arc.

## Verified premise and source evidence

- **VERIFIED (2026-09-24):** `quest_moral_chain_betray_01` is present exactly once in the current `schema_version: 1` `quests` array, which contains 100 entries.
...
**Trigger:** You promised to share medicine with a sick neighbor. The medicine is scarce now.

**Place:** `loc_shelter_gate` · **category:** `share` · **day window:** 10–0 (0 max means unbounded in the current Core contract).

**Existing discovery prose (50 words / 280 characters):**

> Maren's boy has the lung-rot. You told her three days ago, standing at her door, that you'd bring antibiotics when your next ration cycle opened. Now the cycle opened. You have four pills. Your own people have two coughing. The shelter medic looked at you hard when you took them.

**Existing choices, frozen for context:** 1. Deliver the pills as promised; 2. Keep the pills for your own people; 3. Trade the pills to the peacekeepers for favors; 4. Give half — keep the rest

Adjacent chain records: predecessor `none (chain opening)`; successor `quest_moral_chain_betray_02`. Read them in the live catalog before a revision; do not borrow a neighboring record’s reveal, character knowledge, or outcome.
...
```json
{
  ...
  "choices": [
    {
      "empathy_delta": 2,
      "epitaph": "Kept my word. Four pills for a child I barely know. The medic still won't look at me.",
      "label": "Deliver the pills as promised",
      "moral_delta": 8,
      "outcome_text": "You walk to Maren's door in the grey light. She doesn't cry — she's past that. She takes the pills and counts them against her palm. The boy will live. Your shelter's medic doesn't speak to you for two days."
    },
    {
      "empathy_delta": 0,
      "epitaph": "Kept the pills. The boy died on day three. I was busy. I was always busy.",
      "label": "Keep the pills for your own people",
      "moral_delta": -8,
      "outcome_text": "You tell yourself the arithmetic: your people first. The coughing stops by morning. When Maren's boy dies three days later, you don't attend the burial. You were busy. You were always busy.",
      "set_flag": "flag_hoarded_medicine"
    },
    {
      "empathy_delta": 0,
      "epitaph": "Traded antibiotics for a map. The commander didn't count them. I did.",
      "label": "Trade the pills to the peacekeepers for favors",
      "moral_delta": -12,
      "outcome_text": "The outpost commander takes the pills without counting. In return, you get a marked map — supply caches along the eastern road. Maren's boy dies. The map is worth more than one child. You tell yourself that at night, and almost believe it."
    },
    {
      "empathy_delta": 0,
      "epitaph": "Gave her two. Kept two. Enough to slow it. Not enough to stop it.",
      "label": "Give half — keep the rest",
      "moral_delta": 0,
      "outcome_text": "Two pills. Maren looks at your hand and does the math. She takes them without a word. It's enough to slow the rot, not stop it. The boy survives another week. You've bought time for everyone and certainty for no one."
    }
  ],
  "display_name": "The Empty Promise",
  "id": "quest_moral_chain_betray_01",
...
- Compare before/after JSON, proving only `quest_moral_chain_betray_01.discovery` changed; assert all IDs, array order, gates, trigger/category/place, choice payloads, flags, outcomes, and score deltas are unchanged.
