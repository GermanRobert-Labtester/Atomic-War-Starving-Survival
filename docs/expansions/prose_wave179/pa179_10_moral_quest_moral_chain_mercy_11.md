# Subject Plan 179.10 — The Shelter Vote: branching-quest discovery prose
...
Review one field—`discovery`—for selector `quest_moral_chain_mercy_11` in `Assets/StreamingAssets/Data/moral_choice_quests_branching.json`. Current depth: 47 words / 303 characters. The aim is to clarify the authored encounter that players see when this moral-choice quest is offered. The plan does not ask for a longer passage; retain the current text if a revision would add no useful information.
...
This is the Mercy branch record **The Shelter Vote**. The branch identity and actual situation are source facts; do not use this pass to rewrite the larger quest arc.

## Verified premise and source evidence

- **VERIFIED (2026-09-24):** `quest_moral_chain_mercy_11` is present exactly once in the current `schema_version: 1` `quests` array, which contains 100 entries.
...
**Trigger:** Your shelter votes to expel a family accused of hoarding. You have the deciding vote.

**Place:** `loc_shelter_meeting` · **category:** `trust` · **day window:** 115–0 (0 max means unbounded in the current Core contract).

**Existing discovery prose (47 words / 303 characters):**

> The Brennan family — father, mother, teenage son — stands accused of hiding food in their quarters. The evidence is circumstantial: extra weight, closed doors, a neighbor's suspicion. The vote is split. Yours is the deciding voice. Expulsion means the wasteland. Staying means doubt infects the shelter.

**Existing choices, frozen for context:** 1. Vote to keep them — investigate properly first; 2. Vote to keep them with strict monitoring; 3. Abstain — refuse to cast the deciding vote; 4. Vote to expel — the shelter's trust must be maintained

Adjacent chain records: predecessor `quest_moral_chain_mercy_10`; successor `quest_moral_chain_mercy_12`. Read them in the live catalog before a revision; do not borrow a neighboring record’s reveal, character knowledge, or outcome.
...
```json
{
  ...
  "choices": [
    {
      "empathy_delta": 2,
      "epitaph": "Voted to investigate before expelling. Found a jar of peaches. Grief, not hoarding. Shared at the next meal.",
      "label": "Vote to keep them — investigate properly first",
      "moral_delta": 10,
      "outcome_text": "You buy them time. The investigation finds a jar of preserved peaches — not hoarding, grief. The mother preserved them on her daughter's birthday. The daughter didn't survive the exchange. The jar is shared at the next meal. Nobody mentions it again."
    },
    {
      "empathy_delta": 1,
      "epitaph": "Kept the family under monitoring. Suspicion faded. Trust didn't fully return.",
      "label": "Vote to keep them with strict monitoring",
      "moral_delta": 5,
      "outcome_text": "They stay, under watch. The suspicion fades when no evidence surfaces. The family never quite trusts the shelter again. But they survive."
    },
    {
      "empathy_delta": 0,
      "epitaph": "Abstained from the vote. They stayed by default. Everyone noticed the dodge.",
      "label": "Abstain — refuse to cast the deciding vote",
      "moral_delta": 0,
      "outcome_text": "You abstain. The vote fails by default — they stay. But everyone knows you dodged it. The family stays. The trust doesn't."
    },
    {
      "empathy_delta": 0,
      "epitaph": "Voted to expel the family. They left carrying a jar of peaches. I didn't know what it was until later.",
      "label": "Vote to expel — the shelter's trust must be maintained",
      "moral_delta": -8,
      "outcome_text": "You vote out. The Brennan family packs in silence. The son looks at you once — a look that files itself away for later. They walk into the wasteland with a jar of peaches nobody searched for."
    }
  ],
  "display_name": "The Shelter Vote",
  "id": "quest_moral_chain_mercy_11",
...
- Compare before/after JSON, proving only `quest_moral_chain_mercy_11.discovery` changed; assert all IDs, array order, gates, trigger/category/place, choice payloads, flags, outcomes, and score deltas are unchanged.
