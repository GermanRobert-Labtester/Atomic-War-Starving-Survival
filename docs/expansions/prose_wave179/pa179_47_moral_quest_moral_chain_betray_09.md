# Subject Plan 179.47 — The Example: branching-quest discovery prose
...
Review one field—`discovery`—for selector `quest_moral_chain_betray_09` in `Assets/StreamingAssets/Data/moral_choice_quests_branching.json`. Current depth: 67 words / 386 characters. The aim is to clarify the authored encounter that players see when this moral-choice quest is offered. The plan does not ask for a longer passage; retain the current text if a revision would add no useful information.

**Editorial question:** Keep the competing account and the evidence gaps legible. Do not convert suspicion into proof or settle responsibility before the player chooses. Where consequences involve punishment, keep the prose humane and specific; do not make violence decorative or pre-commit the branch.

This is the Betray branch record **The Example**. The branch identity and actual situation are source facts; do not use this pass to rewrite the larger quest arc.

## Verified premise and source evidence

- **VERIFIED (2026-09-24):** `quest_moral_chain_betray_09` is present exactly once in the current `schema_version: 1` `quests` array, which contains 100 entries.
...
**Trigger:** Someone keeps breaking curfew. The shelter needs discipline. You can make an example — or show mercy.

**Place:** `loc_shelter_perimeter` · **category:** `trust` · **day window:** 72–0 (0 max means unbounded in the current Core contract).

**Existing discovery prose (67 words / 386 characters):**

> Bray has broken curfew three times this week. She says she's checking on her brother's grave outside the perimeter. The curfew exists because of the night raids — two people died last month walking after dark. The shelter is watching. They want to see what you do. Mercy looks like weakness. Severity looks like strength. The truth is somewhere in the middle and nobody wants the truth.

**Existing choices, frozen for context:** 1. Escort Bray to her brother's grave yourself; 2. Expel Bray from the shelter; 3. Publicly demote Bray and assign her the worst duties; 4. Make an exception — let Bray visit with an armed escort

Adjacent chain records: predecessor `quest_moral_chain_betray_08`; successor `quest_moral_chain_betray_10`. Read them in the live catalog before a revision; do not borrow a neighboring record’s reveal, character knowledge, or outcome.

## Editorial treatment

Keep the competing account and the evidence gaps legible. Do not convert suspicion into proof or settle responsibility before the player chooses. Where consequences involve punishment, keep the prose humane and specific; do not make violence decorative or pre-commit the branch. The discovery text should set the decision in motion, not answer it. Keep its relation to the separate `trigger` field clear: avoid restating the trigger verbatim when the current text already develops it, and avoid treating a UI heading as new diegetic speech. Use physical and social details supported by this record’s setting. Prefer one precise observation, action, or pressure point over a run of interchangeable bleak images.
...
```json
{
  ...
  "choices": [
    {
      "empathy_delta": 3,
      "epitaph": "Walked her to the grave. A child's drawing under stones. The rule held. So did she.",
      "label": "Escort Bray to her brother's grave yourself",
      "moral_delta": 8,
      "outcome_text": "You go with her at first light. The grave is small — a child's drawing weighted down with stones. You stand there while she cries. On the walk back, you talk about the curfew. She understands. The shelter sees you came back with her, alive, and the rule holds. But the humanity holds too."
    },
    {
      "empathy_delta": 0,
      "epitaph": "Expelled Bray. Her pack was almost empty. The curfew held perfectly after that.",
      "label": "Expel Bray from the shelter",
      "moral_delta": -14,
      "outcome_text": "You announce the expulsion at the morning briefing. Bray doesn't argue. She picks up her pack — it's light, almost empty — and walks through the gate. Nobody follows. The curfew holds perfectly after that. The shelter is quiet in a way that isn't peace. It's the quiet of a held breath.",
      "set_flag": "flag_expelled_survivor"
    },
    {
      "empathy_delta": 0,
      "epitaph": "Demoted Bray publicly. Latrine duty. She stopped asking about her brother's grave.",
      "label": "Publicly demote Bray and assign her the worst duties",
      "moral_delta": -8,
      "outcome_text": "You strip Bray of her ration priority and assign her to the latrine detail. She works in silence. The others watch. The message is clear: break the rules, suffer publicly. Bray's brother's grave goes unvisited. She stops asking. The shelter is disciplined. The shelter is afraid."
    },
    {
      "empathy_delta": 1,
      "epitaph": "Made an exception. Cost a guard each evening. Exceptions breed exceptions.",
      "label": "Make an exception — let Bray visit with an armed escort",
      "moral_delta": 2,
      "outcome_text": "You assign a guard to accompany Bray to the grave each evening. It costs a person from the night watch. Other families hear about the arrangement and want the same. Exceptions breed exceptions. But Bray sleeps better, and the guard reports the eastern road is clear."
    }
  ],
  "display_name": "The Example",
  "id": "quest_moral_chain_betray_09",
...
- Compare before/after JSON, proving only `quest_moral_chain_betray_09.discovery` changed; assert all IDs, array order, gates, trigger/category/place, choice payloads, flags, outcomes, and score deltas are unchanged.
