# Subject Plan 176.24 — Field Hospital 7: Full Tape: focused event prose review
...
Review the exact event `narrative_cassette_field_hospital_7_complete` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 62 whitespace-delimited words (345 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Field Hospital 7: Full Tape` and its exact current field shape is `bodyText, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The tapes from Field Hospital Seven play through in order. An orderly counts cots, argues an X into a star on a transfer list, and records a night shift by generator arithmetic. The last tape is left on the tag board: forty-one names, treatment given, treatment owed, and an instruction to send help where the tags are, not where the lights are.

- **VERIFIED non-prose snapshot:** `{"id": "narrative_cassette_field_hospital_7_complete", "minDay": 18, "title": "Field Hospital 7: Full Tape", "weight": 1}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
