# Subject Plan 176.05 — Before The Walkout: focused event prose review
...
Review the exact event `escalation_walkout_conditions` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 57 whitespace-delimited words (317 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Before The Walkout` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
...
- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "set_conditions_pencil", "effects": [{"setWorldFlag": "friction_walkout_conditions_set", "worldFlagValue": true}], "moraleDelta": -1.0, "text": "Set the conditions in the binder. The doorway follows the binder."}], "conditions": {"RequireWorldFlag": "friction_work_shirker_accusation"}, "id": "escalation_walkout_conditions", "minDay": 37, "title": "Before The Walkout", "weight": 1.0}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
