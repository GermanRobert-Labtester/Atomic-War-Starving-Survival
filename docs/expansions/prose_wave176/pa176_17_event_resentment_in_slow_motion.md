# Subject Plan 176.17 — Resentment In Slow Motion: focused event prose review
...
Review the exact event `ration_resentment_kindling` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 60 whitespace-delimited words (338 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Resentment In Slow Motion` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The pencil wrote four kinds of resentment lines this week. The first was at the scoop. The second was at the sick list. The third was at the walking-out procession. The fourth is at the corridor clock. The resentment has not crossed 0.70 in any line. The resentment is below the threshold. The resentment is below the threshold by design.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "mediate_quietly", "effects": [{"setWorldFlag": "ration_resentment_mediated", "worldFlagValue": true}], "moraleDelta": 2.0, "text": "Pull the four pencils to one meal, alone. The resentment will name itself or it will not."}, {"choiceId": "let_it_run", "effects": [{"setWorldFlag": "ration_resentment_allowed", "worldFlagValue": true}], "moraleDelta": -1.0, "text": "Let the resentment settle below the threshold. Below the threshold is where ration conflict is supposed to live."}], "conditions": {"RequireResentmentBuffer": true}, "id": "ration_resentment_kindling", "minDay": 45, "title": "Resentment In Slow Motion", "weight": 1.1}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
