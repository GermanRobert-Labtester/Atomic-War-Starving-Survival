# Subject Plan 176.02 — First Assist, Then Closure: focused event prose review
...
Review the exact event `apprenticeship_completion_field_dressing` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 54 whitespace-delimited words (292 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `First Assist, Then Closure` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The apprentice closed a wound alone today, under Sasha's eye. The thread did not break, the dressing held, the patient walked out to the cot without wincing. Sasha asked the apprentice to write the time and the line in the log. The apprentice wrote it as if she had been writing it for years.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "document_lesson", "effects": [{"setWorldFlag": "app_field_dressing_complete", "worldFlagValue": true}], "moraleDelta": 3.0, "text": "File it in the medic log and the apprenticeship record."}, {"choiceId": "stage_second_closure", "effects": [{"setWorldFlag": "app_field_dressing_complete", "worldFlagValue": true}], "moraleDelta": 1.0, "text": "Send the apprentice to the second closure the next shift."}], "conditions": {"RequireChildCohort": true}, "id": "apprenticeship_completion_field_dressing", "minDay": 30, "title": "First Assist, Then Closure", "weight": 1.4}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
