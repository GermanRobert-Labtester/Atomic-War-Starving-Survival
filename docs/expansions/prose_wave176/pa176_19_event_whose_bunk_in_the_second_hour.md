# Subject Plan 176.19 — Whose Bunk In The Second Hour: focused event prose review
...
Review the exact event `friction_stealing_sleep_rotation` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 61 whitespace-delimited words (327 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Whose Bunk In The Second Hour` and its exact current field shape is `bodyText, choices, id, minDay, title, weight`.
- **VERIFIED source prose:**

> Twice this week someone has been sleeping in the second bunk of the south corridor during the second hour of second watch, and the bunk does not belong to their rotation. The person it belongs to has found it warm both times, and the warmth is the evidence, and the evidence is being discussed in low voices at the water point.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "confront_visitor", "effects": [{"setWorldFlag": "friction_visitor_assigned", "worldFlagValue": true}], "moraleDelta": -3.0, "text": "Hand the visitor their own bunk assignation. The corridors are not for visitors."}, {"choiceId": "silent_permit", "effects": [{"setWorldFlag": "friction_second_bunk_permitted", "worldFlagValue": true}], "moraleDelta": -1.0, "text": "Say nothing. The bunk has chosen its guest."}], "id": "friction_stealing_sleep_rotation", "minDay": 12, "title": "Whose Bunk In The Second Hour", "weight": 0.9}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
