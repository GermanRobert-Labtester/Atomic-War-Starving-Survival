# Subject Plan 176.36 — The Mechanical Hour: First Lesson: focused event prose review
...
Review the exact event `schooling_mechanics_first_lesson` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 67 whitespace-delimited words (348 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The Mechanical Hour: First Lesson` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The forge daughter shows the children the coke and asks them to listen to it the way she listens to it. The coke makes a sound the children cannot yet hear. Tomorrow it will be the same sound and the children will have heard it once. The third sound is when the children know what they are listening to. The coke does not change; the listener does.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "return_to_basement", "effects": [{"scheduleEventId": "schooling_mechanics_first_lesson", "scheduleOnDay": 33}], "moraleDelta": 2.0, "text": "Take the children to the basement pump for the second lesson."}], "conditions": {"RequireWorldFlag": "schooling_path", "WorldFlagValue": "mechanics"}, "id": "schooling_mechanics_first_lesson", "minDay": 30, "title": "The Mechanical Hour: First Lesson", "weight": 1.0}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
