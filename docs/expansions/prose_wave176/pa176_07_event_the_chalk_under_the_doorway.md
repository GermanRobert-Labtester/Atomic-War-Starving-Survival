# Subject Plan 176.07 — The Chalk Under The Doorway: focused event prose review
...
Review the exact event `escalation_challenge_to_leadership` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 58 whitespace-delimited words (320 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The Chalk Under The Doorway` and its exact current field shape is `bodyText, choices, id, minDay, title, weight`.
- **VERIFIED source prose:**

> There is chalk under the leadership doorway. The chalk says nothing in particular: just the date of the next council meeting, underlined twice, which is not a threat, which is the shape of a threat. Forty-eight hours to the meeting. The chalk has not been picked up, and nobody is going to pick it up before the meeting.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "acknowledge_the_chalk", "effects": [{"setWorldFlag": "escalation_challenge_acknowledged", "worldFlagValue": true}], "moraleDelta": -2.0, "text": "Acknowledge the chalk at the next council sheet. The doorway is one of the surroundings."}, {"choiceId": "step_down_quietly", "effects": [{"setWorldFlag": "escalation_leadership_stepdown", "worldFlagValue": true}], "moraleDelta": 5.0, "text": "Step down before the council sheet. Leadership returns the chalk to its owner."}], "id": "escalation_challenge_to_leadership", "minDay": 80, "title": "The Chalk Under The Doorway", "weight": 1.4}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
