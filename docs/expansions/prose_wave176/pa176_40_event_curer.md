# Subject Plan 176.40 — Curer: focused event prose review
...
Review the exact event `narrative_preservationist_milestone_1` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 68 whitespace-delimited words (430 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Curer` and its exact current field shape is `bodyText, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The first salted meat rack hung above the boiler in the curing room fills the sub-level with a smell that is part salt, part smoke, part something more biological that you learn to ignore. Cold-smoking over compressed peat chips takes eight hours per batch and produces a product that lasts fourteen days without refrigeration. Fourteen days is two expeditions. Two expeditions is the difference between empty-handed and supplied.

- **VERIFIED non-prose snapshot:** `{"id": "narrative_preservationist_milestone_1", "minDay": 10, "title": "Curer", "weight": 1}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
