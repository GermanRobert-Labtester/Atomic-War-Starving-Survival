# Subject Plan 176.25 — A Hundred and Four: focused event prose review
...
Review the exact event `orphan_adoption_archive` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 62 whitespace-delimited words (358 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `A Hundred and Four` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The municipal archive adopted four orphans when the bunker census crossed the binder threshold for the first time. The newest intake was logged under the next four open plot numbers. The binder was three days late because of the intake cycle, and the older children carried the new binders to the new shelf before the new children learned which shelf it was.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "log_archive_intake", "effects": [{"setWorldFlag": "orphan_home", "worldFlagValue": "archive"}], "moraleDelta": 4.0, "text": "Log the intake in the binder. The binder is on the new shelf."}], "conditions": {"RequireCasualtyOrphan": true}, "id": "orphan_adoption_archive", "minDay": 60, "title": "A Hundred and Four", "weight": 1.5}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
