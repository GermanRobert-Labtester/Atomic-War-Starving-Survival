# Subject Plan 176.42 — The Quarantine Tapes: Full Tape: focused event prose review
...
Review the exact event `narrative_cassette_quarantine_tapes_complete` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 69 whitespace-delimited words (380 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The Quarantine Tapes: Full Tape` and its exact current field shape is `bodyText, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The quarantine tapes play through in order. A case definition written in pencil, one door in and one door out, visitors replaced by notes written small on wristbands. On the final tape a doctor releases patients on time and a signature because the test kits left with a convoy that never returned. It was never the rules that failed; it was running out of the means to check them.

- **VERIFIED non-prose snapshot:** `{"id": "narrative_cassette_quarantine_tapes_complete", "minDay": 18, "title": "The Quarantine Tapes: Full Tape", "weight": 1}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
