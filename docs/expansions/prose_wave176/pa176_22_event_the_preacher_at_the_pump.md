# Subject Plan 176.22 — The Preacher at the Pump: focused event prose review
...
Review the exact event `event_belief_charismatic_voice` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 62 whitespace-delimited words (346 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The Preacher at the Pump` and its exact current field shape is `bodyText, choices, id, minDay, title, weight`.
- **VERIFIED source prose:**

> A survivor has started speaking by the water filtration bay every day, not preaching, talking, at first. The talk is about the pumps and the soul in the same sentence, and the crowd grows every shift change, and the crowd is the part leadership has started counting. The voice has not asked for anything yet. The yet is the part leadership hears.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "choice_grant_informal_standing", "effects": [{"setWorldFlag": "charismatic_speaker_consulted", "worldFlagValue": true}], "moraleDelta": 2.5, "text": "Recognize their moral influence and consult them on shelter morale decisions."}, {"choiceId": "choice_warn_against_demagoguery", "effects": [{"setWorldFlag": "charismatic_speaker_reined_in", "worldFlagValue": true}], "moraleDelta": -1.0, "text": "Remind them that leadership is collective and orders flow through formal roster lines."}], "id": "event_belief_charismatic_voice", "minDay": 28, "title": "The Preacher at the Pump", "weight": 1.0}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
