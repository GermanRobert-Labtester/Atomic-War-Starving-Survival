# Subject Plan 176.04 — The Snoring Feud: focused event prose review
...
Review the exact event `friction_snoring_feud` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 55 whitespace-delimited words (315 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.

Editorial question: **Let the record's material details and pauses do the work. Keep speaker, sequence, and recorded claims faithful to the source; do not add a new broadcast, answer, or transmission event.** Work from the current record's facts and voice. Seek a material improvement in clarity, specificity, tone, or continuity. If the only available change would increase length, preserve the current text.

## Live source evidence

- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The Snoring Feud` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> Two adults in the north corridor have not spoken since the rations meeting. The argument is about the snoring. The snoring is about the bunks. The bunks are not movable. The bunkers are not un-snorable. The wall of the corridor has absorbed enough of the argument that the chalk budget is calling itself an apology.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "mediate_immediately", "effects": [{"setWorldFlag": "friction_snoring_feud_mediated", "worldFlagValue": true}, {"needDelta": 3.0, "targetNeed": "morale"}], "moraleDelta": 2.0, "text": "Sit between them at the next meal. Do not say anything. The argument ends when no one says anything."}, {"choiceId": "swap_bunks", "effects": [{"setWorldFlag": "friction_snoring_feud_swapped", "worldFlagValue": true}], "moraleDelta": -1.0, "text": "Reassign one adult to the spare cot in the storeroom. The snoring will be the storeroom's problem now."}], "conditions": {"RequireRoommateFriction": true}, "id": "friction_snoring_feud", "minDay": 14, "title": "The Snoring Feud", "weight": 1.0}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
