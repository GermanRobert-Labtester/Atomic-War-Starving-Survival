# Subject Plan 176.44 — The Signal in the Storm: focused event prose review
...
Review the exact event `event_belief_high_stakes_blindspot` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 70 whitespace-delimited words (372 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The Signal in the Storm` and its exact current field shape is `bodyText, choices, id, minDay, title, weight`.
- **VERIFIED source prose:**

> In the middle of the fallout storm, with the rad counts in the red and the shelter sealed, the adherents say they heard a distress signal from the valley, the holy valley, the one the routes avoid, and they demand a sortie now, while the signal holds. The signal log shows the storm and nothing else. The log is being shown to them, and they are hearing the signal anyway.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "choice_refuse_storm_sortie", "effects": [{"setWorldFlag": "friction_spoken_name_blame", "worldFlagValue": true}], "moraleDelta": -1.0, "text": "Hard refusal. No scout will be sacrificed for an unverified prophecy."}, {"choiceId": "choice_prepare_shielded_scout", "effects": [{"setWorldFlag": "shielded_recon_scheduled", "worldFlagValue": true}], "moraleDelta": 1.0, "text": "Authorize an intensely shielded reconnaissance only after storm peak passes."}], "id": "event_belief_high_stakes_blindspot", "minDay": 40, "title": "The Signal in the Storm", "weight": 0.8}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
