# Subject Plan 176.33 — The Burned-Out Rifle: Disassembly: focused event prose review
...
Review the exact event `schooling_marksmanship_first_lesson` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 65 whitespace-delimited words (350 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The Burned-Out Rifle: Disassembly` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The unloaded rifle was put on the storeroom shelf in pieces, on a cloth, in good light, and the children stood around it in a ring. The armorer showed them how the bolt came out first. The bolt came out first. Then the spring. Then the action. The children took it apart without fumbling, because there were no surprises in it. There were only parts.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "do_reassembly_in_dim", "effects": [{"scheduleEventId": "schooling_marksmanship_first_lesson", "scheduleOnDay": 32}], "moraleDelta": -1.0, "text": "Reassemble in the dim. That is the lesson that decides whether the child owns the rifle or the rifle owns the child."}], "conditions": {"RequireWorldFlag": "schooling_path", "WorldFlagValue": "marksmanship"}, "id": "schooling_marksmanship_first_lesson", "minDay": 30, "title": "The Burned-Out Rifle: Disassembly", "weight": 1.0}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
