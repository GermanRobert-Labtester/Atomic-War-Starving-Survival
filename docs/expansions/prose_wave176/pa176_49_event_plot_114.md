# Subject Plan 176.49 — Plot 114: focused event prose review
...
Review the exact event `orphan_adoption_grange` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 71 whitespace-delimited words (379 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Plot 114` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> A casualty orphan was assigned, by plot number, to a farmer couple who still held the allotment waiting list from before. The plot was 114. It was a plum number on the list. The adults said the plum number killed the convoy that carried the child's old family and were immediately sorry for saying so. The child looked at the plum number and said nothing for the rest of the day.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "honour_list", "effects": [{"setWorldFlag": "orphan_home", "worldFlagValue": "allotment_114"}], "moraleDelta": 3.0, "text": "Keep the plot number on the allotment list, plum spelling and all."}], "conditions": {"RequireCasualtyOrphan": true}, "id": "orphan_adoption_grange", "minDay": 60, "title": "Plot 114", "weight": 1.6}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
