# Subject Plan 176.34 — Master Water Technician: focused event prose review
...
Review the exact event `narrative_trade_mastery_water_technician` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 66 whitespace-delimited words (386 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Master Water Technician` and its exact current field shape is `bodyText, id, minDay, title, weight`.
- **VERIFIED source prose:**

> Pure water from the tap in the sleeping quarters. Hot water from the solar-heated drum in the wash room on the days when there is enough sun. The water technician does not describe their work as remarkable. They describe it as solved. The problem of clean water in this shelter has been solved. Other problems remain. The water technician is already thinking about those other problems.

- **VERIFIED non-prose snapshot:** `{"id": "narrative_trade_mastery_water_technician", "minDay": 40, "title": "Master Water Technician", "weight": 1}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
