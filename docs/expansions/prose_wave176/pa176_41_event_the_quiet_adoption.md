# Subject Plan 176.41 — The Quiet Adoption: focused event prose review
...
Review the exact event `orphan_adoption_warmarms` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 68 whitespace-delimited words (362 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The Quiet Adoption` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> A casualty orphan from the winter convoy was folded into the bunker's census without ceremony. The adults who took the child in were not the biological parents of any child. The cot is shared now. The ration is shared now. There is no ceremony, only a place setting at the next meal. The bunker was kind about it in the way the bunker is kind about quiet things.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "warmarms_keep", "effects": [{"setWorldFlag": "orphan_home", "worldFlagValue": "warmarms"}], "moraleDelta": 4.0, "text": "Keep the casualty orphan with the warm-arms couple."}, {"choiceId": "warmarms_rotate", "effects": [{"setWorldFlag": "orphan_home", "worldFlagValue": "rotation"}], "moraleDelta": -2.0, "text": "Put the orphan on the council-sheet rotation instead."}], "conditions": {"RequireCasualtyOrphan": true}, "id": "orphan_adoption_warmarms", "minDay": 60, "title": "The Quiet Adoption", "weight": 1.8}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
