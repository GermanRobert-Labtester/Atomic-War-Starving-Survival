# Subject Plan 176.26 — The First Tin Disappears: focused event prose review
...
Review the exact event `ration_hoarded_tins_first_sign` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 62 whitespace-delimited words (323 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.

Editorial question: **Keep scarcity concrete and social without changing quantities, access, yield, or allocation. Let the human stakes sit beside the existing material fact rather than becoming a new economy rule.** Work from the current record's facts and voice. Seek a material improvement in clarity, specificity, tone, or continuity. If the only available change would increase length, preserve the current text.

## Live source evidence

- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The First Tin Disappears` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The first tin of sardines has gone from the back shelf, and the taking was deliberate: the tally knows, the shelf knows, and whoever took it knows that both can tell. It is the first tin that matters. The first tin decides whether there is a second one, and the shelter is going about its day, and the day is keeping count.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "log_first_tin_loss", "effects": [{"setWorldFlag": "ration_first_tin_logged", "worldFlagValue": true}], "moraleDelta": 1.0, "text": "Log the loss on the council sheet. The pencil is not for blaming. The pencil is for counting."}, {"choiceId": "share_the_pretend_first_tin", "effects": [{"setWorldFlag": "ration_tin_shared_response", "worldFlagValue": true}], "moraleDelta": 2.0, "text": "Share a tin on the next scoop. The imaginary first tin cannot be missed if the next tin is shared."}], "conditions": {"RequireRationUnevenAllocation": true}, "id": "ration_hoarded_tins_first_sign", "minDay": 35, "title": "The First Tin Disappears", "weight": 1.2}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
