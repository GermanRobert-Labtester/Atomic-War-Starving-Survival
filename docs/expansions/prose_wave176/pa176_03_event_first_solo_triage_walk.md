# Subject Plan 176.03 — First Solo Triage Walk: focused event prose review
...
Review the exact event `apprenticeship_completion_steady_hands` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 54 whitespace-delimited words (294 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `First Solo Triage Walk` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The understudy walked the patient-by-name triaged intake line at 03:00 with no patients in the corridor, so she had to imagine them. She imagined two of them. The protocol let her through. The triage line is not a skill you learn by accident. The triage line is a skill you learn by walking it.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "add_walk_letter", "effects": [{"setWorldFlag": "app_steady_hands_complete", "worldFlagValue": true}], "moraleDelta": 4.0, "text": "Put the understudy on the second-pass letter. She earned it."}], "conditions": {"RequireChildCohort": true}, "id": "apprenticeship_completion_steady_hands", "minDay": 40, "title": "First Solo Triage Walk", "weight": 1.2}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
