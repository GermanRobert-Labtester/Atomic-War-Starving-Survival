# Subject Plan 176.37 — First Stitches: The First Lesson: focused event prose review
...
Review the exact event `schooling_medicine_first_lesson` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 67 whitespace-delimited words (346 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `First Stitches: The First Lesson` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The medic teacher lays out the needle, the thread, the dressing, the saline, and the gloves on the infirmary floor. She says the names of each of them in the same order she will say them in the field. The children repeat the names back. Tomorrow one of the children will pick up a needle at the right end for the first time. The medic will watch.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "infuse_practice_dressing", "effects": [{"scheduleEventId": "schooling_medicine_first_lesson", "scheduleOnDay": 33}], "moraleDelta": 3.0, "text": "Hand the children the saline and the dressing for a practice run."}], "conditions": {"RequireWorldFlag": "schooling_path", "WorldFlagValue": "medicine"}, "id": "schooling_medicine_first_lesson", "minDay": 30, "title": "First Stitches: The First Lesson", "weight": 1.0}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
