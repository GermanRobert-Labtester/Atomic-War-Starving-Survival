# Subject Plan 176.18 — The Sick Get More: focused event prose review
...
Review the exact event `ration_sick_gets_more_dispute` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 60 whitespace-delimited words (317 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The Sick Get More` and its exact current field shape is `bodyText, choices, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The sick in the infirmary get extra, and the extra is written in the medic's pencil, and the dwellers' pencil did not get to agree. The extra is medic-grade and not negotiable at the scoop, and nobody has said it is negotiable, out loud. It is the first item on the next council sheet, which is where the saying goes.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "confirm_medic_extra", "effects": [{"setWorldFlag": "ration_medic_extra_canon", "worldFlagValue": true}], "moraleDelta": 3.0, "text": "Confirm the medic's extra on the council sheet. The infirmary is the infirmary."}, {"choiceId": "make_extra_auditable", "effects": [{"setWorldFlag": "ration_medic_extra_audit", "worldFlagValue": true}], "moraleDelta": -1.0, "text": "Audit the medic's extra in the next quarter. The pencil wants receipts."}], "id": "ration_sick_gets_more_dispute", "minDay": 40, "title": "The Sick Get More", "weight": 1.1}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
