# Subject Plan 176.06 — The Dry-Ration Loss: focused event prose review
...
Review the exact event `friction_ration_stolen_dry_rations` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 57 whitespace-delimited words (327 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The Dry-Ration Loss` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> Three tins of dry rations are gone from the back shelf, and the tally says three, and the shelf says three, and the count has not been reported yet because reporting it means counting everyone, and counting everyone means trusting no one. The shelter walks past the shelf without looking, which is the shelter's way of looking.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "lock_the_bulk_stores", "effects": [{"setWorldFlag": "faction_stores_locked", "worldFlagValue": true}], "moraleDelta": -3.0, "text": "Put a lock on the bulk store. The bunk that did not want lock will not say so."}, {"choiceId": "double_the_signal", "effects": [{"setWorldFlag": "ration_hoarded_tins_first_sign", "worldFlagValue": true}], "moraleDelta": 2.0, "text": "Tally twice a day for a week. The pencil was made for this."}], "conditions": {"RequireRationTheft": true}, "id": "friction_ration_stolen_dry_rations", "minDay": 30, "title": "The Dry-Ration Loss", "weight": 1.2}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
