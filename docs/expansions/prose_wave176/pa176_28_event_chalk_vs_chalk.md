# Subject Plan 176.28 — Chalk Vs Chalk: focused event prose review
...
Review the exact event `friction_two_sides_of_chalk` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 63 whitespace-delimited words (335 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Chalk Vs Chalk` and its exact current field shape is `bodyText, choices, id, minDay, title, weight`.
- **VERIFIED source prose:**

> Two sides of the corridor are arguing about a census number, which stall it belongs to, and the argument has taken to the walls: one side writes in pencil, one side writes in chalk. The writing is polite. The two sides have stopped speaking aloud for a week, and the wall does the talking now, and the wall gets more legible every day.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "compromise_pen_only", "effects": [{"setWorldFlag": "friction_chalk_pen_compromise", "worldFlagValue": true}], "moraleDelta": 3.0, "text": "Adopt pen-only on the census board. The chalk is for letters, not numbers."}, {"choiceId": "leave_pending", "effects": [{"setWorldFlag": "friction_chalk_dispute_pending", "worldFlagValue": true}], "moraleDelta": -1.0, "text": "Leave the dispute for the next council. The corridor will be patient."}], "id": "friction_two_sides_of_chalk", "minDay": 22, "title": "Chalk Vs Chalk", "weight": 1.0}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
