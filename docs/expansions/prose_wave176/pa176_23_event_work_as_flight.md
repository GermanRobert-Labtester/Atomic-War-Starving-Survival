# Subject Plan 176.23 — Work as Flight: focused event prose review
...
Review the exact event `event_grief_conflict_exhaustion_overwork` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 62 whitespace-delimited words (350 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.

Editorial question: **Keep disagreement legible in what people say and withhold. Preserve both sides' stated evidence and every choice consequence; do not settle a dispute the data leaves open.** Work from the current record's facts and voice. Seek a material improvement in clarity, specificity, tone, or continuity. If the only available change would increase length, preserve the current text.

## Live source evidence

- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Work as Flight` and its exact current field shape is `bodyText, choices, id, minDay, title, weight`.
- **VERIFIED source prose:**

> Since the partner died, the survivor has worked three double shifts in the hydroponic trench and refuses the bunk, and the hands are trembling now, and the brine tanks are open, and the walking past them is not getting steadier. The shift peers have started timing the passes, quietly, and the timing is the shelter's way of not waiting for the fall.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "choice_order_mandatory_bunk_rest", "effects": [{"setWorldFlag": "grief_exhaustion_intervened", "worldFlagValue": true}], "moraleDelta": 2.0, "text": "Order them off the roster and assign a companion to stay with them in the dormitory."}, {"choiceId": "choice_allow_light_duty", "effects": [{"setWorldFlag": "grief_light_duty_assigned", "worldFlagValue": true}], "moraleDelta": 1.0, "text": "Transition them to low-risk seated inventory tallying so they stay occupied."}], "id": "event_grief_conflict_exhaustion_overwork", "minDay": 8, "title": "Work as Flight", "weight": 1.2}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
