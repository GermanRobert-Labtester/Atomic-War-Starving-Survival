# Subject Plan 176.13 — Station 14: Full Tape: focused event prose review
...
Review the exact event `narrative_cassette_station_14_complete` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 59 whitespace-delimited words (350 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Station 14: Full Tape` and its exact current field shape is `bodyText, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The Station Fourteen tapes run in sequence. A broadcaster logs one strange minute in an ordinary schedule, then the network feed stops clean at 4:51, and the programme becomes whatever people need read to them: roads, water, the times tables. The final tape is technical. Keep the carrier up; check the mast first, the fuse second, the operator last.

- **VERIFIED non-prose snapshot:** `{"id": "narrative_cassette_station_14_complete", "minDay": 18, "title": "Station 14: Full Tape", "weight": 1}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
