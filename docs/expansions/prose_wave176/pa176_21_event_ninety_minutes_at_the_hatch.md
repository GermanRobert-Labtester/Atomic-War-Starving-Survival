# Subject Plan 176.21 — Ninety Minutes at the Hatch: focused event prose review
...
Review the exact event `apprenticeship_completion_watchful` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 62 whitespace-delimited words (355 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Ninety Minutes at the Hatch` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The hatch watcher's hands were shaking at minute four. By minute forty-five, they were not. The Geiger counter did not exceed a number the bunker accepts. The radio said nothing the bunker did not expect. The morning shift relieved at the appointed minute. The watcher stepped back into the bunker as if the post had never been held by anyone else before.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "extend_roster", "effects": [{"setWorldFlag": "app_watchful_complete", "worldFlagValue": true}], "moraleDelta": 5.0, "text": "Add the watcher to the regular hatch rotation, twice a week."}], "conditions": {"RequireChildCohort": true}, "id": "apprenticeship_completion_watchful", "minDay": 30, "title": "Ninety Minutes at the Hatch", "weight": 1.3}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
