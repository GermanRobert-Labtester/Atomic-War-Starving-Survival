# Subject Plan 176.43 — Master Surveyor: focused event prose review
...
Review the exact event `narrative_trade_mastery_surveyor` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 69 whitespace-delimited words (425 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Master Surveyor` and its exact current field shape is `bodyText, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The shelter's map library is forty-one sheets of hand-drawn cartography, each one indexed, each one dated, each one annotated with the hazard conditions on the day it was surveyed. The surveyor teaches expedition leaders to read the maps by terrain feature, not by memory. Every expedition leader who comes back alive is partly the surveyor's achievement. The surveyor does not claim that credit. The data claims it for them.

- **VERIFIED non-prose snapshot:** `{"id": "narrative_trade_mastery_surveyor", "minDay": 40, "title": "Master Surveyor", "weight": 1}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
