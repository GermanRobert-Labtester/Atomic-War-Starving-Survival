# Subject Plan 176.29 — The First Testimony: focused event prose review
...
Review the exact event `event_belief_first_conviction` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 64 whitespace-delimited words (369 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The First Testimony` and its exact current field shape is `bodyText, choices, id, minDay, title, weight`.
- **VERIFIED source prose:**

> Following a harrowing rad-hazard shift, a survivor gathers peers in the lower workshop. Holding up a charred iron fitting and a handful of grey grit, they speak not of despair, but of purpose: whether through the memory of the Ash Witnesses, the labor of the Rebuilders, or the listening watch of the radio dial, survival must be more than waiting for filters to choke.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "choice_support_testimony", "effects": [{"setWorldFlag": "belief_movement_encouraged", "worldFlagValue": true}], "moraleDelta": 3.0, "text": "Acknowledge their words. The bunker needs conviction to endure."}, {"choiceId": "choice_caution_testimony", "effects": [{"setWorldFlag": "belief_pragmatic_caution_set", "worldFlagValue": true}], "moraleDelta": 0.0, "text": "Urge focus on physical duties. Ideology must not distract from maintenance."}], "id": "event_belief_first_conviction", "minDay": 10, "title": "The First Testimony", "weight": 1.5}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
