# Subject Plan 176.31 — Master Radio Technician: focused event prose review
...
Review the exact event `narrative_trade_mastery_radio_technician` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 64 whitespace-delimited words (388 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Master Radio Technician` and its exact current field shape is `bodyText, id, minDay, title, weight`.
- **VERIFIED source prose:**

> Every distress call, weather shift, faction broadcast, and encrypted signal within range of this shelter now reaches the radio technician's log within minutes of transmission. The log is sixty pages long. The radio technician reads it back to themselves every morning. There is nothing in it they will miss. You cannot afford to miss a distress call when the signal only reaches you once.

- **VERIFIED non-prose snapshot:** `{"id": "narrative_trade_mastery_radio_technician", "minDay": 40, "title": "Master Radio Technician", "weight": 1}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
