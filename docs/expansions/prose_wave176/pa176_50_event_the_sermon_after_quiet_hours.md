# Subject Plan 176.50 — The Sermon After Quiet Hours: focused event prose review
...
Review the exact event `friction_ash_sermon_quiet_hours` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 72 whitespace-delimited words (394 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The Sermon After Quiet Hours` and its exact current field shape is `bodyText, choices, id, minDay, title, weight`.
- **VERIFIED source prose:**

> For three nights someone has been speaking the ash cant to the children after quiet hours, in the cold room. The cant is not in the curriculum, and the curriculum is one hour a day, and the children are listening longer than an hour. The teacher has noticed the children repeating the words during lessons. The teacher has not reported it yet, and the not-yet is starting to look like a choice.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "ban_after_hours", "effects": [{"setWorldFlag": "friction_ash_cant_banned", "worldFlagValue": true}], "moraleDelta": -2.0, "text": "Move the cold room door to require two signatures after quiet hours. The cant will have to wait."}, {"choiceId": "add_to_curriculum", "effects": [{"setWorldFlag": "friction_ash_cant_admitted", "worldFlagValue": true}], "moraleDelta": 4.0, "text": "Add the cant to the curriculum. The teacher's pencil will write it in."}], "id": "friction_ash_sermon_quiet_hours", "minDay": 28, "title": "The Sermon After Quiet Hours", "weight": 1.0}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
