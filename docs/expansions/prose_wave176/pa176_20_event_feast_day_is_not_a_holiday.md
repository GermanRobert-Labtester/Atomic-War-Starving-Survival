# Subject Plan 176.20 — Feast Day Is Not A Holiday: focused event prose review
...
Review the exact event `ration_feast_day_demand` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 61 whitespace-delimited words (331 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Feast Day Is Not A Holiday` and its exact current field shape is `bodyText, choices, id, minDay, title, weight`.
- **VERIFIED source prose:**

> Two adults have requested a feast scoop for the second anniversary of the bunker opening. The feast scoop is not in the schedule, and the schedule is why the request could be made at all, calmly, on paper. The request sits on the council sheet now, and the council sheet is the shelter's way of holding a feast without holding one.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "honor_feast_request", "effects": [{"setWorldFlag": "ration_feast_honored", "worldFlagValue": true}], "moraleDelta": 6.0, "text": "Honor the feast scoop. The pencil rounds; the bowl rounds; the calendar rounds."}, {"choiceId": "defer_feast_request", "effects": [{"setWorldFlag": "ration_feast_deferred", "worldFlagValue": true}], "moraleDelta": -2.0, "text": "Defer to next quarter's review. The schedule is the schedule."}], "id": "ration_feast_day_demand", "minDay": 80, "title": "Feast Day Is Not A Holiday", "weight": 1.0}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
