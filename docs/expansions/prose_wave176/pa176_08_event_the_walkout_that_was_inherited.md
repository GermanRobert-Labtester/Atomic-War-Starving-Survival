# Subject Plan 176.08 — The Walkout That Was Inherited: focused event prose review
...
Review the exact event `friction_inherited_walkout_threat` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 58 whitespace-delimited words (315 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `The Walkout That Was Inherited` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> A survivor has started saying the words walk out, to nobody in particular and to everybody at once, at meals, at the water point. The words came in with the convoy, where somebody else said them and meant them. The room goes quiet every time. The quiet is the threat's audience, and the threat is learning the room.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "let_them_walkout", "effects": [{"setWorldFlag": "escalation_walkout", "worldFlagValue": true}, {"scheduleEventId": "post_walkout_reconsider", "scheduleOnDay": 53}], "moraleDelta": -4.0, "text": "Open the door. They go when they need to go. They come back when they come back."}, {"choiceId": "call_a_council", "effects": [{"setWorldFlag": "friction_walkout_council_held", "worldFlagValue": true}], "moraleDelta": 2.0, "text": "Call the council. The pencil rolls. The threat rolls too."}], "conditions": {"RequireCohortAdoptionPending": true}, "id": "friction_inherited_walkout_threat", "minDay": 50, "title": "The Walkout That Was Inherited", "weight": 1.1}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
