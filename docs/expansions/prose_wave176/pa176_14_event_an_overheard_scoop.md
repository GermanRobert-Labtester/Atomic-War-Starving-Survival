# Subject Plan 176.14 — An Overheard Scoop: focused event prose review
...
Review the exact event `friction_ration_observation_overheard` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 60 whitespace-delimited words (306 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.
...
- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `An Overheard Scoop` and its exact current field shape is `bodyText, choices, conditions, id, minDay, title, weight`.
- **VERIFIED source prose:**

> The morning scoop happened in the corridor at the usual hour, and three adults watched it, and two said nothing, and one said too much: a number, a name, a comparison. The words are still in the corridor at shift change. The one who said them has not taken them back, and there is no way to take them back.

- **VERIFIED non-prose snapshot:** `{"choices": [{"choiceId": "meet_in_private", "effects": [{"setWorldFlag": "friction_ration_observation_discreet", "worldFlagValue": true}], "moraleDelta": 1.0, "text": "Pull the speaker into the council sheet. Without a witness. The corridor has enough."}, {"choiceId": "open_meditation", "effects": [{"setWorldFlag": "friction_ration_observation_open", "worldFlagValue": true}], "moraleDelta": -2.0, "text": "Open the floor at the next council. The corridor heard it; the council can too."}], "conditions": {"RequireRationUnevenAllocation": true}, "id": "friction_ration_observation_overheard", "minDay": 24, "title": "An Overheard Scoop", "weight": 0.9}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
