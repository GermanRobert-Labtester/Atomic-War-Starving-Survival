# Subject Plan 176.32 — Master Wireman: focused event prose review
...
Review the exact event `narrative_trade_mastery_wireman` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 65 whitespace-delimited words (391 characters). This is a review candidate from the residual low-length tranche, not a word-count target or a claim that the source is deficient.

Editorial question: **Let competence appear through an observable check or handoff rather than explanation. Do not add a repair result, machine state, item, or technical procedure that the record does not contain.** Work from the current record's facts and voice. Seek a material improvement in clarity, specificity, tone, or continuity. If the only available change would increase length, preserve the current text.

## Live source evidence

- **VERIFIED:** exact selector exists once in the 240-record `events` array. Its title is `Master Wireman` and its exact current field shape is `bodyText, id, minDay, title, weight`.
- **VERIFIED source prose:**

> Two hundred meters of cable run through concrete and rebar, every splice tight, every junction box sealed against moisture. The Electrician knows where every circuit goes without looking at the diagram because she drew the diagram. The shelter runs on things that nobody sees unless they break. The wire is the one nobody sees until everything breaks. It has not broken. That is the mastery.

- **VERIFIED non-prose snapshot:** `{"id": "narrative_trade_mastery_wireman", "minDay": 40, "title": "Master Wireman", "weight": 1}`. Re-read the whole object before any future edit; this captured shape exists to prevent optional fields from being inferred from another event.
