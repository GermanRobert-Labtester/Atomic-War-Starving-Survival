...
# 2026-09-24 integration expansion — Trophy System
...
**Objective:** one real preserved quarry fact awards a single recipe opportunity and survives reload without a second reward.

**Current evidence:** The trophy system owns an exactly-once award ledger and loads eleven authored trophies. Direct src calls to TrophySystem were not found; wildlife trapping and shelter decor have live hosts. The system awards a crafting opportunity, not automatically a physical mount or item; that distinction controls inventory and UI copy.

**Owner boundary:** Wildlife/trapping owns quarry harvest; TrophySystem owns award/unlock fact; crafting owns recipe execution; decor owns placed display and localized morale. **Core source:** `Assets/Ashfall.Core/Shelter/TrophySystem.cs`. **Current or candidate host:** `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. **Persistence decision:** Persist awarded IDs and unlocked recipes once before consuming the fact in crafting/decor; no mirrored award list in a panel. **Authored data:** `trophies.json`. This follows the master expansion document's C14 and C16 cluster and its four-tier data → Core → host → verification structure. Live source outranks the master when a dated example is stale.

**Non-goals and stop rule:** No trophy from a sighting alone, no automatic item grant, no exploitation reward where the Bone Shop's restraint rule forbids it. This documentation does not register a new save section, modify a host, or certify player reachability. A foreman must claim exact implementation paths under `WORKTREE_OWNERSHIP.md`; a collision or missing authority returns a premise finding rather than an invented parallel subsystem.

## 2. Integration architecture and state flow

```text
trophies.json / canonical owner facts
    -> TrophySystem
    -> WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter
    -> TrophySaveState must be registered or embedded through an approved existing collectible/decor owner; no duplicate award ledger
...
|---|---|---|
| Which Core type actually runs? | `Assets/Ashfall.Core/Shelter/TrophySystem.cs` | Resolve exact constructor, method signatures, input ports, output DTOs, and indirect callers. |
| Which method can the host call? | `LoadCatalogFromJson, RecordQuarryPreserved, IsAwarded, CaptureState, RestoreState` | Use current API; introduce a thin adapter only if source cannot express the bounded outcome. |
| Who owns cross-system state? | `Wildlife/trapping owns quarry harvest; TrophySystem owns award/unlock fact; crafting owns recipe execution; decor owns placed display and localized morale` | One writer per resource, result, and persistent record. |
| What survives save/load? | `TrophySaveState must be registered or embedded through an approved existing collectible/decor owner; no duplicate award ledger` | Capture/restore and old-save default or explicit stateless verdict. |
| What is authored? | `trophies.json` | Verify loader, IDs, references, range validation, and an actual consumer. |
| What does the player see? | `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter` | Trace a command or read route through the current Godot lifecycle. |
| Which expansion ideas apply? | `docs/newest-ashfall-master-expansion-authority-v2-0-complete-compiled-edition-volumes-1-57.md` | Use the C14 and C16 cluster's prose register, continuity, and anti-duplication rules. |
...
|---|---|---|
| `trophy_wolf_head` | `id`, `species_id`, `trophy_item_id`, `recipe_id`, `display_name`, `description`, `localized_morale_delta` | Which loader, owner predicate, host command, and read surface consume this row? |
| `trophy_deer_antlers` | `id`, `species_id`, `trophy_item_id`, `recipe_id`, `display_name`, `description`, `localized_morale_delta` | Which loader, owner predicate, host command, and read surface consume this row? |
| `trophy_boar_tusks` | `id`, `species_id`, `trophy_item_id`, `recipe_id`, `display_name`, `description`, `localized_morale_delta` | Which loader, owner predicate, host command, and read surface consume this row? |
| `trophy_fox_pelt` | `id`, `species_id`, `trophy_item_id`, `recipe_id`, `display_name`, `description`, `localized_morale_delta` | Which loader, owner predicate, host command, and read surface consume this row? |
| `trophy_beetle_carapace` | `id`, `species_id`, `trophy_item_id`, `recipe_id`, `display_name`, `description`, `localized_morale_delta` | Which loader, owner predicate, host command, and read surface consume this row? |
| `trophy_molerat_skull` | `id`, `species_id`, `trophy_item_id`, `recipe_id`, `display_name`, `description`, `localized_morale_delta` | Which loader, owner predicate, host command, and read surface consume this row? |
| `trophy_crow_feathers` | `id`, `species_id`, `trophy_item_id`, `recipe_id`, `display_name`, `description`, `localized_morale_delta` | Which loader, owner predicate, host command, and read surface consume this row? |
| `trophy_pheasant_plume` | `id`, `species_id`, `trophy_item_id`, `recipe_id`, `display_name`, `description`, `localized_morale_delta` | Which loader, owner predicate, host command, and read surface consume this row? |
| `trophy_ash_hound_pelt` | `id`, `species_id`, `trophy_item_id`, `recipe_id`, `display_name`, `description`, `localized_morale_delta` | Which loader, owner predicate, host command, and read surface consume this row? |
| `trophy_gulden_wolf` | `id`, `species_id`, `trophy_item_id`, `recipe_id`, `display_name`, `description`, `localized_morale_delta` | Which loader, owner predicate, host command, and read surface consume this row? |
| `trophy_kestrel_wings` | `id`, `species_id`, `trophy_item_id`, `recipe_id`, `display_name`, `description`, `localized_morale_delta` | Which loader, owner predicate, host command, and read surface consume this row? |

### Review case 001: species encounter under first valid use

**Scene premise.** The player's route reaches **species encounter** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_wolf_head`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “The tagged specimen case is left where the person making the next decision can see it.” is a candidate observation anchored by a tagged specimen case. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `LoadCatalogFromJson, RecordQuarryPreserved, IsAwarded, CaptureState, RestoreState` after its actual input owners have been sampled. Wildlife/trapping owns quarry harvest; TrophySystem owns award/unlock fact; crafting owns recipe execution; decor owns placed display and localized morale. For first valid use, the mandatory boundary is to prove the default state, eligibility, and one visible outcome. Before confirmation, show the canonical cost and status from the same snapshot that will be rechecked at commit. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.

**Save and replay reading.** Persistence decision: Persist awarded IDs and unlocked recipes once before consuming the fact in crafting/decor; no mirrored award list in a panel. The audit must show the before-state, the accepted or refused result, the after-state, and the restore-state. A duplicate event must not spend stock twice. A day transition must not advance twice. Stable ID order and a campaign seed are required where selection or risk is random; where the function is deterministic math, no RNG is added. The panel should obtain the same status after reopening and should release subscriptions when disposed. The author should not refer to a live result until the save and host trace confirms it.

**Acceptance decision.** Mark this card accepted only with source file:line references, a live trigger, the exact method call, a reachable authored ID or owner record, an observable current surface, and a focused assertion matched to prove the default state, eligibility, and one visible outcome. Otherwise mark it deferred or inapplicable. A high-level compilation result does not prove a player route, a catalog file does not prove a loader, and a historical audit count does not prove that the class is still host-unreachable. The first implementation package should choose only the cases necessary to establish one real preserved quarry fact awards a single recipe opportunity and survives reload without a second reward.

### Review case 002: legal harvest under first valid use

**Scene premise.** The player's route reaches **legal harvest** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_deer_antlers`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “A note beside the a tagged specimen case records the uncertainty before anyone writes an outcome.” is a candidate observation anchored by a tagged specimen case. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.
...
### Review case 003: preservation under first valid use

**Scene premise.** The player's route reaches **preservation** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_boar_tusks`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “The tagged specimen case carries a date and a name; neither is proof that the work is finished.” is a candidate observation anchored by a tagged specimen case. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.
...
### Review case 004: catalog match under first valid use

**Scene premise.** The player's route reaches **catalog match** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_fox_pelt`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “Someone checks the a tagged specimen case against the ledger, then leaves the blank field blank.” is a candidate observation anchored by a tagged specimen case. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.
...
### Review case 005: award event under first valid use

**Scene premise.** The player's route reaches **award event** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_beetle_carapace`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “A second hand reaches for the a tagged specimen case, waits, and asks who signed the first entry.” is a candidate observation anchored by a tagged specimen case. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.
...
### Review case 006: recipe unlock under first valid use

**Scene premise.** The player's route reaches **recipe unlock** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_molerat_skull`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 007: crafting bench under first valid use

**Scene premise.** The player's route reaches **crafting bench** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_crow_feathers`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 008: physical mount under first valid use

**Scene premise.** The player's route reaches **physical mount** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_pheasant_plume`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 009: decor placement under first valid use

**Scene premise.** The player's route reaches **decor placement** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_ash_hound_pelt`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 010: morale radius under first valid use

**Scene premise.** The player's route reaches **morale radius** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_gulden_wolf`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 011: duplicate quarry under first valid use

**Scene premise.** The player's route reaches **duplicate quarry** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_kestrel_wings`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
**Scene premise.** The player's route reaches **old save** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_wolf_head`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 013: unknown species under first valid use

**Scene premise.** The player's route reaches **unknown species** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_deer_antlers`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 014: display removal under first valid use

**Scene premise.** The player's route reaches **display removal** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_boar_tusks`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 015: memorial context under first valid use

**Scene premise.** The player's route reaches **memorial context** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_fox_pelt`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 016: quiet archive under first valid use

**Scene premise.** The player's route reaches **quiet archive** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `trophy_beetle_carapace`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 017: species encounter under legacy restore

**Scene premise.** The player's route reaches **species encounter** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_molerat_skull`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “A note beside the a tagged specimen case records the uncertainty before anyone writes an outcome.” is a candidate observation anchored by a tagged specimen case. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `LoadCatalogFromJson, RecordQuarryPreserved, IsAwarded, CaptureState, RestoreState` after its actual input owners have been sampled. Wildlife/trapping owns quarry harvest; TrophySystem owns award/unlock fact; crafting owns recipe execution; decor owns placed display and localized morale. For legacy restore, the mandatory boundary is to default additive state safely and never replay an old effect. Before confirmation, show the canonical cost and status from the same snapshot that will be rechecked at commit. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.
...
### Review case 018: legal harvest under legacy restore

**Scene premise.** The player's route reaches **legal harvest** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_crow_feathers`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 019: preservation under legacy restore

**Scene premise.** The player's route reaches **preservation** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_pheasant_plume`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 020: catalog match under legacy restore

**Scene premise.** The player's route reaches **catalog match** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_ash_hound_pelt`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 021: award event under legacy restore

**Scene premise.** The player's route reaches **award event** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_gulden_wolf`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 022: recipe unlock under legacy restore

**Scene premise.** The player's route reaches **recipe unlock** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_kestrel_wings`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 023: crafting bench under legacy restore

**Scene premise.** The player's route reaches **crafting bench** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_wolf_head`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 024: physical mount under legacy restore

**Scene premise.** The player's route reaches **physical mount** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_deer_antlers`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 025: decor placement under legacy restore

**Scene premise.** The player's route reaches **decor placement** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_boar_tusks`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 026: morale radius under legacy restore

**Scene premise.** The player's route reaches **morale radius** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_fox_pelt`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 027: duplicate quarry under legacy restore

**Scene premise.** The player's route reaches **duplicate quarry** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_beetle_carapace`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
**Scene premise.** The player's route reaches **old save** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_molerat_skull`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 029: unknown species under legacy restore

**Scene premise.** The player's route reaches **unknown species** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_crow_feathers`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 030: display removal under legacy restore

**Scene premise.** The player's route reaches **display removal** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_pheasant_plume`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 031: memorial context under legacy restore

**Scene premise.** The player's route reaches **memorial context** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_ash_hound_pelt`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 032: quiet archive under legacy restore

**Scene premise.** The player's route reaches **quiet archive** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `trophy_gulden_wolf`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 033: species encounter under repeat request

**Scene premise.** The player's route reaches **species encounter** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_kestrel_wings`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “The tagged specimen case carries a date and a name; neither is proof that the work is finished.” is a candidate observation anchored by a tagged specimen case. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `LoadCatalogFromJson, RecordQuarryPreserved, IsAwarded, CaptureState, RestoreState` after its actual input owners have been sampled. Wildlife/trapping owns quarry harvest; TrophySystem owns award/unlock fact; crafting owns recipe execution; decor owns placed display and localized morale. For repeat request, the mandatory boundary is to return the current status without charging or publishing twice. Before confirmation, show the canonical cost and status from the same snapshot that will be rechecked at commit. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.
...
### Review case 034: legal harvest under repeat request

**Scene premise.** The player's route reaches **legal harvest** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_wolf_head`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 035: preservation under repeat request

**Scene premise.** The player's route reaches **preservation** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_deer_antlers`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 036: catalog match under repeat request

**Scene premise.** The player's route reaches **catalog match** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_boar_tusks`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 037: award event under repeat request

**Scene premise.** The player's route reaches **award event** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_fox_pelt`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 038: recipe unlock under repeat request

**Scene premise.** The player's route reaches **recipe unlock** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_beetle_carapace`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 039: crafting bench under repeat request

**Scene premise.** The player's route reaches **crafting bench** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_molerat_skull`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 040: physical mount under repeat request

**Scene premise.** The player's route reaches **physical mount** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_crow_feathers`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 041: decor placement under repeat request

**Scene premise.** The player's route reaches **decor placement** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_pheasant_plume`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 042: morale radius under repeat request

**Scene premise.** The player's route reaches **morale radius** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_ash_hound_pelt`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 043: duplicate quarry under repeat request

**Scene premise.** The player's route reaches **duplicate quarry** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_gulden_wolf`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
**Scene premise.** The player's route reaches **old save** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_kestrel_wings`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 045: unknown species under repeat request

**Scene premise.** The player's route reaches **unknown species** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_wolf_head`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 046: display removal under repeat request

**Scene premise.** The player's route reaches **display removal** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_deer_antlers`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 047: memorial context under repeat request

**Scene premise.** The player's route reaches **memorial context** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_boar_tusks`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 048: quiet archive under repeat request

**Scene premise.** The player's route reaches **quiet archive** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `trophy_fox_pelt`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 049: species encounter under stale preview

**Scene premise.** The player's route reaches **species encounter** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_beetle_carapace`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “Someone checks the a tagged specimen case against the ledger, then leaves the blank field blank.” is a candidate observation anchored by a tagged specimen case. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `LoadCatalogFromJson, RecordQuarryPreserved, IsAwarded, CaptureState, RestoreState` after its actual input owners have been sampled. Wildlife/trapping owns quarry harvest; TrophySystem owns award/unlock fact; crafting owns recipe execution; decor owns placed display and localized morale. For stale preview, the mandatory boundary is to revalidate at the command boundary and explain the new refusal. Before confirmation, show the canonical cost and status from the same snapshot that will be rechecked at commit. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.
...
### Review case 050: legal harvest under stale preview

**Scene premise.** The player's route reaches **legal harvest** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_molerat_skull`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “A second hand reaches for the a tagged specimen case, waits, and asks who signed the first entry.” is a candidate observation anchored by a tagged specimen case. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `LoadCatalogFromJson, RecordQuarryPreserved, IsAwarded, CaptureState, RestoreState` after its actual input owners have been sampled. Wildlife/trapping owns quarry harvest; TrophySystem owns award/unlock fact; crafting owns recipe execution; decor owns placed display and localized morale. For stale preview, the mandatory boundary is to revalidate at the command boundary and explain the new refusal. After a refusal, retain the prior state and give a concrete reason in the player's current surface. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.

**Save and replay reading.** Persistence decision: Persist awarded IDs and unlocked recipes once before consuming the fact in crafting/decor; no mirrored award list in a panel. The audit must show the before-state, the accepted or refused result, the after-state, and the restore-state. A duplicate event must not spend stock twice. A day transition must not advance twice. Stable ID order and a campaign seed are required where selection or risk is random; where the function is deterministic math, no RNG is added. The panel should obtain the same status after reopening and should release subscriptions when disposed. The author should not refer to a live result until the save and host trace confirms it.

**Acceptance decision.** Mark this card accepted only with source file:line references, a live trigger, the exact method call, a reachable authored ID or owner record, an observable current surface, and a focused assertion matched to revalidate at the command boundary and explain the new refusal. Otherwise mark it deferred or inapplicable. A high-level compilation result does not prove a player route, a catalog file does not prove a loader, and a historical audit count does not prove that the class is still host-unreachable. The first implementation package should choose only the cases necessary to establish one real preserved quarry fact awards a single recipe opportunity and survives reload without a second reward.

### Review case 051: preservation under stale preview

**Scene premise.** The player's route reaches **preservation** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_crow_feathers`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “The tagged specimen case is left where the person making the next decision can see it.” is a candidate observation anchored by a tagged specimen case. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `LoadCatalogFromJson, RecordQuarryPreserved, IsAwarded, CaptureState, RestoreState` after its actual input owners have been sampled. Wildlife/trapping owns quarry harvest; TrophySystem owns award/unlock fact; crafting owns recipe execution; decor owns placed display and localized morale. For stale preview, the mandatory boundary is to revalidate at the command boundary and explain the new refusal. After success, the relevant owner may emit one fact; presentation can paraphrase it but cannot create another reward. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.

**Save and replay reading.** Persistence decision: Persist awarded IDs and unlocked recipes once before consuming the fact in crafting/decor; no mirrored award list in a panel. The audit must show the before-state, the accepted or refused result, the after-state, and the restore-state. A duplicate event must not spend stock twice. A day transition must not advance twice. Stable ID order and a campaign seed are required where selection or risk is random; where the function is deterministic math, no RNG is added. The panel should obtain the same status after reopening and should release subscriptions when disposed. The author should not refer to a live result until the save and host trace confirms it.

**Acceptance decision.** Mark this card accepted only with source file:line references, a live trigger, the exact method call, a reachable authored ID or owner record, an observable current surface, and a focused assertion matched to revalidate at the command boundary and explain the new refusal. Otherwise mark it deferred or inapplicable. A high-level compilation result does not prove a player route, a catalog file does not prove a loader, and a historical audit count does not prove that the class is still host-unreachable. The first implementation package should choose only the cases necessary to establish one real preserved quarry fact awards a single recipe opportunity and survives reload without a second reward.

### Review case 052: catalog match under stale preview

**Scene premise.** The player's route reaches **catalog match** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_pheasant_plume`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “A note beside the a tagged specimen case records the uncertainty before anyone writes an outcome.” is a candidate observation anchored by a tagged specimen case. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `LoadCatalogFromJson, RecordQuarryPreserved, IsAwarded, CaptureState, RestoreState` after its actual input owners have been sampled. Wildlife/trapping owns quarry harvest; TrophySystem owns award/unlock fact; crafting owns recipe execution; decor owns placed display and localized morale. For stale preview, the mandatory boundary is to revalidate at the command boundary and explain the new refusal. After restore, rebuild the view from saved owner state without calling the gameplay command again. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.

**Save and replay reading.** Persistence decision: Persist awarded IDs and unlocked recipes once before consuming the fact in crafting/decor; no mirrored award list in a panel. The audit must show the before-state, the accepted or refused result, the after-state, and the restore-state. A duplicate event must not spend stock twice. A day transition must not advance twice. Stable ID order and a campaign seed are required where selection or risk is random; where the function is deterministic math, no RNG is added. The panel should obtain the same status after reopening and should release subscriptions when disposed. The author should not refer to a live result until the save and host trace confirms it.

**Acceptance decision.** Mark this card accepted only with source file:line references, a live trigger, the exact method call, a reachable authored ID or owner record, an observable current surface, and a focused assertion matched to revalidate at the command boundary and explain the new refusal. Otherwise mark it deferred or inapplicable. A high-level compilation result does not prove a player route, a catalog file does not prove a loader, and a historical audit count does not prove that the class is still host-unreachable. The first implementation package should choose only the cases necessary to establish one real preserved quarry fact awards a single recipe opportunity and survives reload without a second reward.

### Review case 053: award event under stale preview

**Scene premise.** The player's route reaches **award event** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_ash_hound_pelt`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 054: recipe unlock under stale preview

**Scene premise.** The player's route reaches **recipe unlock** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_gulden_wolf`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 055: crafting bench under stale preview

**Scene premise.** The player's route reaches **crafting bench** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_kestrel_wings`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 056: physical mount under stale preview

**Scene premise.** The player's route reaches **physical mount** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_wolf_head`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 057: decor placement under stale preview

**Scene premise.** The player's route reaches **decor placement** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_deer_antlers`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 058: morale radius under stale preview

**Scene premise.** The player's route reaches **morale radius** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_boar_tusks`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 059: duplicate quarry under stale preview

**Scene premise.** The player's route reaches **duplicate quarry** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_fox_pelt`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
**Scene premise.** The player's route reaches **old save** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_beetle_carapace`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 061: unknown species under stale preview

**Scene premise.** The player's route reaches **unknown species** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_molerat_skull`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 062: display removal under stale preview

**Scene premise.** The player's route reaches **display removal** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_crow_feathers`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 063: memorial context under stale preview

**Scene premise.** The player's route reaches **memorial context** while the facts used by the panel changed before confirmation. The implementation candidate starts with `trophy_pheasant_plume`. The live source checkpoint is `Assets/Ashfall.Core/Shelter/TrophySystem.cs`; the proposed reader is `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
## 7. Finished planning architecture for Trophy System

The architecture handoff is complete at the planning level: `trophies.json` or current owner facts → `TrophySystem` → `WildlifeTrappingHostSession / ShelterDecorHostSession with one award adapter` → `TrophySaveState must be registered or embedded through an approved existing collectible/decor owner; no duplicate award ledger` only where state is mutable → a truthful player read model. The first package is **one real preserved quarry fact awards a single recipe opportunity and survives reload without a second reward**. The 63 case cards above are a candidate review register; no card is canon or a mandate to build all combinations. The exact files and owner signatures must be rechecked when a foreman promotes the package.
