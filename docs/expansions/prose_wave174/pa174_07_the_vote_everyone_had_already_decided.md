# Subject Plan 174.07 — The Vote Everyone Had Already Decided
Lane: A · Cluster: C10 · Status: PROPOSAL (subject-level, serial queue; not an integration plan)

## Subject
Review and, only if the census supports it, polish prose in `quest_oxygen_equality_04` in `quests_massive_expansion_200.json`. The bounded subject is **children’s air and limits of consensus**. Only `briefing`, `stages[].text`, and `choices[].text` are in scope; a field already doing its job stays unchanged. No new quest, branch, system, flag, or schema field is proposed.

## Premise evidence
- **VERIFIED:** `Assets/StreamingAssets/Data/quests_massive_expansion_200.json` exists with 200 records under `schema_version` / `quests`. This selector has `type=narrative_chain`, `prereq_quest_id=quest_oxygen_equality_03`, `min_day=40`, 1 stage(s), and 2 choices.
- **VERIFIED:** Actual text fields are `briefing` (263 chars), `stages[].text` (75), and `choices[].text` (56); 394 prose characters combined. Choice IDs and `set_flag` values are separate live fields. This record has no `quest_hook` or `objective_text`.
- **VERIFIED source excerpt:** Briefing: “This one is short: fresh oxygen to the children's sector. Lungs that are still growing are being shaped by thin air; the shape is in the infirmary's charts. When the vote is called, the room notices it has already decided. The vote is held anyway, for the record.” Stage: “The vote makes one sound, unanimous. Nobody in the room is surprised by it.” Choices: “Route air to the children.” / “Refuse. Every deck has charts.”.
...
The compiled v2.0 authority names this corpus as A-25 / C10 and directs census-first tranche work. This selector makes the audit tractable: one record, one editorial register, fixed choices. The concrete editorial question is **children’s air and limits of consensus**. Deliverable is a field-by-field decision—polish, retain, or close as sufficient—with a brief continuity rationale. It is not an invitation to expand all 200 records or to make the prose longer for its own sake.

Editorial direction: Show how unanimity formed before the formal vote and whose voices were absent. Distinguish infirmary evidence from the claim that every deck has charts.
...
No numerical health outcomes, new air-routing options, or claim children consented. Preserve flags, order, prerequisite, and Day 40.

Preserve `id=quest_oxygen_equality_04`, `display_name='Oxygen Equality - Stage 4'`, `type=narrative_chain`, prerequisite `'quest_oxygen_equality_03'`, Day 40, stage IDs/order, choice IDs/order, and both `set_flag` values exactly. No field widening, parallel quest state, save changes, route claims, or invented canon. New details remain proposals until checked against neighboring stages and the current world authority.
