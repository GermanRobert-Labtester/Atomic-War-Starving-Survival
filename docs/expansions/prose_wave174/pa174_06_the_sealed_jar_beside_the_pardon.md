# Subject Plan 174.06 — The Sealed Jar beside the Pardon
Lane: A · Cluster: C10 · Status: PROPOSAL (subject-level, serial queue; not an integration plan)

## Subject
Review and, only if the census supports it, polish prose in `quest_forgiveness_doctrine_02` in `quests_massive_expansion_200.json`. The bounded subject is **complaint, evidence, and institutional memory**. Only `briefing`, `stages[].text`, and `choices[].text` are in scope; a field already doing its job stays unchanged. No new quest, branch, system, flag, or schema field is proposed.

## Premise evidence
- **VERIFIED:** `Assets/StreamingAssets/Data/quests_massive_expansion_200.json` exists with 200 records under `schema_version` / `quests`. This selector has `type=narrative_chain`, `prereq_quest_id=quest_forgiveness_doctrine_01`, `min_day=20`, 1 stage(s), and 2 choices.
- **VERIFIED:** Actual text fields are `briefing` (237 chars), `stages[].text` (103), and `choices[].text` (49); 389 prose characters combined. Choice IDs and `set_flag` values are separate live fields. This record has no `quest_hook` or `objective_text`.
- **VERIFIED source excerpt:** Briefing: “Next come the complainers. Raised voices have been logged as offenses here; most of the voices were raised about water. The water was bad; the proposal says so in plain words. While it is read, the water crew stands looking at its hands.” Stage: “On the table by the proposal sits a sealed jar of the old water. Nobody has opened it; nobody needs to.” Choices: “Pardon the complainers.” / “Refuse. Order comes first.”.
...
The compiled v2.0 authority names this corpus as A-25 / C10 and directs census-first tranche work. This selector makes the audit tractable: one record, one editorial register, fixed choices. The concrete editorial question is **complaint, evidence, and institutional memory**. Deliverable is a field-by-field decision—polish, retain, or close as sufficient—with a brief continuity rationale. It is not an invitation to expand all 200 records or to make the prose longer for its own sake.

Editorial direction: Center the sealed jar as evidence no speech replaces. Restore the water crew’s presence and complainants’ fear of being labeled offenders; let pardon remain imperfect.
...
Do not identify a contaminant or create a water-quality system. Keep the contents unknown absent canon evidence; no collectible.

Preserve `id=quest_forgiveness_doctrine_02`, `display_name='The Forgiveness Doctrine - Stage 2'`, `type=narrative_chain`, prerequisite `'quest_forgiveness_doctrine_01'`, Day 20, stage IDs/order, choice IDs/order, and both `set_flag` values exactly. No field widening, parallel quest state, save changes, route claims, or invented canon. New details remain proposals until checked against neighboring stages and the current world authority.
