# Subject Plan 174.14 — A Warm Tap, One Evening at a Time
Lane: A · Cluster: C10 · Status: PROPOSAL (subject-level, serial queue; not an integration plan)

## Subject
Review and, only if the census supports it, polish prose in `quest_water_gift_03` in `quests_massive_expansion_200.json`. The bounded subject is **comfort as a human need**. Only `briefing`, `stages[].text`, and `choices[].text` are in scope; a field already doing its job stays unchanged. No new quest, branch, system, flag, or schema field is proposed.

## Premise evidence
- **VERIFIED:** `Assets/StreamingAssets/Data/quests_massive_expansion_200.json` exists with 200 records under `schema_version` / `quests`. This selector has `type=narrative_chain`, `prereq_quest_id=quest_water_gift_02`, `min_day=30`, 1 stage(s), and 2 choices.
- **VERIFIED:** Actual text fields are `briefing` (245 chars), `stages[].text` (122), and `choices[].text` (51); 418 prose characters combined. Choice IDs and `set_flag` values are separate live fields. This record has no `quest_hook` or `objective_text`.
- **VERIFIED source excerpt:** Briefing: “The third ask is comfort: warm water, for washing that is not need. The proposal is a single paragraph, most of it one list: hands, faces, whatever else the heart asks. The room reads that line twice. It passes on the first call, without debate.” Stage: “The first evening the warm tap runs, a man shaves for the first time since the exchange. He does not say why; nobody asks.” Choices: “Give the comfort water.” / “Refuse. Comfort is a luxury.”.
...
The compiled v2.0 authority names this corpus as A-25 / C10 and directs census-first tranche work. This selector makes the audit tractable: one record, one editorial register, fixed choices. The concrete editorial question is **comfort as a human need**. Deliverable is a field-by-field decision—polish, retain, or close as sufficient—with a brief continuity rationale. It is not an invitation to expand all 200 records or to make the prose longer for its own sake.

Editorial direction: Keep the request ordinary: hands, faces, a shave, permission to feel like oneself. Add a witness to scarcity without making relief limitless; end on one specific human moment.
...
No hot-water production, fuel cost, bathing mechanic, or need meter. Do not promise indefinite service; preserve Day 30 and flags.

Preserve `id=quest_water_gift_03`, `display_name='The Water Gift - Stage 3'`, `type=narrative_chain`, prerequisite `'quest_water_gift_02'`, Day 30, stage IDs/order, choice IDs/order, and both `set_flag` values exactly. No field widening, parallel quest state, save changes, route claims, or invented canon. New details remain proposals until checked against neighboring stages and the current world authority.
