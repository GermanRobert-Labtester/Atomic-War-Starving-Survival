# Subject Plan 174.02 — The Iodine Smell Has Left the Clinic
Lane: A · Cluster: C10 · Status: PROPOSAL (subject-level, serial queue; not an integration plan)

## Subject
Review and, only if the census supports it, polish prose in `quest_medical_attrition_04` in `quests_massive_expansion_200.json`. The bounded subject is **institutional privilege at the clinic door**. Only `briefing`, `stages[].text`, and `choices[].text` are in scope; a field already doing its job stays unchanged. No new quest, branch, system, flag, or schema field is proposed.

## Premise evidence
- **VERIFIED:** `Assets/StreamingAssets/Data/quests_massive_expansion_200.json` exists with 200 records under `schema_version` / `quests`. This selector has `type=narrative_chain`, `prereq_quest_id=quest_medical_attrition_03`, `min_day=40`, 1 stage(s), and 2 choices.
- **VERIFIED:** Actual text fields are `briefing` (233 chars), `stages[].text` (72), and `choices[].text` (64); 369 prose characters combined. Choice IDs and `set_flag` values are separate live fields. This record has no `quest_hook` or `objective_text`.
- **VERIFIED source excerpt:** Briefing: “Antiseptics are next, and the order includes a justification this time: elite personnel face greater risk and require priority dosing. The clinic has been asked to dilute what remains for general use. What remains is already diluted.” Stage: “The clinic smells of boiled water now, where it used to smell of iodine.” Choices: “Dilute and hand it over.” / “Keep the bottles. Mark them clinic-only.”.
...
The compiled v2.0 authority names this corpus as A-25 / C10 and directs census-first tranche work. This selector makes the audit tractable: one record, one editorial register, fixed choices. The concrete editorial question is **institutional privilege at the clinic door**. Deliverable is a field-by-field decision—polish, retain, or close as sufficient—with a brief continuity rationale. It is not an invitation to expand all 200 records or to make the prose longer for its own sake.

Editorial direction: Contrast “priority dosing” with the clinic’s physical evidence. Show what dilution means to staff and patients without turning the scene into a medical manual; keep custody of the bottles central.
...
No antiseptic chemistry, ratios, treatment protocol, or inventory mechanic. Preserve the two flags and clinic-only alternative.

Preserve `id=quest_medical_attrition_04`, `display_name='Medical Attrition - Stage 4'`, `type=narrative_chain`, prerequisite `'quest_medical_attrition_03'`, Day 40, stage IDs/order, choice IDs/order, and both `set_flag` values exactly. No field widening, parallel quest state, save changes, route claims, or invented canon. New details remain proposals until checked against neighboring stages and the current world authority.
