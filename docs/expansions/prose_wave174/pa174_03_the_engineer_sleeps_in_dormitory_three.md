# Subject Plan 174.03 — The Engineer Sleeps in Dormitory Three
Lane: A · Cluster: C10 · Status: PROPOSAL (subject-level, serial queue; not an integration plan)

## Subject
Review and, only if the census supports it, polish prose in `quest_oxygen_triage_04` in `quests_massive_expansion_200.json`. The bounded subject is **self-sacrifice inside a rationing order**. Only `briefing`, `stages[].text`, and `choices[].text` are in scope; a field already doing its job stays unchanged. No new quest, branch, system, flag, or schema field is proposed.

## Premise evidence
- **VERIFIED:** `Assets/StreamingAssets/Data/quests_massive_expansion_200.json` exists with 200 records under `schema_version` / `quests`. This selector has `type=narrative_chain`, `prereq_quest_id=quest_oxygen_triage_03`, `min_day=40`, 1 stage(s), and 2 choices.
- **VERIFIED:** Actual text fields are `briefing` (235 chars), `stages[].text` (84), and `choices[].text` (55); 374 prose characters combined. Choice IDs and `set_flag` values are separate live fields. This record has no `quest_hook` or `objective_text`.
- **VERIFIED source excerpt:** Briefing: “Dormitory 3 is next on the list, and the people in it know the pattern: the air thins one door at a time. The engineer who signed the report sleeps in Dormitory 3, and he has asked that the cut start with his own bunk, and he means it.” Stage: “The engineer stands at the manifold with his wrench, waiting for the order he wrote.” Choices: “Cut Dormitory 3 as planned.” / “Refuse. Find the fuel first.”.
...
The compiled v2.0 authority names this corpus as A-25 / C10 and directs census-first tranche work. This selector makes the audit tractable: one record, one editorial register, fixed choices. The concrete editorial question is **self-sacrifice inside a rationing order**. Deliverable is a field-by-field decision—polish, retain, or close as sufficient—with a brief continuity rationale. It is not an invitation to expand all 200 records or to make the prose longer for its own sake.

Editorial direction: Keep authorship, residence, and consent distinct: the engineer wrote the report, lives in Dormitory 3, and offers his bunk first. Test what consent means when neighbors are affected.
...
No oxygen/fuel figures, health outcomes, or claim one person speaks for the dormitory. Preserve Day 40, prerequisite, and “find fuel first.”

Preserve `id=quest_oxygen_triage_04`, `display_name='Oxygen Triage - Stage 4'`, `type=narrative_chain`, prerequisite `'quest_oxygen_triage_03'`, Day 40, stage IDs/order, choice IDs/order, and both `set_flag` values exactly. No field widening, parallel quest state, save changes, route claims, or invented canon. New details remain proposals until checked against neighboring stages and the current world authority.
