# Subject Plan 174.19 — The Librarian Spends Her Week Reading
Lane: A · Cluster: C10 · Status: PROPOSAL (subject-level, serial queue; not an integration plan)

## Subject
Review and, only if the census supports it, polish prose in `quest_propaganda_control_02` in `quests_massive_expansion_200.json`. The bounded subject is **censorship resisted through care for books**. Only `briefing`, `stages[].text`, and `choices[].text` are in scope; a field already doing its job stays unchanged. No new quest, branch, system, flag, or schema field is proposed.

## Premise evidence
- **VERIFIED:** `Assets/StreamingAssets/Data/quests_massive_expansion_200.json` exists with 200 records under `schema_version` / `quests`. This selector has `type=narrative_chain`, `prereq_quest_id=quest_propaganda_control_01`, `min_day=20`, 1 stage(s), and 2 choices.
- **VERIFIED:** Actual text fields are `briefing` (319 chars), `stages[].text` (136), and `choices[].text` (48); 503 prose characters combined. Choice IDs and `set_flag` values are separate live fields. This record has no `quest_hook` or `objective_text`.
- **VERIFIED source excerpt:** Briefing: “The office has traced the panic to the library. Books written before describe plenty, plenty invites comparison, and comparison is despair; the ban arrives with a list four pages long. The librarian was asked to carry it out. She asked for a week to prepare. She was given the week, and she has spent all of it reading.” Stage: “She sits in the archive with the list beside her, reading. By the door the book cart stands empty, on the spot marked for the burn pile.” Choices: “Enforce the ban.” / “Let her hide them. Sign nothing.”.
...
The compiled v2.0 authority names this corpus as A-25 / C10 and directs census-first tranche work. This selector makes the audit tractable: one record, one editorial register, fixed choices. The concrete editorial question is **censorship resisted through care for books**. Deliverable is a field-by-field decision—polish, retain, or close as sufficient—with a brief continuity rationale. It is not an invitation to expand all 200 records or to make the prose longer for its own sake.

Editorial direction: Make the librarian’s week concrete through reading and the empty cart. Hold former promises of plenty against present scarcity without inventing titles; keep enforcement/refusal as the fixed alternatives.
...
No hidden-book inventory, meter, real-world tactics, or claim books are saved absent flag semantics. Preserve chain, Day 20, IDs/flags.

Preserve `id=quest_propaganda_control_02`, `display_name='Propaganda Control - Stage 2'`, `type=narrative_chain`, prerequisite `'quest_propaganda_control_01'`, Day 20, stage IDs/order, choice IDs/order, and both `set_flag` values exactly. No field widening, parallel quest state, save changes, route claims, or invented canon. New details remain proposals until checked against neighboring stages and the current world authority.
