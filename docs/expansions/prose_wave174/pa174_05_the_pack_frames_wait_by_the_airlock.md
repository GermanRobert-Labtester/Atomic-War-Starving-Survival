# Subject Plan 174.05 — The Pack Frames Wait by the Airlock
Lane: A · Cluster: C10 · Status: PROPOSAL (subject-level, serial queue; not an integration plan)

## Subject
Review and, only if the census supports it, polish prose in `quest_expedition_sacrifices_02` in `quests_massive_expansion_200.json`. The bounded subject is **bureaucratic arithmetic versus field knowledge**. Only `briefing`, `stages[].text`, and `choices[].text` are in scope; a field already doing its job stays unchanged. No new quest, branch, system, flag, or schema field is proposed.

## Premise evidence
- **VERIFIED:** `Assets/StreamingAssets/Data/quests_massive_expansion_200.json` exists with 200 records under `schema_version` / `quests`. This selector has `type=narrative_chain`, `prereq_quest_id=quest_expedition_sacrifices_01`, `min_day=20`, 1 stage(s), and 2 choices.
- **VERIFIED:** Actual text fields are `briefing` (234 chars), `stages[].text` (100), and `choices[].text` (50); 384 prose characters combined. Choice IDs and `set_flag` values are separate live fields. This record has no `quest_hook` or `objective_text`.
- **VERIFIED source excerpt:** Briefing: “Revised upward: scouts now carry double loads on the return leg. The weight was worked out in the operations office by a man who has never carried a pack. Shown the arithmetic, the team laughed once, briefly. Nobody has laughed since.” Stage: “The pack frames stand by the airlock, loaded twice as tall as regulation. Nobody has lifted one yet.” Choices: “Load them double.” / “Cut the loads. Run twice instead.”.
...
The compiled v2.0 authority names this corpus as A-25 / C10 and directs census-first tranche work. This selector makes the audit tractable: one record, one editorial register, fixed choices. The concrete editorial question is **bureaucratic arithmetic versus field knowledge**. Deliverable is a field-by-field decision—polish, retain, or close as sufficient—with a brief continuity rationale. It is not an invitation to expand all 200 records or to make the prose longer for its own sake.

Editorial direction: Carry the double-height pack frames from written order to team assessment. Let field knowledge expose one practical cost omitted by the office, while leaving the decision to the existing two choices.
...
No load stat, injury simulation, or encounter. “Run twice instead” stays an authored alternative, not a guaranteed route or safety promise.

Preserve `id=quest_expedition_sacrifices_02`, `display_name='Expedition Sacrifices - Stage 2'`, `type=narrative_chain`, prerequisite `'quest_expedition_sacrifices_01'`, Day 20, stage IDs/order, choice IDs/order, and both `set_flag` values exactly. No field widening, parallel quest state, save changes, route claims, or invented canon. New details remain proposals until checked against neighboring stages and the current world authority.
