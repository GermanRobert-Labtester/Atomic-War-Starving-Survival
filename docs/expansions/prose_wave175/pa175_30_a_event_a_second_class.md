# Subject Plan 175.30 — A Second Class: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_teacher_milestone_2` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **competence shown through a witnessed moment**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `A Second Class`. Current body: 39 whitespace-delimited words / 242 characters.
- **VERIFIED source text:**

> The teacher opens a second class for the adults, evenings, after the shifts. The subjects are practical: rations math, map reading, medicine names. The children attend the first one, the adults the second, and both sessions end with homework.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_teacher_milestone_2", `minDay`=20, `title`="A Second Class", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
