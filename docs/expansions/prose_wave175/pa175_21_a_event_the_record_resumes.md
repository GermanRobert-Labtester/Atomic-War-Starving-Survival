# Subject Plan 175.21 — The Record Resumes: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_typewriter_restored` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **a repaired object re-entering communal life**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `The Record Resumes`. Current body: 37 whitespace-delimited words / 210 characters.
- **VERIFIED source text:**

> The typewriter carriage returns with a sound the corridor has not heard in years. The clerk types the date, the weather, the ration count, and then, after a pause, a sentence about the snow. The record resumes.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_typewriter_restored", `minDay`=12, `title`="The Record Resumes", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
