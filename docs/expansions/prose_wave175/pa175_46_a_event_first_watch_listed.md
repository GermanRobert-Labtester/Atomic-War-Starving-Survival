# Subject Plan 175.46 — First Watch, Listed: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `post_maturation_watch_readiness` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **interpersonal pressure and distinct voices**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `First Watch, Listed`. Current body: 51 whitespace-delimited words / 273 characters.
- **VERIFIED source text:**

> The watcher's name is now on the standing hatch rotation, twice a week, in pencil, on the wall sheet above the duty board. The pencil will become a pen when the watcher has held the post for a full month without complaint. The pencil is the corridor between role and habit.
...
Editorial question: **Give each person a discernible position in “First Watch, Listed” and let the pressure change through action or silence. Avoid a narrator’s verdict that settles the conflict ahead of the current event outcome.** The output is a decision per field with a continuity reason. If the only benefit is greater length, retain the current text.
...
Preserve every non-prose field byte-for-byte in meaning: `choices`=[{"choiceId": "schedule_first_standby", "text": "Put the watcher on the next open standby slot.", "moraleDelta": 4.0, "effects": []}], `conditions`={"RequireChildCohort": true, "RequireMaturationEligible": true}, `id`="post_maturation_watch_readiness", `minDay`=91, `title`="First Watch, Listed", `weight`=1.0. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
