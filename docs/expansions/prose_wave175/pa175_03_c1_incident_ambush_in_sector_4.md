# Subject Plan 175.03 — The ambush on the salvage run
Lane: A · Cluster: C1 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `incident_ambush_sector_4` in `Assets/StreamingAssets/Data/incidents.json` and polish it only if the editorial pass identifies a material gap. Its register is **field confusion and the cost of a disrupted return**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `Ambush in Sector 4`. Current body: 29 whitespace-delimited words / 181 characters.
- **VERIFIED source text:**

> A routine salvage run in Sector 4 collapses into chaos. Ghost-faced raiders dropped from the ruined overpass, claiming one life and stripping our cargo before the dust even settled.
...
Editorial question: **Clarify the crew’s immediate perception of the ruined overpass and raiders while keeping the encounter’s uncertainty intact. Give the scene an observable turn that does not pre-answer what the shelter should do.** The output is a decision per field with a continuity reason. If the only benefit is greater length, retain the current text.

## What must not change
No combat mechanics, casualty count, tactical instruction, new faction, or changed incident weight/day. Keep violence restrained and the existing event resolution surface untouched.

Preserve every non-prose field byte-for-byte in meaning: `id`="incident_ambush_sector_4", `minDay`=12, `title`="Ambush in Sector 4", `weight`=1.0. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
