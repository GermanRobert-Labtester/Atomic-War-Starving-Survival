# Subject Plan 175.08 — The Second Shift: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_nurse_milestone_2` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **competence shown through a witnessed moment**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `The Second Shift`. Current body: 32 whitespace-delimited words / 201 characters.
- **VERIFIED source text:**

> The nurse runs a second shift on the sickbeds without being asked. They move between the bunks checking pulses, adjusting bandages, speaking quietly. Somebody says the clinic feels like a clinic again.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_nurse_milestone_2", `minDay`=20, `title`="The Second Shift", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
