# Subject Plan 175.35 — North, Still There: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_brass_compass_restored` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **a repaired object re-entering communal life**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `North, Still There`. Current body: 41 whitespace-delimited words / 232 characters.
- **VERIFIED source text:**

> The needle circles once and settles, exactly where it has always pointed. Someone lays it beside the shelter map and checks the bearing twice. It tells them nothing they did not know, and they check it again the next morning anyway.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_brass_compass_restored", `minDay`=10, `title`="North, Still There", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
