# Subject Plan 175.28 — The Camera Sees: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_camera_restored` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **a repaired object re-entering communal life**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `The Camera Sees`. Current body: 39 whitespace-delimited words / 205 characters.
- **VERIFIED source text:**

> The old camera is wound, focused, and pointed at the bunker. The first photograph is of the doorway, the second of the children, the third of the person who fixed it. There is film left for a handful more.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_camera_restored", `minDay`=14, `title`="The Camera Sees", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
