# Subject Plan 175.45 — The Sky Has Detail: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_telescope_restored` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **a repaired object re-entering communal life**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `The Sky Has Detail`. Current body: 49 whitespace-delimited words / 275 characters.
- **VERIFIED source text:**

> The telescope is carried up to the surface hatch at dusk and aimed at a sky everyone usually reads only for weather. Two people quietly argue over which bright point is a planet. The argument lasts until the cold sends everyone below, and nobody settles it, and nobody minds.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_telescope_restored", `minDay`=15, `title`="The Sky Has Detail", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
