# Subject Plan 175.01 — The reservoir turns into a countable loss
Lane: A · Cluster: C1 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `incident_water_contamination` in `Assets/StreamingAssets/Data/incidents.json` and polish it only if the editorial pass identifies a material gap. Its register is **water scarcity and the immediate choice**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `Water Contamination`. Current body: 23 whitespace-delimited words / 154 characters.
- **VERIFIED source text:**

> The reservoir alarms blare a sickening two-tone klaxon. Corrosive silt has breached the primary purifiers, turning our drinking water a rusty, oily brown.
...
Editorial question: **Develop the sensory clue and the moment the reading confirms contamination, then keep the quantity of clean water visible through one person’s practical response. Preserve the current dilemma: discard the supply and accept thirst, without implying boiling or improvised treatment makes it safe.** The output is a decision per field with a continuity reason. If the only benefit is greater length, retain the current text.

## What must not change
Do not add a contaminant, medical claim, purifier repair, water amount, or choice. Keep this within the sealed five-field incident surface; do not invent a follow-up dispatch.

Preserve every non-prose field byte-for-byte in meaning: `id`="incident_water_contamination", `minDay`=15, `title`="Water Contamination", `weight`=1.0. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
