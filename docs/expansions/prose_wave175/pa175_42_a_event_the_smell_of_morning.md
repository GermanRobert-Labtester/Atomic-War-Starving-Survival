# Subject Plan 175.42 — The Smell of Morning: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_coffee_grinder_restored` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **a repaired object re-entering communal life**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `The Smell of Morning`. Current body: 45 whitespace-delimited words / 262 characters.
- **VERIFIED source text:**

> The crank complains through its first turn and finds its rhythm by the fifth. There is barely enough coffee left to justify grinding, but someone measures out a handful anyway. The smell reaches the far corridor, and people find reasons to walk past the kitchen.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_coffee_grinder_restored", `minDay`=12, `title`="The Smell of Morning", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
