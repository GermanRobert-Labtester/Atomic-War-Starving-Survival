# Subject Plan 175.49 — Grain Refiner: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_miller_milestone_2` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **competence shown through a witnessed moment**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `Grain Refiner`. Current body: 51 whitespace-delimited words / 291 characters.
- **VERIFIED source text:**

> The millstone is dressed, the spindle balanced, and the flour comes out fine enough to pass through cloth. The morning mash has become proper loaves, and the shelter ate the first one quietly, the way you eat something you were afraid was gone. The miller watched the eating, not the praise.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_miller_milestone_2", `minDay`=20, `title`="Grain Refiner", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
