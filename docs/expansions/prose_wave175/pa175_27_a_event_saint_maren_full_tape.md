# Subject Plan 175.27 — Saint Maren: Full Tape: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_cassette_saint_maren_complete` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **archival sound, listener response, and a bounded memory**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `Saint Maren: Full Tape`. Current body: 38 whitespace-delimited words / 215 characters.
- **VERIFIED source text:**

> The tape from Saint Maren plays through. A woman's voice describes the clinic before the exchange: beds, names, a harvest festival. Then the ash came, and the voice says only that the clinic held until it could not.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_cassette_saint_maren_complete", `minDay`=18, `title`="Saint Maren: Full Tape", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
