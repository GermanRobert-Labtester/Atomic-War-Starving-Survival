# Subject Plan 175.11 — Master Electrician: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_trade_mastery_electrician` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **competence shown through a witnessed moment**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `Master Electrician`. Current body: 34 whitespace-delimited words / 211 characters.
- **VERIFIED source text:**

> The electrician has mastered the pre-war trade of their profession. Their hands find the right wire by instinct. The bunker's lights flicker less often now, and the hum of the generators sounds almost confident.
...
Editorial question: **Let mastery appear in a specific piece of work and in how someone else relies on it. Preserve the authored profession and claim already in “Master Electrician”; do not turn recognition into a new reward announcement.** The output is a decision per field with a continuity reason. If the only benefit is greater length, retain the current text.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_trade_mastery_electrician", `minDay`=40, `title`="Master Electrician", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
