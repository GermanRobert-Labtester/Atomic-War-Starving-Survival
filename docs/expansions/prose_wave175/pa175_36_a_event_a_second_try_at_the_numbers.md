# Subject Plan 175.36 — A Second Try at the Numbers: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `apprenticeship_completion_signal_ear` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **learning as a shared task, without new progression rules**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `A Second Try at the Numbers`. Current body: 42 whitespace-delimited words / 252 characters.
- **VERIFIED source text:**

> The radio apprentice wrote the numbers station transmission twice in two nights without being asked, in the same hand, on the same slips of paper, and slipped them under the operator's door before dawn. The operator confirmed both. Neither was a fluke.
...
Editorial question: **Keep the apprentice’s work and mentor’s response distinct. The scene may show uncertainty, correction, or earned trust already implied by “A Second Try at the Numbers,” but it must not award a skill, change task state, or invent a progression threshold.** The output is a decision per field with a continuity reason. If the only benefit is greater length, retain the current text.
...
Preserve every non-prose field byte-for-byte in meaning: `choices`=[{"choiceId": "promote_to_listener", "text": "Add the apprentice to the listening rotation. Saturday only.", "moraleDelta": 4.0, "effects": [{"setWorldFlag": "app_signal_ear_complete", "worldFlagValue": true}]}], `conditions`={"RequireChildCohort": true}, `id`="apprenticeship_completion_signal_ear", `minDay`=35, `title`="A Second Try at the Numbers", `weight`=1.2. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
