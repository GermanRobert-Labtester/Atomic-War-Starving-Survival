# Subject Plan 175.18 — Decoder Back Online: a focused prose pass
Lane: A · Cluster: C8 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `frequency_decoder_reset` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **radio repair as a small return of shared information**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `Decoder Back Online`. Current body: 36 whitespace-delimited words / 224 characters.
- **VERIFIED source text:**

> The frequency decoder hums back to life at the communications array. The tuning dial finds its marks again and the signal ladder climbs. Whatever the wasteland is saying on its hidden frequencies, someone is listening again.
...
Editorial question: **Keep the repair and first test audible in the existing communications context. Clarify what listeners verify, but do not introduce a new station, decoded message, or signal-follow-up chain.** The output is a decision per field with a continuity reason. If the only benefit is greater length, retain the current text.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="frequency_decoder_reset", `minDay`=25, `title`="Decoder Back Online", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
