# Subject Plan 175.20 — The Music Box Turns: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_music_box_restored` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **a repaired object re-entering communal life**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `The Music Box Turns`. Current body: 37 whitespace-delimited words / 199 characters.
- **VERIFIED source text:**

> The music box plays its little tune to the end without skipping. Somebody is crying quietly in the corner, and nobody pretends it is the dust. The tune is three minutes long; it will be played again.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_music_box_restored", `minDay`=10, `title`="The Music Box Turns", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
