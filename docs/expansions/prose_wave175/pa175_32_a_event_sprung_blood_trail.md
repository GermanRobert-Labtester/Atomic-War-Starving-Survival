# Subject Plan 175.32 — Sprung, Blood Trail: a focused prose pass
Lane: A · Cluster: C15 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `trap_sprung_blood_trail` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **evidence at the perimeter and unresolved cause**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `Sprung, Blood Trail`. Current body: 40 whitespace-delimited words / 231 characters.
- **VERIFIED source text:**

> The trap is pulled crooked and the ground beneath it is dark with blood. Whatever touched the wire left under its own strength—or something else carried it away. The trail vanishes where the wind has swept dust across the concrete.
...
Editorial question: **Build the passage around the physical evidence named by “Sprung, Blood Trail.” Leave explanations open where the record does; do not change a trap result or guarantee a creature identity.** The output is a decision per field with a continuity reason. If the only benefit is greater length, retain the current text.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="trap_sprung_blood_trail", `minDay`=1, `title`="Sprung, Blood Trail", `weight`=1.0. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
