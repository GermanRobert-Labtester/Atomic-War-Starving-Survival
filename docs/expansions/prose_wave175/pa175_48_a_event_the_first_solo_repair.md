# Subject Plan 175.48 — The First Solo Repair: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `apprenticeship_completion_rough_repairs` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **learning as a shared task, without new progression rules**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `The First Solo Repair`. Current body: 51 whitespace-delimited words / 283 characters.
- **VERIFIED source text:**

> Eli's apprentice turned the elbow joint alone today. The packing went in clean, the thread tape was the right width, and the joint held when Marcus ran the test pressure. The apprentice did not say anything. Eli did not say anything. The bunker was quiet for the rest of the evening.

- **VERIFIED:** this record’s actual fields are `bodyText, choices, conditions, id, minDay, title, weight`. Body text is the sole proposed prose surface; optional per-record fields must not be inferred from a different catalog record.
...
Editorial question: **Keep the apprentice’s work and mentor’s response distinct. The scene may show uncertainty, correction, or earned trust already implied by “The First Solo Repair,” but it must not award a skill, change task state, or invent a progression threshold.** The output is a decision per field with a continuity reason. If the only benefit is greater length, retain the current text.
...
Preserve every non-prose field byte-for-byte in meaning: `choices`=[{"choiceId": "praise_apprentice", "text": "Tell the apprentice, in front of the bunker, that it held.", "moraleDelta": 4.0, "effects": [{"setWorldFlag": "apprentice_praised", "worldFlagValue": true}, {"setWorldFlag": "app_rough_repairs_complete", "worldFlagValue": true}]}, {"choiceId": "silent_recognition", "text": "Say nothing. The apprentice will learn that work is its own reward.", "moraleDelta": -1.0, "effects": [{"setWorldFlag": "app_rough_repairs_complete", "worldFlagValue": true}]}], `conditions`={"RequireChildCohort": true}, `id`="apprenticeship_completion_rough_repairs", `minDay`=30, `title`="The First Solo Repair", `weight`=1.4. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
