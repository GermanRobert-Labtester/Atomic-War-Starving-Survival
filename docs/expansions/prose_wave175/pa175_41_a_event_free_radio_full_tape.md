# Subject Plan 175.41 — Free Radio: Full Tape: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_cassette_free_radio_complete` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **archival sound, listener response, and a bounded memory**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `Free Radio: Full Tape`. Current body: 45 whitespace-delimited words / 247 characters.
- **VERIFIED source text:**

> The free radio tape plays to its end. A pirate broadcaster reads the news of the last week before the exchange: weather, a football result, a recipe, a joke. The signal signs off with the station's name, said proudly, into a world about to change.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_cassette_free_radio_complete", `minDay`=18, `title`="Free Radio: Full Tape", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
