# Subject Plan 175.25 — First Circuit Restored: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_electrician_milestone_1` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **competence shown through a witnessed moment**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `First Circuit Restored`. Current body: 38 whitespace-delimited words / 212 characters.
- **VERIFIED source text:**

> The first repaired circuit lights a single bulb in the workshop. It is not much: one bulb, one switch, one length of wire made whole. The electrician stands under it with their hands in their pockets, not moving.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_electrician_milestone_1", `minDay`=30, `title`="First Circuit Restored", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
