# Subject Plan 175.44 — Family Bunker: Full Tape: a focused prose pass
Lane: A · Cluster: C9 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `narrative_cassette_family_bunker_complete` in `Assets/StreamingAssets/Data/events.json` and polish it only if the editorial pass identifies a material gap. Its register is **archival sound, listener response, and a bounded memory**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `Family Bunker: Full Tape`. Current body: 49 whitespace-delimited words / 253 characters.
- **VERIFIED source text:**

> The family bunker tape plays in full. A father narrates the building of the shelter, the fitting of the door, the stocking of the shelves, explaining it all to a child too young to understand. At the end he says: if you are hearing this, we made it out.
...
Editorial question: **Focus on listening and the listeners’ present reaction to “Family Bunker: Full Tape.” Do not write new quoted tape dialogue or alter what the archive establishes; provenance and completeness remain as authored.** The output is a decision per field with a continuity reason. If the only benefit is greater length, retain the current text.
...
Preserve every non-prose field byte-for-byte in meaning: `id`="narrative_cassette_family_bunker_complete", `minDay`=18, `title`="Family Bunker: Full Tape", `weight`=1. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
