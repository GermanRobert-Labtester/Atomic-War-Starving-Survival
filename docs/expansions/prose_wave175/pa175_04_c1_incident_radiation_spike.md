# Subject Plan 175.04 — The dosimeters rise in Sector Four
Lane: A · Cluster: C1 · Status: PROPOSAL (subject-level; serial queue; not an integration plan)

## Subject
Review the current `bodyText` for `incident_radiation_spike` in `Assets/StreamingAssets/Data/incidents.json` and polish it only if the editorial pass identifies a material gap. Its register is **measured danger at the ventilation boundary**. Deepen the existing content rather than create a system or imply an unencoded outcome. “Text is sufficient” is an acceptable no-change closeout.

## Premise evidence
- **VERIFIED:** exact selector is live. Current title: `Radiation Spike`. Current body: 33 whitespace-delimited words / 191 characters.
- **VERIFIED source text:**

> The dosimeters scream to life in Sector 4. A toxic cloud of irradiated ash has drifted over the ventilation intakes, spiking levels to a dangerous 1.2 mSv/hr. The air tastes faintly of metal.
...
Editorial question: **Expand the shift from instrument alarm to shelter response: who notices the reading, what boundary they are watching, and what the quiet cost is while the ash cloud passes. Treat the existing reading as fact and avoid adding a radiation rule.** The output is a decision per field with a continuity reason. If the only benefit is greater length, retain the current text.

## What must not change
No new dose figures, medical symptoms, immunity claims, exposure procedure, or protection item. Preserve all non-prose fields and the Case D seal.

Preserve every non-prose field byte-for-byte in meaning: `id`="incident_radiation_spike", `minDay`=20, `title`="Radiation Spike", `weight`=1.0. Only `bodyText` may be considered; nested choices, costs, flags, deltas, conditions, gates, and order remain unchanged. Keep JSON valid and escape any new line breaks. Do not disturb encounter scheduling, event weights, day gates, deterministic selection, or sealed vocabulary boundaries. No new identifier, trigger contract, save state, route, or panel is in scope.
