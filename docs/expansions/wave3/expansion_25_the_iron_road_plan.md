...
# 2026-09-24 integration expansion — Railway Interlock Engine
...
**Objective:** one authored junction denies an illegal dispatch, reserves a legal route once, and releases it after movement or cancellation.

**Current evidence:** RailwaySystem and RailTrackMaintenanceLedger are hosted. RailwayInterlockEngine has three authored junctions and mutable route locks, reservations, signals, obstruction, and tamper state, but no direct src caller. The Iron Road plan itself says the interlock alone authorizes routes, so rail dispatch cannot simply infer legality from maintenance feasibility.

**Owner boundary:** RailwaySystem owns topology/dispatch; interlock owns junction legality and reservations; maintenance ledger owns wear and load feasibility. **Core source:** `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`. **Current or candidate host:** `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. **Persistence decision:** The interlock's own CaptureState/RestoreState must be composed with the railway lifecycle; choose one registered section after a save-shape audit. **Authored data:** `railway_interlock_catalog.json`. This follows the master expansion document's C5 and C15 cluster and its four-tier data → Core → host → verification structure. Live source outranks the master when a dated example is stale.

**Non-goals and stop rule:** No second rail topology, no route reservation without a dispatch ID, and no forced path through a denied signal. This documentation does not register a new save section, modify a host, or certify player reachability. A foreman must claim exact implementation paths under `WORKTREE_OWNERSHIP.md`; a collision or missing authority returns a premise finding rather than an invented parallel subsystem.

## 2. Integration architecture and state flow

```text
railway_interlock_catalog.json / canonical owner facts
    -> RailwayInterlockEngine
    -> Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession
    -> railway topology/dispatch and rail_maintenance are existing sections; decide interlock state location without duplicating either
...
|---|---|---|
| Which Core type actually runs? | `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs` | Resolve exact constructor, method signatures, input ports, output DTOs, and indirect callers. |
| Which method can the host call? | `BindCatalog, RestoreJunction, RequestRoute, ReleaseRoute, IsExpeditionPathLegal, TickDay, CaptureState, RestoreState` | Use current API; introduce a thin adapter only if source cannot express the bounded outcome. |
| Who owns cross-system state? | `RailwaySystem owns topology/dispatch; interlock owns junction legality and reservations; maintenance ledger owns wear and load feasibility` | One writer per resource, result, and persistent record. |
| What survives save/load? | `railway topology/dispatch and rail_maintenance are existing sections; decide interlock state location without duplicating either` | Capture/restore and old-save default or explicit stateless verdict. |
| What is authored? | `railway_interlock_catalog.json` | Verify loader, IDs, references, range validation, and an actual consumer. |
| What does the player see? | `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession` | Trace a command or read route through the current Godot lifecycle. |
...
|---|---|---|
| `junction_holdfast_terminal` | `junction_id`, `display_name`, `map_node_id`, `connected_route_ids`, `main_route_id`, `siding_route_ids`, `interlock_type` | Which loader, owner predicate, host command, and read surface consume this row? |
| `junction_delta` | `junction_id`, `display_name`, `map_node_id`, `connected_route_ids`, `main_route_id`, `siding_route_ids`, `interlock_type` | Which loader, owner predicate, host command, and read surface consume this row? |
| `junction_switchyard` | `junction_id`, `display_name`, `map_node_id`, `connected_route_ids`, `main_route_id`, `siding_route_ids`, `interlock_type` | Which loader, owner predicate, host command, and read surface consume this row? |

### Review case 001: junction restoration under first valid use

**Scene premise.** The player's route reaches **junction restoration** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “The locked points lever is left where the person making the next decision can see it.” is a candidate observation anchored by a locked points lever. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `BindCatalog, RestoreJunction, RequestRoute, ReleaseRoute, IsExpeditionPathLegal, TickDay, CaptureState, RestoreState` after its actual input owners have been sampled. RailwaySystem owns topology/dispatch; interlock owns junction legality and reservations; maintenance ledger owns wear and load feasibility. For first valid use, the mandatory boundary is to prove the default state, eligibility, and one visible outcome. Before confirmation, show the canonical cost and status from the same snapshot that will be rechecked at commit. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.

**Save and replay reading.** Persistence decision: The interlock's own CaptureState/RestoreState must be composed with the railway lifecycle; choose one registered section after a save-shape audit. The audit must show the before-state, the accepted or refused result, the after-state, and the restore-state. A duplicate event must not spend stock twice. A day transition must not advance twice. Stable ID order and a campaign seed are required where selection or risk is random; where the function is deterministic math, no RNG is added. The panel should obtain the same status after reopening and should release subscriptions when disposed. The author should not refer to a live result until the save and host trace confirms it.

**Acceptance decision.** Mark this card accepted only with source file:line references, a live trigger, the exact method call, a reachable authored ID or owner record, an observable current surface, and a focused assertion matched to prove the default state, eligibility, and one visible outcome. Otherwise mark it deferred or inapplicable. A high-level compilation result does not prove a player route, a catalog file does not prove a loader, and a historical audit count does not prove that the class is still host-unreachable. The first implementation package should choose only the cases necessary to establish one authored junction denies an illegal dispatch, reserves a legal route once, and releases it after movement or cancellation.

### Review case 002: switch alignment under first valid use

**Scene premise.** The player's route reaches **switch alignment** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “A note beside the a locked points lever records the uncertainty before anyone writes an outcome.” is a candidate observation anchored by a locked points lever. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.
...
### Review case 003: route request under first valid use

**Scene premise.** The player's route reaches **route request** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “The locked points lever carries a date and a name; neither is proof that the work is finished.” is a candidate observation anchored by a locked points lever. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.
...
### Review case 004: signal aspect under first valid use

**Scene premise.** The player's route reaches **signal aspect** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “Someone checks the a locked points lever against the ledger, then leaves the blank field blank.” is a candidate observation anchored by a locked points lever. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.
...
### Review case 005: reservation under first valid use

**Scene premise.** The player's route reaches **reservation** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “A second hand reaches for the a locked points lever, waits, and asks who signed the first entry.” is a candidate observation anchored by a locked points lever. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.
...
### Review case 006: obstruction under first valid use

**Scene premise.** The player's route reaches **obstruction** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 007: tamper report under first valid use

**Scene premise.** The player's route reaches **tamper report** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 008: denial arm under first valid use

**Scene premise.** The player's route reaches **denial arm** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 009: denial trigger under first valid use

**Scene premise.** The player's route reaches **denial trigger** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 010: dispatch handoff under first valid use

**Scene premise.** The player's route reaches **dispatch handoff** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 011: train crossing under first valid use

**Scene premise.** The player's route reaches **train crossing** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 012: release route under first valid use

**Scene premise.** The player's route reaches **release route** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 013: track repair under first valid use

**Scene premise.** The player's route reaches **track repair** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 014: bridge load under first valid use

**Scene premise.** The player's route reaches **bridge load** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 015: old rail save under first valid use

**Scene premise.** The player's route reaches **old rail save** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 016: signal panel under first valid use

**Scene premise.** The player's route reaches **signal panel** while the canonical owner has the required record and no earlier accepted command. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 017: junction restoration under legacy restore

**Scene premise.** The player's route reaches **junction restoration** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “A note beside the a locked points lever records the uncertainty before anyone writes an outcome.” is a candidate observation anchored by a locked points lever. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `BindCatalog, RestoreJunction, RequestRoute, ReleaseRoute, IsExpeditionPathLegal, TickDay, CaptureState, RestoreState` after its actual input owners have been sampled. RailwaySystem owns topology/dispatch; interlock owns junction legality and reservations; maintenance ledger owns wear and load feasibility. For legacy restore, the mandatory boundary is to default additive state safely and never replay an old effect. Before confirmation, show the canonical cost and status from the same snapshot that will be rechecked at commit. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.
...
### Review case 018: switch alignment under legacy restore

**Scene premise.** The player's route reaches **switch alignment** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 019: route request under legacy restore

**Scene premise.** The player's route reaches **route request** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 020: signal aspect under legacy restore

**Scene premise.** The player's route reaches **signal aspect** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 021: reservation under legacy restore

**Scene premise.** The player's route reaches **reservation** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 022: obstruction under legacy restore

**Scene premise.** The player's route reaches **obstruction** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 023: tamper report under legacy restore

**Scene premise.** The player's route reaches **tamper report** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 024: denial arm under legacy restore

**Scene premise.** The player's route reaches **denial arm** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 025: denial trigger under legacy restore

**Scene premise.** The player's route reaches **denial trigger** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 026: dispatch handoff under legacy restore

**Scene premise.** The player's route reaches **dispatch handoff** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 027: train crossing under legacy restore

**Scene premise.** The player's route reaches **train crossing** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 028: release route under legacy restore

**Scene premise.** The player's route reaches **release route** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 029: track repair under legacy restore

**Scene premise.** The player's route reaches **track repair** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 030: bridge load under legacy restore

**Scene premise.** The player's route reaches **bridge load** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 031: old rail save under legacy restore

**Scene premise.** The player's route reaches **old rail save** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 032: signal panel under legacy restore

**Scene premise.** The player's route reaches **signal panel** while the campaign was saved before the proposed adapter existed. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 033: junction restoration under repeat request

**Scene premise.** The player's route reaches **junction restoration** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “The locked points lever carries a date and a name; neither is proof that the work is finished.” is a candidate observation anchored by a locked points lever. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `BindCatalog, RestoreJunction, RequestRoute, ReleaseRoute, IsExpeditionPathLegal, TickDay, CaptureState, RestoreState` after its actual input owners have been sampled. RailwaySystem owns topology/dispatch; interlock owns junction legality and reservations; maintenance ledger owns wear and load feasibility. For repeat request, the mandatory boundary is to return the current status without charging or publishing twice. Before confirmation, show the canonical cost and status from the same snapshot that will be rechecked at commit. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.
...
### Review case 034: switch alignment under repeat request

**Scene premise.** The player's route reaches **switch alignment** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 035: route request under repeat request

**Scene premise.** The player's route reaches **route request** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 036: signal aspect under repeat request

**Scene premise.** The player's route reaches **signal aspect** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 037: reservation under repeat request

**Scene premise.** The player's route reaches **reservation** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 038: obstruction under repeat request

**Scene premise.** The player's route reaches **obstruction** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 039: tamper report under repeat request

**Scene premise.** The player's route reaches **tamper report** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 040: denial arm under repeat request

**Scene premise.** The player's route reaches **denial arm** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 041: denial trigger under repeat request

**Scene premise.** The player's route reaches **denial trigger** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 042: dispatch handoff under repeat request

**Scene premise.** The player's route reaches **dispatch handoff** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 043: train crossing under repeat request

**Scene premise.** The player's route reaches **train crossing** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 044: release route under repeat request

**Scene premise.** The player's route reaches **release route** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 045: track repair under repeat request

**Scene premise.** The player's route reaches **track repair** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 046: bridge load under repeat request

**Scene premise.** The player's route reaches **bridge load** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 047: old rail save under repeat request

**Scene premise.** The player's route reaches **old rail save** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 048: signal panel under repeat request

**Scene premise.** The player's route reaches **signal panel** while the same player request arrives twice before the next campaign tick. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 049: junction restoration under stale preview

**Scene premise.** The player's route reaches **junction restoration** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “Someone checks the a locked points lever against the ledger, then leaves the blank field blank.” is a candidate observation anchored by a locked points lever. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `BindCatalog, RestoreJunction, RequestRoute, ReleaseRoute, IsExpeditionPathLegal, TickDay, CaptureState, RestoreState` after its actual input owners have been sampled. RailwaySystem owns topology/dispatch; interlock owns junction legality and reservations; maintenance ledger owns wear and load feasibility. For stale preview, the mandatory boundary is to revalidate at the command boundary and explain the new refusal. Before confirmation, show the canonical cost and status from the same snapshot that will be rechecked at commit. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.
...
### Review case 050: switch alignment under stale preview

**Scene premise.** The player's route reaches **switch alignment** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “A second hand reaches for the a locked points lever, waits, and asks who signed the first entry.” is a candidate observation anchored by a locked points lever. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `BindCatalog, RestoreJunction, RequestRoute, ReleaseRoute, IsExpeditionPathLegal, TickDay, CaptureState, RestoreState` after its actual input owners have been sampled. RailwaySystem owns topology/dispatch; interlock owns junction legality and reservations; maintenance ledger owns wear and load feasibility. For stale preview, the mandatory boundary is to revalidate at the command boundary and explain the new refusal. After a refusal, retain the prior state and give a concrete reason in the player's current surface. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.

**Save and replay reading.** Persistence decision: The interlock's own CaptureState/RestoreState must be composed with the railway lifecycle; choose one registered section after a save-shape audit. The audit must show the before-state, the accepted or refused result, the after-state, and the restore-state. A duplicate event must not spend stock twice. A day transition must not advance twice. Stable ID order and a campaign seed are required where selection or risk is random; where the function is deterministic math, no RNG is added. The panel should obtain the same status after reopening and should release subscriptions when disposed. The author should not refer to a live result until the save and host trace confirms it.

**Acceptance decision.** Mark this card accepted only with source file:line references, a live trigger, the exact method call, a reachable authored ID or owner record, an observable current surface, and a focused assertion matched to revalidate at the command boundary and explain the new refusal. Otherwise mark it deferred or inapplicable. A high-level compilation result does not prove a player route, a catalog file does not prove a loader, and a historical audit count does not prove that the class is still host-unreachable. The first implementation package should choose only the cases necessary to establish one authored junction denies an illegal dispatch, reserves a legal route once, and releases it after movement or cancellation.

### Review case 051: route request under stale preview

**Scene premise.** The player's route reaches **route request** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “The locked points lever is left where the person making the next decision can see it.” is a candidate observation anchored by a locked points lever. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `BindCatalog, RestoreJunction, RequestRoute, ReleaseRoute, IsExpeditionPathLegal, TickDay, CaptureState, RestoreState` after its actual input owners have been sampled. RailwaySystem owns topology/dispatch; interlock owns junction legality and reservations; maintenance ledger owns wear and load feasibility. For stale preview, the mandatory boundary is to revalidate at the command boundary and explain the new refusal. After success, the relevant owner may emit one fact; presentation can paraphrase it but cannot create another reward. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.

**Save and replay reading.** Persistence decision: The interlock's own CaptureState/RestoreState must be composed with the railway lifecycle; choose one registered section after a save-shape audit. The audit must show the before-state, the accepted or refused result, the after-state, and the restore-state. A duplicate event must not spend stock twice. A day transition must not advance twice. Stable ID order and a campaign seed are required where selection or risk is random; where the function is deterministic math, no RNG is added. The panel should obtain the same status after reopening and should release subscriptions when disposed. The author should not refer to a live result until the save and host trace confirms it.

**Acceptance decision.** Mark this card accepted only with source file:line references, a live trigger, the exact method call, a reachable authored ID or owner record, an observable current surface, and a focused assertion matched to revalidate at the command boundary and explain the new refusal. Otherwise mark it deferred or inapplicable. A high-level compilation result does not prove a player route, a catalog file does not prove a loader, and a historical audit count does not prove that the class is still host-unreachable. The first implementation package should choose only the cases necessary to establish one authored junction denies an illegal dispatch, reserves a legal route once, and releases it after movement or cancellation.

### Review case 052: signal aspect under stale preview

**Scene premise.** The player's route reaches **signal aspect** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?

**Possible prose register.** “A note beside the a locked points lever records the uncertainty before anyone writes an outcome.” is a candidate observation anchored by a locked points lever. Its final form must identify an actual speaker or document type, avoid knowledge the source cannot possess, and use the result's real status language. The line is deferred if the item, person, or record is not present. A journal note may record the player's knowledge after a fact is published; a panel can describe current eligibility; neither can invent an upstream event. The content editor should compare nearby JSON rows and avoid a second paragraph that only repeats this one in different words.

**Command and owner trace.** The relevant Core call comes from `BindCatalog, RestoreJunction, RequestRoute, ReleaseRoute, IsExpeditionPathLegal, TickDay, CaptureState, RestoreState` after its actual input owners have been sampled. RailwaySystem owns topology/dispatch; interlock owns junction legality and reservations; maintenance ledger owns wear and load feasibility. For stale preview, the mandatory boundary is to revalidate at the command boundary and explain the new refusal. After restore, rebuild the view from saved owner state without calling the gameplay command again. If the named system is a pure evaluator, the caller applies any accepted result and the evaluator keeps no independent state. If it is mutable, the host captures its state through the selected owner. The decision must not create a parallel inventory, medical record, route reservation, award list, threat contact, or merchant stock entry.

**Save and replay reading.** Persistence decision: The interlock's own CaptureState/RestoreState must be composed with the railway lifecycle; choose one registered section after a save-shape audit. The audit must show the before-state, the accepted or refused result, the after-state, and the restore-state. A duplicate event must not spend stock twice. A day transition must not advance twice. Stable ID order and a campaign seed are required where selection or risk is random; where the function is deterministic math, no RNG is added. The panel should obtain the same status after reopening and should release subscriptions when disposed. The author should not refer to a live result until the save and host trace confirms it.

**Acceptance decision.** Mark this card accepted only with source file:line references, a live trigger, the exact method call, a reachable authored ID or owner record, an observable current surface, and a focused assertion matched to revalidate at the command boundary and explain the new refusal. Otherwise mark it deferred or inapplicable. A high-level compilation result does not prove a player route, a catalog file does not prove a loader, and a historical audit count does not prove that the class is still host-unreachable. The first implementation package should choose only the cases necessary to establish one authored junction denies an illegal dispatch, reserves a legal route once, and releases it after movement or cancellation.

### Review case 053: reservation under stale preview

**Scene premise.** The player's route reaches **reservation** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 054: obstruction under stale preview

**Scene premise.** The player's route reaches **obstruction** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 055: tamper report under stale preview

**Scene premise.** The player's route reaches **tamper report** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 056: denial arm under stale preview

**Scene premise.** The player's route reaches **denial arm** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 057: denial trigger under stale preview

**Scene premise.** The player's route reaches **denial trigger** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 058: dispatch handoff under stale preview

**Scene premise.** The player's route reaches **dispatch handoff** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 059: train crossing under stale preview

**Scene premise.** The player's route reaches **train crossing** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 060: release route under stale preview

**Scene premise.** The player's route reaches **release route** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_switchyard`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 061: track repair under stale preview

**Scene premise.** The player's route reaches **track repair** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_holdfast_terminal`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
### Review case 062: bridge load under stale preview

**Scene premise.** The player's route reaches **bridge load** while the facts used by the panel changed before confirmation. The implementation candidate starts with `junction_delta`. The live source checkpoint is `Assets/Ashfall.Core/Expeditions/RailwayInterlockEngine.cs`; the proposed reader is `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession`. The scene exists only if the current catalog or owner provides this trigger. No date, character, equipment, hazard, or reward is asserted merely because the heading names it. This makes the card a precise question for the promotion audit: what real fact causes this view or command to exist at this point in the campaign?
...
## 7. Finished planning architecture for Railway Interlock Engine

The architecture handoff is complete at the planning level: `railway_interlock_catalog.json` or current owner facts → `RailwayInterlockEngine` → `Main.Railway / RailwaySystem plus RailTrackMaintenanceHostSession` → `railway topology/dispatch and rail_maintenance are existing sections; decide interlock state location without duplicating either` only where state is mutable → a truthful player read model. The first package is **one authored junction denies an illegal dispatch, reserves a legal route once, and releases it after movement or cancellation**. The 62 case cards above are a candidate review register; no card is canon or a mandate to build all combinations. The exact files and owner signatures must be rechecked when a foreman promotes the package.
