# Subject Plan 177.24 — A Tin With Bearings: focused event prose review
...
Review the exact event `ration_theft_with_evidence` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 76 whitespace-delimited words / 371 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `ration_theft_with_evidence` exists once in the current 240-record `events` array.
- **VERIFIED:** title `A Tin With Bearings`; exact observed fields: `bodyText, choices, conditions, id, minDay, title, weight`.
- **Current source prose:**

> A tin was taken, and someone who did not need it saw the taking, and the sawdust block fell out of a pocket, and the block has a print on it, and the print belongs to the second bunk, and the second bunk takes the lead medic's spoon in the morning. The evidence sits in a cup on the council table, and the cup has not been discussed, and the cup is going to be discussed.

- **Frozen non-prose snapshot:** `{"choices": [{"choiceId": "confront_in_private", "effects": [{"setWorldFlag": "ration_theft_second_bunk_named", "worldFlagValue": true}, {"scheduleEventId": "escalation_sabotage", "scheduleOnDay": 51}], "moraleDelta": -5.0, "text": "Pull the second-bunk into the council sheet. The fingerprint is the evidence. The council sheet is the verdict."}, {"choiceId": "tally_more_often", "effects": [{"setWorldFlag": "ration_double_tally_week", "worldFlagValue": true}], "moraleDelta": -2.0, "text": "Tally twice a day for a week. The pencil sits down. The lead does not name."}], "conditions": {"RequireRationTheft": true}, "id": "ration_theft_with_evidence", "minDay": 50, "title": "A Tin With Bearings", "weight": 1.3}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
