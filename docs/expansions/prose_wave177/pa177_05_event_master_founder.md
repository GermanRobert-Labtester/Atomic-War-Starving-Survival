# Subject Plan 177.05 — Master Founder: focused event prose review
...
- **VERIFIED:** exact selector `narrative_metallurgist_mastery` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Master Founder`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> A pump impeller cast in sand, machined on the lathe, pressed onto the shaft with an interference fit. Zero casting voids found on inspection. The pump runs quieter than the original because the impeller is balanced closer to true than the factory part was. The metallurgist does not explain why a salvage-cast impeller would outperform a factory part. They accept that it does and document the mold geometry in the technical file.

- **Frozen non-prose snapshot:** `{"id": "narrative_metallurgist_mastery", "minDay": 30, "title": "Master Founder", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
