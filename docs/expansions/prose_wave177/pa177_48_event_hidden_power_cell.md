# Subject Plan 177.48 — Hidden Power Cell: focused event prose review
...
Review the exact event `power_cell_discovery` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 120 whitespace-delimited words / 690 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `power_cell_discovery` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Hidden Power Cell`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> While clearing debris from the collapsed storage alcove, your hand closes on something solid: a sealed power cell, military markings, still showing a full charge on the test strip. It has been sitting behind the wall panel since before the exchange, untouched, waiting. It could power the radio for weeks, and the radio is your only window on the world outside the bunker: the broadcasts, the band channels, the weather. Or it could run the air filtration at night, or keep the store lights on when the generator sleeps. The cell is one. The needs are many. Everyone has an opinion about where it should go, and they are all reasonable, and they are all wrong about each other's priorities.

- **Frozen non-prose snapshot:** `{"id": "power_cell_discovery", "minDay": 15, "title": "Hidden Power Cell", "weight": 0.7}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
