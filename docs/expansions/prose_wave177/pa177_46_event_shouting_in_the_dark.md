# Subject Plan 177.46 — Shouting in the Dark: focused event prose review
...
Review the exact event `morale_crisis` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 119 whitespace-delimited words / 649 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `morale_crisis` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Shouting in the Dark`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> Voices carry through the bunker like they own it. Two of your people, the ones who rationed the water together last winter, are standing over the medical tin arguing about the last iodine pill. The tin is empty after this. Both of them have been in the open since the storm, both have the headaches that mean a dose, and both are right. The argument is loud enough that nobody in the sleeping quarters pretends to sleep. Whatever you do now will be remembered. Take the pill and you choose a side. Split it and you insult both. Order them apart and the question just waits in the tin, unanswered, until someone decides to answer it with their hands.

- **Frozen non-prose snapshot:** `{"id": "morale_crisis", "minDay": 8, "title": "Shouting in the Dark", "weight": 1.0}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
