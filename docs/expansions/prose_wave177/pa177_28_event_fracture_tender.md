# Subject Plan 177.28 — Fracture Tender: focused event prose review
...
Review the exact event `narrative_bone_setter_milestone_1` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 78 whitespace-delimited words / 425 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `narrative_bone_setter_milestone_1` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Fracture Tender`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> A fractured fibula set without anesthetic using traction cord looped over the bunk frame and a folded towel as the fulcrum. The patient bit through the corner of a leather belt and did not cry out, which was a mercy for everyone else in the medical bay. The bone is straight now. By next month it will be load-bearing. By spring, it will carry the person into the field again. Medicine in this shelter is measured in springs.

- **Frozen non-prose snapshot:** `{"id": "narrative_bone_setter_milestone_1", "minDay": 10, "title": "Fracture Tender", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
