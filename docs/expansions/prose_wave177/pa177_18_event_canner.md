# Subject Plan 177.18 — Canner: focused event prose review
...
Review the exact event `narrative_preservationist_milestone_2` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 75 whitespace-delimited words / 439 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `narrative_preservationist_milestone_2` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Canner`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> Sealing glass jars with paraffin wax and processing them in a boiling water bath is the oldest preservation technique that still works when the glass is not cracked and the wax is not contaminated and the water is actually boiling and not just steaming. All of those conditions are now reliably met in this shelter. The pantry board shows three weeks of preserved stores for the first time. Nobody says anything. Everybody reads the board.

- **Frozen non-prose snapshot:** `{"id": "narrative_preservationist_milestone_2", "minDay": 20, "title": "Canner", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
