# Subject Plan 177.44 — Radiation Spike: focused event prose review
...
Review the exact event `radiation_leak` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 117 whitespace-delimited words / 672 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `radiation_leak` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Radiation Spike`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> The dosimeter alarm triggers at 7 AM, a shrill note that brings everyone to the corridor. Radiation levels inside the bunker have spiked: the wall-mounted unit reads three times what it read yesterday. Something is leaking. The airlock. The hatch seal. A contaminated item brought in with the last salvage. The source is somewhere in this building, and every hour it leaks, it leaks into the people you are responsible for. You can sweep the bunker room by room with the wand, you can seal off the sections you suspect, or you can move everyone into the smallest room and let the bunker stay open to whatever is bleeding. The alarm keeps sounding until you acknowledge it.

- **Frozen non-prose snapshot:** `{"id": "radiation_leak", "minDay": 8, "title": "Radiation Spike", "weight": 2.0}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
