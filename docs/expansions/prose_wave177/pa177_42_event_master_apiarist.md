# Subject Plan 177.42 — Master Apiarist: focused event prose review
...
Review the exact event `narrative_trade_mastery_apiarist` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 86 whitespace-delimited words / 474 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `narrative_trade_mastery_apiarist` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Master Apiarist`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> The hum from the sub-level growing room is constant now — the bees working, the Botanist working, the two operations interdependent and both better for it. The apiarist does not romanticize the bees. They are insects responding to stimulus and they will sting you if you open the hive wrong. But they are alive and producing and they were brought through the end of the world in a wooden box carried down two flights of stairs on the first day. That earns a kind of respect.

- **Frozen non-prose snapshot:** `{"id": "narrative_trade_mastery_apiarist", "minDay": 40, "title": "Master Apiarist", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
