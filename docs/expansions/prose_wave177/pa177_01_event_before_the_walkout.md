# Subject Plan 177.01 — Before The Walkout: focused event prose review
...
Review the exact event `escalation_walkout_conditions` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 57 whitespace-delimited words / 317 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `escalation_walkout_conditions` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Before The Walkout`; exact observed fields: `bodyText, choices, conditions, id, minDay, title, weight`.
- **Current source prose:**

> The pencil has written the conditions on the wall by the corridor door: write when you leave, write why you leave, write when you come back. The conditions are not a law. The conditions are what the shelter could agree on with its hands shaking, and the doorway watches the pencil, and the pencil watches the doorway.

- **Frozen non-prose snapshot:** `{"choices": [{"choiceId": "set_conditions_pencil", "effects": [{"setWorldFlag": "friction_walkout_conditions_set", "worldFlagValue": true}], "moraleDelta": -1.0, "text": "Set the conditions in the binder. The doorway follows the binder."}], "conditions": {"RequireWorldFlag": "friction_work_shirker_accusation"}, "id": "escalation_walkout_conditions", "minDay": 37, "title": "Before The Walkout", "weight": 1.0}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
