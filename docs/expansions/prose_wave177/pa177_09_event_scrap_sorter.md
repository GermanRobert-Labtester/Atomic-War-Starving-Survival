# Subject Plan 177.09 — Scrap Sorter: focused event prose review
...
- **VERIFIED:** exact selector `narrative_appraiser_milestone_1` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Scrap Sorter`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> Alloy sorting by hardness test and spark grind has saved the workshop from three bad batches of scrap that would have produced tools that cracked on the first use. The appraiser knows the difference between cast iron and wrought iron by the sound of the grinder and the color of the spark trail. That knowledge costs nothing to apply and has already saved more in failed components than the appraiser's entire resource allocation.

- **Frozen non-prose snapshot:** `{"id": "narrative_appraiser_milestone_1", "minDay": 10, "title": "Scrap Sorter", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
