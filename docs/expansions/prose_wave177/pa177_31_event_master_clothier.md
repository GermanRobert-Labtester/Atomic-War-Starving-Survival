# Subject Plan 177.31 — Master Clothier: focused event prose review
...
Review the exact event `narrative_tailor_mastery` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 79 whitespace-delimited words / 494 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `narrative_tailor_mastery` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Master Clothier`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> The thermal-barrier parka is the shelter's best piece of cold-weather equipment. Salvage insulation quilted between two layers of salvage canvas, hood sealed with drawstring to leave only the mask aperture exposed, vent panels at the armpits for heat management during work. The blizzard rating has not been formally tested because the blizzards are real and not a testing environment. But three scouts wore the parkas through the worst storm of the winter and came back with all their fingers.

- **Frozen non-prose snapshot:** `{"id": "narrative_tailor_mastery", "minDay": 30, "title": "Master Clothier", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
