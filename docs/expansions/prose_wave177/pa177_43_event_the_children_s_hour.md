# Subject Plan 177.43 — The Children's Hour: focused event prose review
...
Review the exact event `schooling_curriculum_choice` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 90 whitespace-delimited words / 455 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.

Editorial question: **Show learning through a bounded observed action and the source's existing response. Do not add a training step, skill gain, or guaranteed competence beyond the record.** Add specificity only when it sharpens the existing event's person, decision, evidence, or consequence. Keep the text restrained, grounded, and compatible with the ASHFALL encounter register.

## Live-source evidence

- **VERIFIED:** exact selector `schooling_curriculum_choice` exists once in the current 240-record `events` array.
- **VERIFIED:** title `The Children's Hour`; exact observed fields: `bodyText, choices, conditions, id, minDay, title, weight`.
- **Current source prose:**

> There are four teachers and four hours. The teacher in the cold room says the children need to read because the only books left are the ones the adults are willing to speak aloud. The forge teacher says the children need to feel the coke so they know when it is right and when it is wrong. The medic says the children need the steady hands. The armorer says the children need the rifle so they know when to put it down. There is only one hour a day. Pick.

- **Frozen non-prose snapshot:** `{"choices": [{"choiceId": "choose_letters", "effects": [{"setWorldFlag": "schooling_path", "worldFlagValue": "letters"}, {"scheduleEventId": "schooling_letters_first_lesson", "scheduleOnDay": 30}], "moraleDelta": 2.0, "text": "The Reading Hour. Reading first."}, {"choiceId": "choose_mechanics", "effects": [{"setWorldFlag": "schooling_path", "worldFlagValue": "mechanics"}, {"scheduleEventId": "schooling_mechanics_first_lesson", "scheduleOnDay": 30}], "moraleDelta": 2.0, "text": "The Mechanical Hour. Hands first."}, {"choiceId": "choose_medicine", "effects": [{"setWorldFlag": "schooling_path", "worldFlagValue": "medicine"}, {"scheduleEventId": "schooling_medicine_first_lesson", "scheduleOnDay": 30}], "moraleDelta": 4.0, "text": "The First Stitches. The medic teacher gets the hour."}, {"choiceId": "choose_marksmanship", "effects": [{"setWorldFlag": "schooling_path", "worldFlagValue": "marksmanship"}, {"scheduleEventId": "schooling_marksmanship_first_lesson", "scheduleOnDay": 30}], "moraleDelta": -2.0, "text": "The Burned-Out Rifle. Better that the children know it than the rifle know the children."}], "conditions": {"RequireChildCohort": true}, "id": "schooling_curriculum_choice", "minDay": 20, "title": "The Children's Hour", "weight": 1.5}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
