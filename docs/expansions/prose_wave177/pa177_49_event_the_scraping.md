# Subject Plan 177.49 — The Scraping: focused event prose review
...
Review the exact event `silent_knock_part2b_scraping` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 121 whitespace-delimited words / 642 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.

Editorial question: **Clarify the event's immediate human point with a detail already implied by this record. Preserve its point of view, scope, timing, uncertainty, and existing choices; create no second event or mechanic.** Add specificity only when it sharpens the existing event's person, decision, evidence, or consequence. Keep the text restrained, grounded, and compatible with the ASHFALL encounter register.

## Live-source evidence

- **VERIFIED:** exact selector `silent_knock_part2b_scraping` exists once in the current 240-record `events` array.
- **VERIFIED:** title `The Scraping`; exact observed fields: `bodyText, choices, conditions, id, minDay, title, weight`.
- **Current source prose:**

> On Day 38 it starts again, but this time it is not knocking. A long, slow scraping against the outer hatch seal, metal on metal, dragged in one direction and then back. Methodical. It stops for six hours. It starts again. Two days of it, the sound of something patient working at the door, and then nothing. The seal holds. You check it twice, three times a day, and each time the scrape resumes you find yourself listening to the direction of the sound, trying to read intent into the rhythm of it. Whoever it is has decided not to knock anymore. That is a decision in itself. The door is still whole. The question of what comes next is not.

- **Frozen non-prose snapshot:** `{"choices": [{"choiceId": "scraping_accept", "effects": [{"setWorldFlag": "stranger_died_outside", "worldFlagValue": true}], "moraleDelta": -10.0, "text": "You made your choice. Keep the hatch sealed."}], "conditions": {"MinDay": 38, "RequiredFlagId": "stranger_ignored"}, "id": "silent_knock_part2b_scraping", "minDay": 38, "title": "The Scraping", "weight": 0.0}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
