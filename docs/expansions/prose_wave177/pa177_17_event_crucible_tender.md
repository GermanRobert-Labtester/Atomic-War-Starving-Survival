# Subject Plan 177.17 — Crucible Tender: focused event prose review
...
Review the exact event `narrative_metallurgist_milestone_1` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 75 whitespace-delimited words / 445 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `narrative_metallurgist_milestone_1` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Crucible Tender`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> Scrap rebar melted in a refractory clay crucible over a coke forge at eighteen hundred degrees Celsius and poured into sand molds — workable foundry iron, rough but usable. The first pour cracked three molds and produced nothing. The second pour cracked one mold and produced two useful ingots. The third pour produced four clean ingots and the metallurgist did not say anything except to note the mold geometry that worked in the reference log.

- **Frozen non-prose snapshot:** `{"id": "narrative_metallurgist_milestone_1", "minDay": 10, "title": "Crucible Tender", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
