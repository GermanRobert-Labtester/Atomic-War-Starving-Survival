# Subject Plan 177.11 — Master Miller: focused event prose review
...
Review the exact event `narrative_trade_mastery_miller` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 73 whitespace-delimited words / 383 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `narrative_trade_mastery_miller` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Master Miller`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> The miller's hands know the flour now by the way it falls from the millstone — too coarse and the bread won't hold, too fine and it absorbs all the water and bakes into something hard enough to break a tooth. The bread on Day Two tasted of survival. The bread today tastes of craft, which is a different thing and means something for morale that you cannot measure on a ration board.

- **Frozen non-prose snapshot:** `{"id": "narrative_trade_mastery_miller", "minDay": 40, "title": "Master Miller", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
