# Subject Plan 177.37 — Filter Hand: focused event prose review
...
Review the exact event `narrative_water_tech_milestone_1` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 81 whitespace-delimited words / 462 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `narrative_water_tech_milestone_1` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Filter Hand`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> The filter column is six layers: coarse gravel at the base to trap sediment, medium gravel above it for mechanical filtration, fine sand in the middle, crushed activated charcoal for chemical sorption, a second fine sand layer to retain the charcoal particles, and a fine mesh screen at the outlet. The water that comes out the bottom is not clean by pre-war standards. It is clean enough. It has been clean enough for forty days. Nobody has gotten sick from it.

- **Frozen non-prose snapshot:** `{"id": "narrative_water_tech_milestone_1", "minDay": 10, "title": "Filter Hand", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
