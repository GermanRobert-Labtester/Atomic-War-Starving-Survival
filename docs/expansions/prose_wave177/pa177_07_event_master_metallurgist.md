# Subject Plan 177.07 — Master Metallurgist: focused event prose review
...
Review the exact event `narrative_trade_mastery_metallurgist` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 72 whitespace-delimited words / 435 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `narrative_trade_mastery_metallurgist` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Master Metallurgist`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> Scrap metal enters the foundry and leaves as parts that work. That is the whole sentence. The metallurgist does not require recognition for this. They require coke, charcoal flux, clean clay, and time. Given those things, they produce the metal components that everything else in the shelter depends on. They have never missed a production commitment. That is the only professional record they keep and it is the only one that matters.

- **Frozen non-prose snapshot:** `{"id": "narrative_trade_mastery_metallurgist", "minDay": 40, "title": "Master Metallurgist", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
