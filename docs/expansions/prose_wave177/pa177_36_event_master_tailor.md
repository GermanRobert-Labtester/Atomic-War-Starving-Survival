# Subject Plan 177.36 — Master Tailor: focused event prose review
...
Review the exact event `narrative_trade_mastery_tailor` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 81 whitespace-delimited words / 460 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.

Editorial question: **Show competence through an observable check or handoff. Add no repair result, machine state, object, or technical procedure absent from the source.** Add specificity only when it sharpens the existing event's person, decision, evidence, or consequence. Keep the text restrained, grounded, and compatible with the ASHFALL encounter register.

## Live-source evidence

- **VERIFIED:** exact selector `narrative_trade_mastery_tailor` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Master Tailor`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> Every survivor in the shelter has durable outer garments, functional gloves, and footwear that will not fail in the cold. The tailor assessed each person's kit before winter and repaired or replaced what would not last. That assessment took twelve days. Nobody asked the tailor to do it. They did it because nobody else was going to and because a shelter where someone freezes for lack of a functional zipper is a shelter that did not need to lose that person.

- **Frozen non-prose snapshot:** `{"id": "narrative_trade_mastery_tailor", "minDay": 40, "title": "Master Tailor", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
