# Subject Plan 177.25 — The Contested Mattress: focused event prose review
...
Review the exact event `event_grief_conflict_bunk_reassignment` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 77 whitespace-delimited words / 410 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.

Editorial question: **Make disagreement legible through what people say, notice, or withhold. Preserve each side's evidence and the current unresolved state; do not settle the conflict.** Add specificity only when it sharpens the existing event's person, decision, evidence, or consequence. Keep the text restrained, grounded, and compatible with the ASHFALL encounter register.

## Live-source evidence

- **VERIFIED:** exact selector `event_grief_conflict_bunk_reassignment` exists once in the current 240-record `events` array.
- **VERIFIED:** title `The Contested Mattress`; exact observed fields: `bodyText, choices, id, minDay, title, weight`.
- **Current source prose:**

> The new arrival sleeps on the hallway concrete, and leadership has assigned them the bunk of the dweller who died last week, and the dead one's bunk neighbor stands at the ladder and will not move: there is a custom, twenty-four hours, and the custom is not about the bunk, and everyone in the corridor knows what it is about. The new arrival stands in the hallway with the bag, and the bag is not the problem.

- **Frozen non-prose snapshot:** `{"choices": [{"choiceId": "choice_honor_empty_bunk_custom", "effects": [{"setWorldFlag": "empty_bunk_custom_upheld", "worldFlagValue": true}], "moraleDelta": 1.5, "text": "Honor the empty-bunk night. Provide extra blankets in the common room for tonight."}, {"choiceId": "choice_assign_bunk_immediately", "effects": [{"setWorldFlag": "friction_lucky_bunk_dispute", "worldFlagValue": true}], "moraleDelta": -2.0, "text": "The living take precedence over the dead. Assign the mattress immediately."}], "id": "event_grief_conflict_bunk_reassignment", "minDay": 14, "title": "The Contested Mattress", "weight": 1.1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
