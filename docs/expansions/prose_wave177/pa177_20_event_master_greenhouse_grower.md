# Subject Plan 177.20 — Master Greenhouse Grower: focused event prose review
...
Review the exact event `narrative_trade_mastery_greenhouse_grower` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 75 whitespace-delimited words / 478 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `narrative_trade_mastery_greenhouse_grower` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Master Greenhouse Grower`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> The holdfast greenhouse operates without the Botanist's direct intervention for forty-eight hours at a stretch. Everything the Botanist knows about keeping plants alive underground is in the procedure manual they wrote and the three people they trained and the sixty-four varieties of seed stored in the cold cabinet behind the pump. The shelter will grow food after the Botanist. That is the mastery. The mastery is making yourself redundant in the most essential possible way.

- **Frozen non-prose snapshot:** `{"id": "narrative_trade_mastery_greenhouse_grower", "minDay": 40, "title": "Master Greenhouse Grower", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
