# Subject Plan 177.50 — Black Rain: Water Contamination: focused event prose review
...
Review the exact event `weather_black_rain_contamination` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 121 whitespace-delimited words / 623 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.

Editorial question: **Let the source's medium, omissions, and sequence carry the beat. Preserve who recorded it, what is actually heard, and what remains unknown; do not invent a reply or transmission.** Add specificity only when it sharpens the existing event's person, decision, evidence, or consequence. Keep the text restrained, grounded, and compatible with the ASHFALL encounter register.

## Live-source evidence

- **VERIFIED:** exact selector `weather_black_rain_contamination` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Black Rain: Water Contamination`; exact observed fields: `bodyText, choices, conditions, id, minDay, title, weight`.
- **Current source prose:**

> The rain is black, and it is hitting the roof in sheets, and it sounds like someone throwing gravel. It runs off the eaves in dark streaks, and the water barrels are collecting it, and the barrels are the only water you have. The rain is radioactive. It always is. But this rain is worse than the last rain, and the barrels are full of it, and the dosimeter near the barrels is reading in numbers that are not a suggestion. You can pour it out and be thirsty, or you can boil it and hope, or you can run the purifier on overtime and burn the filter. The rain keeps coming down, black and heavy, and the barrels are filling.

- **Frozen non-prose snapshot:** `{"choices": [{"choiceId": "pour_out", "effects": [{"itemAmount": -5, "itemId": "dirty_water"}], "moraleDelta": -5.0, "text": "Pour the barrels out. Start over when the rain cleans."}, {"choiceId": "boil_it", "effects": [{"itemAmount": -2, "itemId": "fuel"}, {"needDelta": 4.0, "targetNeed": "radiation"}], "moraleDelta": 0.0, "text": "Boil what you can. The fire is cheaper than the dose."}, {"choiceId": "run_purifier", "effects": [{"itemAmount": -1, "itemId": "water_filter"}, {"needDelta": 1.0, "targetNeed": "radiation"}], "moraleDelta": 3.0, "text": "Run the purifier on overtime. Burn the filter."}], "conditions": {"MinDay": 4, "RequireWeather": "BlackRain"}, "id": "weather_black_rain_contamination", "minDay": 4, "title": "Black Rain: Water Contamination", "weight": 2.0}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
