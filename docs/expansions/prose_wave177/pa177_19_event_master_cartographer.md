# Subject Plan 177.19 — Master Cartographer: focused event prose review
...
Review the exact event `narrative_surveyor_mastery` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 75 whitespace-delimited words / 441 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `narrative_surveyor_mastery` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Master Cartographer`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> The subterranean fissure network beneath the shelter has been fully mapped from geophone readings and core borings. The surface sinkhole risk zones are marked in red on the expedition planning board. Every route the shelter uses now avoids those zones by a safety margin. The surveyor cannot prevent the ground from moving. They can tell you exactly where it moved and exactly what to avoid. In the ash field, that is the same as prevention.

- **Frozen non-prose snapshot:** `{"id": "narrative_surveyor_mastery", "minDay": 30, "title": "Master Cartographer", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
