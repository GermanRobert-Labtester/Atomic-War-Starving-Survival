# Subject Plan 177.47 — The Contaminated Water Trade: focused event prose review
...
Review the exact event `contaminated_water_trade` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 120 whitespace-delimited words / 660 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.

Editorial question: **Keep scarcity material and social without changing amounts, access, yield, or allocation. No supply promise or new economy rule.** Add specificity only when it sharpens the existing event's person, decision, evidence, or consequence. Keep the text restrained, grounded, and compatible with the ASHFALL encounter register.

## Live-source evidence

- **VERIFIED:** exact selector `contaminated_water_trade` exists once in the current 240-record `events` array.
- **VERIFIED:** title `The Contaminated Water Trade`; exact observed fields: `bodyText, choices, id, minDay, title, weight`.
- **Current source prose:**

> A gaunt woman pounds on the hatch with both fists, and behind her a child coughs, a wet rattling sound that goes on longer than a cough should. 'Please,' she begs, the word worn flat from repeating. 'Clean water. Just enough for my daughter.' She holds up ammunition in both hands, military-grade rounds, boxed and sealed, the kind of stock that does not walk around loose. It's yours, she says, all of it, for one canteen. The bunker's water reserves are already low, measured in cups, and the barrels are at the bottom. The ammo is real, the rounds are bright, and the cough is real too, and the daughter is small enough to be hidden behind her mother's hip.

- **Frozen non-prose snapshot:** `{"choices": [{"choiceId": "trade_water", "effects": [{"amount": 10, "type": "remove_water"}, {"amount": 30, "itemId": "ammo_556", "type": "add_item"}], "moraleDelta": 15.0, "text": "Trade the water. A child's life is worth it."}, {"choiceId": "refuse_trade", "effects": [{"traitId": "guilt_water_trade", "type": "add_trait"}], "moraleDelta": -15.0, "text": "We can't spare it. Every drop counts."}, {"choiceId": "offer_half", "effects": [{"amount": 5, "type": "remove_water"}], "moraleDelta": 5.0, "text": "Half now. Half if she brings more ammunition."}], "id": "contaminated_water_trade", "minDay": 10, "title": "The Contaminated Water Trade", "weight": 2.5}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
