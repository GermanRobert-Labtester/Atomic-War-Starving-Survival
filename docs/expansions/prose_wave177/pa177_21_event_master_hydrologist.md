# Subject Plan 177.21 — Master Hydrologist: focused event prose review
...
Review the exact event `narrative_water_tech_mastery` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 75 whitespace-delimited words / 459 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.
...
- **VERIFIED:** exact selector `narrative_water_tech_mastery` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Master Hydrologist`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> The deep well pump draws from an aquifer thirty-two meters below the shelter floor. The pump is submersible, the cable is insulated, and the pressure tank in the pump room maintains four atmospheres of delivery pressure to all living quarters. The water technician rebuilt the pump from service manual and salvaged components over sixty days. When they started, the shelter was rationing at two liters per person per day. The current allocation is six liters.

- **Frozen non-prose snapshot:** `{"id": "narrative_water_tech_mastery", "minDay": 30, "title": "Master Hydrologist", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
