# Subject Plan 177.16 — Master Salvage Appraiser: focused event prose review
...
Review the exact event `narrative_trade_mastery_salvage_appraiser` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` is 74 whitespace-delimited words / 459 characters. This is a source-grounded review proposal, not a defect finding or target length. A no-change disposition is appropriate where the current prose already communicates its scene clearly.

Editorial question: **Keep the terms and uncertainty already authored. Add no stock, price, delivery, debt, or reputation effect.** Add specificity only when it sharpens the existing event's person, decision, evidence, or consequence. Keep the text restrained, grounded, and compatible with the ASHFALL encounter register.

## Live-source evidence

- **VERIFIED:** exact selector `narrative_trade_mastery_salvage_appraiser` exists once in the current 240-record `events` array.
- **VERIFIED:** title `Master Salvage Appraiser`; exact observed fields: `bodyText, id, minDay, title, weight`.
- **Current source prose:**

> The appraiser has assessed everything that has come through the shelter gates for eighty days. They know what the shelter has, what it is worth, and what it should be traded for. That knowledge, documented in the valuation ledger, belongs to the shelter and not to any single person. The appraiser made that point explicitly when they handed the ledger to the quartermaster and asked them to countersign each assessment. Institutional knowledge. Not personal.

- **Frozen non-prose snapshot:** `{"id": "narrative_trade_mastery_salvage_appraiser", "minDay": 40, "title": "Master Salvage Appraiser", "weight": 1}`. This serializes every observed field except the single editable prose value. Re-read the whole live object before any future edit.
