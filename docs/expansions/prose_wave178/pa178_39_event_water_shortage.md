# Subject Plan 178.39 — Dry Pipes: focused event prose review
...
Review only the exact selector `water_shortage` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` contains 124 words / 672 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `water_shortage` exists once in current `events.json` (`events`, 240 records), with fields `bodyText, id, minDay, title, weight`.
- **VERIFIED:** `bodyText` is the sole prose field proposed for review; current value is 672 characters / 124 whitespace-delimited words.
- **VERIFIED:** current non-prose snapshot is `{"id": "water_shortage", "minDay": 5, "title": "Dry Pipes", "weight": 2.5}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> The water barrel is down to the last few centimetres, a dark circle in the bottom that reflects the light when you lean over. Rationing began nine days ago, a cup per person per day, and the cups have gotten smaller twice. Thirst is setting in: lips crack, the arguments come faster, and the children have stopped asking for more. The nearest clean source is the creek two hours east, and the last person who walked that road came back empty and limping. Melted snow is a gamble, the rain barrels are half ash, and every option costs either distance or risk or both. People are watching you measure the cups now. The barrel does not care about the count. It is just empty.
