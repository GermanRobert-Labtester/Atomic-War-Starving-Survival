# Subject Plan 178.32 — The Tempest Water-Plant Maintenance Log: focused broadcast prose review
...
Review only the exact selector `radio_verdict_valve_s36` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 76 words / 450 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_verdict_valve_s36` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 450 characters / 76 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 252, "frequency": "96.400 MHz", "id": "radio_verdict_valve_s36", "isEmergency": false, "signalStrength": "S4", "source": "The Tempest Water-Plant Maintenance Log"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> Valve S36 accessed. Access time: shift boundary. Water drawn: 480 liters. Service order: Section 36. Authorized hand: logged. [The valve knows which hand turned it. The meter keeps the account. Whoever turned valve S36 on Day 252 was using an authorization code from a charter that had been suspended for forty days. The system accepted it. The water flowed. The record says it was authorized. Whether it was is not a question the record can answer.]
...
{
  "dayTrigger": 252,
  "frequency": "96.400 MHz",
  "id": "radio_verdict_valve_s36",
  "isEmergency": false,
  "signalStrength": "S4",
  "source": "The Tempest Water-Plant Maintenance Log"
}
```
