# Subject Plan 178.11 — Highway 12 Convoy Control: focused broadcast prose review
...
Review only the exact selector `radio_supply_corps_convoy_tracking` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 35 words / 231 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_supply_corps_convoy_tracking` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 231 characters / 35 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 288, "frequency": "88.400 MHz", "id": "radio_supply_corps_convoy_tracking", "isEmergency": true, "signalStrength": "S7", "source": "Highway 12 Convoy Control"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> CONVOY FOUR UPDATE: Half-track lead is entering High Granite Pass. Heavy sniper fire reported from switchback 4. Transport 03 threw a track in snowdrifts. Sentry escort returning fire. Maintain radio silence until past the viaduct.
...
```json
{
  ...
  "id": "radio_supply_corps_convoy_tracking",
  "isEmergency": true,
  "signalStrength": "S7",
  "source": "Highway 12 Convoy Control"
}
```
