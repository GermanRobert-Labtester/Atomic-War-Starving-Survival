# Subject Plan 178.04 — Vitrified Crater Penitent Choir: focused broadcast prose review
...
Review only the exact selector `radio_cult_martyrdom_chant` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 44 words / 251 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_cult_martyrdom_chant` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 251 characters / 44 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 270, "frequency": "104.200 MHz", "id": "radio_cult_martyrdom_chant", "isEmergency": false, "signalStrength": "S3", "source": "Vitrified Crater Penitent Choir"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> ...(slow rhythmic chanting with brass chimes)... 'From the fire we came, in the frost we wait, by the black silt we are made clean. Blessed is the lead that shields no sin. Blessed is the bone that glows in the dark.'...(chanting fades into static)...
...
```json
{
  ...
  "id": "radio_cult_martyrdom_chant",
  "isEmergency": false,
  "signalStrength": "S3",
  "source": "Vitrified Crater Penitent Choir"
}
```
