# Subject Plan 178.43 — Old Diary: focused event prose review
...
Review only the exact selector `found_diary` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` contains 126 words / 693 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `found_diary` exists once in current `events.json` (`events`, 240 records), with fields `bodyText, id, minDay, title, weight`.
- **VERIFIED:** `bodyText` is the sole prose field proposed for review; current value is 693 characters / 126 whitespace-delimited words.
- **VERIFIED:** current non-prose snapshot is `{"id": "found_diary", "minDay": 8, "title": "Old Diary", "weight": 0.5}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> Behind a loose panel in the storage room, pushed back behind the shelving, a diary. It belonged to the bunker's original occupant, a man who kept records like they mattered: fuel levels, water usage, the date the power lines went dead. The entries are calm for the first two weeks. Then the handwriting changes, shorter sentences, wider spacing. The last entry is dated three weeks after the bombs. It ends mid-sentence, with a pen stroke that dragged off the page. You have read it twice. It is not a happy document, and it tells you something about this bunker that no one who built it intended you to know. What it means for how you run things, what you tell the others, that part is yours.
