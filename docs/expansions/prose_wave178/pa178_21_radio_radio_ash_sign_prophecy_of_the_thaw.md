# Subject Plan 178.21 — Ash Sign Penitent Speaker: focused broadcast prose review
...
Review only the exact selector `radio_ash_sign_prophecy_of_the_thaw` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 46 words / 248 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_ash_sign_prophecy_of_the_thaw` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 248 characters / 46 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 295, "frequency": "104.200 MHz", "id": "radio_ash_sign_prophecy_of_the_thaw", "isEmergency": false, "signalStrength": "S4", "source": "Ash Sign Penitent Speaker"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> Hear the word of the crater: When the snow turns to grease and the earth smells of rotten sulfur, the great machine will sleep. The iron soldiers will cast off their breastplates and weep in the ditches. Walk out into the rain and receive the mark.
...
```json
{
  ...
  "id": "radio_ash_sign_prophecy_of_the_thaw",
  "isEmergency": false,
  "signalStrength": "S4",
  "source": "Ash Sign Penitent Speaker"
}
```
