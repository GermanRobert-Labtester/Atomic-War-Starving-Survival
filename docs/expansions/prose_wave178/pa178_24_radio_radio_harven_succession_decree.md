# Subject Plan 178.24 — Sector 4 Garrison Command (Acting): focused broadcast prose review
...
Review only the exact selector `radio_harven_succession_decree` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 70 words / 475 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_harven_succession_decree` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, message, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 475 characters / 70 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 240, "frequency": "106.300 MHz", "id": "radio_harven_succession_decree", "source": "Sector 4 Garrison Command (Acting)"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> TO ALL SHELTERS. BY ORDER OF THE CONTINUITY RECLAMATION DECREE, EFFECTIVE IMMEDIATELY: Colonel Harven assumes command of Sector 4 Garrison. All standing orders issued under the previous command are revoked. Movement permits must be re-issued at Checkpoint Gamma. The previous command's coastal liaison detachments are closed. Reassignment orders dated prior to this broadcast are void. AMENDED: no further announcements will be made on this channel. KEEP THE WIRE CLEAR. CZC.
...
{
  "dayTrigger": 240,
  "frequency": "106.300 MHz",
  "id": "radio_harven_succession_decree",
  "source": "Sector 4 Garrison Command (Acting)"
}
```
