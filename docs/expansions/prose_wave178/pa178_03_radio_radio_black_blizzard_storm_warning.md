# Subject Plan 178.03 — Granite Pass Weather Observatory: focused broadcast prose review
...
Review only the exact selector `radio_black_blizzard_storm_warning` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 43 words / 294 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_black_blizzard_storm_warning` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 294 characters / 43 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 204, "frequency": "101.500 MHz", "id": "radio_black_blizzard_storm_warning", "isEmergency": true, "signalStrength": "S8", "source": "Granite Pass Weather Observatory"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> SEVERE STORM WARNING: Barometer reading 938 hPa and falling rapidly. A polar vortex is driving pulverized radioactive soot south at sixty knots. Zero visibility for next seventy-two hours. Surface travel is fatal. Secure all air intakes and switch to internal recirculating filters immediately.
...
{
  "dayTrigger": 204,
  "frequency": "101.500 MHz",
  "id": "radio_black_blizzard_storm_warning",
  "isEmergency": true,
  "signalStrength": "S8",
  "source": "Granite Pass Weather Observatory"
}
```
