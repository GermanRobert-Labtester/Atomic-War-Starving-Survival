# Subject Plan 178.47 — Filter Wear: focused event prose review
...
Review only the exact selector `filter_degradation` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` contains 125 words / 695 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `filter_degradation` exists once in current `events.json` (`events`, 240 records), with fields `bodyText, id, minDay, title, weight`.
- **VERIFIED:** `bodyText` is the sole prose field proposed for review; current value is 695 characters / 125 whitespace-delimited words.
- **VERIFIED:** current non-prose snapshot is `{"id": "filter_degradation", "minDay": 10, "title": "Filter Wear", "weight": 2.0}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> The air filter indicator has dropped into the yellow, a small window with the needle resting at the edge of the green zone. The numbers have been falling for a week, slowly, the way everything degrades in the wasteland. The spare is in the stores, one unit, wrapped in plastic and oiled paper, and it represents a trade decision you have been putting off. Replace the filter now and you spend the spare with months of winter ahead. Wait and the air gets heavier, the smell of dust and bodies comes back, and the next storm will finish the job this filter started. The indicator does not blink and does not hurry. It just waits, yellow, patient, while the bunker breathes through a clogging lung.
