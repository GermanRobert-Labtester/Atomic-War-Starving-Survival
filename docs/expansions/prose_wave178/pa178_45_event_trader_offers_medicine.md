# Subject Plan 178.45 — Medicine Trade: focused event prose review
...
Review only the exact selector `trader_offers_medicine` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` contains 133 words / 694 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `trader_offers_medicine` exists once in current `events.json` (`events`, 240 records), with fields `bodyText, id, minDay, title, weight`.
- **VERIFIED:** `bodyText` is the sole prose field proposed for review; current value is 694 characters / 133 whitespace-delimited words.
- **VERIFIED:** current non-prose snapshot is `{"id": "trader_offers_medicine", "minDay": 10, "title": "Medicine Trade", "weight": 1.0}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> A trader at the gate claims to have anti-rad medication, real decorporation pills from a pre-war pharmacy, and holds up a strip of foil to prove it. They want food in return, a specific list: two tins of meat and a bag of flour. The deal feels too good. Anti-rad is scarce enough that people kill for it, and this trader is offering it at market price with a smile that does not reach their eyes. The foil could hold real pills. It could hold chalk dust and aspirin. The food is yours either way, and it is food you need. You have seen too many good deals in the wasteland, and every one of them cost somebody something. The question is whether this one costs you your stock or just your pride.
...
```json
{
  "id": "trader_offers_medicine",
  "minDay": 10,
  "title": "Medicine Trade",
  "weight": 1.0
}
```
