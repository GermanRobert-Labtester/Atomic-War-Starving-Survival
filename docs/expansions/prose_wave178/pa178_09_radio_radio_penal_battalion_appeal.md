# Subject Plan 178.09 — 8th Penal Pioneer Sump Outpost: focused broadcast prose review
...
Review only the exact selector `radio_penal_battalion_appeal` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 46 words / 281 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_penal_battalion_appeal` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 281 characters / 46 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 275, "frequency": "142.850 MHz", "id": "radio_penal_battalion_appeal", "isEmergency": true, "signalStrength": "S4", "source": "8th Penal Pioneer Sump Outpost"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> APPEAL FOR MEDICAL AID: Sump Trench Sector is suffering 60% casualties from acute radiation sickness and trench foot. We have no iodine, no clean bandages, and no rations. Provost guards have locked the gates. If any medical personnel hear this, throw supplies over the south wire.
...
```json
{
  ...
  "id": "radio_penal_battalion_appeal",
  "isEmergency": true,
  "signalStrength": "S4",
  "source": "8th Penal Pioneer Sump Outpost"
}
```
