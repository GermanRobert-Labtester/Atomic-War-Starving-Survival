# Subject Plan 178.07 — High Granite Munitions Directorate: focused broadcast prose review
...
Review only the exact selector `radio_granite_arsenal_mobilization_broadcast` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 42 words / 252 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_granite_arsenal_mobilization_broadcast` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 252 characters / 42 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 255, "frequency": "88.400 MHz", "id": "radio_granite_arsenal_mobilization_broadcast", "isEmergency": false, "signalStrength": "S8", "source": "High Granite Munitions Directorate"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> ORDNANCE DIRECTIVE: The Granite Foundry is operating at maximum capacity. All brass scrap, copper wire, and spent shell casings must be turned in to the Arsenal rail siding. Compensation will be issued in flour scrip at 200 grams per kilogram of brass.
...
```json
{
  ...
  "id": "radio_granite_arsenal_mobilization_broadcast",
  "isEmergency": false,
  "signalStrength": "S8",
  "source": "High Granite Munitions Directorate"
}
```
