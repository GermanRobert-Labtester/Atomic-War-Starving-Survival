# Subject Plan 178.12 — Central Upland Defense League: focused broadcast prose review
...
Review only the exact selector `radio_ash_militia_communal_defense_edict` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 38 words / 247 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_ash_militia_communal_defense_edict` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 247 characters / 38 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 245, "frequency": "101.500 MHz", "id": "radio_ash_militia_communal_defense_edict", "isEmergency": false, "signalStrength": "S7", "source": "Central Upland Defense League"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> To all farmsteads on the eastern terraces: Garrison requisition trucks have been spotted near the lower sluice. Roll the deadfall logs across the road. Extinguish all chimney smoke. Send runners to the stone redoubt if armored half-tracks advance.
...
```json
{
  ...
  "id": "radio_ash_militia_communal_defense_edict",
  "isEmergency": false,
  "signalStrength": "S7",
  "source": "Central Upland Defense League"
}
```
