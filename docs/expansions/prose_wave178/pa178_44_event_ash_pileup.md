# Subject Plan 178.44 — Ash Accumulation: focused event prose review
...
Review only the exact selector `ash_pileup` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` contains 123 words / 648 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `ash_pileup` exists once in current `events.json` (`events`, 240 records), with fields `bodyText, id, minDay, title, weight`.
- **VERIFIED:** `bodyText` is the sole prose field proposed for review; current value is 648 characters / 123 whitespace-delimited words.
- **VERIFIED:** current non-prose snapshot is `{"id": "ash_pileup", "minDay": 7, "title": "Ash Accumulation", "weight": 2.0}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> Radioactive ash has drifted against the air intake, a grey dune that leans up the wall and presses into the louver. The wind keeps feeding it. If it is not cleared, the filters will clog within hours, and a clogged filter is a seized filter, and a seized filter is the air problem you cannot fix at midnight. Clearing it means sending someone out in the gear, half an hour in the open with a shovel and a mask, ash working its way into every seam. Waiting means watching the airflow gauge drop and doing nothing about it. The intake is not decorative. It is the difference between air you can breathe and air that gets into you one cough at a time.
