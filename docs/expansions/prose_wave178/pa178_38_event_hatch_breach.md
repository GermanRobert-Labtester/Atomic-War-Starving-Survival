# Subject Plan 178.38 — Hatch Seal Breach: focused event prose review
...
Review only the exact selector `hatch_breach` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` contains 133 words / 716 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `hatch_breach` exists once in current `events.json` (`events`, 240 records), with fields `bodyText, id, minDay, title, weight`.
- **VERIFIED:** `bodyText` is the sole prose field proposed for review; current value is 716 characters / 133 whitespace-delimited words.
- **VERIFIED:** current non-prose snapshot is `{"id": "hatch_breach", "minDay": 4, "title": "Hatch Seal Breach", "weight": 2.0}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> A high-pitched whistle cuts through the bunker, thin and constant, like air escaping a punctured tire. It is coming from the outer hatch. The rubber gasket has cracked along its length, a dry split that flexes when you press it, and through the gap you can feel the faint movement of air moving inward. The dosimeter near the hatch reads a little higher every time you look. The air outside is not clean, and now it has a way in. Replacing the gasket means using the spare seal, the only one you have, and spending an hour in the cold with the hatch open. Patching it means tape and rubber from the stores, a repair that will hold for a week and then ask the question again. The whistle keeps going, steady, patient.
