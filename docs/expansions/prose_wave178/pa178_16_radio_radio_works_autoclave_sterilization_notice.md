# Subject Plan 178.16 — The Works Medical Department: focused broadcast prose review
...
Review only the exact selector `radio_works_autoclave_sterilization_notice` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 30 words / 205 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_works_autoclave_sterilization_notice` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 205 characters / 30 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 310, "frequency": "101.500 MHz", "id": "radio_works_autoclave_sterilization_notice", "isEmergency": false, "signalStrength": "S6", "source": "The Works Medical Department"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> The Allotment steam autoclave is operating on 24-hour cycle. We can sterilize surgical dressings, bone chisels, and seed containers for neighboring shelters. Bring clean items in sealed tins on Wednesdays.
...
```json
{
  ...
  "id": "radio_works_autoclave_sterilization_notice",
  "isEmergency": false,
  "signalStrength": "S6",
  "source": "The Works Medical Department"
}
```
