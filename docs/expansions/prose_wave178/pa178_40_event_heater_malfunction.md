# Subject Plan 178.40 — Heater Overheat: focused event prose review
...
Review only the exact selector `heater_malfunction` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` contains 128 words / 667 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `heater_malfunction` exists once in current `events.json` (`events`, 240 records), with fields `bodyText, id, minDay, title, weight`.
- **VERIFIED:** `bodyText` is the sole prose field proposed for review; current value is 667 characters / 128 whitespace-delimited words.
- **VERIFIED:** current non-prose snapshot is `{"id": "heater_malfunction", "minDay": 6, "title": "Heater Overheat", "weight": 1.5}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> The heater unit in the main room glows red at the seams, and the metal skin is warm enough to blister a hand. An acrid smell fills the shelter, hot dust and scorched wiring, the smell of something burning from the inside. The thermostat is stuck, the fan is grinding against a warped blade, and the unit has been pushing heat it was never meant to push for the better part of an hour. Shut it down and the bunker cools fast, the kind of cold that settles into the concrete and stays. Open the panel and you risk the wiring or your fingers. The smell is getting worse. Whatever you decide, it has to happen before the unit decides for you, and the unit is not patient.
...
```json
{
  "id": "heater_malfunction",
  "minDay": 6,
  "title": "Heater Overheat",
  "weight": 1.5
}
```
