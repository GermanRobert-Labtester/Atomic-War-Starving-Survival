# Subject Plan 178.01 — 3rd Corps Provost Marshal: focused broadcast prose review
...
Review only the exact selector `radio_garrison_deserter_bulletin` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 52 words / 353 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_garrison_deserter_bulletin` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 353 characters / 52 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 260, "frequency": "88.400 MHz", "id": "radio_garrison_deserter_bulletin", "isEmergency": false, "signalStrength": "S8", "source": "3rd Corps Provost Marshal"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> BULLETIN: The following personnel have deserted their posts at Checkpoint Gamma carrying service weapons: Corporal Felix Vane, Gunner Mikhail, Private Solokov. A reward of fifty kilograms of flour and two tins of diesel will be paid for information leading to their apprehension. Harboring military deserters carries the death penalty under martial law.
...
{
  "dayTrigger": 260,
  "frequency": "88.400 MHz",
  "id": "radio_garrison_deserter_bulletin",
  "isEmergency": false,
  "signalStrength": "S8",
  "source": "3rd Corps Provost Marshal"
}
```
