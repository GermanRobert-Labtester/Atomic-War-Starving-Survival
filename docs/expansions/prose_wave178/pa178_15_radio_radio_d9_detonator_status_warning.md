# Subject Plan 178.15 — STD-9 Automated Telemetry: focused broadcast prose review
...
Review only the exact selector `radio_d9_detonator_status_warning` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 27 words / 221 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_d9_detonator_status_warning` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 221 characters / 27 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 290, "frequency": "96.100 MHz", "id": "radio_d9_detonator_status_warning", "isEmergency": true, "signalStrength": "S9", "source": "STD-9 Automated Telemetry"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> TELEMETRY LOG: Railway Bridge 44 seismic sensors registering heavy vehicular vibrations. Automatic 30-minute detonation sequence initiated. To cancel countdown, enter verified five-digit cipher on local substation keypad.
...
```json
{
  ...
  "id": "radio_d9_detonator_status_warning",
  "isEmergency": true,
  "signalStrength": "S9",
  "source": "STD-9 Automated Telemetry"
}
```
