# Subject Plan 178.17 — 3rd Corps Provost Marshal: focused broadcast prose review
...
Review only the exact selector `radio_provost_martial_execution_roll` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 34 words / 226 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_provost_martial_execution_roll` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 226 characters / 34 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 299, "frequency": "88.400 MHz", "id": "radio_provost_martial_execution_roll", "isEmergency": false, "signalStrength": "S8", "source": "3rd Corps Provost Marshal"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> PROVOST REPORT: Sixteen conscripts from 2nd Sentry Company were executed this morning at Kilometre 12 for mutinous refusal to clear radioactive fallout debris. Discipline in the ranks will be maintained by all available means.
...
```json
{
  ...
  "id": "radio_provost_martial_execution_roll",
  "isEmergency": false,
  "signalStrength": "S8",
