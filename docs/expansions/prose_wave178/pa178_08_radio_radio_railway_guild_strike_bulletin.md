# Subject Plan 178.08 — Shattered Rail Union Telegraph Station: focused broadcast prose review
...
Review only the exact selector `radio_railway_guild_strike_bulletin` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 42 words / 247 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_railway_guild_strike_bulletin` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 247 characters / 42 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 266, "frequency": "101.500 MHz", "id": "radio_railway_guild_strike_bulletin", "isEmergency": true, "signalStrength": "S6", "source": "Shattered Rail Union Telegraph Station"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> TO ALL SWITCHMEN AND LINESMEN: The Garrison has placed armed provosts at Mile Marker 8. Lock all track switches in the siding position. Remove the brass armature relays from repeater huts 12 through 16. Do not let munitions trains pass the canyon.
...
```json
{
  ...
  "id": "radio_railway_guild_strike_bulletin",
  "isEmergency": true,
  "signalStrength": "S6",
  "source": "Shattered Rail Union Telegraph Station"
}
```
