# Subject Plan 178.29 — Office of Censuses (Automated Tape Loop): focused broadcast prose review
...
Review only the exact selector `radio_verdict_off_count_assessed` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 77 words / 420 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_verdict_off_count_assessed` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 420 characters / 77 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 241, "frequency": "96.400 MHz", "id": "radio_verdict_off_count_assessed", "isEmergency": false, "signalStrength": "S5", "source": "Office of Censuses (Automated Tape Loop)"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> Off-count is a penalty assessed against the holder. [Six seconds. The tape repeats. Six seconds again. The phrase is from the middle of a longer document — not its beginning, not its end. Someone set the tape to loop at this phrase specifically. The off-count penalty is what happens when the count is open and you do not present your people. The tape does not say what the penalty is. It only says it will be assessed.]
...
```json
{
  ...
  "id": "radio_verdict_off_count_assessed",
  "isEmergency": false,
  "signalStrength": "S5",
  "source": "Office of Censuses (Automated Tape Loop)"
}
```
