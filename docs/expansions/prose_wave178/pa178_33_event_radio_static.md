# Subject Plan 178.33 — Radio Transmission: focused event prose review
...
Review only the exact selector `radio_static` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` contains 122 words / 685 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_static` exists once in current `events.json` (`events`, 240 records), with fields `bodyText, id, minDay, title, weight`.
- **VERIFIED:** `bodyText` is the sole prose field proposed for review; current value is 685 characters / 122 whitespace-delimited words.
- **VERIFIED:** current non-prose snapshot is `{"id": "radio_static", "minDay": 7, "title": "Radio Transmission", "weight": 1.0}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> The radio wakes you at dawn, spitting layers of static, and then a voice cuts through: thin, tired, repeating a set of coordinates three times. A supply cache, they say. Sealed container, pre-war, buried in the collapsed general store east of the rail bridge. Then the warning: something is in there with it. They do not say what. The signal dies mid-sentence and the static returns. You write the coordinates on the wall in chalk. The rail bridge is a half-day walk through open country, and open country has its own residents now. The cache could be real. The warning could be the honest part. The only way to know is to send someone, and everyone you have is already stretched thin.
