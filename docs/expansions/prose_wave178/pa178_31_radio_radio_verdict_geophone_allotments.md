# Subject Plan 178.31 — The Tempest Geophone Array (Unlabeled Feed): focused broadcast prose review
...
Review only the exact selector `radio_verdict_geophone_allotments` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 65 words / 387 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_verdict_geophone_allotments` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 387 characters / 65 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 210, "frequency": "96.400 MHz", "id": "radio_verdict_geophone_allotments", "isEmergency": false, "signalStrength": "S4", "source": "The Tempest Geophone Array (Unlabeled Feed)"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> Tap. Tap. Tap. [A seismic readout from sensors under the Allotments district, transmitted without label or attribution. The rhythm varies — heavy during the working hours, lighter in the evening, absent in the deep night. The array is recording the movement of people who never signed a census form. They are in the record as vibration. The earth counted them when the office could not.]
...
```json
{
  ...
  "id": "radio_verdict_geophone_allotments",
  "isEmergency": false,
  "signalStrength": "S4",
  "source": "The Tempest Geophone Array (Unlabeled Feed)"
}
```
