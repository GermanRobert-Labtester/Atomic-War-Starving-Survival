# Subject Plan 178.30 — Office of Censuses (Full Broadcast, Amateur Band Relay): focused broadcast prose review
...
Review only the exact selector `radio_verdict_count_is_open_amateur` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 80 words / 487 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_verdict_count_is_open_amateur` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 487 characters / 80 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 240, "frequency": "88.500 MHz", "id": "radio_verdict_count_is_open_amateur", "isEmergency": false, "signalStrength": "S3", "source": "Office of Censuses (Full Broadcast, Amateur Band Relay)"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> This is the Office of Censuses. The count is open. All persons having custody of persons must present them. Off-count is a penalty assessed against the holder. This message will repeat. The count is open. [Heard on the amateur band, not the carrier frequency. Someone relayed the full text. The signal is weaker here, more compressed, broadcast on the frequencies where ordinary people had their radios. The carrier band was for officials. This band was for whoever was still listening.]
...
```json
{
  ...
  "id": "radio_verdict_count_is_open_amateur",
  "isEmergency": false,
  "signalStrength": "S3",
  "source": "Office of Censuses (Full Broadcast, Amateur Band Relay)"
}
```
