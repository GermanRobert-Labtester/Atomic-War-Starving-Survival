# Subject Plan 178.50 — Wall Crack: focused event prose review
...
Review only the exact selector `structural_damage` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` contains 134 words / 709 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `structural_damage` exists once in current `events.json` (`events`, 240 records), with fields `bodyText, id, minDay, title, weight`.
- **VERIFIED:** `bodyText` is the sole prose field proposed for review; current value is 709 characters / 134 whitespace-delimited words.
- **VERIFIED:** current non-prose snapshot is `{"id": "structural_damage", "minDay": 12, "title": "Wall Crack", "weight": 1.5}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> A hairline crack has appeared in the bunker wall, running from the floor to the ceiling in the storage corridor. It is thin as a fingernail now, but water seeps through it when it rains, a dark line that grows after every storm. Concrete does not crack for no reason: the ground is moving, or the wall is carrying weight it was not designed to carry, or the frame is tired. If the crack widens, the shelter is compromised, and a compromised shelter is a different kind of problem entirely, one that no rationing can solve. The stores have cement and rod, enough for one patch job. The crack is patient. You have to decide whether to patch it now or wait until it decides for you, and the rain is not done yet.
