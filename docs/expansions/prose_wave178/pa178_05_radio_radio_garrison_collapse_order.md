# Subject Plan 178.05 — Garrison Checkpoint Gamma: focused broadcast prose review
...
Review only the exact selector `radio_garrison_collapse_order` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 36 words / 251 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_garrison_collapse_order` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 251 characters / 36 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 357, "frequency": "88.400 MHz", "id": "radio_garrison_collapse_order", "isEmergency": true, "signalStrength": "S6", "source": "Garrison Checkpoint Gamma"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> TO ALL 3RD CORPS OUTPOSTS: Checkpoint Gamma is abandoned. Headquarters staff is evacuating north by motor transport. All conscripts are released from service obligations. Destroy all codebooks and spike remaining artillery tubes. May God preserve you.
...
```json
{
  ...
  "id": "radio_garrison_collapse_order",
  "isEmergency": true,
  "signalStrength": "S6",
  "source": "Garrison Checkpoint Gamma"
}
```
