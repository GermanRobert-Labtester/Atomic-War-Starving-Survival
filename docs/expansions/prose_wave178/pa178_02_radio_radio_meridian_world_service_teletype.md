# Subject Plan 178.02 — Meridian Maritime World Service: focused broadcast prose review
...
Review only the exact selector `radio_meridian_world_service_teletype` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 46 words / 315 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_meridian_world_service_teletype` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 315 characters / 46 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 330, "frequency": "142.850 MHz", "id": "radio_meridian_world_service_teletype", "isEmergency": false, "signalStrength": "S6", "source": "Meridian Maritime World Service"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> ZCZC 990 012 330 ... CONTINENTAL OBSERVATORY REPORT ... STRATOSPHERIC ASH CONCENTRATION DROPPING 4% WEEKLY ... SURFACE TEMPERATURES IN SOUTHERN LATITUDES RETURNING TO PLUS FIVE CELSIUS ... SEED VAULT NETWORK CONFIRMED INTACT AT SVALBARD AND GREENLAND ... CIVIL RECONSTRUCTION COUNCILS FORMING ... HOLD FAST ... NNNN
...
```json
{
  ...
  "id": "radio_meridian_world_service_teletype",
  "isEmergency": false,
  "signalStrength": "S6",
  "source": "Meridian Maritime World Service"
}
```
