# Subject Plan 178.36 — Rats in the Stores: focused event prose review
...
Review only the exact selector `rat_infestation` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` contains 132 words / 699 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `rat_infestation` exists once in current `events.json` (`events`, 240 records), with fields `bodyText, id, minDay, title, weight`.
- **VERIFIED:** `bodyText` is the sole prose field proposed for review; current value is 699 characters / 132 whitespace-delimited words.
- **VERIFIED:** current non-prose snapshot is `{"id": "rat_infestation", "minDay": 6, "title": "Rats in the Stores", "weight": 1.5}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> Droppings in the food crate this morning, black and dry, pressed into the corner where the flour bags lean. Something has gnawed through two ration packs and left the torn foil where anyone can step on it. The scratching comes from the walls at night, a light relentless sound that moves in short bursts. Rats carry things in their fur, and things in their fur live on in the food. The traps you have are two, and the poison is one tin, and the stores are the only thing between the bunker and hunger. Set the poison and you risk the water. Use the traps and you risk missing the nest. Seal the stores and the rats stay in the walls, breeding, patient, waiting for the day you open the crates again.
