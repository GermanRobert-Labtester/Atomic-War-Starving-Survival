# Subject Plan 178.18 — Low-Background Salt Chamber Laboratory: focused broadcast prose review
...
Review only the exact selector `radio_low_background_salt_vault_invitation` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 42 words / 256 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_low_background_salt_vault_invitation` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 256 characters / 42 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 335, "frequency": "104.200 MHz", "id": "radio_low_background_salt_vault_invitation", "isEmergency": false, "signalStrength": "S5", "source": "Low-Background Salt Chamber Laboratory"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> Dr. Dahl to all scientific personnel in Sector 4: The 400m counting chamber is dry and fully powered by emergency battery banks. We have five open berths for physicists, doctors, or precision machinists. Radiation is 0.02 µSv/h. Contact via the salt hoist.
...
```json
{
  ...
  "id": "radio_low_background_salt_vault_invitation",
  "isEmergency": false,
  "signalStrength": "S5",
  "source": "Low-Background Salt Chamber Laboratory"
}
```
