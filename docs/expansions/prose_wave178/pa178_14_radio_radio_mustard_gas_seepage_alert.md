# Subject Plan 178.14 — Sector 8 Environmental Monitoring: focused broadcast prose review
...
Review only the exact selector `radio_mustard_gas_seepage_alert` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 36 words / 239 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_mustard_gas_seepage_alert` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 239 characters / 36 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 308, "frequency": "142.850 MHz", "id": "radio_mustard_gas_seepage_alert", "isEmergency": true, "signalStrength": "S8", "source": "Sector 8 Environmental Monitoring"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> CHEMICAL ALERT: Thawing floodwaters have breached an abandoned chemical weapons cache in the lower swale. Mustard gas vapors are pooling in low-lying craters and basements. Yellow oily film reported on water surfaces. Avoid the marsh road.
...
```json
{
  ...
  "id": "radio_mustard_gas_seepage_alert",
  "isEmergency": true,
  "signalStrength": "S8",
  "source": "Sector 8 Environmental Monitoring"
}
```
