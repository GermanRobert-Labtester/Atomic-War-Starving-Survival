# Subject Plan 178.13 — Civil Defense Monitoring Network: focused broadcast prose review
...
Review only the exact selector `radio_cyanide_gas_warning_sector_4` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 33 words / 237 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_cyanide_gas_warning_sector_4` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 237 characters / 33 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 246, "frequency": "142.850 MHz", "id": "radio_cyanide_gas_warning_sector_4", "isEmergency": true, "signalStrength": "S8", "source": "Civil Defense Monitoring Network"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> HAZARD ADVISORY: Subterranean foam insulation fires in Sector 4 infrastructure shafts are generating lethal concentrations of hydrogen cyanide gas. Do not enter unventilated lower levels without Model 58 respirators or amyl nitrite kits.
...
```json
{
  ...
  "id": "radio_cyanide_gas_warning_sector_4",
  "isEmergency": true,
  "signalStrength": "S8",
  "source": "Civil Defense Monitoring Network"
}
```
