# Subject Plan 178.34 — Sick Stranger: focused event prose review
...
Review only the exact selector `stranger_sickness` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` contains 138 words / 730 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `stranger_sickness` exists once in current `events.json` (`events`, 240 records), with fields `bodyText, id, minDay, title, weight`.
- **VERIFIED:** `bodyText` is the sole prose field proposed for review; current value is 730 characters / 138 whitespace-delimited words.
- **VERIFIED:** current non-prose snapshot is `{"id": "stranger_sickness", "minDay": 4, "title": "Sick Stranger", "weight": 1.5}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> A cough echoes off the airlock walls, then a body crumples against the inner door. The figure is wearing a civilian coat under a cracked gas mask, and the skin of their hands is mottled with radiation burns, dark patches that have not healed properly. They try to speak through the cough and fail. The mask lens is fogged from the inside. They came in through the outer hatch, so whatever they carried on their clothes is now on the floor of the airlock. The bunker has no quarantine space, no spare beds, and the med kit is already thin. What they have might be simple exhaustion. What they have might be a dose that finishes the job in a week. Either way, the door between you and them is the only real decision you get to make.
