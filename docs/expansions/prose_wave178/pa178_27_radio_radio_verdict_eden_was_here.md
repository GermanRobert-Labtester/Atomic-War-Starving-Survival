# Subject Plan 178.27 — Eden Vale (Recorded Relay): focused broadcast prose review
...
Review only the exact selector `radio_verdict_eden_was_here` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 79 words / 410 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_verdict_eden_was_here` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 410 characters / 79 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 220, "frequency": "104.700 MHz", "id": "radio_verdict_eden_was_here", "isEmergency": false, "signalStrength": "S5", "source": "Eden Vale (Recorded Relay)"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> Still here. Static's thinning. That is not good news — that is a storm on the way. If anyone is reading, the array is drawing again. I do not know what it is drawing for. There was a count pulse this morning on the census carrier. Someone is still running the machine. I filed my own numbers into the sub-band just to see if anyone would answer. No answer. But the tape accepted it. The tape always accepts it.
...
{
  "dayTrigger": 220,
  "frequency": "104.700 MHz",
  "id": "radio_verdict_eden_was_here",
  "isEmergency": false,
  "signalStrength": "S5",
  "source": "Eden Vale (Recorded Relay)"
}
```
