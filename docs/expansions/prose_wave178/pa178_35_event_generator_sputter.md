# Subject Plan 178.35 — Generator Failure: focused event prose review
...
Review only the exact selector `generator_sputter` in `Assets/StreamingAssets/Data/events.json`. Its current `bodyText` contains 129 words / 683 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `generator_sputter` exists once in current `events.json` (`events`, 240 records), with fields `bodyText, id, minDay, title, weight`.
- **VERIFIED:** `bodyText` is the sole prose field proposed for review; current value is 683 characters / 129 whitespace-delimited words.
- **VERIFIED:** current non-prose snapshot is `{"id": "generator_sputter", "minDay": 3, "title": "Generator Failure", "weight": 2.0}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> The lights flicker twice, hold, and die. The generator coughs once, a hard mechanical bark, and then the bunker goes silent in a way that has weight. Cold seeps in immediately, the way it does when the heaters are gone, and the only light left is the red glow of the emergency strip. Fuel gauge shows a quarter tank. The spark plug is fouled and the air filter on the generator is clogged with the same grey dust that gets into everything. You can rebuild the spark with wire and patience, or you can spend the fuel keeping the battery topped up, or you can let it sit and freeze the pipes. People are already pulling blankets off the bunks. The dark is not the problem. The cold is.
