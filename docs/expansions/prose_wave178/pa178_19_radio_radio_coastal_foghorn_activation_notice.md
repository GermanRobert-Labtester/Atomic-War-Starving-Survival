# Subject Plan 178.19 — Northern Sound Maritime Navigation: focused broadcast prose review
...
Review only the exact selector `radio_coastal_foghorn_activation_notice` in `Assets/StreamingAssets/Data/year_of_ash_radio.json`. Its current `message` contains 35 words / 221 characters. This is not a defect finding, a quota, or a target length. Preserve it unchanged when a revision would merely make it longer.
...
- **VERIFIED:** exact selector `radio_coastal_foghorn_activation_notice` exists once in current `year_of_ash_radio.json` (`broadcasts`, 50 records), with fields `dayTrigger, frequency, id, isEmergency, message, signalStrength, source`.
- **VERIFIED:** `message` is the sole prose field proposed for review; current value is 221 characters / 35 whitespace-delimited words.
- **VERIFIED:** current authoring metadata is frozen as `{"dayTrigger": 348, "frequency": "142.850 MHz", "id": "radio_coastal_foghorn_activation_notice", "isEmergency": false, "signalStrength": "S9", "source": "Northern Sound Maritime Navigation"}`.
- **VERIFIED:** this ID is absent from the current expansion-plan indexes searched for this batch.

**Current source prose:**

> NOTICE TO MARINERS AND ICE SLED CONVOYS: The compressed-air foghorn at High Bluff is operational on diesel power. Sounding one five-second blast every two minutes on tone C-flat to guide northern evacuees through sea fog.
...
```json
{
  ...
  "id": "radio_coastal_foghorn_activation_notice",
  "isEmergency": false,
  "signalStrength": "S9",
  "source": "Northern Sound Maritime Navigation"
}
```
