...
- Concern: **Duty Roster**. Current owner: `DutyRosterSystem`.
...
- Persistence seam: `duty_roster / DutyRosterSave v3`. Capture and restore must be proven through the live save registry before changing any field.
- Authored data family: `duty_roster_quests.json and the existing Duty Roster catalog family`. Every new ID must pass schema, reference, consumer, and reachability checks.
- Current source finding: Assignment and overflow engines are internal to DutyRosterSystem; the host owns the player route.
- Outcome sought: a pencilled shift that changes who must carry a burden; the actionable route is to request a lawful assignment through the roster owner; the observable result is that the roster card and next shift show the same assignee.
...
The first promoted slice should be an assignment that changes a visible shift card and one real operational consumer. `DutyRosterAssignmentEngine` and `DutyRosterOverflowEngine` are internal collaborators of `DutyRosterSystem`; they are not independent host services. Audit `Main.DutyRoster.cs` for the existing player command and `DutyRosterDayOwner` for phase-3 sampling. The hatch dilemma magnitudes in the original creative pack are pinned narrative mechanics: 50 rads/h on let-in, 10 on forced decontamination, and morale 20 on denial for every other living survivor. Do not alter them through a new duty modifier. A proposed scene about a watch transfer must use an existing role ID and an actor who can lawfully hold it. An absent actor yields a reasoned refusal, while the shift card continues to show the previous assignment. The scenario register below is a review matrix for voice and boundary handling; the authored creative pack above remains the source of candidate player prose.

## Current API checkpoints for the first slice

| Role | Current source | Symbol or audit action |
|---|---|---|
| Core assignment | `Assets/Ashfall.Core/DutyRoster/DutyRosterSystem.cs` | `AssignWithResult, ExecuteAssign, GetAssignment` |
| Core overflow | `Assets/Ashfall.Core/DutyRoster/DutyRosterSystem.cs` | `GrantOverflowAccess, RegisterOverflowVisit, CaptureOverflowState` |
| Host and section | `src/Main.DutyRoster.cs` | `SetupDutyRoster, SaveDutyRoster` |
...
public readonly record struct DutyRosterActionRequest(
...
### Duty Roster probe 001 — hatch watch / fresh campaign

**Situation.** A player reaches hatch watch before the next watch. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, there is no prior feature state and no optional catalog row has been unlocked. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The record keeps the names; it cannot make the choice for them.” This is candidate presentation copy, contingent on the state actually supporting it.

**Authority trace.** Read the live projection from `DutyRosterSystem`; do not infer the result from the panel's previous label. Send a single request through `DutyRosterHostSession and Main.DutyRoster.cs` only after the route exists. The Core transition must validate the identifier, eligibility, and current day against its own state. The host then applies only authorized neighboring effects, in documented order, and captures `duty_roster / DutyRosterSave v3` through its current store. Content comes from `duty_roster_quests.json and the existing Duty Roster catalog family` by stable ID. If that data family cannot represent this scene, classify it as an authoring proposal; do not add a shadow table.

**Failure and recovery.** For fresh campaign, the expected no-op is “No entry made. The previous record remains in force.” Retrying after the underlying condition changes must re-query the owner. Reloading must reproduce the same resolved state and must not replay the first successful event. Compare the assigned role and survivor against the pre-command roster; a refused shift leaves both unchanged. Fitness warnings require the current confirmation path, and no narrator line may imply that a tired survivor became fit merely because their name was written on the slate. If a survivor, faction, object, or location is missing, use the existing fallback presentation and preserve the save envelope.
...
### Duty Roster probe 002 — hatch watch / legacy restore

**Situation.** A player reaches hatch watch while the corridor is quiet. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, an older save lacks the newest additive field while retaining its original checksum contract. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “Someone has left room on the page for what happens next.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 003 — hatch watch / repeated command

**Situation.** A player reaches hatch watch at the marked change of shift. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, the same player action is submitted twice before the next day advances. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “No one in the room calls this a victory.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 004 — hatch watch / day rollover

**Situation.** A player reaches hatch watch after the shelter lights dip. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, the action is requested on the boundary between campaign days. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The count is exact. The reason for it is harder to file.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 006 — hatch watch / survivor death

**Situation.** A player reaches hatch watch before the next watch. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, a referenced survivor dies between preview and commitment. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The record keeps the names; it cannot make the choice for them.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 007 — hatch watch / resource shortage

**Situation.** A player reaches hatch watch while the corridor is quiet. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, the owner ledger has less stock than the preview advertised. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “Someone has left room on the page for what happens next.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 008 — hatch watch / power interruption

**Situation.** A player reaches hatch watch at the marked change of shift. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, the shelter power owner reports no usable supply for the relevant facility. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “No one in the room calls this a victory.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 010 — hatch watch / reopened panel

**Situation.** A player reaches hatch watch as the ledger is closed. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, the panel is disposed and reopened after a state change. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The object is small enough to miss, and too specific to replace.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 012 — hatch watch / cross-system signal

**Situation.** A player reaches hatch watch while the corridor is quiet. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, a neighboring owner publishes a fact during the same day phase. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “Someone has left room on the page for what happens next.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 013 — hatch watch / late campaign

**Situation.** A player reaches hatch watch at the marked change of shift. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, the relevant branch is encountered after the nominal story window. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “No one in the room calls this a victory.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 014 — hatch watch / zero participants

**Situation.** A player reaches hatch watch after the shelter lights dip. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, the roster projection contains no eligible living survivor. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The count is exact. The reason for it is harder to file.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 015 — hatch watch / many participants

**Situation.** A player reaches hatch watch as the ledger is closed. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, a large roster and dense catalog make iteration order visible. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The object is small enough to miss, and too specific to replace.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 016 — hatch watch / conflicting command

**Situation.** A player reaches hatch watch before the next watch. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, a second command requests an incompatible state transition. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The record keeps the names; it cannot make the choice for them.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 017 — hatch watch / malformed input

**Situation.** A player reaches hatch watch while the corridor is quiet. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, the input has an empty, unknown, or wrong-domain identifier. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “Someone has left room on the page for what happens next.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 019 — hatch watch / restore after event

**Situation.** A player reaches hatch watch after the shelter lights dip. A pencil stub gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, the game restores immediately after event publication. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The count is exact. The reason for it is harder to file.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 021 — infirmary relief / fresh campaign

**Situation.** A player reaches infirmary relief after the shelter lights dip. A wet sleeve gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, there is no prior feature state and no optional catalog row has been unlocked. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The count is exact. The reason for it is harder to file.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 022 — infirmary relief / legacy restore

**Situation.** A player reaches infirmary relief as the ledger is closed. A wet sleeve gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, an older save lacks the newest additive field while retaining its original checksum contract. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The object is small enough to miss, and too specific to replace.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 023 — infirmary relief / repeated command

**Situation.** A player reaches infirmary relief before the next watch. A wet sleeve gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, the same player action is submitted twice before the next day advances. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The record keeps the names; it cannot make the choice for them.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 024 — infirmary relief / day rollover

**Situation.** A player reaches infirmary relief while the corridor is quiet. A wet sleeve gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, the action is requested on the boundary between campaign days. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “Someone has left room on the page for what happens next.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 025 — infirmary relief / missing catalog reference

**Situation.** A player reaches infirmary relief at the marked change of shift. A wet sleeve gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, an authored ID cannot be resolved by the current loader. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “No one in the room calls this a victory.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 026 — infirmary relief / survivor death
...
### Duty Roster probe 027 — infirmary relief / resource shortage
...
### Duty Roster probe 028 — infirmary relief / power interruption
...
### Duty Roster probe 029 — infirmary relief / blocked route
...
### Duty Roster probe 030 — infirmary relief / reopened panel
...
### Duty Roster probe 031 — infirmary relief / same seed replay
...
### Duty Roster probe 032 — infirmary relief / cross-system signal
...
### Duty Roster probe 033 — infirmary relief / late campaign
...
### Duty Roster probe 034 — infirmary relief / zero participants
...
### Duty Roster probe 035 — infirmary relief / many participants
...
### Duty Roster probe 036 — infirmary relief / conflicting command
...
### Duty Roster probe 037 — infirmary relief / malformed input
...
### Duty Roster probe 038 — infirmary relief / failed save write
...
### Duty Roster probe 039 — infirmary relief / restore after event
...
### Duty Roster probe 040 — infirmary relief / accessibility route
...
### Duty Roster probe 041 — mess preparation / fresh campaign

**Situation.** A player reaches mess preparation while the corridor is quiet. A shift slate gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, there is no prior feature state and no optional catalog row has been unlocked. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “Someone has left room on the page for what happens next.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 042 — mess preparation / legacy restore

**Situation.** A player reaches mess preparation at the marked change of shift. A shift slate gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, an older save lacks the newest additive field while retaining its original checksum contract. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “No one in the room calls this a victory.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 043 — mess preparation / repeated command

**Situation.** A player reaches mess preparation after the shelter lights dip. A shift slate gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, the same player action is submitted twice before the next day advances. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The count is exact. The reason for it is harder to file.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 044 — mess preparation / day rollover

**Situation.** A player reaches mess preparation as the ledger is closed. A shift slate gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, the action is requested on the boundary between campaign days. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The object is small enough to miss, and too specific to replace.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 045 — mess preparation / missing catalog reference

**Situation.** A player reaches mess preparation before the next watch. A shift slate gives the scene a physical anchor. The request concerns a pencilled shift that changes who must carry a burden. In this variant, an authored ID cannot be resolved by the current loader. That detail matters because a preview made one moment earlier may no longer describe the command that can lawfully run now. The player should be able to read the current condition before committing, and should see a specific refusal if the condition changed. The text should remain human and precise: “The record keeps the names; it cannot make the choice for them.” This is candidate presentation copy, contingent on the state actually supporting it.
...
### Duty Roster probe 046 — mess preparation / survivor death
...
### Duty Roster probe 047 — mess preparation / resource shortage
...
### Duty Roster probe 048 — mess preparation / power interruption
...
### Duty Roster probe 049 — mess preparation / blocked route
...
### Duty Roster probe 050 — mess preparation / reopened panel
...
### Duty Roster probe 051 — mess preparation / same seed replay
...
### Duty Roster probe 052 — mess preparation / cross-system signal
...
### Duty Roster probe 053 — mess preparation / late campaign
...
The integration architecture for **Duty Roster** is therefore: authored JSON `duty_roster_quests.json and the existing Duty Roster catalog family` → `DutyRosterSystem` → `DutyRosterHostSession and Main.DutyRoster.cs` → existing save owner `duty_roster / DutyRosterSave v3` → a read-only Godot projection. This is the entire intended direction of dependency. A consumer may ask the owner for a fact; it may not mutate the owner's private copy or introduce a parallel clock, resource, outcome, or ledger. The phase-0 audit must replace the illustrative framework with exact current signatures and record any contrary source evidence.
