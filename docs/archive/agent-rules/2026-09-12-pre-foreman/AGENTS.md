...
# Autonomous Agent Rules: Test Execution

## MANDATORY: Targeted Testing Only
- NEVER run the full test suite (`gut_cmdln.gd` without arguments, `pytest`, or global test runners).
- ONLY run the single test file corresponding to the modified code.
- Exception being multiple tests that are in the region of modification! Never run more than 100 tests!
- Prioritise running lower amount of tests rather than closer to 100
- If you are a debugging agent or are searching for bugs and code issues plus wiring then you are free to run closer to 2- 00-500 tests per time Prioritise region test over full test suite
- If you are not implementing new code or expansion, then you are classified as a ai agent that seeks to improve the game code, you have clearance to run higher amount of tests with priority of not  exceeding 500 tests and hard cap of 700 tests, also please review the tests them selfs and see if the tests them selfs can be merged and or un-needed tests discarded!
- The rest of the info here is for implementing agents, integrating new code and coding in content for the game means you are a ai agent that primarly focuses on integrating rather than auditting, clearance no more than 100 tests per run and must adhere to region specific, avoid conflicting with other ai agents that may be coding aswell.
- If another ai agent is faster at fixing a bug you may have found while implementing and the ai agent seems to be fixing files faster than you can edit, the ai agent has clearly high tokens/s output and hence please allow any changes that they make, even if it breaks the specific part, however after a small amount of time retest and if the other ai agent simply made a mistake and forgot about the code and it made broken code you may fix the broken code!
- Make sure to run godot with 15fps instead of 24-30-45-60!! This is a HARD RULE!
- When creating a new test file, execute ONLY that file using:
  `./scripts/run_test.sh <path_to_test_file>`
- If no specific test path is provided, you must ask for clarification instead of running all tests.
- Parse and display ONLY the failure summary and stack trace. Do not dump the full stdout.
