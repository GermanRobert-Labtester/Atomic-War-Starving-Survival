...
`ProjectSettings/ProjectVersion.txt` pins Unity `6000.5.5f1`.

`Packages/manifest.json` contains a broad package/service surface including rendering, 2D tools, navigation, Input System, Visual Scripting, Test Framework, Remote Config, Analytics, Cloud Save/Cloud Code/Economy/Friends/Leaderboards/Moderation/Push Notifications, LevelPlay, and other Unity packages.

This matters even if Godot is the active gameplay host: if Unity builds remain part of release/support plans, dependency lifecycle, privacy consent, initialization ordering, and platform SDK behavior remain maintenance responsibilities.
