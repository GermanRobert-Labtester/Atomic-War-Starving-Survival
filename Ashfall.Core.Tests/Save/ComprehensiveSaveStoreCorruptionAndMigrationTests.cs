...
namespace Ashfall.Core.Tests.Save
{
    public class ComprehensiveSaveStoreCorruptionAndMigrationTests : IDisposable
    {
        ...
        {
            // Enforces that every section in the contract matrix (61 sections) is accounted for
            Assert.Equal(61, SaveSectionRegistry.All.Count);
            var keys = new HashSet<string>(SaveSectionRegistry.All.Select(s => s.SectionKey));
            Assert.Equal(61, keys.Count);
        }
    }
}
