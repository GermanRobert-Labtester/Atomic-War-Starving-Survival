#!/usr/bin/env bash
# ASHFALL — Google Jules setup / snapshot bootstrap (v5)
# Repository-audited and Jules-log-validated: 2026-09-19
#
# Purpose
# -------
...
cleanup_snapshot_worktree() {
  set_stage "restore clean Git worktree for Jules snapshot"

  # Godot's importer can generate legacy *.import sidecars next to source
  # images outside the canonical assets/ tree. Jules performs an independent
  # post-setup cleanliness check and rejects the snapshot if ANY tracked
  # modifications or non-ignored untracked files remain.
  #
  # This setup always starts from a fresh Jules clone, so restoring HEAD here
  # is intentional. Crucially, neither command removes ignored warm caches:
  #   .godot/                 (Godot import cache)
  #   bin/, obj/              (.NET build outputs)
  #   build/                  (setup/gate reports)
  #
  # git clean WITHOUT -x/-X does not remove ignored files.
  local before
  before="$(git status --porcelain=v1 --untracked-files=all || true)"

  if [[ -n "$before" ]]; then
    log "Setup generated worktree changes; restoring repository bytes to HEAD."
    printf '%s\n' "$before" | sed -n '1,120p'

    git reset --hard HEAD
    git clean -fd
  else
    log "Git worktree is already clean"
  fi

  local after
  after="$(git status --porcelain=v1 --untracked-files=all || true)"

  if [[ -n "$after" ]]; then
    printf '%s\n' "$after" >&2
    die "Jules snapshot requires a clean Git worktree, but setup artifacts remain."
  fi

  log "Git worktree clean; ignored .godot/.NET/build caches preserved"
}
...
final_sanity() {
  ...
  printf 'Git worktree:       CLEAN (snapshot-safe)\n'
