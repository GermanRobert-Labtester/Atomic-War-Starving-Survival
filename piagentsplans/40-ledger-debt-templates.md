...
- The historical plan asked for a new file because the old baseline had no `ledger_debt_templates.json`. That file is now current, schema-versioned and loaded by the shared `ExpansionHostSession` into one catalog instance shared by the ledger, dispatcher and credit paths.
- The live contract is template row → `DebtContract` presentation/terms → two-read signature → daily term countdown/forfeit → exactly-once consequence dispatch → faction embargo/labor/market effects → `ExpansionHubSave` capture/restore.
- The valuable expansion is precision: prove every template resolves to a current item/faction/consequence, every default has one owner, and every restore cannot replay a consequence. New credit products or a parallel ledger are out of scope.

**Bounded outcome:** Retire the old missing-catalog premise. The current authority has 15 debt templates and 10 consequence rows, `DebtTemplateCatalogLoader` validates them, `LedgerDebtSystem` owns contracts, and the existing `ExpansionHubSave` carries ledger, dispatcher, embargo and bridge state. The next work is a bounded economic/reachability audit, not a second debt ledger or arbitrary interest system.
...
- `ledger_debt_templates.json` is valid schema version 1 with 15 templates and 10 consequences; IDs, principal item references, terms, rates, forfeit descriptions and consequence references are loader concerns.
- `LedgerDebtSystem` has explicit `ReadsRequired=2`, `StandingFreshDays=3`, contract state, closed contracts, tamper state, capture/restore and named events; it does not compound interest and says so in source.
- `ExpansionHostSession` owns one `DebtTemplateCatalog`, one `DebtConsequenceDispatcher`, the `LedgerDebtSystem` and the canonical faction embargo ledger; `ExpansionHubSave` is the existing cross-host envelope.
- Historical DEC-299 focused tests cover the catalog, contract and debt-consequence paths; this document does not turn those historical pass counts into a fresh test result.
...
- C11 Economy cluster: ledger, market, faction standing and trade consequences remain separate owners.
...
- Replace the missing-file brief with a 15-template/10-consequence current census and a cross-reference matrix.
- Document the exact transaction boundary between presenting a contract, reading it twice, signing it, transferring principal, cancelling a failed draft, and dispatching default consequences.
- Audit current host/UI reachability and player-visible refusal/settlement states without creating a second credit manager.
- Preserve the `ExpansionHubSave` migration chain and exactly-once dispatcher/embargo/bridge records.
...
| --- | --- | --- | --- |
| template/consequence definitions and validation | DebtTemplateCatalogLoader | `Assets/Ashfall.Core/DebtTemplateCatalog.cs` | Owns static authored debt policy; it does not own signed contracts. |
| contract state, terms, reads, signature, term and settlement | LedgerDebtSystem | `Assets/Ashfall.Core/LedgerDebtSystem.cs` | Sole contract authority; simple non-compounding math is intentional. |
| one-time default consequence dispatch | DebtConsequenceDispatcher | `Assets/Ashfall.Core/DebtConsequenceDispatcher.cs` | Consumes current ledger/catalog and writes the fired-set/dispatcher state. |
| ledger, dispatcher, embargo and labor bridge persistence | ExpansionHubSave | `Assets/Ashfall.Core/ExpansionHubSave.cs` | Existing cross-host envelope; no new debt section. |
| catalog composition, commands and player projection | ExpansionHostSession/Main.DebtCredit | `src/Host/ExpansionHostSession.cs; src/Main.DebtCredit.cs; src/UI/SubterraneanDebtLedgerPanel.cs` | Host routes commands and presents owner state; it is not a second ledger. |
| contract, consequence and room/credit integration proof | Debt focused tests | `Ashfall.Core.Tests/LedgerDebtSystemTests.cs; Ashfall.Core.Tests/DebtConsequenceIntegrationTests.cs; Ashfall.Core.Tests/Shelter/Plan40_41DebtRoomIntegrationTests.cs` | Focused executable evidence. |
...
Authored JSON / current persisted owner state
                ...
                ▼
┌──────────────────────────────────────────────────────────────┐
│ Ledger Debt Templates, Consequence Dispatch and Save-Aware Credit
│ Integration route: extend current seams; no parallel authority │
└──────────────────────────────────────────────────────────────┘
│ DebtTemplateCatalogLoader
│   template/consequence definitions and validation
│ LedgerDebtSystem
│   contract state, terms, reads, signature, term and settlement
│ DebtConsequenceDispatcher
│   one-time default consequence dispatch
│ ExpansionHubSave
│   ledger, dispatcher, embargo and labor bridge persistence
│ ExpansionHostSession/Main.DebtCredit
│   catalog composition, commands and player projection
                │
                ▼
...
1. **Preserve current state ownership.** DebtTemplateCatalogLoader owns template/consequence definitions and validation: Owns static authored debt policy; it does not own signed contracts.
...
1. load and validate the shared debt catalog
2. resolve a template and canonical principal item
3. present a draft and record each reading
4. require the second reading and sign at the current day
5. atomically transfer/record principal through the existing credit/inventory seam
6. tick the term and expose forfeit state
7. dispatch each default consequence once
8. capture the existing expansion hub envelope and restore every owner
...
- Template definitions are immutable catalog data; signed contracts and closed contracts are owner state.
- A contract cannot be signed before two reads, cannot be silently overwritten while open, and retains a closed historical record after settlement.
- Forfeit/payment/forgiveness state is explicit; `TotalOwed` is a simple principal-plus-rate calculation, not a compounding ledger.
- Dispatcher fired records, embargo state and labor obligations are durable in the existing expansion hub envelope.
...
- Unknown template, item, creditor or consequence references fail before a contract is signed.
- A repeated sign/pay/forgive/default action is idempotent or returns a named refusal without double consequence.
- A cancelled draft cannot remove an unrelated draft because expected creditor/template guards are checked.
- Restore verifies checksum and does not replay already-fired dispatcher records.
...
- `ledger_debt_templates.json` is the sole template/consequence authority.
- Do not copy rows into a UI catalog, credit catalog or debt save file.
- A future row needs a current principal item, creditor/faction reference, distinct terms and a real dispatcher consequence.
...
- Use `ExpansionHubSave` and its current version/migration chain; no new `ledger_debt_save` section is justified.
- The debt state, dispatcher fired-set, embargo ledger and debt bridge must round-trip together at the owning save boundary.
- Older envelope versions must retain their frozen checksum shape and default missing fields to empty/neutral values.
...
- The ledger has no random selection; ordering is list order and contract lookup is deterministic.
- Consequence dispatch and catalog iteration must use stable IDs and ordinal ordering.
- Paired runs compare contract state, closed history, dispatcher records, embargoes and labor obligations.
...
- Ledger contract events report signed, paid, forgiven, renegotiated, forfeit and tamper facts.
- Dispatcher/bridge events are consumed by existing faction/market/labor owners; the debt system does not own their state.
- A failed transaction must cancel only the matching unsigned draft.
...
- Debt terms are physical, named and legible; the tone is grounded and restrained rather than gamified lending.
- A default consequence should identify the actual creditor, item/service and cost without pretending a prose outcome replaced the simulation.
- Do not add real-world currencies, countries, copied contracts or exploitative real-world advice.
...
| --- | --- | --- | --- |
| F-01 | A template references an item or consequence that the loader cannot resolve. | DebtTemplateCatalogLoader | Focused negative test or static source gate; no broad-suite dependency. |
| F-02 | A repeated daily tick dispatches the same default twice. | LedgerDebtSystem | Focused negative test or static source gate; no broad-suite dependency. |
| F-03 | A new draft replaces an open contract or a failed transfer removes the wrong draft. | DebtConsequenceDispatcher | Focused negative test or static source gate; no broad-suite dependency. |
| F-04 | A panel shows a zero balance while the owner still owns a contract. | ExpansionHubSave | Focused negative test or static source gate; no broad-suite dependency. |
| F-05 | A new save section creates a second debt authority. | ExpansionHostSession/Main.DebtCredit | Focused negative test or static source gate; no broad-suite dependency. |
...
1. `bash scripts/run_test.sh Ashfall.Core.Tests/LedgerDebtSystemTests.cs`
2. `bash scripts/run_test.sh Ashfall.Core.Tests/DebtConsequenceIntegrationTests.cs`
3. `bash scripts/run_test.sh Ashfall.Core.Tests/Shelter/Plan40_41DebtRoomIntegrationTests.cs`
...
| --- | --- | --- | --- |
| 0 — current census | Read catalog, loader, system, dispatcher, host and save envelope. | 15/10 counts and one authority chain are proven. | No production path until the owning implementation package is separately claimed. |
| 1 — transaction trace | Follow present/read/sign/transfer/tick/default/settle. | Every mutation and rollback has one owner. | No production path until the owning implementation package is separately claimed. |
| 2 — migration and replay proof | Exercise current and legacy envelopes plus repeated dispatch. | No double consequence or lost contract occurs. | No production path until the owning implementation package is separately claimed. |
| 3 — UI truthfulness pass | Audit debt panel states and accessible command feedback. | No shadow balance or misleading control remains. | No production path until the owning implementation package is separately claimed. |
...
| --- | --- | --- |
| Assets/StreamingAssets/Data/ledger_debt_templates.json | READ ONLY; MODIFY only for a proven reference/content gap | 15/10 authority |
| Assets/Ashfall.Core/LedgerDebtSystem.cs | READ ONLY | Contract owner |
| Assets/Ashfall.Core/ExpansionHubSave.cs | READ ONLY | Existing envelope |
| src/UI/SubterraneanDebtLedgerPanel.cs | READ ONLY; MODIFY only under a new UI claim | Projection |
...
| Risk | Control |
| --- | --- |
| Reintroducing compounding interest contrary to current source. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |
| Copying the catalog into a new credit authority. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |
| Losing the dispatcher fired-set during migration. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |
| Treating historical test counts as current runtime proof. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |

# 22. Explicit Non-Goals

- No new debt products.
- No new interest formula.
- No new save section.
...
- Future data or codec changes retain the prior valid catalog and frozen save fixtures.
...
- 15 templates and 10 consequences are documented against current owners.
- Contract/dispatcher/embargo/bridge save boundaries are explicit.
- Exactly-once, rollback, migration and UI truthfulness obligations are named.
- Focused commands are listed without fresh-pass claims.
...
**Authority clause — concern-specific ownership.** Every mutable value named in this plan is governed by the owner shown for that concern in the matrix: template/consequence definitions and validation → DebtTemplateCatalogLoader; contract state, terms, reads, signature, term and settlement → LedgerDebtSystem; one-time default consequence dispatch → DebtConsequenceDispatcher; ledger, dispatcher, embargo and labor bridge persistence → ExpansionHubSave; catalog composition, commands and player projection → ExpansionHostSession/Main.DebtCredit; contract, consequence and room/credit integration proof → Debt focused tests. A host object, panel, derived snapshot, generated document or test fixture is not an authority merely because it contains the same field name.

**Data clause.** A row in `Assets/StreamingAssets/Data/` is authored content, not reachability. The row must resolve to a loader, validator rule, runtime consumer, player or host command, and test surface. This applies to every catalog listed for Plan 40.

**Save clause.** Capture must be deep, restore must normalize only documented legacy absence, and a checksum must change when authoritative state changes. Plan 40 does not authorize a new save section when an existing owner can carry the fact.
...
**UI clause.** The interface reads the current owner projection, previews a real command and renders named refusals. It must not recompute state owned by DebtTemplateCatalogLoader or any other authority, hide uncertainty, or introduce a gameplay-only counter.
...
# Appendix B.02 — Current Code Architecture: `Assets/Ashfall.Core/LedgerDebtSystem.cs`
...
# Appendix B.03 — Current Code Architecture: `Assets/Ashfall.Core/DebtTemplateCatalog.cs`

### `Assets/Ashfall.Core/DebtTemplateCatalog.cs`

- Evidence status: **CURRENT FILE PRESENT**.
- Size: 190 lines / 8121 bytes.
- SHA-256: `9efbaeef6f290164f33ad09a70c590b2f99a4a6d231cf802d2b5f551e45a4290`.
...
# Appendix B.04 — Current Code Architecture: `Assets/Ashfall.Core/DebtConsequenceDispatcher.cs`

### `Assets/Ashfall.Core/DebtConsequenceDispatcher.cs`

- Evidence status: **CURRENT FILE PRESENT**.
- Size: 316 lines / 13769 bytes.
- SHA-256: `8878d927372fd361345bfa3b853318c29150a1684ca4f832bd56fda6b3e22ac8`.
- Architecture signals: seeded references=0; save/restore symbols=2; typed event declarations=26; textual Godot mentions=0; textual Unity/JsonUtility mentions=0; `System.Random`=0; wall-clock DateTime reads=0; dynamic GUIDs=0.
- Integration reading: This evidence contributes to the current architecture map and must be interpreted through the ownership matrix below.
...
# Appendix B.05 — Current Code Architecture: `Assets/Ashfall.Core/ExpansionHubSave.cs`
...
# Appendix B.06 — Current Code Architecture: `src/Host/ExpansionHostSession.cs`
...
# Appendix B.07 — Current Code Architecture: `src/Main.DebtCredit.cs`

### `src/Main.DebtCredit.cs`

- Evidence status: **CURRENT FILE PRESENT**.
- Size: 166 lines / 7064 bytes.
- SHA-256: `54f2d8bbf05ba706a80a0f8becd7ed61f3fa4df963312b291b2133e2d7ec6e85`.
...
# Appendix B.08 — Current Code Architecture: `src/UI/SubterraneanDebtLedgerPanel.cs`

### `src/UI/SubterraneanDebtLedgerPanel.cs`

- Evidence status: **CURRENT FILE PRESENT**.
- Size: 100 lines / 5234 bytes.
- SHA-256: `cb99fcb90c6d8afe071b3506aacbdf14ab262f21f478c9aac48c0b8dc450cbd0`.
...
# Appendix C.09 — Catalog Census: `Assets/StreamingAssets/Data/ledger_debt_templates.json`

### `Assets/StreamingAssets/Data/ledger_debt_templates.json`

- Parse: valid strict JSON.
- Schema version: `1`.
- Size: 14240 bytes / 14190 characters.
- SHA-256: `58a93a97dd02d2e11174d8c799d30ddbf8350dd3f094ee5bc3dcb5e1310f1f16`.
- Root keys: `consequences`, `schema_version`, `templates`.

Array-path census (minimum, maximum, observed rows):

```text
consequences: min=10, max=10, observed_paths=1
templates: min=15, max=15, observed_paths=1
```
...
# Appendix C.11 — Catalog Census: `Assets/StreamingAssets/Data/faction_territory.json`
...
# Appendix D.12 — Existing Focused Test Inventory: `Ashfall.Core.Tests/LedgerDebtSystemTests.cs`

### `Ashfall.Core.Tests/LedgerDebtSystemTests.cs`

- Current test declarations: Fact=22, Theory=0, InlineData=0.
- File lines: 429; SHA-256: `aae26054d31896970bb1a4421f01e84b6d1b62e41bcdc891584b329b146e3a77`.
...
```text
PresentRejectsInvalidTerms
SigningRequiresTwoReadings
SignedContractFreezesTermsAndBlocksNewDraft
TermExpiryTriggersNamedForfeit
PayResolvesBeforeForfeit
PayStillHonoursAfterForfeitDue
CannotPayUnsignedOrTwice
RenegotiationReplacesAndRequiresFreshReadings
RenegotiationRejectsPaidForfeitedAndUnknown
TamperIsOneShot
TotalOwedIsFlatRate
TermEndRenegotiationExtendsSignedInk
SaveRoundTripPreservesContractsAndTamper
RestoreStateNullSafeAndIdempotent
StateChangedFiresOnMutations
TotalOwedDropsToZeroOncePaid
SettledContractIsArchivedNotOverwritten
UnresolvedForfeitBlocksNewDraft
ContestedRenegotiationRequiresAFreshStanding
UncontestedRenegotiationIgnoresTheStandingGate
SaveRoundTripPreservesClosedContracts
HeadlessDemoPasses
```


# Appendix D.13 — Existing Focused Test Inventory: `Ashfall.Core.Tests/DebtConsequenceIntegrationTests.cs`

### `Ashfall.Core.Tests/DebtConsequenceIntegrationTests.cs`

- Current test declarations: Fact=24, Theory=0, InlineData=0.
- File lines: 593; SHA-256: `c883857448062a308e1b54dfb527559d5b5bcdad0524282c65c6042d827052dd`.
- The list below is an inventory, not a newly executed pass result. Focused tests must be rerun when the owning implementation changes.

```text
FiredStateRoundtripPreventsRedispatch
ConsequenceIdentity_IsStable_AndSplitsContractInstances
StandingEvent_CarriesAuthoredDelta_AndCreditorFallback
EscalationChain_DispatchesBountyAfterModerateStanding
CollateralSeizure_FallsBackToTemplatePrincipal
DispatchIsCatalogDriven_NotHardCodedToStanding
ForgiveContract_ClearsBalance_WithoutPayment
ForgiveContract_RefusesUnsignedDraft
ForgivenState_PersistsRoundtrip_AndAllowsNewDraft
ForgivenessConsequence_ChangesLedgerState_NotEventOnly
Embargo_WindowBoundaries_AreDayDerived
Embargo_SameSourceIsIdempotent
Embargo_RestoreRoundtrip
Bridge_StandingAppliedExactlyOnce_WithClamping
Bridge_StandingClampsAtLowerBound
Bridge_EmbargoBlocksCreditor_OtherFactionsUnaffected
Bridge_EmbargoDoesNotDuplicateAfterRestore
Bridge_BountyRoutedToRaidAuthority_OncePerConsequenceStage
Bridge_CollateralSeizure_RemovesExactlyOnce
Bridge_CollateralShortfall_SeizesNothing
Bridge_LaborObligation_IsBounded_AndSurvivesSaveLoad
Bridge_LaborObligation_DoesNotDuplicateForSameConsequence
Bridge_TeardownAndRebuild_DoesNotLeakSubscriptions
Bridge_ForgivenessViaBridge_ChangesLedgerState
```


# Appendix D.14 — Existing Focused Test Inventory: `Ashfall.Core.Tests/Shelter/Plan40_41DebtRoomIntegrationTests.cs`

### `Ashfall.Core.Tests/Shelter/Plan40_41DebtRoomIntegrationTests.cs`

- Current test declarations: Fact=3, Theory=0, InlineData=0.
- File lines: 218; SHA-256: `08078ef664ead4b529c88602ed50b2d1cb14474aa38104c8cb779f7b7a4bbf9b`.
- The list below is an inventory, not a newly executed pass result. Focused tests must be rerun when the owning implementation changes.

```text
LedgerDebtTemplates_LoadsAll15TemplatesAnd10Consequences_FromAuthoritativeJson
ShelterRoomCatalog_LoadsAll23RoomsAnd12Rules_FromAuthoritativeJson
LedgerDebtSystem_And_ShelterRoomSystem_ExecuteConcurrentlyWithoutInterference
```


# Appendix E.15 — Supporting Code Evidence: `Assets/Ashfall.Core/LedgerDebtSystem.cs`
...
# Appendix E.16 — Supporting Code Evidence: `Assets/Ashfall.Core/DebtTemplateCatalog.cs`
...
# Appendix E.17 — Supporting Code Evidence: `Assets/Ashfall.Core/DebtConsequenceDispatcher.cs`
...
# Appendix E.19 — Supporting Code Evidence: `src/Host/ExpansionHostSession.cs`
...
# Appendix G.20 — Supporting Regression Evidence: `Ashfall.Core.Tests/LedgerDebtSystemTests.cs`
...
# Appendix G.21 — Supporting Regression Evidence: `Ashfall.Core.Tests/DebtConsequenceIntegrationTests.cs`
...
# Appendix G.22 — Supporting Regression Evidence: `Ashfall.Core.Tests/Shelter/Plan40_41DebtRoomIntegrationTests.cs`
...
| --- | --- | --- | --- | --- |
| template/consequence definitions and validation | DebtTemplateCatalogLoader | contract state, terms, reads, signature, term and settlement | LedgerDebtSystem | Owner emits/reads a typed fact; no mirror state. |
| template/consequence definitions and validation | DebtTemplateCatalogLoader | one-time default consequence dispatch | DebtConsequenceDispatcher | Owner emits/reads a typed fact; no mirror state. |
| template/consequence definitions and validation | DebtTemplateCatalogLoader | ledger, dispatcher, embargo and labor bridge persistence | ExpansionHubSave | Owner emits/reads a typed fact; no mirror state. |
| template/consequence definitions and validation | DebtTemplateCatalogLoader | catalog composition, commands and player projection | ExpansionHostSession/Main.DebtCredit | Owner emits/reads a typed fact; no mirror state. |
| template/consequence definitions and validation | DebtTemplateCatalogLoader | contract, consequence and room/credit integration proof | Debt focused tests | Owner emits/reads a typed fact; no mirror state. |
| contract state, terms, reads, signature, term and settlement | LedgerDebtSystem | template/consequence definitions and validation | DebtTemplateCatalogLoader | Owner emits/reads a typed fact; no mirror state. |
| contract state, terms, reads, signature, term and settlement | LedgerDebtSystem | one-time default consequence dispatch | DebtConsequenceDispatcher | Owner emits/reads a typed fact; no mirror state. |
| contract state, terms, reads, signature, term and settlement | LedgerDebtSystem | ledger, dispatcher, embargo and labor bridge persistence | ExpansionHubSave | Owner emits/reads a typed fact; no mirror state. |
| contract state, terms, reads, signature, term and settlement | LedgerDebtSystem | catalog composition, commands and player projection | ExpansionHostSession/Main.DebtCredit | Owner emits/reads a typed fact; no mirror state. |
| contract state, terms, reads, signature, term and settlement | LedgerDebtSystem | contract, consequence and room/credit integration proof | Debt focused tests | Owner emits/reads a typed fact; no mirror state. |
| one-time default consequence dispatch | DebtConsequenceDispatcher | template/consequence definitions and validation | DebtTemplateCatalogLoader | Owner emits/reads a typed fact; no mirror state. |
| one-time default consequence dispatch | DebtConsequenceDispatcher | contract state, terms, reads, signature, term and settlement | LedgerDebtSystem | Owner emits/reads a typed fact; no mirror state. |
| one-time default consequence dispatch | DebtConsequenceDispatcher | ledger, dispatcher, embargo and labor bridge persistence | ExpansionHubSave | Owner emits/reads a typed fact; no mirror state. |
| one-time default consequence dispatch | DebtConsequenceDispatcher | catalog composition, commands and player projection | ExpansionHostSession/Main.DebtCredit | Owner emits/reads a typed fact; no mirror state. |
| one-time default consequence dispatch | DebtConsequenceDispatcher | contract, consequence and room/credit integration proof | Debt focused tests | Owner emits/reads a typed fact; no mirror state. |
| ledger, dispatcher, embargo and labor bridge persistence | ExpansionHubSave | template/consequence definitions and validation | DebtTemplateCatalogLoader | Owner emits/reads a typed fact; no mirror state. |
| ledger, dispatcher, embargo and labor bridge persistence | ExpansionHubSave | contract state, terms, reads, signature, term and settlement | LedgerDebtSystem | Owner emits/reads a typed fact; no mirror state. |
| ledger, dispatcher, embargo and labor bridge persistence | ExpansionHubSave | one-time default consequence dispatch | DebtConsequenceDispatcher | Owner emits/reads a typed fact; no mirror state. |
| ledger, dispatcher, embargo and labor bridge persistence | ExpansionHubSave | catalog composition, commands and player projection | ExpansionHostSession/Main.DebtCredit | Owner emits/reads a typed fact; no mirror state. |
| ledger, dispatcher, embargo and labor bridge persistence | ExpansionHubSave | contract, consequence and room/credit integration proof | Debt focused tests | Owner emits/reads a typed fact; no mirror state. |
| catalog composition, commands and player projection | ExpansionHostSession/Main.DebtCredit | template/consequence definitions and validation | DebtTemplateCatalogLoader | Owner emits/reads a typed fact; no mirror state. |
| catalog composition, commands and player projection | ExpansionHostSession/Main.DebtCredit | contract state, terms, reads, signature, term and settlement | LedgerDebtSystem | Owner emits/reads a typed fact; no mirror state. |
| catalog composition, commands and player projection | ExpansionHostSession/Main.DebtCredit | one-time default consequence dispatch | DebtConsequenceDispatcher | Owner emits/reads a typed fact; no mirror state. |
| catalog composition, commands and player projection | ExpansionHostSession/Main.DebtCredit | ledger, dispatcher, embargo and labor bridge persistence | ExpansionHubSave | Owner emits/reads a typed fact; no mirror state. |
| catalog composition, commands and player projection | ExpansionHostSession/Main.DebtCredit | contract, consequence and room/credit integration proof | Debt focused tests | Owner emits/reads a typed fact; no mirror state. |
| contract, consequence and room/credit integration proof | Debt focused tests | template/consequence definitions and validation | DebtTemplateCatalogLoader | Owner emits/reads a typed fact; no mirror state. |
| contract, consequence and room/credit integration proof | Debt focused tests | contract state, terms, reads, signature, term and settlement | LedgerDebtSystem | Owner emits/reads a typed fact; no mirror state. |
| contract, consequence and room/credit integration proof | Debt focused tests | one-time default consequence dispatch | DebtConsequenceDispatcher | Owner emits/reads a typed fact; no mirror state. |
| contract, consequence and room/credit integration proof | Debt focused tests | ledger, dispatcher, embargo and labor bridge persistence | ExpansionHubSave | Owner emits/reads a typed fact; no mirror state. |
| contract, consequence and room/credit integration proof | Debt focused tests | catalog composition, commands and player projection | ExpansionHostSession/Main.DebtCredit | Owner emits/reads a typed fact; no mirror state. |
...
| --- | --- | --- | --- |
| R-01 | Replace the missing-file brief with a 15-template/10-consequence current census and a cross-reference matrix. | Focused negative/edge test; host/runtime proof where player-visible. | BLOCKED if no named owner can accept the fact. |
| R-02 | Document the exact transaction boundary between presenting a contract, reading it twice, signing it, transferring principal, cancelling a failed draft, and dispatching default consequences. | Focused negative/edge test; host/runtime proof where player-visible. | BLOCKED if no named owner can accept the fact. |
| R-03 | Audit current host/UI reachability and player-visible refusal/settlement states without creating a second credit manager. | Focused negative/edge test; host/runtime proof where player-visible. | BLOCKED if no named owner can accept the fact. |
| R-04 | Preserve the `ExpansionHubSave` migration chain and exactly-once dispatcher/embargo/bridge records. | Focused negative/edge test; host/runtime proof where player-visible. | BLOCKED if no named owner can accept the fact. |
...
The subject is the contract lifecycle and its existing cross-host debt consequence boundary. The plan expands the transaction, migration, replay and player-feedback architecture while preserving the current non-compounding ledger semantics.

- **template/consequence definitions and validation** remains with `DebtTemplateCatalogLoader` at `Assets/Ashfall.Core/DebtTemplateCatalog.cs`. Owns static authored debt policy; it does not own signed contracts.
- **contract state, terms, reads, signature, term and settlement** remains with `LedgerDebtSystem` at `Assets/Ashfall.Core/LedgerDebtSystem.cs`. Sole contract authority; simple non-compounding math is intentional.
- **one-time default consequence dispatch** remains with `DebtConsequenceDispatcher` at `Assets/Ashfall.Core/DebtConsequenceDispatcher.cs`. Consumes current ledger/catalog and writes the fired-set/dispatcher state.
- **ledger, dispatcher, embargo and labor bridge persistence** remains with `ExpansionHubSave` at `Assets/Ashfall.Core/ExpansionHubSave.cs`. Existing cross-host envelope; no new debt section.
- **catalog composition, commands and player projection** remains with `ExpansionHostSession/Main.DebtCredit` at `src/Host/ExpansionHostSession.cs; src/Main.DebtCredit.cs; src/UI/SubterraneanDebtLedgerPanel.cs`. Host routes commands and presents owner state; it is not a second ledger.
- **contract, consequence and room/credit integration proof** remains with `Debt focused tests` at `Ashfall.Core.Tests/LedgerDebtSystemTests.cs; Ashfall.Core.Tests/DebtConsequenceIntegrationTests.cs; Ashfall.Core.Tests/Shelter/Plan40_41DebtRoomIntegrationTests.cs`. Focused executable evidence.
...
| --- | --- | --- | --- | --- | --- |
| S-01 | 40-01 template load and cross-reference validation | load and validate the shared debt catalog | Template definitions are immutable catalog data; signed contracts and closed contracts are owner state. | A template references an item or consequence that the loader cannot resolve. | DebtTemplateCatalogLoader |
| S-02 | 40-02 first contract reading | resolve a template and canonical principal item | A contract cannot be signed before two reads, cannot be silently overwritten while open, and retains a closed historical record after settlement. | A repeated daily tick dispatches the same default twice. | DebtTemplateCatalogLoader |
| S-03 | 40-03 second reading and signature | present a draft and record each reading | Forfeit/payment/forgiveness state is explicit; `TotalOwed` is a simple principal-plus-rate calculation, not a compounding ledger. | A new draft replaces an open contract or a failed transfer removes the wrong draft. | DebtTemplateCatalogLoader |
| S-04 | 40-04 failed transfer draft cancellation | require the second reading and sign at the current day | Dispatcher fired records, embargo state and labor obligations are durable in the existing expansion hub envelope. | A panel shows a zero balance while the owner still owns a contract. | DebtTemplateCatalogLoader |
| S-05 | 40-05 term expiry and forfeit | atomically transfer/record principal through the existing credit/inventory seam | Template definitions are immutable catalog data; signed contracts and closed contracts are owner state. | A new save section creates a second debt authority. | DebtTemplateCatalogLoader |
| S-06 | 40-06 payment after forfeit | tick the term and expose forfeit state | A contract cannot be signed before two reads, cannot be silently overwritten while open, and retains a closed historical record after settlement. | A template references an item or consequence that the loader cannot resolve. | DebtTemplateCatalogLoader |
| S-07 | 40-07 forgiveness path | dispatch each default consequence once | Forfeit/payment/forgiveness state is explicit; `TotalOwed` is a simple principal-plus-rate calculation, not a compounding ledger. | A repeated daily tick dispatches the same default twice. | DebtTemplateCatalogLoader |
| S-08 | 40-08 restore and dispatcher replay | capture the existing expansion hub envelope and restore every owner | Dispatcher fired records, embargo state and labor obligations are durable in the existing expansion hub envelope. | A new draft replaces an open contract or a failed transfer removes the wrong draft. | DebtTemplateCatalogLoader |
...
| --- | --- | --- | --- | --- |
| T-01 | 40-TC-01 schema and duplicate template IDs | data | schema and duplicate template IDs; verify the current owner and its negative boundary without inventing a second authority. | DebtTemplateCatalogLoader |
| T-02 | 40-TC-02 principal item resolution | unit | principal item resolution; verify the current owner and its negative boundary without inventing a second authority. | DebtTemplateCatalogLoader |
| T-03 | 40-TC-03 creditor/faction reference check | persistence | creditor/faction reference check; verify the current owner and its negative boundary without inventing a second authority. | DebtTemplateCatalogLoader |
| T-04 | 40-TC-04 two-read signature gate | determinism | two-read signature gate; verify the current owner and its negative boundary without inventing a second authority. | DebtTemplateCatalogLoader |
| T-05 | 40-TC-05 open contract replacement refusal | host | open contract replacement refusal; verify the current owner and its negative boundary without inventing a second authority. | DebtTemplateCatalogLoader |
| T-06 | 40-TC-06 term countdown boundary | UI/accessibility | term countdown boundary; verify the current owner and its negative boundary without inventing a second authority. | DebtTemplateCatalogLoader |
| T-07 | 40-TC-07 forfeit/pay/forgive exclusivity | cross-system | forfeit/pay/forgive exclusivity; verify the current owner and its negative boundary without inventing a second authority. | DebtTemplateCatalogLoader |
| T-08 | 40-TC-08 closed contract history | data | closed contract history; verify the current owner and its negative boundary without inventing a second authority. | DebtTemplateCatalogLoader |
| T-09 | 40-TC-09 dispatcher exactly-once | unit | dispatcher exactly-once; verify the current owner and its negative boundary without inventing a second authority. | DebtTemplateCatalogLoader |
| T-10 | 40-TC-10 expansion hub migration | persistence | expansion hub migration; verify the current owner and its negative boundary without inventing a second authority. | DebtTemplateCatalogLoader |
| T-11 | 40-TC-11 deep-copy save isolation | determinism | deep-copy save isolation; verify the current owner and its negative boundary without inventing a second authority. | DebtTemplateCatalogLoader |
| T-12 | 40-TC-12 UI state and refusal projection | host | UI state and refusal projection; verify the current owner and its negative boundary without inventing a second authority. | DebtTemplateCatalogLoader |
...
| --- | --- | --- |
| 69 | `Assets/Ashfall.Core/ExpansionHubSave.cs` | current reference count; inspect the caller before treating it as a live route |
| 43 | `Ashfall.Core.Tests/DebtConsequenceIntegrationTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 24 | `Ashfall.Core.Tests/ExpansionHubSaveV5Tests.cs` | current reference count; inspect the caller before treating it as a live route |
| 17 | `src/Host/ExpansionHostSession.cs` | current reference count; inspect the caller before treating it as a live route |
| 16 | `Ashfall.Core.Tests/ExpansionHubSaveTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 16 | `src/Host/ExpansionHubSaveStore.cs` | current reference count; inspect the caller before treating it as a live route |
| 14 | `Ashfall.Core.Tests/DiseaseSystemTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 14 | `Assets/Ashfall.Core/LedgerDebtSystem.cs` | current reference count; inspect the caller before treating it as a live route |
| 11 | `Ashfall.Core.Tests/LedgerDebtSystemTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 11 | `Assets/Ashfall.Core/DebtConsequenceHostBridge.cs` | current reference count; inspect the caller before treating it as a live route |
| 11 | `Assets/Ashfall.Core/DebtTemplateCatalog.cs` | current reference count; inspect the caller before treating it as a live route |
| 11 | `Assets/Ashfall.Core/LedgerDebtHeadlessDemo.cs` | current reference count; inspect the caller before treating it as a live route |
| 9 | `Ashfall.Core.Tests/SilentFoundrySystemTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 8 | `Ashfall.Core.Tests/SilentFoundryConsequenceTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 8 | `src/Host/HostCli.PanelTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 7 | `Assets/Ashfall.Core/DebtConsequenceDispatcher.cs` | current reference count; inspect the caller before treating it as a live route |
| 5 | `Ashfall.Core.Tests/TradeCreditCoordinatorTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 4 | `Ashfall.Core.Tests/CrossingQuestSystemTests.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 4 | `Assets/Ashfall.Core/Economy/TradeCreditCoordinator.cs` | current reference count; inspect the caller before treating it as a live route |
| 4 | `src/Main.ExpansionHub.cs` | current reference count; inspect the caller before treating it as a live route |
| 3 | `Ashfall.Core.Tests/Shelter/Plan40_41DebtRoomIntegrationTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 3 | `src/UI/CrossingQuestPanel.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 2 | `Ashfall.Core.Tests/VersionReportContractTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `Assets/Ashfall.Core/Economy/LoanSharkEnforcerEngine.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `Assets/Ashfall.Core/HostCliRegistry.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `src/Host/HostCli.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `src/Host/OrphanSealWave1HostSessions.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `src/Host/ShelterOperationsHostSession.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `src/Main.Application.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `src/Main.Holdfast.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `src/Main.OrphanSealWave1.cs` | current reference count; inspect the caller before treating it as a live route |
...
# Appendix — Complete Catalog Audit: `Assets/StreamingAssets/Data/ledger_debt_templates.json`

### `Assets/StreamingAssets/Data/ledger_debt_templates.json` — complete current row audit

- Parse status: **valid JSON**.
- Bytes: 14240; characters: 14190.
- SHA-256: `58a93a97dd02d2e11174d8c799d30ddbf8350dd3f094ee5bc3dcb5e1310f1f16`.
- Row presence is not reachability. Each row below is an authored fact requiring a loader/consumer check.

Root keys: `schema_version`, `templates`, `consequences`

#### `templates` — 15 current rows

- Row 001 `debt_supply_corps_rations`: `{"consequenceId":"conseq_standing_loss_mild","creditorId":"faction_supply_corps","description":"The Supply Corps extends emergency rations against the shelter's next allocation. Eight tins, twenty days, fifteen percent. The quartermaster m…`
- Row 002 `debt_supply_corps_fuel`: `{"consequenceId":"conseq_standing_loss_mild","creditorId":"faction_supply_corps","description":"Fuel for the generator, against the shelter's word. The Corps charges twenty percent — not predatory, but the quartermaster remembers who borro…`
- Row 003 `debt_supply_corps_medical`: `{"consequenceId":"conseq_standing_loss_mild","creditorId":"faction_supply_corps","description":"Medical supplies on credit. The Corps keeps the rate low for medicine — dead debtors pay nothing. Fifteen days. The nurse watches the door.","d…`
- Row 004 `debt_hydro_barons_water`: `{"consequenceId":"conseq_embargo_trade","creditorId":"faction_hydro_barons","description":"The Barons control the aquifer. Twelve litres, eighteen days, twenty-five percent. They do not negotiate — water is leverage, and they know it.","di…`
- Row 005 `debt_hydro_barons_filter`: `{"consequenceId":"conseq_embargo_trade","creditorId":"faction_hydro_barons","description":"Filters are infrastructure. The Barons lend them at thirty percent — steep, but a shelter without clean water is a shelter that pays any price.","di…`
- Row 006 `debt_hydro_barons_purification`: `{"consequenceId":"conseq_embargo_trade","creditorId":"faction_hydro_barons","description":"Purification tablets on credit. The Barons prefer this to lending hardware — tablets run out, and the borrower comes back.","displayName":"Hydro Bar…`
- Row 007 `debt_railway_guild_fuel`: `{"consequenceId":"conseq_standing_loss_and_embargo","creditorId":"faction_railway_guild","description":"The Guild keeps the trains running on diesel. Ten litres, twenty-eight days. They charge less than the Barons — but default, and the tr…`
- Row 008 `debt_railway_guild_parts`: `{"consequenceId":"conseq_standing_loss_and_embargo","creditorId":"faction_railway_guild","description":"Mechanical parts for shelter maintenance. The Guild's rate is fair — they need the shelters functional as much as the shelters need par…`
- Row 009 `debt_railway_guild_transport`: `{"consequenceId":"conseq_bounty_moderate","creditorId":"faction_railway_guild","description":"An engine is capital equipment. The Guild lends one at thirty-five percent over forty-five days — long term, high stakes. Default here, and the G…`
- Row 010 `debt_ordnance_foundry_ammo`: `{"consequenceId":"conseq_bounty_moderate","creditorId":"faction_ordnance_foundry","description":"The Foundry produces. The shelter borrows. Forty rounds, fourteen days. The Foundry does not forget — and they have enforcers.","displayName":…`
- Row 011 `debt_ordnance_foundry_tools`: `{"consequenceId":"conseq_standing_loss_moderate","creditorId":"faction_ordnance_foundry","description":"Tools on credit. The Foundry charges fifteen percent — they want the shelters capable, not desperate. Default, and the next requisition…`
- Row 012 `debt_ordnance_foundry_armor`: `{"consequenceId":"conseq_bounty_moderate","creditorId":"faction_ordnance_foundry","description":"Gas masks are survival equipment. The Foundry lends three at twenty-five percent — and expects them back in working order.","displayName":"Ord…`
- Row 013 `debt_scavengers_food`: `{"consequenceId":"conseq_collateral_seizure","creditorId":"faction_scavengers","description":"The Scavengers don't ask questions. Fifteen packs, twelve days, thirty percent. They want collateral up front — and they know what's in your shel…`
- Row 014 `debt_scavengers_medicine`: `{"consequenceId":"conseq_collateral_seizure","creditorId":"faction_scavengers","description":"Black-market antibiotics. Ten days, thirty-five percent. The Scavengers don't do instalments — pay in full, or they take what they're owed.","dis…`
- Row 015 `debt_scavengers_equipment`: `{"consequenceId":"conseq_collateral_seizure","creditorId":"faction_scavengers","description":"Scavenged dosimeters, sold on credit. Twenty days, twenty-five percent. The Scavengers know the value of what they lend — and the value of what t…`

#### `consequences` — 10 current rows

- Row 001 `conseq_standing_loss_mild`: `{"bountyLevel":"","collateralItemId":"","description":"The creditor notes the default. Standing drops — not catastrophically, but the ledger remembers.","displayName":"Creditor Displeasure","effectType":"standing_loss","embargoDurationDays…`
- Row 002 `conseq_standing_loss_moderate`: `{"bountyLevel":"","collateralItemId":"","description":"A serious default. The creditor's standing drops sharply, and word spreads — other factions take note.","displayName":"Creditor Contempt","effectType":"standing_loss","embargoDurationD…`
- Row 003 `conseq_embargo_trade`: `{"bountyLevel":"","collateralItemId":"","description":"The creditor suspends trade relations. Fourteen days of closed markets — the shelter must find supplies elsewhere, or negotiate a settlement.","displayName":"Trade Suspension","effectT…`
- Row 004 `conseq_standing_loss_and_embargo`: `{"bountyLevel":"","collateralItemId":"","description":"Default damages standing AND closes the creditor's markets. Ten days of embargo, a lasting reputation hit, and the threat of escalation.","displayName":"Broken Trust","effectType":"sta…`
- Row 005 `conseq_bounty_moderate`: `{"bountyLevel":"moderate","collateralItemId":"","description":"The creditor posts a bounty. Collectors may visit — not an army, but enough to remind the shelter that debts have consequences. Escalation looms.","displayName":"Debt Collector…`
- Row 006 `conseq_collateral_seizure`: `{"bountyLevel":"low","collateralItemId":"","description":"The creditor seizes pledged collateral and sends a warning. The shelter loses the asset but keeps market access — for now.","displayName":"Collateral Forfeiture","effectType":"bount…`
- Row 007 `conseq_raid_severe`: `{"bountyLevel":"severe","collateralItemId":"","description":"Unresolved debt escalates to a shelter raid. The creditor's enforcers arrive — this is not negotiation. Standing damage is severe and lasting.","displayName":"Enforcement Raid","…`
- Row 008 `conseq_labor_obligation`: `{"bountyLevel":"","collateralItemId":"","description":"The creditor invokes a labor clause. Seven days of compulsory service — the shelter loses a survivor's labour but keeps market access and avoids escalation.","displayName":"Bonded Labo…`
- Row 009 `conseq_treaty_breach`: `{"bountyLevel":"","collateralItemId":"","description":"A treaty-backed debt defaults. This is not just a broken promise — it is a diplomatic incident. Standing collapses, and the treaty itself is at risk.","displayName":"Treaty Violation",…`
- Row 010 `conseq_forgiveness_rare`: `{"bountyLevel":"","collateralItemId":"","description":"In rare circumstances — humanitarian crisis, prior service, faction doctrine — the creditor absorbs the loss. Standing improves slightly. This does not happen twice.","displayName":"Cr…`
