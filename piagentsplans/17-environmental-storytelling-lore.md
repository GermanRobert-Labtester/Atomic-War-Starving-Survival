# Plan 17 — Environmental Storytelling & Lore
...
This is a planning and architecture artifact for **Environmental text, atmosphere, archive transcription, codex and location-memory legibility**. It preserves the original intent: Make environmental writing reachable and state-aware while preserving the current text, atmosphere, archive, codex and location-memory owners.

The current residual premise is: The plan must distinguish authored text presence from loader selection, visit-state reactivity, weather binding, archive consumption, codex unlock and actual player presentation.
...
The bounded objective is to make the **Environmental Storytelling & Lore** opportunity implementable without creating a parallel gameplay authority. The plan must identify:
...
### Current source: `Assets/Ashfall.Core/EnvironmentalTextSystem.cs`
- Evidence status: **CURRENT FILE PRESENT**; 175 lines / 6504 bytes; SHA-256 `4e4de727b949e75f976aae7f18338b37d527a9960b04a114aad4a188acf9ea63`.
...
6: {
7:     /// <summary>
8:     /// Read-only runtime consumer for environmental text entries.
9:     /// Indexes entries by location, type, and tags for fast lookup.
   10:     /// No save/load needed — this is a static catalog (like AtmosphereTextSystem).
   11:     /// Engine-agnostic: zero engine references (Invariant 1).
   12:     /// </summary>
   13:     public sealed class EnvironmentalTextSystem
   14:     {
   15:         /// <summary>All entries keyed by location id (case-insensitive).</summary>
   16:         private readonly Dictionary<string, List<EnvironmentalTextEntry>> _byLocation =
   17:             new Dictionary<string, List<EnvironmentalTextEntry>>(StringComparer.OrdinalIgnoreCase);
   18:
   19:         /// <summary>All entries keyed by type (case-insensitive).</summary>
   20:         private readonly Dictionary<string, List<EnvironmentalTextEntry>> _byType =
   21:             new Dictionary<string, List<EnvironmentalTextEntry>>(StringComparer.OrdinalIgnoreCase);
   22:
   23:         /// <summary>Flat list of every loaded entry.</summary>
   24:         private readonly List<EnvironmentalTextEntry> _all = new List<EnvironmentalTextEntry>();
   25:
   26:         /// <summary>All entries keyed by their unique id (case-insensitive).</summary>
   27:         private readonly Dictionary<string, EnvironmentalTextEntry> _byId =
   28:             new Dictionary<string, EnvironmentalTextEntry>(StringComparer.OrdinalIgnoreCase);
   29:
   30:         /// <summary>Read-only access to the full catalog.</summary>
   31:         public IReadOnlyList<EnvironmentalTextEntry> AllEntries => _all;
   32:
   33:         /// <summary>Total number of loaded entries.</summary>
   34:         public int Count => _all.Count;
   35:
   36:         /// <summary>
   37:         /// Constructs the system from a pre-loaded list of entries.
   38:         /// Builds internal indexes for efficient querying.
   39:         /// </summary>
   40:         public EnvironmentalTextSystem(List<EnvironmentalTextEntry> entries)
   41:         {
   42:             if (entries != null)
   43:                 LoadCatalog(entries);
   44:         }
   45:
   46:         /// <summary>
   47:         /// Default constructor — call <see cref="LoadCatalog"/> afterwards.
   48:         /// </summary>
   49:         public EnvironmentalTextSystem() { }
   50:
   51:         /// <summary>
   52:         /// (Re)populates the system from a list of entries, rebuilding all indexes.
   53:         /// </summary>
   54:         public void LoadCatalog(List<EnvironmentalTextEntry> entries)
   55:         {
```
### Current source: `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs`
- Evidence status: **CURRENT FILE PRESENT**; 85 lines / 3213 bytes; SHA-256 `9c71c25e05cfd61d7d7db567ba1e5e1d56d2f28b81aec4fcbe0a2f65daeb2225`.
The following is a bounded source excerpt, not a generated API and not a claim that every declaration is production-reachable.
```text
1: // SPDX-License-Identifier: MIT
2: using System;
...
5:
6: namespace Ashfall.Core
7: {
8:     /// <summary>
9:     /// Single entry from environmental_texts_expansion_05.json.
   10:     /// Each entry is a location-anchored environmental text (sign, note, diary, broadcast, etc.)
   11:     /// with optional filtering dimensions (type, tags).
   12:     /// </summary>
   13:     [Serializable]
   14:     public sealed class EnvironmentalTextEntry
   15:     {
   16:         /// <summary>Unique snake_case id, e.g. "env_bunker_perimeter_sign".</summary>
   17:         public string id;
   18:
   19:         /// <summary>Location id this text is anchored to, e.g. "bunker_perimeter".</summary>
   20:         public string location;
   21:
   22:         /// <summary>The environmental text/prose itself.</summary>
   23:         public string text;
   24:
   25:         /// <summary>Entry type, e.g. "warning", "note", "diary", "broadcast".</summary>
   26:         public string type;
   27:
   28:         /// <summary>Free-form tags, e.g. ["danger", "bunker", "warning"].</summary>
   29:         public string[] tags;
   30:     }
   31:
   32:     /// <summary>Root shape of environmental_texts_expansion_05.json.</summary>
   33:     [Serializable]
   34:     public sealed class EnvironmentalTextCatalogFile
   35:     {
   36:         public int schema_version;
   37:         public List<EnvironmentalTextEntry> environmental_texts = new List<EnvironmentalTextEntry>();
   38:     }
   39:
   40:     /// <summary>
   41:     /// Loads environmental text entries from JSON.
   42:     /// Engine-agnostic: uses IFileIO and IJsonSerializer ports (Invariant 2).
   43:     /// </summary>
   44:     public static class EnvironmentalTextCatalogLoader
   45:     {
   46:         public const string DefaultFileName = "environmental_texts_expansion_05.json";
   47:
   48:         /// <summary>
   49:         /// Loads all environmental text entries from the data directory.
   50:         /// Returns an empty list when the file is missing, empty, or malformed.
   51:         /// </summary>
   52:         public static List<EnvironmentalTextEntry> Load(string dataDir, IFileIO fileIO, IJsonSerializer json)
   53:         {
   54:             if (fileIO == null || json == null || string.IsNullOrEmpty(dataDir))
   55:                 return new List<EnvironmentalTextEntry>();
```
### Current source: `Assets/Ashfall.Core/AtmosphereTextSystem.cs`
- Evidence status: **CURRENT FILE PRESENT**; 265 lines / 10299 bytes; SHA-256 `ee3426627ec9ea29f5777e1ddaaf9792937a994144cff5a2939ed7e69ba8a9f6`.
The following is a bounded source excerpt, not a generated API and not a claim that every declaration is production-reachable.
```text
1: // SPDX-License-Identifier: MIT
2: using System;
...
6: {
7:     /// <summary>
8:     /// Read-only runtime consumer for atmosphere text entries.
9:     /// Indexes entries by location for fast lookup and provides filtered queries
   10:     /// by weather, condition/state, time-phase, and sense.
   11:     /// No save/load needed — this is a static catalog (like OralLoreCatalog).
   12:     /// Engine-agnostic: zero engine references (Invariant 1).
   13:     /// </summary>
   14:     public sealed class AtmosphereTextSystem
   15:     {
   16:         /// <summary>All entries keyed by location id (case-insensitive).</summary>
   17:         private readonly Dictionary<string, List<AtmosphereTextEntry>> _byLocation =
   18:             new Dictionary<string, List<AtmosphereTextEntry>>(StringComparer.OrdinalIgnoreCase);
   19:
   20:         /// <summary>Flat list of every loaded entry.</summary>
   21:         private readonly List<AtmosphereTextEntry> _all = new List<AtmosphereTextEntry>();
   22:
   23:         /// <summary>All entries keyed by their unique id (case-insensitive).</summary>
   24:         private readonly Dictionary<string, AtmosphereTextEntry> _byId =
   25:             new Dictionary<string, AtmosphereTextEntry>(StringComparer.OrdinalIgnoreCase);
   26:
   27:         /// <summary>Read-only access to the full catalog.</summary>
   28:         public IReadOnlyList<AtmosphereTextEntry> AllEntries => _all;
   29:
   30:         /// <summary>Total number of loaded entries.</summary>
   31:         public int Count => _all.Count;
   32:
   33:         /// <summary>
   34:         /// Plan 49 / C1[16] — Delegate seam invoked whenever atmosphere text is delivered for a location.
   35:         /// </summary>
   36:         public Action<string, AtmosphereTextEntry>? AtmosphereTextDeliveredSeam { get; set; }
   37:
   38:         /// <summary>
   39:         /// Constructs the system from a pre-loaded list of entries.
   40:         /// Builds internal indexes for efficient querying.
   41:         /// </summary>
   42:         public AtmosphereTextSystem(List<AtmosphereTextEntry> entries)
   43:         {
   44:             if (entries != null)
   45:                 LoadCatalog(entries);
   46:         }
   47:
   48:         /// <summary>
   49:         /// Default constructor — call <see cref="LoadCatalog"/> afterwards.
   50:         /// </summary>
   51:         public AtmosphereTextSystem() { }
   52:
   53:         /// <summary>
   54:         /// (Re)populates the system from a list of entries, rebuilding all indexes.
   55:         /// </summary>
```
### Current source: `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs`
- Evidence status: **CURRENT FILE PRESENT**; 104 lines / 3935 bytes; SHA-256 `669c254aff14ce001a916d6194b1c2aba40336caf62443da05ab928a5b1bc79a`.
The following is a bounded source excerpt, not a generated API and not a claim that every declaration is production-reachable.
```text
1: // SPDX-License-Identifier: MIT
2: using System;
...
7: {
8:     /// <summary>
9:     /// Single entry from environmental_atmosphere_expansion.json.
   10:     /// Each entry is a location-flavoured atmosphere text with optional
   11:     /// filtering dimensions (weather, time-phase, condition, sense, tags).
   12:     /// </summary>
   13:     [Serializable]
   14:     public sealed class AtmosphereTextEntry
   15:     {
   16:         /// <summary>Unique snake_case id, e.g. "atm_loc_approach_thermal_plant".</summary>
   17:         public string id;
   18:
   19:         /// <summary>Location id this text is anchored to, e.g. "geothermal_plant_ruins".</summary>
   20:         public string location;
   21:
   22:         /// <summary>The atmospheric prose itself.</summary>
   23:         public string text;
   24:
   25:         /// <summary>Entry type, e.g. "location_description".</summary>
   26:         public string type;
   27:
   28:         /// <summary>Free-form tags, e.g. ["approach", "thermal", "steam", "hazard"].</summary>
   29:         public string[] tags;
   30:
   31:         /// <summary>Atmosphere mood keywords, e.g. ["danger", "industrial", "unstable"].</summary>
   32:         public string[] atmosphere;
   33:
   34:         /// <summary>Primary sense channel, e.g. "smell_sulfur_heat", "sight".</summary>
   35:         public string sense;
   36:
   37:         /// <summary>Time-of-day filter: "any", "day", "night", "dusk", "dawn".</summary>
   38:         public string time_phase;
   39:
   40:         /// <summary>Weather filter: "any", "clear", "storm", "fog", etc.</summary>
   41:         public string weather;
   42:
   43:         /// <summary>Author attribution, e.g. "narrator_observation".</summary>
   44:         public string author;
   45:
   46:         /// <summary>Location condition filter: "intact", "normal", "damaged", "destroyed".</summary>
   47:         public string condition;
   48:     }
   49:
   50:     /// <summary>Root shape of environmental_atmosphere_expansion.json.</summary>
   51:     [Serializable]
   52:     public sealed class AtmosphereCatalogFile
   53:     {
   54:         public int schema_version;
   55:         public string collection_id;
   56:         public List<AtmosphereTextEntry> environmental_texts = new List<AtmosphereTextEntry>();
   57:     }
   58:
   59:     /// <summary>
   60:     /// Loads atmosphere text entries from JSON.
   61:     /// Engine-agnostic: uses IFileIO and IJsonSerializer ports (Invariant 2).
   62:     /// </summary>
   63:     public static class AtmosphereCatalogLoader
   64:     {
   65:         public const string DefaultFileName = "environmental_atmosphere_expansion.json";
   66:
   67:         /// <summary>
```
### Current source: `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs`
- Evidence status: **CURRENT FILE PRESENT**; 376 lines / 14778 bytes; SHA-256 `bc5f40ddb09efb75299b76b2638aae147c0768ded345cd1bacca9dc3a0da6039`.
The following is a bounded source excerpt, not a generated API and not a claim that every declaration is production-reachable.
```text
1: // SPDX-License-Identifier: MIT
2: using System;
...
7:
8: namespace Ashfall.Core.Shelter
9: {
   10:     public enum AtmosphereMoodCategory
   11:     {
   12:         Bleak = 0,
   13:         Tense = 1,
   14:         Neutral = 2,
   ...
   18:     }
   19:
   20:     public enum AtmosphereProfileType
   21:     {
   22:         Industrial = 0,
   23:         Sterile = 1,
   24:         LivedIn = 2,
   ...
   29:     }
   30:
   31:     public sealed class AtmosphereModifiers
   32:     {
   33:         public float MoraleModifier { get; set; }
   34:         public float ProductivityModifier { get; set; }
   35:         public float StressReliefModifier { get; set; }
   36:     }
   37:
   38:     public sealed class AtmosphereProfileDefinition
   39:     {
   40:         [JsonPropertyName("profile_type")]
   41:         public string ProfileType { get; set; } = string.Empty;
   42:
   43:         [JsonPropertyName("name")]
   44:         public string Name { get; set; } = string.Empty;
   45:
   46:         [JsonPropertyName("description")]
   47:         public string Description { get; set; } = string.Empty;
   48:
   49:         [JsonPropertyName("morale_modifier")]
   50:         public float MoraleModifier { get; set; }
   51:
   52:         [JsonPropertyName("productivity_modifier")]
   53:         public float ProductivityModifier { get; set; }
   54:
   55:         [JsonPropertyName("stress_relief_modifier")]
  134:         public ShelterAtmosphereSystem(AtmosphereState? state = null)
  135:         {
  136:             _state = state ?? new AtmosphereState();
  137:         }
  138:
  139:         public void UpdateEnvironmentalInputs(
  140:             float lighting,
  141:             float acousticComfort,
  142:             float airPurity,
  143:             float thermalComfort,
  144:             float cleanliness,
  145:             float socialWarmth,
  146:             float decorationLevel,
  147:             int currentDay)
  148:         {
  149:             _state.LightingQuality = Math.Clamp(lighting, 0f, 100f);
  150:             _state.AcousticComfort = Math.Clamp(acousticComfort, 0f, 100f);
```
### Current source: `src/Main.ShelterAtmosphere.cs`
- Evidence status: **CURRENT FILE PRESENT**; 208 lines / 7934 bytes; SHA-256 `5199ad7341ab49a2106c3cba24253f4688f9ec53056f1901f8ab2ab731ac8a72`.
The following is a bounded source excerpt, not a generated API and not a claim that every declaration is production-reachable.
```text
1: // SPDX-License-Identifier: MIT
2: using System;
3: using Godot;
4: using Ashfall.Core.Shelter;
5: using Ashfall.Core.Survivors;
6: using AtomicWar.GodotApp.UI;
7:
8: namespace AtomicWar.GodotApp
9: {
   10:     public partial class Main : Control
   11:     {
   12:         private ShelterAtmosphereHostSession _shelterAtmosphere = null!;
   13:         private ShelterAtmospherePanel _shelterAtmospherePanel = null!;
   14:         private bool _shelterAtmosphereDirty;
   15:
   16:         public ShelterAtmosphereHostSession ShelterAtmosphere => EnsureShelterAtmosphere();
   17:
   18:         public ShelterAtmosphereHostSession EnsureShelterAtmosphere()
   19:         {
   20:             if (_shelterAtmosphere != null) return _shelterAtmosphere;
   21:
   22:             SetupPowerGrid();
   23:             SetupStartingLevel();
   24:
   25:             var atmoState = ShelterAtmosphereSaveStore.TryLoad() ?? new AtmosphereState();
   26:             var noiseState = ShelterNoiseSaveStore.TryLoad() ?? new ShelterNoiseState();
   27:
   28:             var atmoSys = new ShelterAtmosphereSystem(atmoState);
   29:             var noiseSys = new ShelterNoiseSystem(noiseState);
   30:
   31:             // Plan 220 — bind the authored atmosphere profiles so ambiance modifiers
   32:             // come from the canonical catalog rather than hardcoded defaults.
   33:             string atmoCatalogPath = CatalogPath.ResolveCatalog("atmosphere_profiles.json");
   34:             var atmoCatalogIo = CatalogPath.CreateFileIOForDataDir(CatalogPath.ResolveDataDir());
   35:             if (atmoCatalogIo.FileExists(atmoCatalogPath))
   36:             {
   37:                 atmoSys.LoadCatalog(atmoCatalogIo.ReadAllText(atmoCatalogPath));
   38:             }
   39:
   40:             // Plan 205 — bind the authored noise source definitions so acoustic
   41:             // sourcing is catalog-driven (the baseline seeding below stays a fallback).
   42:             string noiseCatalogPath = CatalogPath.ResolveCatalog("noise_sources.json");
   43:             if (atmoCatalogIo.FileExists(noiseCatalogPath))
   44:             {
   45:                 noiseSys.LoadCatalog(atmoCatalogIo.ReadAllText(noiseCatalogPath));
   46:             }
   47:
   48:             // If new game / pristine state, register standard baseline shelter acoustic profile
   49:             if (noiseState.Sources.Count == 0)
   50:             {
   51:                 noiseSys.AddNoiseSource(NoiseSourceType.Generator, "power_room", 60f, NoiseFrequency.Low);
   52:                 noiseSys.AddNoiseSource(NoiseSourceType.Ventilation, "hvac_room", 40f, NoiseFrequency.Medium);
   53:                 noiseSys.AddNoiseSource(NoiseSourceType.Machinery, "workshop", 50f, NoiseFrequency.Medium);
   54:                 noiseSys.SoundproofRoom("sleeping_quarters", 35f, 25f);
   55:                 noiseSys.SoundproofRoom("rec_room", 30f, 20f);
  136:             {
  137:                 int count = _shelterDecor.System.State?.Placements?.Count ?? 0;
  138:                 decorationLevel = Math.Clamp(count * 10f + 25f, 10f, 95f);
  139:             }
  140:
  141:             _shelterAtmosphere.UpdateEnvironmentalAtmosphere(
  142:                 day,
  143:                 lighting,
  144:                 airPurity,
  145:                 thermalComfort,
  ...
  148:                 decorationLevel);
  149:
  150:             // Apply active atmospheric morale delta across living survivors
  151:             var mods = _shelterAtmosphere.Modifiers;
  152:             if (Math.Abs(mods.MoraleModifier) > 0.01f && _survivors?.RosterState != null && _survivors.Needs != null)
```
### Current source: `src/UI/ShelterAtmospherePanel.cs`
- Evidence status: **CURRENT FILE PRESENT**; 220 lines / 10158 bytes; SHA-256 `91ad42e1016d7b7e7578e3beb7fbbb2c53494cc68657f2631c65c198cb380229`.
The following is a bounded source excerpt, not a generated API and not a claim that every declaration is production-reachable.
```text
1: // SPDX-License-Identifier: MIT
2: using System;
...
4: using System.Text;
5: using Godot;
6: using Ashfall.Core.Shelter;
7: using AtomicWar.GodotApp;
8: using DesignTheme = Ashfall.Core.UI.Theme;
9:
   10: namespace AtomicWar.GodotApp.UI
   11: {
   12:     /// <summary>
   13:     /// Plan 220 & 205 — Shelter Atmosphere, Ambiance & Acoustic Discipline Panel.
   14:     /// Presents the holistic shelter environmental profile: composite mood score,
   15:     /// active ambiance profile, 7 environmental facet levels, holistic modifiers,
   16:     /// noise sources, and quiet hours controls.
   17:     /// </summary>
   18:     public partial class ShelterAtmospherePanel : Control, IBindablePanel
   19:     {
   20:         public event Action? OnClose;
   21:
   22:         private AshfallDashboardShell _shell = null!;
   23:         private AshfallStatusRail? _statusRail;
   24:         private VBoxContainer _contentStack = null!;
   25:         private Label _breakdownText = null!;
   26:         private Label _modifiersText = null!;
   27:         private Label _noiseSourcesText = null!;
   28:         private Button _toggleQuietHoursBtn = null!;
   29:         private Button _soundproofWorkshopBtn = null!;
   30:
   31:         private ShelterAtmosphereHostSession? _host;
   32:
   33:         public bool IsBound => _host != null;
   34:
   35:         public void Bind(ShelterAtmosphereHostSession session)
   36:         {
   37:             _host = session;
   38:             if (_host != null) _host.StateChanged += RefreshView;
   39:             RefreshView();
   40:         }
   41:
   ...
   44:             if (_host != null)
   45:             {
   46:                 _host.StateChanged -= RefreshView;
   47:                 _host = null;
   48:             }
   49:         }
   50:
   51:         public void RefreshView()
   52:         {
   53:             if (_host == null || _statusRail == null) return;
   54:
   55:             var atmo = _host.Atmosphere;
   84:             _statusRail.Set("mood", $"{atmo.OverallMoodScore:F1} · {atmo.CurrentMoodCategory}", moodCriticality);
   85:             _statusRail.Set("profile", $"{atmo.ActiveProfile}", AshfallMetricCard.Criticality.Normal);
   86:             _statusRail.Set("noise", $"{noise.OverallNoiseLevel:F1} dB", noiseCriticality);
   87:             _statusRail.Set("risk", $"{noise.DetectionRisk:F1}%", riskCriticality);
   88:
   89:             // 1. Environmental Breakdown
   90:             var sb = new StringBuilder();
   91:             sb.AppendLine($"Overall Mood Score: {atmo.OverallMoodScore:F1}/100 ({atmo.CurrentMoodCategory.ToString().ToUpperInvariant()})");
   92:             sb.AppendLine($"  Lighting Quality:   {atmo.LightingQuality:F1}/100");
   93:             sb.AppendLine($"  Acoustic Comfort:   {atmo.AcousticComfort:F1}/100 (Shelter Noise: {noise.OverallNoiseLevel:F1} dB)");
   94:             sb.AppendLine($"  Air Purity:         {atmo.AirPurity:F1}/100");
   95:             sb.AppendLine($"  Thermal Comfort:    {atmo.ThermalComfort:F1}/100");
   96:             sb.AppendLine($"  Cleanliness:        {atmo.Cleanliness:F1}/100");
   97:             sb.AppendLine($"  Social Warmth:      {atmo.SocialWarmth:F1}/100");
   98:             sb.AppendLine($"  Decoration Level:   {atmo.DecorationLevel:F1}/100");
   99:             if (!string.IsNullOrEmpty(_host.LastEvent))
  100:             {
```

# Appendix B — Current Data and Catalog Audit

- `Assets/StreamingAssets/Data/environmental_atmosphere_expansion.json` — 160920 bytes; SHA-256 `0dbbbaea552c9f71bffcadd8a482707cfb9eeaab5a9761632a1de2600d0d7845`.
  - `root` object keys (3): `schema_version`, `collection_id`, `environmental_texts`
  - `root.environmental_texts` list rows: **189**
- sample row key: `atm_loc_approach_thermal_plant`; fields: `id`, `location`, `text`, `type`, `tags`, `atmosphere`, `sense`, `time_phase`, `weather`, `author`, `condition`
- sample row key: `atm_loc_arrival_flooded_subway`; fields: `id`, `location`, `text`, `type`, `tags`, `atmosphere`, `sense`, `time_phase`, `weather`, `author`, `condition`
- sample row key: `atm_loc_first_sight_seed_vault`; fields: `id`, `location`, `text`, `type`, `tags`, `atmosphere`, `sense`, `time_phase`, `weather`, `author`, `condition`
- `Assets/StreamingAssets/Data/environmental_texts_expansion_05.json` — 14494 bytes; SHA-256 `6f4f63b8d7d5b06909bf85ea63574608efbb8cbcab64d0c32d9df5f9449c2d24`.
  - `root` object keys (2): `schema_version`, `environmental_texts`
  - `root.environmental_texts` list rows: **42**
- sample row key: `env_bunker_perimeter_sign`; fields: `id`, `location`, `text`, `type`, `tags`
- sample row key: `env_scavenger_camp_note`; fields: `id`, `location`, `text`, `type`, `tags`
- sample row key: `env_radiation_warning_sign`; fields: `id`, `location`, `text`, `type`, `tags`
- `Assets/StreamingAssets/Data/archive_inks.json` — 3191 bytes; SHA-256 `83fc3d37907fc3e632829d56da3dcc4a83605f91001bee0f102a854ac25e9a5e`.
  - `root` object keys (3): `schema_version`, `collection_id`, `inks`
  - `root.inks` list rows: **12**
- sample row key: `None`; fields: `ink_id`, `display_name`, `legibility_score`, `archival_longevity_days`, `fade_rate_per_day`, `required_item_id`, `required_amount`
- sample row key: `None`; fields: `ink_id`, `display_name`, `legibility_score`, `archival_longevity_days`, `fade_rate_per_day`, `required_item_id`, `required_amount`
- sample row key: `None`; fields: `ink_id`, `display_name`, `legibility_score`, `archival_longevity_days`, `fade_rate_per_day`, `required_item_id`, `required_amount`
- `Assets/StreamingAssets/Data/world_history.json` — 45166 bytes; SHA-256 `f830a503a02eff8650228e33af1ba13b80c843ae6be3f9540847b932f055a035`.
  - `root` object keys (2): `schema_version`, `items`
  - `root.items` list rows: **79**
- sample row key: `None`; fields: `era`, `year_month`, `title`, `body`, `discovery_location_id`, `discovery_trigger`, `knowledge_key`
- sample row key: `None`; fields: `era`, `year_month`, `title`, `body`, `discovery_location_id`, `discovery_trigger`, `knowledge_key`
- sample row key: `None`; fields: `era`, `year_month`, `title`, `body`, `discovery_location_id`, `discovery_trigger`, `knowledge_key`

# Appendix C — Current Test Inventory

- `Ashfall.Core.Tests/Plan17BAtmosphereTests.cs` — 297 lines; SHA-256 `185adad041dd793fb3ac1ce185695345d548981905ebe4f0e430f1c416ab0f02`; test attributes 10; declaration lines 19.
...
- `Ashfall.Core.Tests/Content/Plan49ContentAtmosphereIntegrationTests.cs` — 165 lines; SHA-256 `3df844e7659515a81cfef0385a49a299b54e3118c9f4c7452bdcc1ece655c7aa`; test attributes 4; declaration lines 5.
...
- `Ashfall.Core.Tests/Shelter/ShelterAtmosphereSystemTests.cs` — 100 lines; SHA-256 `e71228c0fdd96fad41aecc63890600db7a827745041ba982fc8841ac49a18bfe`; test attributes 5; declaration lines 6.
...
### Mechanical references to `EnvironmentalTextSystem`
Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs:70: /// Loads the catalog and feeds entries into an <see cref="EnvironmentalTextSystem"/>.
Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs:74: EnvironmentalTextSystem system,
Assets/Ashfall.Core/EnvironmentalTextSystem.cs:13: public sealed class EnvironmentalTextSystem
Assets/Ashfall.Core/EnvironmentalTextSystem.cs:40: public EnvironmentalTextSystem(List<EnvironmentalTextEntry> entries)
Assets/Ashfall.Core/EnvironmentalTextSystem.cs:49: public EnvironmentalTextSystem() { }
src/Host/WorldHostSession.cs:38: public EnvironmentalTextSystem EnvironmentalTexts { get; } = new EnvironmentalTextSystem();
...
### Mechanical references to `EnvironmentalTextCatalogLoader`
Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs:44: public static class EnvironmentalTextCatalogLoader
src/Host/WorldHostSession.cs:185: EnvironmentalTextCatalogLoader.LoadAndRegister(session.EnvironmentalTexts, dataDir, files, json);
Ashfall.Core.Tests/Tooling/LoaderWiringGateTests.cs:108: Assert.True(Regex.IsMatch(srcText, @"\bEnvironmentalTextCatalogLoader\s*\.\s*LoadAndRegister\b"),
Ashfall.Core.Tests/Tooling/LoaderWiringGateTests.cs:109: "EnvironmentalTextCatalogLoader.LoadAndRegister must be called from production src/");
Interpretation: these are search hits only; an integrating builder must trace the actual call path, setup binding, event subscription and save registration before claiming reachability.
### Mechanical references to `AtmosphereTextSystem`
Assets/Ashfall.Core/AtmosphereCatalogLoader.cs:89: /// Loads the catalog and feeds entries into an <see cref="AtmosphereTextSystem"/>.
Assets/Ashfall.Core/AtmosphereCatalogLoader.cs:93: AtmosphereTextSystem system,
Assets/Ashfall.Core/AtmosphereTextSystem.cs:14: public sealed class AtmosphereTextSystem
Assets/Ashfall.Core/AtmosphereTextSystem.cs:42: public AtmosphereTextSystem(List<AtmosphereTextEntry> entries)
Assets/Ashfall.Core/AtmosphereTextSystem.cs:51: public AtmosphereTextSystem() { }
Assets/Ashfall.Core/EnvironmentalTextSystem.cs:10: /// No save/load needed — this is a static catalog (like AtmosphereTextSystem).
src/Main.ContentCertification.cs:60: session.MarkConsumerActive("AtmosphereTextSystem", _world?.AtmosphereTexts != null);
src/Host/WorldHostSession.cs:35: public AtmosphereTextSystem AtmosphereTexts { get; } = new AtmosphereTextSystem();
src/Host/ContentCertificationHostSession.cs:53: new ContentCertificationFamily("place_atmosphere_texts", "PLACE / ATMOSPHERE", "environmental_atmosphere_expansion.json", "AtmosphereTextSystem"),
Ashfall.Core.Tests/Plan17BAtmosphereTests.cs:4: // validates query patterns that a future AtmosphereTextSystem will consume.
Ashfall.Core.Tests/Plan17BAtmosphereTests.cs:5: // Since AtmosphereTextSystem is not yet implemented, these tests operate
Ashfall.Core.Tests/Plan17BAtmosphereTests.cs:267: /// Mirrors the contract a future AtmosphereTextSystem will expose.
Ashfall.Core.Tests/Content/Plan49ContentAtmosphereIntegrationTests.cs:17: new ContentCandidateRow("atm_loc_approach_thermal_plant", "environmental_atmosphere_expansion.json", "AtmosphereTextSystem", true),
Ashfall.Core.Tests/Content/Plan49ContentAtmosphereIntegrationTests.cs:24: "AtmosphereTextSystem",
Interpretation: these are search hits only; an integrating builder must trace the actual call path, setup binding, event subscription and save registration before claiming reachability.
### Mechanical references to `AtmosphereCatalogLoader`
Assets/Ashfall.Core/AtmosphereCatalogLoader.cs:63: public static class AtmosphereCatalogLoader
src/Host/WorldHostSession.cs:184: AtmosphereCatalogLoader.LoadAndRegister(session.AtmosphereTexts, dataDir, files, json);
Ashfall.Core.Tests/Tooling/LoaderWiringGateTests.cs:106: Assert.True(Regex.IsMatch(srcText, @"\bAtmosphereCatalogLoader\s*\.\s*LoadAndRegister\b"),
Ashfall.Core.Tests/Tooling/LoaderWiringGateTests.cs:107: "AtmosphereCatalogLoader.LoadAndRegister must be called from production src/");
Interpretation: these are search hits only; an integrating builder must trace the actual call path, setup binding, event subscription and save registration before claiming reachability.
### Mechanical references to `ShelterAtmosphereSystem`
Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs:101: public sealed class ShelterAtmosphereSystem
Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs:134: public ShelterAtmosphereSystem(AtmosphereState? state = null)
src/Main.ShelterAtmosphere.cs:28: var atmoSys = new ShelterAtmosphereSystem(atmoState);
src/Host/ShelterAtmosphereHostSession.cs:10: /// Composes ShelterAtmosphereSystem (interpretive ambiance read-model) and
src/Host/ShelterAtmosphereHostSession.cs:16: public ShelterAtmosphereSystem Atmosphere { get; }
src/Host/ShelterAtmosphereHostSession.cs:23: public ShelterAtmosphereHostSession(ShelterAtmosphereSystem? atmosphere = null, ShelterNoiseSystem? noise = null)
src/Host/ShelterAtmosphereHostSession.cs:25: Atmosphere = atmosphere ?? new ShelterAtmosphereSystem();
src/Host/ShelterAtmosphereSelfTest.cs:37: var atmoSys = new ShelterAtmosphereSystem();
src/Host/ShelterAtmosphereSelfTest.cs:84: var atmoRestored = new ShelterAtmosphereSystem();
Ashfall.Core.Tests/Shelter/ShelterAtmosphereSystemTests.cs:10: public sealed class ShelterAtmosphereSystemTests
Ashfall.Core.Tests/Shelter/ShelterAtmosphereSystemTests.cs:15: var system = new ShelterAtmosphereSystem();
Ashfall.Core.Tests/Shelter/ShelterAtmosphereSystemTests.cs:38: var system = new ShelterAtmosphereSystem();
Ashfall.Core.Tests/Shelter/ShelterAtmosphereSystemTests.cs:56: var system = new ShelterAtmosphereSystem();
Ashfall.Core.Tests/Shelter/ShelterAtmosphereSystemTests.cs:74: var system = new ShelterAtmosphereSystem();
Interpretation: these are search hits only; an integrating builder must trace the actual call path, setup binding, event subscription and save registration before claiming reachability.
### Mechanical references to `Main.ShelterAtmosphere`
src/Host/ShelterAtmosphereSaveStore.cs:5: // Host Caller: Main.ShelterAtmosphere / ShelterAtmosphereHostSession
src/Host/ShelterNoiseSaveStore.cs:5: // Host Caller: Main.ShelterAtmosphere / ShelterAtmosphereHostSession
Interpretation: these are search hits only; an integrating builder must trace the actual call path, setup binding, event subscription and save registration before claiming reachability.
### Mechanical references to `ShelterAtmospherePanel`
src/Main.ShelterAtmosphere.cs:13: private ShelterAtmospherePanel _shelterAtmospherePanel = null!;
src/Main.ShelterAtmosphere.cs:188: private void SetupShelterAtmospherePanel()
src/Main.ShelterAtmosphere.cs:190: if (_shelterAtmospherePanel != null && _shelterAtmospherePanel.IsInsideTree())
src/Main.ShelterAtmosphere.cs:194: _shelterAtmospherePanel = new ShelterAtmospherePanel();
src/Main.ShelterAtmosphere.cs:195: _shelterAtmospherePanel.Bind(_shelterAtmosphere);
src/Main.ShelterAtmosphere.cs:196: _shelterAtmospherePanel.OnClose += () => _shelterAtmospherePanel.Visible = false;
src/Main.ShelterAtmosphere.cs:197: _shelterAtmospherePanel.Visible = false;
src/Main.ShelterAtmosphere.cs:198: AddChild(_shelterAtmospherePanel);
src/Main.ShelterAtmosphere.cs:201: public void ShowShelterAtmospherePanel()
src/Main.ShelterAtmosphere.cs:203: SetupShelterAtmospherePanel();
src/Main.ShelterAtmosphere.cs:204: _shelterAtmospherePanel.Visible = true;
src/Main.ShelterAtmosphere.cs:205: _shelterAtmospherePanel.RefreshView();
src/Main.ExpandedShelterSystems.cs:663: ShowShelterAtmospherePanel();
src/Host/ShelterAtmosphereSelfTest.cs:96: var panel = new ShelterAtmospherePanel();
Interpretation: these are search hits only; an integrating builder must trace the actual call path, setup binding, event subscription and save registration before claiming reachability.
...
140: | C17 | Ambient environmental text and atmosphere cues for panels rendering newer systems with sparse surface prose | INFERENCE — verify via `--ui-layout-selftest` and snapshot coverage |
141:
142: ### 3.2 Lane B — Mechanics and systems functionality
143:
authority lines 931-936:
931: Each map lists the cluster's canon owners, live catalogs, host sessions, and current factory openings. These are planning instruments: a session picks a cluster, reads its map, and consumes its seeds. All catalog and system names below are carried from the verified v1.0 inventory and the live 2026-09-24 listings; per-field internals remain session-verify territory.
932:
...
936:
authority lines 1125-1130:
1125: - 2026-09-24 — Volume 4: prose specification library part 1 (eight worked genre contracts plus usage rules) — ~5,500 — cumulative ~125,500
1126:
1127: - 2026-09-25 — Volume 5: prose specification library part 2 (twenty-one additional worked genre contracts: journal, unsent letter, communiqué, directive, liturgy, cipher, dispatch/debrief, graffiti, folklore, almanac, gazetteer, treaty, permit, glitch report, provenance, eulogy, rumor, environmental clue, item inspection, relationship reaction, quest/outcome texts, codex, map texts, world-state notification; genre list now fully covered) — ~17,700 — cumulative ~143,000
1128: - 2026-09-25 — Volume 6: twenty full subject plans expanded from Lane A seeds (FP-A01…FP-A27 selection, with wave sequencing F6-1 through F6-4) — ~16,600 — cumulative ~160,000
1129: - 2026-09-25 — Volume 7: catalog authoring contract library (field-inventory protocol, six CANON record contracts, item authoring rules, integrity checklist, schema-sheet registry with priority inventory order) — ~8,700 — cumulative ~169,000
1130: - 2026-09-25 — Volume 8: balance harness specification library (twelve harness specs H-C1…H-C12 plus publication and consumption rules) — ~10,300 — cumulative ~179,000
authority lines 1141-1146:
1141: - 2026-09-27 — Volume 17: cluster-by-cluster expansion roadmaps C1–C17, each anchored to its deep map with Phase I–IV structure and five cross-cluster sequencing rules — ~16,900 — cumulative ~330,000
1142: - 2026-09-27 — Volume 18: re-audit against the repository's public surface — four drift-register candidates (DR-16 communiqué board and tick-gate, DR-17 map-atlas repair, DR-18 hardening/quarantine cleanup, DR-19 post-v1.0 subsystem families absent from the deep maps), five premise corrections, five evidence-gated seed replenishments (A-31, A-32, B-26, G-09, E-11), and the factory self-audit — ~9,900 — cumulative ~342,000
1143: - 2026-09-27 — Volume 19: public-surface confirmation pass — DR-16, DR-17, DR-18 upgraded to MERGED-AS-LISTED with merge dates; four new entries (DR-20 wave-5/6 subsystem inventory, DR-21 shelter atmosphere/noise panels with follow-up repair, DR-22 commitment-event semantic parity incident corroborating DP-04, DR-23 port-contract seam ratchet cleanup); six premise corrections — ~9,700 — cumulative ~352,000
1144: - 2026-09-27 — Volume 20: authoring session runbook library — eight runbooks (RB-CENSUS, RB-COLLISION, RB-PAIR, RB-SEALGUARD, RB-KNOWLEDGE, RB-QUANT, RB-WAVE, RB-DRCONFIRM) turning every open-premise class into an executable session procedure — ~7,700 — cumulative ~359,000
1145: - 2026-09-27 — Volume 21: remaining Lane B and all Lane C seed expansions — FP-B04, FP-B06, FP-B14, FP-B15 plus the full economy set FP-C01 through FP-C14 with the lane rule that no Lane C plan changes a number in its first tranche; Lane B and Lane C compressed backlogs now zero (GATE items excepted, correctly) — ~23,100 — cumulative ~382,000
1146: - 2026-09-27 — Volume 22: prose and document contract supplement, merged-surface edition — atmosphere-reading and noise-complaint genre contracts (DR-21), release-craft documentation contract (DR-16), agent-skills index contract, commitment-event authoring rule (DR-22/DR-23); DR-19 domains deliberately left uncontracted pending confirmation — ~6,800 — cumulative ~389,000
authority lines 1444-1452:
1444: ```
1445:
1446: ## 5.18 Environmental clue (embedded-history register)
1447:
1448: ```text
1449: prose_field: environmental_clue
1450: purpose: wear and residue as history
1451: trigger: location inspection
1452: length: 25-60 words
authority lines 3976-3981:
3976: | ShelterArchiveSystem | PRESENT | `Assets/Ashfall.Core/Shelter/ShelterArchiveSystem.cs` |
3977: | ShelterSecuritySystem | PRESENT | `Assets/Ashfall.Core/Shelter/ShelterSecuritySystem.cs` |
3978: | ShelterAtmosphereSystem / ShelterNoiseSystem (DR-21) | PRESENT | `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs`, `ShelterNoiseSystem.cs`; plus `AtmosphereTextSystem.cs`, `AtmosphereCatalogLoader.cs`, `EnvironmentalTextSystem.cs`, `EnvironmentalTextCatalogLoader.cs` at Core top level |
...
```text
bash scripts/run_test.sh Ashfall.Core.Tests/Plan17BAtmosphereTests.cs
bash scripts/run_test.sh Ashfall.Core.Tests/Content/Plan49ContentAtmosphereIntegrationTests.cs
bash scripts/run_test.sh Ashfall.Core.Tests/Shelter/ShelterAtmosphereSystemTests.cs
...
## Audit cycle 01, lens 01: Environmental Storytelling & Lore boundary

**Question 01.01.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.

**Current evidence to recheck.** `Assets/StreamingAssets/Data/environmental_texts_expansion_05.json` must be parsed and its IDs/references counted. The result must distinguish content presence from a live consumer. If the catalog is absent, malformed, orphaned or not loaded, record a blocker rather than creating a replacement loader.
...
## Audit cycle 01, lens 03: Environmental Storytelling & Lore boundary

**Question 01.03.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.

**Current evidence to recheck.** `Assets/StreamingAssets/Data/world_history.json` must be parsed and its IDs/references counted. The result must distinguish content presence from a live consumer. If the catalog is absent, malformed, orphaned or not loaded, record a blocker rather than creating a replacement loader.
...
## Audit cycle 01, lens 04: Environmental Storytelling & Lore boundary

**Question 01.04.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.

**Current evidence to recheck.** `Assets/StreamingAssets/Data/environmental_atmosphere_expansion.json` must be parsed and its IDs/references counted. The result must distinguish content presence from a live consumer. If the catalog is absent, malformed, orphaned or not loaded, record a blocker rather than creating a replacement loader.
...
## Audit cycle 01, lens 05: Environmental Storytelling & Lore boundary

**Question 01.05.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 06: Environmental Storytelling & Lore boundary

**Question 01.06.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 07: Environmental Storytelling & Lore boundary

**Question 01.07.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 08: Environmental Storytelling & Lore boundary

**Question 01.08.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 09: Environmental Storytelling & Lore boundary

**Question 01.09.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 10: Environmental Storytelling & Lore boundary

**Question 01.10.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 11: Environmental Storytelling & Lore boundary

**Question 01.11.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 12: Environmental Storytelling & Lore boundary

**Question 01.12.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 01: Environmental Storytelling & Lore boundary

**Question 02.01.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 02: Environmental Storytelling & Lore boundary

**Question 02.02.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 03: Environmental Storytelling & Lore boundary

**Question 02.03.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 04: Environmental Storytelling & Lore boundary

**Question 02.04.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 05: Environmental Storytelling & Lore boundary

**Question 02.05.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 06: Environmental Storytelling & Lore boundary

**Question 02.06.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 07: Environmental Storytelling & Lore boundary

**Question 02.07.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 08: Environmental Storytelling & Lore boundary

**Question 02.08.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 09: Environmental Storytelling & Lore boundary

**Question 02.09.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 10: Environmental Storytelling & Lore boundary

**Question 02.10.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 11: Environmental Storytelling & Lore boundary

**Question 02.11.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 12: Environmental Storytelling & Lore boundary

**Question 02.12.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 01: Environmental Storytelling & Lore boundary

**Question 03.01.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 02: Environmental Storytelling & Lore boundary

**Question 03.02.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 03: Environmental Storytelling & Lore boundary

**Question 03.03.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 04: Environmental Storytelling & Lore boundary

**Question 03.04.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 05: Environmental Storytelling & Lore boundary

**Question 03.05.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 06: Environmental Storytelling & Lore boundary

**Question 03.06.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 07: Environmental Storytelling & Lore boundary

**Question 03.07.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 08: Environmental Storytelling & Lore boundary

**Question 03.08.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 09: Environmental Storytelling & Lore boundary

**Question 03.09.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 10: Environmental Storytelling & Lore boundary

**Question 03.10.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 11: Environmental Storytelling & Lore boundary

**Question 03.11.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 12: Environmental Storytelling & Lore boundary

**Question 03.12.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 01: Environmental Storytelling & Lore boundary

**Question 04.01.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 02: Environmental Storytelling & Lore boundary

**Question 04.02.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 03: Environmental Storytelling & Lore boundary

**Question 04.03.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 04: Environmental Storytelling & Lore boundary

**Question 04.04.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 05: Environmental Storytelling & Lore boundary

**Question 04.05.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 06: Environmental Storytelling & Lore boundary

**Question 04.06.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 07: Environmental Storytelling & Lore boundary

**Question 04.07.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 08: Environmental Storytelling & Lore boundary

**Question 04.08.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 09: Environmental Storytelling & Lore boundary

**Question 04.09.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 10: Environmental Storytelling & Lore boundary

**Question 04.10.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 11: Environmental Storytelling & Lore boundary

**Question 04.11.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 12: Environmental Storytelling & Lore boundary

**Question 04.12.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 01: Environmental Storytelling & Lore boundary

**Question 05.01.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 02: Environmental Storytelling & Lore boundary

**Question 05.02.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 03: Environmental Storytelling & Lore boundary

**Question 05.03.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 04: Environmental Storytelling & Lore boundary

**Question 05.04.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 05: Environmental Storytelling & Lore boundary

**Question 05.05.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 06: Environmental Storytelling & Lore boundary

**Question 05.06.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 07: Environmental Storytelling & Lore boundary

**Question 05.07.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 08: Environmental Storytelling & Lore boundary

**Question 05.08.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 09: Environmental Storytelling & Lore boundary

**Question 05.09.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 10: Environmental Storytelling & Lore boundary

**Question 05.10.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 11: Environmental Storytelling & Lore boundary

**Question 05.11.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 12: Environmental Storytelling & Lore boundary

**Question 05.12.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 01: Environmental Storytelling & Lore boundary

**Question 06.01.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 02: Environmental Storytelling & Lore boundary

**Question 06.02.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 03: Environmental Storytelling & Lore boundary

**Question 06.03.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 04: Environmental Storytelling & Lore boundary

**Question 06.04.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 05: Environmental Storytelling & Lore boundary

**Question 06.05.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 06: Environmental Storytelling & Lore boundary

**Question 06.06.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 07: Environmental Storytelling & Lore boundary

**Question 06.07.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 08: Environmental Storytelling & Lore boundary

**Question 06.08.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 09: Environmental Storytelling & Lore boundary

**Question 06.09.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 10: Environmental Storytelling & Lore boundary

**Question 06.10.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 11: Environmental Storytelling & Lore boundary

**Question 06.11.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 12: Environmental Storytelling & Lore boundary

**Question 06.12.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 01: Environmental Storytelling & Lore boundary

**Question 07.01.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 02: Environmental Storytelling & Lore boundary

**Question 07.02.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 03: Environmental Storytelling & Lore boundary

**Question 07.03.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 04: Environmental Storytelling & Lore boundary

**Question 07.04.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 05: Environmental Storytelling & Lore boundary

**Question 07.05.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 06: Environmental Storytelling & Lore boundary

**Question 07.06.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 07: Environmental Storytelling & Lore boundary

**Question 07.07.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 08: Environmental Storytelling & Lore boundary

**Question 07.08.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 09: Environmental Storytelling & Lore boundary

**Question 07.09.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 10: Environmental Storytelling & Lore boundary

**Question 07.10.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 11: Environmental Storytelling & Lore boundary

**Question 07.11.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 12: Environmental Storytelling & Lore boundary

**Question 07.12.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 01: Environmental Storytelling & Lore boundary

**Question 08.01.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 02: Environmental Storytelling & Lore boundary
...
## Audit cycle 08, lens 03: Environmental Storytelling & Lore boundary

**Question 08.03.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 04: Environmental Storytelling & Lore boundary

**Question 08.04.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 05: Environmental Storytelling & Lore boundary

**Question 08.05.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 06: Environmental Storytelling & Lore boundary

**Question 08.06.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 07: Environmental Storytelling & Lore boundary

**Question 08.07.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 08: Environmental Storytelling & Lore boundary

**Question 08.08.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 09: Environmental Storytelling & Lore boundary

**Question 08.09.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 10: Environmental Storytelling & Lore boundary

**Question 08.10.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 11: Environmental Storytelling & Lore boundary

**Question 08.11.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 12: Environmental Storytelling & Lore boundary

**Question 08.12.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 01: Environmental Storytelling & Lore boundary

**Question 09.01.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 02: Environmental Storytelling & Lore boundary

**Question 09.02.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 03: Environmental Storytelling & Lore boundary

**Question 09.03.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 04: Environmental Storytelling & Lore boundary

**Question 09.04.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 05: Environmental Storytelling & Lore boundary

**Question 09.05.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 06: Environmental Storytelling & Lore boundary

**Question 09.06.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 07: Environmental Storytelling & Lore boundary

**Question 09.07.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 08: Environmental Storytelling & Lore boundary

**Question 09.08.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 09: Environmental Storytelling & Lore boundary

**Question 09.09.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 10: Environmental Storytelling & Lore boundary

**Question 09.10.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 11: Environmental Storytelling & Lore boundary

**Question 09.11.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 12: Environmental Storytelling & Lore boundary

**Question 09.12.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 01: Environmental Storytelling & Lore boundary

**Question 10.01.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 02: Environmental Storytelling & Lore boundary

**Question 10.02.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 03: Environmental Storytelling & Lore boundary

**Question 10.03.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 04: Environmental Storytelling & Lore boundary

**Question 10.04.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 05: Environmental Storytelling & Lore boundary

**Question 10.05.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 06: Environmental Storytelling & Lore boundary

**Question 10.06.** Does `Assets/Ashfall.Core/EnvironmentalTextCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 07: Environmental Storytelling & Lore boundary

**Question 10.07.** Does `Assets/Ashfall.Core/AtmosphereTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 08: Environmental Storytelling & Lore boundary

**Question 10.08.** Does `Assets/Ashfall.Core/AtmosphereCatalogLoader.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 09: Environmental Storytelling & Lore boundary

**Question 10.09.** Does `Assets/Ashfall.Core/Shelter/ShelterAtmosphereSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 10: Environmental Storytelling & Lore boundary

**Question 10.10.** Does `src/Main.ShelterAtmosphere.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 11: Environmental Storytelling & Lore boundary

**Question 10.11.** Does `src/UI/ShelterAtmospherePanel.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 12: Environmental Storytelling & Lore boundary

**Question 10.12.** Does `Assets/Ashfall.Core/EnvironmentalTextSystem.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
> **Pre-polish body length at generation time:** 265,985 characters.
> **Target interpretation:** 150k–170k is the first checkpoint; 250k+ is the evidence-backed depth target and is not a ceiling.

# Post-250K Deep Polishing Pass — Environmental Storytelling & Lore
...
**Outcome:** `Environmental Storytelling & Lore`
