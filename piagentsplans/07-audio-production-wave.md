...
This is a planning and architecture artifact for **Audio cue authority, event bridging, shelter acoustics, accessibility and production playback**. It preserves the original intent: Deepen the audio production plan around current cue catalogs, event bridges, acoustic state, accessibility settings and host playback rather than inventing a second audio manager.

The current residual premise is: Audio data presence, cue integrity, host bridging, source registration, loudness policy and accessibility recovery must be separated and proven against the current audio owner chain.
...
The bounded objective is to make the **Audio Production Wave** opportunity implementable without creating a parallel gameplay authority. The plan must identify:
...
### Current source: `src/Audio/AudioCueCatalog.cs`
- Evidence status: **CURRENT FILE PRESENT**; 504 lines / 24731 bytes; SHA-256 `c66047437733c6f29de1c44e73cac9ab345ef9b8df56113b0e1ecd5cf8f64ab3`.
...
4:
5: namespace AtomicWar.GodotApp.Audio
6: {
7:     /// <summary>
8:     /// Audio bus names used by the cue catalog and playback service.
9:     /// </summary>
   10:     public static class AudioBusNames
   11:     {
   12:         public const string Master = "Master";
   13:         public const string Music = "Music";
   14:         public const string Ambience = "Ambience";
   15:         public const string Sfx = "SFX";
   16:         public const string Ui = "UI";
   17:         public const string Voice = "Voice";
   18:         public const string Alerts = "Alerts";
   19:         public const string Generator = "Generator";
   20:         public const string Ventilation = "Ventilation";
   21:         public const string Radio = "Radio";
   22:         public const string Medical = "Medical";
   23:         public const string Surface = "Surface";
   24:         public const string Machinery = "Machinery";
   25:         public const string ShelterSocial = "ShelterSocial";
   26:         public const string Subterranean = "Subterranean";
   27:     }
   28:
   29:     /// <summary>
   30:     /// Defines a single audio cue: stable snake_case ID → resource, bus, behavior.
   31:     /// </summary>
   32:     public sealed class AudioCueDef
   33:     {
   34:         public string Id { get; }
   35:         public System.Collections.Generic.IReadOnlyList<string> ResourcePaths { get; }
   36:         public string ResourcePath => ResourcePaths.Count > 0 ? ResourcePaths[0] : "";
   37:         public string Bus { get; }
   38:         public bool Loop { get; }
   39:         public float DefaultVolumeDb { get; }
   40:         public float VolumeJitterDb { get; }
   41:         public float PitchMin { get; }
   42:         public float PitchMax { get; }
   43:         public int MaxInstances { get; }
   44:         public int Priority { get; }
   45:         public float CooldownSeconds { get; }
   46:         public float FadeInSeconds { get; }
   47:         public float FadeOutSeconds { get; }
   48:         public string? FallbackCueId { get; }
   49:
   50:         public AudioCueDef(string id, string resourcePath, string bus,
   51:             bool loop = false, float defaultVolumeDb = 0f,
   52:             float cooldownSeconds = 0f, string? fallbackCueId = null,
   53:             System.Collections.Generic.IEnumerable<string>? resourcePaths = null,
   54:             float volumeJitterDb = 0f, float pitchMin = 1f, float pitchMax = 1f,
   55:             int maxInstances = 4, int priority = 50, float fadeInSeconds = 0f, float fadeOutSeconds = 0f)
   83:     /// <summary>
   84:     /// Data-driven catalog of all audio cues.
   85:     /// Stable snake_case IDs. Missing resources fail safely.
   86:     /// </summary>
   87:
   88:     public sealed class AudioCueCatalogDto
   89:     {
   90:         [System.Text.Json.Serialization.JsonPropertyName("schema_version")]
   91:         public int SchemaVersion { get; set; }
   92:
   93:         [System.Text.Json.Serialization.JsonPropertyName("cues")]
   94:         public System.Collections.Generic.List<AudioCueDto> Cues { get; set; } = new();
   95:     }
   96:
   97:     public sealed class AudioCueDto
   98:     {
   99:         [System.Text.Json.Serialization.JsonPropertyName("id")]
  140:
  141:         [System.Text.Json.Serialization.JsonPropertyName("fallback_cue_id")]
  142:         public string? FallbackCueId { get; set; }
  143:     }
  144:
  145:     public static class AudioCueCatalog
  146:     {
  147:         private static readonly Dictionary<string, AudioCueDef> s_cues = new();
  148:
  149:         // ── Cue IDs (stable, snake_case) ────────────────────────
  150:
  151:         // UI
  152:         public const string UiClick = "ui_click";
  153:         public const string UiConfirm = "ui_confirm";
  154:         public const string UiWarning = "ui_warning";
  155:         public const string UiCancel = "ui_cancel";
  156:         public const string UiTabChange = "ui_tab_change";
  397:         public const string MethaneAlarmBeep = "methane_alarm_beep";
  398:         public const string WaterPumpHumLoop = "water_pump_hum_loop";
  399:         public const string BulkheadHydraulicSlam = "bulkhead_hydraulic_slam";
  400:         public const string CaveInCollapseRumble = "cave_in_collapse_rumble";
  401:
  402:         static AudioCueCatalog()
  403:         {
  404:             RegisterAll();
  405:         }
  406:
  407:         private const string GodotCatalogPath = "res://assets/StreamingAssets/Data/audio_cues.json";
  408:
  409:         public static void RegisterAll()
  410:         {
  411:             s_cues.Clear();
  412:             // Plan 26A: resolve through the one data-path authority.
  413:             bool loaded = false;
  414:             try { loaded = LoadFromJson(CatalogPath.ResolveCatalog("audio_cues.json")); }
  415:             catch { /* CatalogPath may throw if segment is invalid; fall through */ }
  416:             if (!loaded)
  417:             {
  418:                 LoadFromJson(GodotCatalogPath);
  419:             }
  420:         }
  421:
  422:         public static void Reload() => RegisterAll();
  423:
  424:         // ── Lookup ──────────────────────────────────────────────
  425:
```
### Current source: `src/Audio/AudioManager.cs`
- Evidence status: **CURRENT FILE PRESENT**; 1068 lines / 44591 bytes; SHA-256 `80c99fe790b2ba0d356f070afb9b600bfe4dc7aa44826623bdcd49631bd22126`.
The following is a bounded source excerpt, not a generated API and not a claim that every declaration is production-reachable.
```text
1: // SPDX-License-Identifier: MIT
2: using Godot;
...
5: using System.IO;
6:
7: namespace AtomicWar.GodotApp.Audio
8: {
9:     /// <summary>
   10:     /// Complete audio playback service for ASHFALL.
   11:     /// Owns bus topology, stream caching, pooled one-shot players,
   12:     /// cooldown/dedup, music crossfade, ambience looping, and settings integration.
   13:     /// No simulation logic — presentation only.
   14:     /// </summary>
   15:     public partial class AudioManager : Node
   16:     {
   17:         public static AudioManager? Instance { get; internal set; }
   18:
   19:         // ── Bus topology ────────────────────────────────────────
   20:
   21:         private static readonly string[] AllBuses = {
   22:             AudioBusNames.Music,
   23:             AudioBusNames.Ambience,
   24:             AudioBusNames.Sfx,
   25:             AudioBusNames.Ui,
   26:             AudioBusNames.Voice,
   27:             AudioBusNames.Alerts,
   28:             AudioBusNames.Generator,
   29:             AudioBusNames.Ventilation,
   30:             AudioBusNames.Radio,
   31:             AudioBusNames.Medical,
   32:             AudioBusNames.Surface,
   33:             AudioBusNames.Machinery,
   34:             AudioBusNames.ShelterSocial,
   35:             AudioBusNames.Subterranean,
   36:         };
   37:
   38:         // ── Players ─────────────────────────────────────────────
   39:
   40:         private AudioStreamPlayer _musicPlayerA = null!;
   41:         private AudioStreamPlayer _musicPlayerB = null!;
   42:         private bool _musicActiveA;
   43:         private float _musicCrossfade;
   44:         private bool _crossfading;
   45:
   46:         // Loops are keyed by cue ID. Generator, ventilation, Geiger, weather,
   47:         // and ambience therefore coexist instead of replacing one shared stream.
   48:         private readonly Dictionary<string, AudioStreamPlayer> _loopPlayers = new();
   49:         private readonly List<string> _loopKeys = new();
   50:
   51:         // One-shot pool: bounded to prevent unbounded allocation
   52:         private const int MaxOneShotPlayers = 16;
   53:         private readonly Stack<AudioStreamPlayer> _pool = new();
   54:         private readonly List<AudioStreamPlayer> _activeOneShots = new();
   55:
   65:         private const float CooldownEpsilon = 0.01f;
   66:
   67:         // ── Headless detection ──────────────────────────────────
   68:
   69:         private bool _headless;
   70:         private AudioEventBridge? _eventBridge;
   71:         private ExpansionAudioBridge? _expansionEventBridge;
   72:         private AudioConditionHostBridge? _conditionBridge;
   73:         private AudioStateCoordinator? _stateCoordinator;
   74:         private ShelterAudioController? _shelterAudio;
   75:         private ShelterOperationsAudioBridge? _shelterOperationsBridge;
   76:         private SurfaceAmbienceController? _surfaceAmbience;
   77:         private ScarcityAudioController? _scarcityAudio;
   78:         private IAudioDomainProvider? _domainProvider;
   79:         private Action? _settingsChangedHandler;
   80:         private int _oneShotDroppedCount;
   81:
   82:         // ── Lifecycle ───────────────────────────────────────────
   86:             Instance = this;
   87:             _headless = !DisplayServer.WindowCanDraw();
   88:
   89:             SetupBuses();
   90:             SetupPlayers();
   91:             ApplySettings(AudioSettings.Instance);
   92:             _settingsChangedHandler = () => ApplySettings(AudioSettings.Instance);
   93:             AudioSettings.Instance.OnSettingsChanged += _settingsChangedHandler;
   94:
   95:             _eventBridge = new AudioEventBridge(this);
   96:             _expansionEventBridge = new ExpansionAudioBridge(this);
   97:             _conditionBridge = new AudioConditionHostBridge(this);
   98:             _stateCoordinator = new AudioStateCoordinator();
   99:             AddChild(_stateCoordinator);
  100:             _shelterAudio = new ShelterAudioController(this);
  101:             _shelterOperationsBridge = new ShelterOperationsAudioBridge(this);
  102:             _surfaceAmbience = new SurfaceAmbienceController(this);
  103:             _scarcityAudio = new ScarcityAudioController(this);
  104:             _domainProvider = GetParent() as IAudioDomainProvider;
  105:             RefreshDomainBindings();
  106:
  107:             GD.Print($"[AudioManager] Ready — {AllBuses.Length + 1} buses, " +
  108:                      $"pool={MaxOneShotPlayers}, headless={_headless}");
  109:         }
  110:
  111:         private AudioEffectLowPassFilter? _surfaceOcclusionEffect;
  112:         private bool _bunkerOccluded = true;
  113:         public bool IsBunkerOccluded => _bunkerOccluded;
  114:
  115:         private void SetupBuses()
  116:         {
  117:             foreach (string bus in AllBuses)
  118:                 EnsureBus(bus);
  177:         }
  178:
  179:         public void TriggerConcussionDeafness(float durationSeconds = 2.5f)
  180:         {
  181:             if (_headless) return;
  182:             PlayCue(AudioCueCatalog.UiWarning);
  183:
  184:             int masterIdx = AudioServer.GetBusIndex(AudioBusNames.Master);
  185:             if (masterIdx >= 0)
  186:             {
  187:                 float currentVol = AudioServer.GetBusVolumeDb(masterIdx);
  188:                 AudioServer.SetBusVolumeDb(masterIdx, currentVol - 12f);
  189:                 if (!AtomicWar.GodotApp.Settings.AccessibilityPresentation.MotionAllowed)
  190:                 {
  191:                     AudioServer.SetBusVolumeDb(masterIdx, currentVol);
  192:                     return;
  193:                 }
  368:         /// Play a cue by its stable snake_case ID.
  369:         /// Respects cooldown, settings, headless mode, and missing-resource fallback.
  370:         /// </summary>
  371:         public void PlayCue(string cueId)
  372:         {
  373:             var cue = AudioCueCatalog.Resolve(cueId);
  374:             if (cue == null)
  375:             {
  376:                 LogMissingOnce($"cue:{cueId}");
  377:                 return;
  378:             }
  379:             PlayCueDef(cue);
  380:         }
  381:
  382:         /// <summary>
  383:         /// Play a cue definition directly.
  384:         /// </summary>
  401:             if (stream == null)
  402:             {
  403:                 // Try fallback
  404:                 if (cue.FallbackCueId != null)
  405:                 {
  406:                     var fallback = AudioCueCatalog.Resolve(cue.FallbackCueId);
  407:                     if (fallback != null) PlayCueDef(fallback);
  408:                 }
  409:                 return;
  410:             }
  411:
  412:             // Apply cooldown
  413:             if (cue.CooldownSeconds > 0)
  414:                 _cooldowns[cue.Id] = cue.CooldownSeconds;
  415:
  416:             float effectiveDb = cue.DefaultVolumeDb + GetBusVolumeOffset(cue.Bus);
  417:             if (cue.VolumeJitterDb > 0f)
```
...
9: using Ashfall.Core.Radiation;
   ...
   58:         // C2 / Plan 17C Phase G — geiger loop lifecycle from real exposure state.
   59:         private readonly HashSet<string> _exposedSurvivors = new(StringComparer.Ordinal);
   60:         private string? _activeGeigerLoopCue;
   61:         private bool _disposed;
   62:
   63:         public AudioEventBridge(AudioManager audio)
   64:             : this(
   65:                 (audio ?? throw new ArgumentNullException(nameof(audio))).PlayCue,
   66:                 audio.StopCue)
   67:         {
   68:         }
   69:
   70:         internal AudioEventBridge(Action<string> playCue, Action<string>? stopCue = null)
   71:         {
   72:             _playCue = playCue ?? throw new ArgumentNullException(nameof(playCue));
   73:             _stopCue = stopCue ?? (_ => { });
   74:         }
   75:
   76:         /// <summary>
   77:         /// Bind both domains. Safe to call every frame: unchanged references
   78:         /// are no-ops, while replaced sessions are unsubscribed before binding.
   79:         /// </summary>
   80:         public void SubscribeAll(
   81:             RadiationSystem? radiation = null,
  168:
  169:         private void OnCombatEvent(CombatState state, CombatEvent evt)
  170:         {
  171:             string? cueId = evt.Kind switch
  172:             {
  173:                 "encounter_start" => AudioCueCatalog.CombatStart,
  174:                 "fire" => ResolveWeaponFireCue(evt.Detail),
  175:                 "suppress" => AudioCueCatalog.WeaponLmgBurst,
  176:                 "weapon_jam" => AudioCueCatalog.CombatJam,
  177:                 "jam_persist" => AudioCueCatalog.CombatDryFire,
  178:                 "weapon_burst" => AudioCueCatalog.CombatWeaponBurst,
  179:                 "reload" => evt.Detail.Contains("shotgun") ? AudioCueCatalog.WeaponShotgunRack : AudioCueCatalog.CombatReload,
  180:                 "clear_jam" => AudioCueCatalog.CombatReload,
  181:                 "downed" => AudioCueCatalog.HeavyImpactFall,
  182:                 "death" => AudioCueCatalog.CombatDowned,
  183:                 "mutual_kill" => AudioCueCatalog.CombatDowned,
  184:                 "victory" => AudioCueCatalog.CombatVictory,
  185:                 "defeat" => AudioCueCatalog.CombatDefeat,
  186:                 "trap" => AudioCueCatalog.CombatHit,
  187:                 "retreat_fail" => AudioCueCatalog.CombatHit,
  378:
  379:         private void OnOutcomeResolved(string survivorId, string diseaseId, bool recovered)
  380:         {
  381:             // Fatal outcomes enter the all-cause SurvivorFate pipeline, which
  382:             // owns the distinct loss cue. Recovery gets the quieter clearance
  383:             // feedback here; AudioManager's cue cooldown absorbs same-tick
  384:             // recovery + automatic quarantine-release pairs.
  385:             if (recovered)
  386:                 _playCue(AudioCueCatalog.MedQuarantineClear);
  387:         }
  388:
  389:         /// <summary>
  390:         /// Binds the unified Core death pipeline rather than individual sources.
  391:         /// The fate system emits only after its idempotent all-cause cascade, so
  392:         /// one real campaign loss produces one death cue.
  393:         /// </summary>
  394:         public void BindSurvivorFate(SurvivorFateSystem? survivorFate)
```
### Current source: `src/Audio/ExpansionAudioBridge.cs`
- Evidence status: **CURRENT FILE PRESENT**; 169 lines / 4991 bytes; SHA-256 `0df73a4f96e2b2852b99cf4b2fdf11954e9f411b266448207ff26a0086ff9bb7`.
The following is a bounded source excerpt, not a generated API and not a claim that every declaration is production-reachable.
```text
1: // SPDX-License-Identifier: MIT
2: using System;
...
7: using Ashfall.Core.Combat;
8: using Ashfall.Core.Expeditions;
9:
   10: namespace AtomicWar.GodotApp.Audio
   11: {
   12:     public interface IExpansionAudioProvider
   13:     {
   14:         DesperationSystem? AudioDesperation { get; }
   15:         MutationSystem? AudioMutation { get; }
   16:         ChemWarfareSystem? AudioChemWarfare { get; }
   17:         RailwaySystem? AudioRailway { get; }
   18:     }
   19:
   20:     public sealed class ExpansionAudioBridge : IDisposable
   21:     {
   22:         private readonly Action<string> _playCue;
   23:         private DesperationSystem? _desperation;
   24:         private MutationSystem? _mutation;
   25:         private ChemWarfareSystem? _chemWarfare;
   26:         private RailwaySystem? _railway;
   27:         private bool _disposed;
   28:
   29:         public ExpansionAudioBridge(AudioManager audio)
   30:             : this(audio != null ? audio.PlayCue : throw new ArgumentNullException(nameof(audio)))
   31:         {
   32:         }
   33:
   34:         internal ExpansionAudioBridge(Action<string> playCue)
   35:         {
   36:             _playCue = playCue ?? throw new ArgumentNullException(nameof(playCue));
   37:         }
   38:
   39:         public void SubscribeAll(IExpansionAudioProvider? provider)
   40:         {
   ...
   46:             }
   47:
   48:             BindDesperation(provider.AudioDesperation);
   49:             BindMutation(provider.AudioMutation);
   50:             BindChemWarfare(provider.AudioChemWarfare);
   51:             BindRailway(provider.AudioRailway);
   52:         }
   53:
   54:         public void BindDesperation(DesperationSystem? next)
   55:         {
  115:             }
  116:         }
  117:
  118:         private void HandleTabooBroken(DesperationActRecord record)
  119:         {
  120:             _playCue(AudioCueCatalog.InterrogationSlam);
  121:         }
  122:
  123:         private void HandleMutationAcquired(string survivorId, string mutationId, List<string> tags)
  124:         {
  125:             _playCue(AudioCueCatalog.BioMutationPulse);
  126:         }
  127:
  128:         private void HandleHazardDeployed(ToxicHazardZoneState hazard)
  129:         {
  130:             _playCue(AudioCueCatalog.HazardToxicSizzle);
  131:         }
  132:
  133:         private void HandleDerailment(string trainId, string segmentId)
  134:         {
  135:             _playCue(AudioCueCatalog.TrainScreechCrash);
  136:         }
  137:
  138:         public void UnbindAll()
  139:         {
  140:             if (_desperation != null)
  141:             {
```
### Current source: `src/Audio/ShelterAcousticBridge.cs`
- Evidence status: **CURRENT FILE PRESENT**; 77 lines / 2502 bytes; SHA-256 `63240f3c79c4f70d0ad44b70de2ba93b7199f1090c4d3beb1d9b57af0794d4c2`.
The following is a bounded source excerpt, not a generated API and not a claim that every declaration is production-reachable.
```text
1: // SPDX-License-Identifier: MIT
2: using System;
3: using Ashfall.Core.Audio;
4:
5: namespace AtomicWar.GodotApp.Audio
6: {
7:     /// <summary>
8:     /// Bridges the Core ShelterAcousticDirector to the Godot AudioManager presentation layer.
9:     /// Thin presentation adapter with zero gameplay or simulation logic.
   10:     /// </summary>
   11:     public sealed class ShelterAcousticBridge : IDisposable
   12:     {
   13:         private readonly Action<string> _playCue;
   14:         private readonly Action<string, string> _startLoop;
   15:         private readonly Action<string, string, float, float> _updateLoop;
   16:         private readonly Action<string, string> _stopLoop;
   17:
   18:         private ShelterAcousticDirector? _director;
   19:         private bool _disposed;
   20:
   21:         public ShelterAcousticBridge(AudioManager audio)
   22:             : this(
   23:                 (audio ?? throw new ArgumentNullException(nameof(audio))).PlayCue,
   24:                 audio.StartLoop,
   25:                 audio.UpdateLoop,
   26:                 audio.StopLoop)
   27:         {
   28:         }
   29:
   30:         public ShelterAcousticBridge(
   31:             Action<string> playCue,
   32:             Action<string, string>? startLoop = null,
   33:             Action<string, string, float, float>? updateLoop = null,
   34:             Action<string, string>? stopLoop = null)
   35:         {
   36:             _playCue = playCue ?? throw new ArgumentNullException(nameof(playCue));
   37:             _startLoop = startLoop ?? ((_, _) => { });
   38:             _updateLoop = updateLoop ?? ((_, _, _, _) => { });
   39:             _stopLoop = stopLoop ?? ((_, _) => { });
   40:         }
   41:
   42:         public void BindDirector(ShelterAcousticDirector? director)
   43:         {
   44:             _director = director;
   45:         }
   46:
   47:         public void SyncAcoustics()
   48:         {
   49:             if (_disposed || _director == null) return;
   50:
   51:             var snapshot = _director.EvaluateSnapshot();
   52:
   53:             // 1. Dispatch one-shots
   54:             foreach (var cueId in snapshot.pendingOneShotCues)
   55:             {
```
### Current source: `src/Audio/AudioSettings.cs`
- Evidence status: **CURRENT FILE PRESENT**; 337 lines / 12962 bytes; SHA-256 `3bba0d8faa63bdd6b284f8a43e0e3807b7a519a948fb67ec880d5141ffbcbdcd`.
The following is a bounded source excerpt, not a generated API and not a claim that every declaration is production-reachable.
```text
1: // SPDX-License-Identifier: MIT
2: using Godot;
...
6: using Ashfall.Core.Audio;
7: using Ashfall.Core.Settings;
8:
9: namespace AtomicWar.GodotApp.Audio
   10: {
   11:     /// <summary>
   12:     /// Versioned, persisted audio user preferences.
   13:     /// Separate from gameplay saves. Stored at user://audio_settings.json.
   14:     /// Hardened for malformed-file recovery: restores defaults without exception,
   15:     /// while preserving valid values whenever partial parsing is possible.
   16:     /// </summary>
   17:     public sealed class AudioSettings
   18:     {
   19:         public const int CurrentVersion = AudioSettingsData.CurrentVersion;
   20:         private const string FileName = "audio_settings.json";
   21:
   22:         private static AudioSettings? _instance;
   23:         private static string? _lastDiagnosticMessage;
   24:
   25:         public static AudioSettings Instance => _instance ??= Load();
   26:
   27:         /// <summary>
   28:         /// Diagnostic message from the most recent load/save/recovery operation, or null if clean.
   29:         /// </summary>
   30:         public static string? LastDiagnosticMessage => _lastDiagnosticMessage;
   31:
   32:         /// <summary>
   33:         /// True if the most recent load or save encountered an issue and had to recover.
   34:         /// </summary>
   35:         public static bool HasDiagnosticError => !string.IsNullOrEmpty(_lastDiagnosticMessage);
   36:
   37:         public static void ClearDiagnosticMessage() => _lastDiagnosticMessage = null;
   38:
   39:         /// <summary>
   40:         /// Applies the five user-facing mix controls from the unified settings
   41:         /// store. Advanced per-domain buses remain in this legacy audio store,
   42:         /// but the controls exposed by SettingsPanel have one authority.
   43:         /// </summary>
   44:         public static void ApplyUnifiedMix(UserSettingsData data)
   45:         {
   46:             if (data == null) return;
   47:             var settings = Instance;
   48:             settings.MasterVolume = Math.Clamp(data.MasterVolume * 100f, 0f, 100f);
   49:             settings.MusicVolume = Math.Clamp(data.MusicVolume * 100f, 0f, 100f);
   50:             settings.AmbienceVolume = Math.Clamp(data.AmbienceVolume * 100f, 0f, 100f);
   51:             settings.SfxVolume = Math.Clamp(data.SfxVolume * 100f, 0f, 100f);
   52:             settings.RadioVolume = Math.Clamp(data.RadioVolume * 100f, 0f, 100f);
   53:             settings.MasterMute = data.MuteAll;
   54:             settings.MusicMute = data.MuteAll;
   55:             settings.AmbienceMute = data.MuteAll;
```
### Current source: `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs`
- Evidence status: **CURRENT FILE PRESENT**; 54 lines / 1916 bytes; SHA-256 `bbe2904b48a9bad69702acdf84429554564b1513b90ff3376d01972570ba31db`.
The following is a bounded source excerpt, not a generated API and not a claim that every declaration is production-reachable.
```text
1: // SPDX-License-Identifier: MIT
2: using System;
3: using System.Collections.Generic;
4:
5: namespace Ashfall.Core.Audio
6: {
7:     [Serializable]
8:     public sealed class ShelterAudioCueDefinition
9:     {
   10:         public string id = string.Empty;
   11:         public string display_name = string.Empty;
   12:         public string bus_id = string.Empty;
   13:         public string playback_mode = "one_shot"; // "loop", "one_shot", "procedural_density"
   14:         public float base_volume_db;
   15:         public int pitch_jitter_permille;
   16:         public string ducking_group = "none";
   17:         public string description = string.Empty;
   18:     }
   19:
   20:     [Serializable]
   21:     public sealed class AcousticMixProfileDefinition
   22:     {
   23:         public string id = string.Empty;
   24:         public string display_name = string.Empty;
   25:         public float reverb_room_size;
   26:         public int lowpass_cutoff_hz = 20000;
   27:         public float attenuation_db;
   28:     }
   29:
   30:     [Serializable]
   31:     public sealed class ShelterAudioCueCatalog
   32:     {
   33:         public int schema_version = 1;
   34:         public List<ShelterAudioCueDefinition> cues = new List<ShelterAudioCueDefinition>();
   35:         public List<AcousticMixProfileDefinition> mix_profiles = new List<AcousticMixProfileDefinition>();
   36:     }
   37:
   38:     public static class ShelterAudioCueCatalogLoader
   39:     {
   40:         public static ShelterAudioCueCatalog Load(string json, IJsonSerializer serializer)
   41:         {
   42:             if (string.IsNullOrWhiteSpace(json))
   43:                 throw new ArgumentException("Shelter audio cues JSON string cannot be null or empty.", nameof(json));
   44:             if (serializer == null)
   45:                 throw new ArgumentNullException(nameof(serializer));
   46:
   47:             var catalog = serializer.Deserialize<ShelterAudioCueCatalog>(json);
   48:             if (catalog == null)
   49:                 throw new InvalidOperationException("Failed to deserialize shelter audio cues catalog.");
   50:
   51:             return catalog;
   52:         }
   53:     }
...
- `Assets/StreamingAssets/Data/audio_cues.json` — 112400 bytes; SHA-256 `afec6f7eb662d456667f2e60b41255f689278524593f06d19e93a9d63a207bd0`.
  - `root` object keys (2): `schema_version`, `cues`
  - `root.cues` list rows: **196**
- sample row key: `action_crafting`; fields: `id`, `resource_path`, `resource_paths`, `bus`, `loop`, `default_volume_db`, `volume_jitter_db`, `pitch_min`, `pitch_max`, `max_instances`, `priority`, `cooldown_seconds`, `fade_in_seconds`, `fade_out_seconds`, `fallback_cue_id`
- sample row key: `action_injection`; fields: `id`, `resource_path`, `resource_paths`, `bus`, `loop`, `default_volume_db`, `volume_jitter_db`, `pitch_min`, `pitch_max`, `max_instances`, `priority`, `cooldown_seconds`, `fade_in_seconds`, `fade_out_seconds`, `fallback_cue_id`
- sample row key: `action_interrogation_slam`; fields: `id`, `resource_path`, `resource_paths`, `bus`, `loop`, `default_volume_db`, `volume_jitter_db`, `pitch_min`, `pitch_max`, `max_instances`, `priority`, `cooldown_seconds`, `fade_in_seconds`, `fade_out_seconds`, `fallback_cue_id`
- `Assets/StreamingAssets/Data/audio_settings.json` — missing at generation time; no catalog claim is made.

# Appendix C — Current Test Inventory

- `Ashfall.Core.Tests/Audio/AudioCueIntegrityTests.cs` — missing at generation time; do not cite as executable evidence.
- `Ashfall.Core.Tests/Audio/Plan169AudioAccessibilityIntegrationTests.cs` — 134 lines; SHA-256 `a575ea90262be129621761a91d90fa0acc4b9095afedd68d20f157483a809310`; test attributes 6; declaration lines 8.
...
- `Ashfall.Core.Tests/AudioEventIntegrationTests.cs` — 301 lines; SHA-256 `8142a3cb139ee16bc343412539302bb3637441f3bd9b115190c5b28044543cd7`; test attributes 14; declaration lines 17.
...
### Mechanical references to `AudioCueCatalog`
Assets/Ashfall.Core/CatalogIntegrityValidator.cs:536: // host-side AudioCueCatalog rather than a gameplay JSON catalog, but
Assets/Ashfall.Core/Content/ContentUtilizationScanner.cs:523: ["shelter_audio_cues.json"] = new[] { "ShelterAudioCueCatalogLoader", "ShelterAcousticDirector" },
Assets/Ashfall.Core/Audio/ShelterAcousticDirector.cs:39: public ShelterAcousticDirector(ShelterAudioCueCatalog? catalog = null, ISeededRng? rng = null)
Assets/Ashfall.Core/Audio/ShelterAcousticDirector.cs:48: public void LoadCatalog(ShelterAudioCueCatalog catalog)
Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs:31: public sealed class ShelterAudioCueCatalog
Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs:38: public static class ShelterAudioCueCatalogLoader
Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs:40: public static ShelterAudioCueCatalog Load(string json, IJsonSerializer serializer)
Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs:47: var catalog = serializer.Deserialize<ShelterAudioCueCatalog>(json);
src/Main.FactionBranch.cs:76: AtomicWar.GodotApp.Audio.AudioManager.Instance?.PlayCue(AtomicWar.GodotApp.Audio.AudioCueCatalog.UiConfirm);
src/Main.MoralChoice.cs:214: AtomicWar.GodotApp.Audio.AudioManager.Instance?.PlayCue(AtomicWar.GodotApp.Audio.AudioCueCatalog.UiConfirm);
src/Main.Holdfast.cs:280: _audio?.PlayCue(AtomicWar.GodotApp.Audio.AudioCueCatalog.DayTransition);
src/Main.ShelterInfrastructure.cs:223: cueId => AtomicWar.GodotApp.Audio.AudioCueCatalog.Resolve(cueId)?.Loop ?? false);
src/Main.SaveOrchestrator.cs:641: _audio?.PlayCue(AtomicWar.GodotApp.Audio.AudioCueCatalog.SaveSuccess);
src/Main.GameFlow.cs:366: AtomicWar.GodotApp.Audio.AudioManager.Instance?.PlayCue(AtomicWar.GodotApp.Audio.AudioCueCatalog.UiConfirm);
...
### Mechanical references to `AudioManager`
Assets/Ashfall.Core/HostCliRegistry.cs:1017: "Audio cue catalog, AudioManager wiring, and sound event verification"),
Assets/Ashfall.Core/Radio/DistressAudioCueResolver.cs:21: /// RNG, no time, no hash ordering), no AudioManager calls, no gameplay
Assets/Ashfall.Core/Radio/SignalTrustLedger.cs:64: /// radio/distress runtime, free of UI/AudioManager dependencies. Every
Assets/Ashfall.Core/Settings/UserSettingsData.cs:78: /// reduced_stimulation). Applied by AudioManager at startup and on change.
src/Main.FactionBranch.cs:76: AtomicWar.GodotApp.Audio.AudioManager.Instance?.PlayCue(AtomicWar.GodotApp.Audio.AudioCueCatalog.UiConfirm);
src/Main.Audio.cs:19: /// AudioManager discovers this through its parent and safely rebinds when
src/Main.MoralChoice.cs:214: AtomicWar.GodotApp.Audio.AudioManager.Instance?.PlayCue(AtomicWar.GodotApp.Audio.AudioCueCatalog.UiConfirm);
src/Main.UiPanels.cs:50: private AudioManager _audio = null!;
src/Main.UiPanels.cs:234: _audio = new AudioManager();
src/Main.AudioAccessibility.cs:4: // Binds the Core AudioAccessibilityCoordinator to the live AudioManager and to
src/Main.GameFlow.cs:366: AtomicWar.GodotApp.Audio.AudioManager.Instance?.PlayCue(AtomicWar.GodotApp.Audio.AudioCueCatalog.UiConfirm);
src/Main.PlayerSurfaces.cs:1088: /// all apply); the AudioManager cue cooldown absorbs the second confirm.
src/Host/AutopsyHostSession.cs:52: AtomicWar.GodotApp.Audio.AudioManager.Instance?.PlayCue(AtomicWar.GodotApp.Audio.AudioCueCatalog.ShelterAirFilter);
src/Host/HoldfastTerminalPanel.cs:173: AtomicWar.GodotApp.Audio.AudioManager.Instance?.PlayCue(AtomicWar.GodotApp.Audio.AudioCueCatalog.ActionTrade);
Interpretation: these are search hits only; an integrating builder must trace the actual call path, setup binding, event subscription and save registration before claiming reachability.
...
Interpretation: these are search hits only; an integrating builder must trace the actual call path, setup binding, event subscription and save registration before claiming reachability.
### Mechanical references to `ExpansionAudioBridge`
src/Audio/AudioManager.cs:71: private ExpansionAudioBridge? _expansionEventBridge;
src/Audio/AudioManager.cs:96: _expansionEventBridge = new ExpansionAudioBridge(this);
src/Audio/ExpansionAudioBridge.cs:20: public sealed class ExpansionAudioBridge : IDisposable
src/Audio/ExpansionAudioBridge.cs:29: public ExpansionAudioBridge(AudioManager audio)
src/Audio/ExpansionAudioBridge.cs:34: internal ExpansionAudioBridge(Action<string> playCue)
src/Audio/AudioSelfTest.cs:718: var expansionBridge = new ExpansionAudioBridge(expansionBridgeEmitted.Add);
src/Audio/AudioSelfTest.cs:730: Check("Repeated refresh on ExpansionAudioBridge produces exactly one callback",
src/Audio/AudioSelfTest.cs:737: Check("Disposed ExpansionAudioBridge produces zero callbacks",
src/Audio/AudioSelfTest.cs:743: var expansionBridge2 = new ExpansionAudioBridge(expansionBridgeEmitted2.Add);
src/Audio/AudioSelfTest.cs:753: Check("Rebound ExpansionAudioBridge does not trigger on stale provider",
src/Audio/AudioSelfTest.cs:758: Check("Rebound ExpansionAudioBridge triggers on active provider",
src/Audio/AudioSelfTest.cs:810: var probeBridge = new ExpansionAudioBridge(probeEmitted.Add);
Interpretation: these are search hits only; an integrating builder must trace the actual call path, setup binding, event subscription and save registration before claiming reachability.
### Mechanical references to `ShelterAcousticBridge`
src/Main.Plans50_53.cs:24: private ShelterAcousticBridge? _shelterAcousticBridge;
src/Main.Plans50_53.cs:34: public ShelterAcousticBridge? ShelterAcousticBridge => _shelterAcousticBridge;
src/Main.Plans50_53.cs:224: _shelterAcousticBridge = new ShelterAcousticBridge(_audio);
src/Main.Plans50_53.cs:225: _shelterAcousticBridge.BindDirector(_shelterAcousticDirector);
src/Main.Plans50_53.cs:264: _shelterAcousticBridge?.SyncAcoustics();
src/Audio/ShelterAcousticBridge.cs:11: public sealed class ShelterAcousticBridge : IDisposable
src/Audio/ShelterAcousticBridge.cs:21: public ShelterAcousticBridge(AudioManager audio)
src/Audio/ShelterAcousticBridge.cs:30: public ShelterAcousticBridge(
Interpretation: these are search hits only; an integrating builder must trace the actual call path, setup binding, event subscription and save registration before claiming reachability.
### Mechanical references to `AudioSettings`
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:13: public static class AudioSettingsCodec
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:25: public static (AudioSettingsData Data, string? DiagnosticMessage) DeserializeWithRecovery(string? json)
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:29: return (new AudioSettingsData(), "[AudioSettingsCodec] Settings JSON is empty or null. Restored safe defaults.");
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:35: var data = JsonSerializer.Deserialize<AudioSettingsData>(json, JsonOpts);
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:53: return (new AudioSettingsData(), "[AudioSettingsCodec] JSON root is not an object. Restored safe defaults.");
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:56: var data = new AudioSettingsData();
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:90: ? "[AudioSettingsCodec] Partially recovered settings: " + string.Join("; ", warnings)
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:91: : sanitizeMsg ?? "[AudioSettingsCodec] Recovered settings with valid values.";
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:97: return (new AudioSettingsData(), $"[AudioSettingsCodec] Malformed JSON syntax ({ex.Message}). Restored safe defaults.");
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:133: public static AudioSettingsData Sanitize(AudioSettingsData? data, out string? diagnosticMessage)
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:137: diagnosticMessage = "[AudioSettingsCodec] Null data provided for sanitization. Returning defaults.";
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:138: return new AudioSettingsData();
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:157: ? "[AudioSettingsCodec] Sanitized volumes: " + string.Join("; ", warnings)
Assets/Ashfall.Core/Audio/AudioSettingsCodec.cs:182: public static string Serialize(AudioSettingsData data)
Interpretation: these are search hits only; an integrating builder must trace the actual call path, setup binding, event subscription and save registration before claiming reachability.
### Mechanical references to `ShelterAudioCueCatalog`
Assets/Ashfall.Core/Content/ContentUtilizationScanner.cs:523: ["shelter_audio_cues.json"] = new[] { "ShelterAudioCueCatalogLoader", "ShelterAcousticDirector" },
Assets/Ashfall.Core/Audio/ShelterAcousticDirector.cs:39: public ShelterAcousticDirector(ShelterAudioCueCatalog? catalog = null, ISeededRng? rng = null)
Assets/Ashfall.Core/Audio/ShelterAcousticDirector.cs:48: public void LoadCatalog(ShelterAudioCueCatalog catalog)
Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs:31: public sealed class ShelterAudioCueCatalog
...
Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs:40: public static ShelterAudioCueCatalog Load(string json, IJsonSerializer serializer)
Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs:47: var catalog = serializer.Deserialize<ShelterAudioCueCatalog>(json);
src/Main.Plans50_53.cs:213: var catalog = ShelterAudioCueCatalogLoader.Load(json, new SystemTextJsonSerializer());
Ashfall.Core.Tests/Audio/ShelterAcousticDirectorTests.cs:11: private static ShelterAudioCueCatalog CreateSampleCatalog()
Ashfall.Core.Tests/Audio/ShelterAcousticDirectorTests.cs:13: return new ShelterAudioCueCatalog
Ashfall.Core.Tests/Campaign/Plans50_53_SharedIntegrationTests.cs:27: private static (VehicleGarageCatalog GarageCat, FactionIntelligenceCatalog EspionageCat, PsychologicalTraumaCatalog MentalCat, ShelterAudioCueCatalog
Ashfall.Core.Tests/Campaign/Plans50_53_SharedIntegrationTests.cs:42: var audioCat = ShelterAudioCueCatalogLoader.Load(audioJson, serializer);
Interpretation: these are search hits only; an integrating builder must trace the actual call path, setup binding, event subscription and save registration before claiming reachability.
...
bash scripts/run_test.sh Ashfall.Core.Tests/AudioEventIntegrationTests.cs
...
## Audit cycle 01, lens 01: Audio Production Wave boundary

**Question 01.01.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.

**Current evidence to recheck.** `Assets/StreamingAssets/Data/audio_settings.json` must be parsed and its IDs/references counted. The result must distinguish content presence from a live consumer. If the catalog is absent, malformed, orphaned or not loaded, record a blocker rather than creating a replacement loader.
...
## Audit cycle 01, lens 02: Audio Production Wave boundary

**Question 01.02.** Does `src/Audio/AudioEventBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.

**Current evidence to recheck.** `Assets/StreamingAssets/Data/audio_cues.json` must be parsed and its IDs/references counted. The result must distinguish content presence from a live consumer. If the catalog is absent, malformed, orphaned or not loaded, record a blocker rather than creating a replacement loader.
...
## Audit cycle 01, lens 03: Audio Production Wave boundary

**Question 01.03.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 04: Audio Production Wave boundary

**Question 01.04.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 05: Audio Production Wave boundary

**Question 01.05.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 06: Audio Production Wave boundary

**Question 01.06.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 07: Audio Production Wave boundary

**Question 01.07.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 08: Audio Production Wave boundary

**Question 01.08.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 09: Audio Production Wave boundary
...
## Audit cycle 01, lens 10: Audio Production Wave boundary

**Question 01.10.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 11: Audio Production Wave boundary

**Question 01.11.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 01, lens 12: Audio Production Wave boundary

**Question 01.12.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 01: Audio Production Wave boundary

**Question 02.01.** Does `src/Audio/AudioEventBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 02: Audio Production Wave boundary

**Question 02.02.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 03: Audio Production Wave boundary

**Question 02.03.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 04: Audio Production Wave boundary

**Question 02.04.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 05: Audio Production Wave boundary

**Question 02.05.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 06: Audio Production Wave boundary

**Question 02.06.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 07: Audio Production Wave boundary

**Question 02.07.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 08: Audio Production Wave boundary
...
## Audit cycle 02, lens 09: Audio Production Wave boundary

**Question 02.09.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 10: Audio Production Wave boundary

**Question 02.10.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 11: Audio Production Wave boundary

**Question 02.11.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 02, lens 12: Audio Production Wave boundary

**Question 02.12.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 01: Audio Production Wave boundary

**Question 03.01.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 02: Audio Production Wave boundary

**Question 03.02.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 03: Audio Production Wave boundary

**Question 03.03.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 04: Audio Production Wave boundary

**Question 03.04.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 05: Audio Production Wave boundary

**Question 03.05.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 06: Audio Production Wave boundary

**Question 03.06.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 07: Audio Production Wave boundary
...
## Audit cycle 03, lens 08: Audio Production Wave boundary

**Question 03.08.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 09: Audio Production Wave boundary

**Question 03.09.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 10: Audio Production Wave boundary

**Question 03.10.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 11: Audio Production Wave boundary

**Question 03.11.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 03, lens 12: Audio Production Wave boundary

**Question 03.12.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 01: Audio Production Wave boundary

**Question 04.01.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 02: Audio Production Wave boundary

**Question 04.02.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 03: Audio Production Wave boundary

**Question 04.03.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 04: Audio Production Wave boundary

**Question 04.04.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 05: Audio Production Wave boundary

**Question 04.05.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 06: Audio Production Wave boundary
...
## Audit cycle 04, lens 07: Audio Production Wave boundary

**Question 04.07.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 08: Audio Production Wave boundary

**Question 04.08.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 09: Audio Production Wave boundary

**Question 04.09.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 10: Audio Production Wave boundary

**Question 04.10.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 11: Audio Production Wave boundary

**Question 04.11.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 04, lens 12: Audio Production Wave boundary

**Question 04.12.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 01: Audio Production Wave boundary

**Question 05.01.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 02: Audio Production Wave boundary

**Question 05.02.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 03: Audio Production Wave boundary

**Question 05.03.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 04: Audio Production Wave boundary

**Question 05.04.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 05: Audio Production Wave boundary
...
## Audit cycle 05, lens 06: Audio Production Wave boundary

**Question 05.06.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 07: Audio Production Wave boundary

**Question 05.07.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 08: Audio Production Wave boundary

**Question 05.08.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 09: Audio Production Wave boundary

**Question 05.09.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 10: Audio Production Wave boundary

**Question 05.10.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 11: Audio Production Wave boundary

**Question 05.11.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 05, lens 12: Audio Production Wave boundary

**Question 05.12.** Does `src/Audio/AudioEventBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 01: Audio Production Wave boundary

**Question 06.01.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 02: Audio Production Wave boundary

**Question 06.02.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 03: Audio Production Wave boundary

**Question 06.03.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 04: Audio Production Wave boundary
...
## Audit cycle 06, lens 05: Audio Production Wave boundary

**Question 06.05.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 06: Audio Production Wave boundary

**Question 06.06.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 07: Audio Production Wave boundary

**Question 06.07.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 08: Audio Production Wave boundary

**Question 06.08.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 09: Audio Production Wave boundary

**Question 06.09.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 10: Audio Production Wave boundary

**Question 06.10.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 11: Audio Production Wave boundary

**Question 06.11.** Does `src/Audio/AudioEventBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 06, lens 12: Audio Production Wave boundary

**Question 06.12.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 01: Audio Production Wave boundary

**Question 07.01.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 02: Audio Production Wave boundary

**Question 07.02.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 03: Audio Production Wave boundary
...
## Audit cycle 07, lens 04: Audio Production Wave boundary

**Question 07.04.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 05: Audio Production Wave boundary

**Question 07.05.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 06: Audio Production Wave boundary

**Question 07.06.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 07: Audio Production Wave boundary

**Question 07.07.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 08: Audio Production Wave boundary

**Question 07.08.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 09: Audio Production Wave boundary

**Question 07.09.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 10: Audio Production Wave boundary

**Question 07.10.** Does `src/Audio/AudioEventBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 11: Audio Production Wave boundary

**Question 07.11.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 07, lens 12: Audio Production Wave boundary

**Question 07.12.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 01: Audio Production Wave boundary

**Question 08.01.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 02: Audio Production Wave boundary
...
## Audit cycle 08, lens 03: Audio Production Wave boundary

**Question 08.03.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 04: Audio Production Wave boundary

**Question 08.04.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 05: Audio Production Wave boundary

**Question 08.05.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 06: Audio Production Wave boundary

**Question 08.06.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 07: Audio Production Wave boundary

**Question 08.07.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 08: Audio Production Wave boundary

**Question 08.08.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 09: Audio Production Wave boundary

**Question 08.09.** Does `src/Audio/AudioEventBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 10: Audio Production Wave boundary

**Question 08.10.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 11: Audio Production Wave boundary

**Question 08.11.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 08, lens 12: Audio Production Wave boundary

**Question 08.12.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 01: Audio Production Wave boundary
...
## Audit cycle 09, lens 02: Audio Production Wave boundary

**Question 09.02.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 03: Audio Production Wave boundary

**Question 09.03.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 04: Audio Production Wave boundary

**Question 09.04.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 05: Audio Production Wave boundary

**Question 09.05.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 06: Audio Production Wave boundary

**Question 09.06.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 07: Audio Production Wave boundary

**Question 09.07.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 08: Audio Production Wave boundary

**Question 09.08.** Does `src/Audio/AudioEventBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 09: Audio Production Wave boundary

**Question 09.09.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 10: Audio Production Wave boundary

**Question 09.10.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 11: Audio Production Wave boundary

**Question 09.11.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 09, lens 12: Audio Production Wave boundary

**Question 09.12.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 01: Audio Production Wave boundary

**Question 10.01.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 02: Audio Production Wave boundary

**Question 10.02.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 03: Audio Production Wave boundary

**Question 10.03.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 04: Audio Production Wave boundary

**Question 10.04.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 05: Audio Production Wave boundary

**Question 10.05.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 06: Audio Production Wave boundary

**Question 10.06.** Does `src/Audio/AudioManager.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 07: Audio Production Wave boundary

**Question 10.07.** Does `src/Audio/AudioEventBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 08: Audio Production Wave boundary

**Question 10.08.** Does `src/Audio/ExpansionAudioBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 09: Audio Production Wave boundary

**Question 10.09.** Does `src/Audio/ShelterAcousticBridge.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 10: Audio Production Wave boundary

**Question 10.10.** Does `src/Audio/AudioSettings.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 11: Audio Production Wave boundary

**Question 10.11.** Does `Assets/Ashfall.Core/Audio/ShelterAudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
...
## Audit cycle 10, lens 12: Audio Production Wave boundary

**Question 10.12.** Does `src/Audio/AudioCueCatalog.cs` remain the narrowest valid seam for this concern, or is the proposed feature only a projection over another owner? The implementation must answer with a source-level call path, a current public contract, a setup/lifecycle binding and a save answer. A class declaration, a test fixture or a panel name is not sufficient.
