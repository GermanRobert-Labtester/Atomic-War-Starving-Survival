# Plan 103 — Foundry Treaty Consequences, Stance and Market Modifiers

> **Rebuild status:** COMPLETE 15-POLICY DATA/POLICY LOOP — ASSESSMENT, DISPATCH AND MARKET-ATTRIBUTION AUDIT
>
...
- The current `foundry_treaty_consequences.json` has 15 policies across the current treaty set, each with a standing delta and optional market demand modifiers. `SilentFoundryConsequencePolicyCatalog` validates unique treaty/outcome keys and known outcomes.
- The live route is treaty ratification/compliance state in `SilentFoundrySystem` → policy lookup → `FoundryConsequenceRecord` → `ExpansionHubSave` consequence ledger → host `SilentFoundryHostSession` applying faction stance and existing market modifiers.
- The plan expands the architecture around exactly-once cycle markers, correct `violated` semantics, bounded modifiers, restore-before-apply behavior, and visible player feedback. It does not reinterpret policy prose as a new labor or market authority.

**Bounded outcome:** Retire the old 6→15 pure-data brief and correct its vocabulary: the current Core policy uses `met`, `missed`, and `violated`, not a speculative `breached` outcome. The 15-policy catalog, policy loader, Silent Foundry consequence state, host surface application and existing expansion-hub save are live. The remaining plan is a treaty-cycle/idempotence/market/standing audit, not more policies or a second treaty manager.
...
- `foundry_treaty_consequences.json` is valid schema version 1 with 15 `policies`; current data uses `outcome` values `met`, `missed`, and `violated` and references market goods through `market_modifiers`.
- `SilentFoundryConsequencePolicy` defines the typed outcome enum, policy DTOs, durable `FoundryConsequenceState`, and catalog validation; `KnownOutcomes` is `met`, `missed`, `violated`.
- `SilentFoundrySystem` binds treaty ratification anchors and policy, records treaty compliance and captures consequence state; the host applies the resulting record to `FactionStanceEngine` and `MarketSystem` through existing surfaces.
- Historical DEC-268 focused tests cover 15 policies and integration with the journal/treaty family; this plan does not claim a fresh runtime pass.
...
- C7/C11 cluster: treaty outcome, faction stance, market modifiers and foundry production are separate owners joined by a host adapter.
...
- Replace the old 6→15/“breached” language with a 15-policy current census and exact `met`/`missed`/`violated` vocabulary.
- Trace treaty assessment day/cycle marker → policy lookup → exactly-once record → faction stance and market modifier application.
- Audit the current expansion-hub restore ordering so a record is marked applied before downstream effects are projected or re-applied.
- Preserve authored magnitudes and reasons; do not tune treaty economy in this documentation package.
...
| --- | --- | --- | --- |
| policy parsing, validation and treaty/outcome lookup | SilentFoundryConsequencePolicyCatalog | `Assets/Ashfall.Core/Foundry/SilentFoundryConsequencePolicy.cs` | Owns static policy semantics and errors; it does not apply a modifier. |
| treaty compliance, consequence records and durable foundry state | SilentFoundrySystem | `Assets/Ashfall.Core/Foundry/SilentFoundrySystem.cs; Assets/Ashfall.Core/Foundry/SilentFoundrySystem.TreatyLabor.cs` | Owns assessment/cycle and idempotency state. |
| consequence ledger persistence | ExpansionHubSave | `Assets/Ashfall.Core/ExpansionHubSave.cs` | Existing envelope carries foundry and consequence state; no new section. |
| stance/market surface application | SilentFoundryHostSession | `src/Foundry/SilentFoundryHostSession.cs` | Thin adapter; it writes existing faction/market owners, not policy state. |
| policy outcome presentation | Foundry panel/host UI | `src/UI/SilentFoundryPanel.cs; src/Main.UiTests.SilentFoundry.cs` | Presentation and diagnostic surface; no policy authority. |
| policy, treaty and host consequence proof | Foundry focused tests | `Ashfall.Core.Tests/FoundryTreatyConsequenceExpansionTests.cs; Ashfall.Core.Tests/Narrative/Plan95_103JournalTreatyIntegrationTests.cs; Ashfall.Core.Tests/SilentFoundryConsequenceTests.cs` | Focused evidence surface. |
...
Authored JSON / current persisted owner state
                ...
                ▼
┌──────────────────────────────────────────────────────────────┐
│ Foundry Treaty Consequences, Stance and Market Modifiers
│ Integration route: extend current seams; no parallel authority │
└──────────────────────────────────────────────────────────────┘
│ SilentFoundryConsequencePolicyCatalog
│   policy parsing, validation and treaty/outcome lookup
│ SilentFoundrySystem
│   treaty compliance, consequence records and durable foundry state
│ ExpansionHubSave
│   consequence ledger persistence
│ SilentFoundryHostSession
│   stance/market surface application
│ Foundry panel/host UI
│   policy outcome presentation
                │
                ▼
...
1. **Preserve current state ownership.** SilentFoundryConsequencePolicyCatalog owns policy parsing, validation and treaty/outcome lookup: Owns static policy semantics and errors; it does not apply a modifier.
...
1. load the 15 policy rows and current accord ratification anchors
2. bind policy catalog to SilentFoundrySystem
3. assess treaty outcome on the current cycle day
4. look up exact treaty/outcome policy
5. check the durable applied marker
6. record the consequence and net standing once
7. host applies faction stance and market modifiers
8. capture/restore the existing expansion hub envelope and refresh the panel
...
- Policy rows are immutable; `SilentFoundryConsequenceState.applied` is the exactly-once ledger and `guildStanding` is the authoritative net policy standing.
- A treaty/cycle marker is applied at most once; reloading the same save must not apply standing or market modifiers twice.
- Outcome values are finite and known; an unknown outcome is a validation error, not a fallback to success.
- Market modifiers are bounded authored deltas applied through the existing MarketSystem owner; they are not copied into a foundry-only price table.
...
- Every policy key is unique by treaty/outcome and every outcome is one of the three current tokens.
- A repeated assessment for the same treaty/cycle returns the existing record without double application.
- A missing policy records the treaty outcome but does not fabricate a modifier or standing delta.
- Restore replays the saved net standing with `SetTrust`-style semantics rather than applying each historical record again.
...
- `foundry_treaty_consequences.json` is the sole policy authority.
- Do not create a second consequence catalog or convert `violated` into an undocumented `breached` synonym.
- New policy rows require current treaty/faction/good references, a known outcome, bounded modifiers and a host consumer.
...
- Use `ExpansionHubSave` current version and `SilentFoundryConsequenceState`; no new foundry-consequence section.
- Capture the applied records, cycle markers, modifiers and guild standing together.
- Legacy saves with no consequence ledger restore neutral and do not retroactively apply historical policy.
...
- Policy lookup and list order are ordinal; no random policy selection.
- Foundry RNG remains the existing production stream; consequence assessment is deterministic from treaty state/day.
- Paired runs compare policy key, cycle marker, record, standing and market modifier deltas.
...
- Foundry emits typed treaty quota/met/missed and consequence-applied facts after owner state commits.
- The host consumes `FoundryConsequenceRecord` to update existing faction stance and market surfaces.
- Journal/audio/presentation facts are downstream and must not become the source of policy state.
...
- Treaty consequences should identify material cause and cost without turning labor harm into spectacle.
- Use the current fictional faction/treaty canon and avoid real-world political or legal imitation.
- A policy reason is explanatory metadata; it cannot override the structured deltas.
...
| --- | --- | --- | --- |
| F-01 | An old `breached` token is introduced into current data. | SilentFoundryConsequencePolicyCatalog | Focused negative test or static source gate; no broad-suite dependency. |
| F-02 | The same cycle applies a market modifier twice after reload. | SilentFoundrySystem | Focused negative test or static source gate; no broad-suite dependency. |
| F-03 | A missing good reference causes a partial host mutation. | ExpansionHubSave | Focused negative test or static source gate; no broad-suite dependency. |
| F-04 | A panel displays a consequence the ledger did not apply. | SilentFoundryHostSession | Focused negative test or static source gate; no broad-suite dependency. |
| F-05 | A new policy store duplicates ExpansionHubSave. | Foundry panel/host UI | Focused negative test or static source gate; no broad-suite dependency. |
...
1. `bash scripts/run_test.sh Ashfall.Core.Tests/FoundryTreatyConsequenceExpansionTests.cs`
2. `bash scripts/run_test.sh Ashfall.Core.Tests/Narrative/Plan95_103JournalTreatyIntegrationTests.cs`
3. `bash scripts/run_test.sh Ashfall.Core.Tests/SilentFoundryConsequenceTests.cs`
...
| --- | --- | --- | --- |
| 0 — policy census | Read 15 rows, current enum, loader, system, host and save. | Vocabulary and owner are exact. | No production path until the owning implementation package is separately claimed. |
| 1 — cycle/replay trace | Exercise met/missed/violated, duplicate cycles and restore. | Exactly-once effects are proven. | No production path until the owning implementation package is separately claimed. |
| 2 — surface audit | Trace stance/market/journal/panel effects. | Every visible effect has a current owner. | No production path until the owning implementation package is separately claimed. |
| 3 — precision/tone pass | Review missing-policy behavior, migration and restrained feedback. | No duplicate authority or unsupported balance claim. | No production path until the owning implementation package is separately claimed. |
...
| --- | --- | --- |
| Assets/StreamingAssets/Data/foundry_treaty_consequences.json | READ ONLY; MODIFY only for a proven policy/reference gap | 15-row authority |
| Assets/Ashfall.Core/Foundry/SilentFoundryConsequencePolicy.cs | READ ONLY | Policy owner |
| Assets/Ashfall.Core/ExpansionHubSave.cs | READ ONLY | Existing consequence persistence |
| src/Foundry/SilentFoundryHostSession.cs | READ ONLY; MODIFY only under a new host claim | Surface adapter |
...
| Risk | Control |
| --- | --- |
| Using stale outcome vocabulary. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |
| Applying a record before its durable marker is safely captured. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |
| Copying market prices into the policy catalog. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |
| Changing historical balance numbers without a balance package. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |

# 22. Explicit Non-Goals

- No new policy rows in this package.
- No new foundry/treaty manager.
- No new save section.
...
- Future policy/host changes retain the prior valid JSON, expansion-hub fixture and replay tests.
...
- 15 current policies and exact outcome vocabulary are documented.
- Cycle idempotence, restore, stance and market boundaries are explicit.
- No stale `breached` path or parallel policy owner remains in the handoff.
...
0 — policy census — re-read the current owner APIs, reproduce the focused baseline, and write down any premise that differs from this plan. Stop with `STALE_PLAN` if the owner, save path or data contract has moved.


# Appendix A — Contract Clause Library

**Authority clause — concern-specific ownership.** Every mutable value named in this plan is governed by the owner shown for that concern in the matrix: policy parsing, validation and treaty/outcome lookup → SilentFoundryConsequencePolicyCatalog; treaty compliance, consequence records and durable foundry state → SilentFoundrySystem; consequence ledger persistence → ExpansionHubSave; stance/market surface application → SilentFoundryHostSession; policy outcome presentation → Foundry panel/host UI; policy, treaty and host consequence proof → Foundry focused tests. A host object, panel, derived snapshot, generated document or test fixture is not an authority merely because it contains the same field name.

**Data clause.** A row in `Assets/StreamingAssets/Data/` is authored content, not reachability. The row must resolve to a loader, validator rule, runtime consumer, player or host command, and test surface. This applies to every catalog listed for Plan 103.

**Save clause.** Capture must be deep, restore must normalize only documented legacy absence, and a checksum must change when authoritative state changes. Plan 103 does not authorize a new save section when an existing owner can carry the fact.
...
**UI clause.** The interface reads the current owner projection, previews a real command and renders named refusals. It must not recompute state owned by SilentFoundryConsequencePolicyCatalog or any other authority, hide uncertainty, or introduce a gameplay-only counter.
...
# Appendix B.02 — Current Code Architecture: `Assets/Ashfall.Core/Foundry/SilentFoundryConsequencePolicy.cs`
...
# Appendix B.04 — Current Code Architecture: `Assets/Ashfall.Core/Foundry/SilentFoundrySystem.TreatyLabor.cs`
...
# Appendix B.06 — Current Code Architecture: `src/Foundry/SilentFoundryHostSession.cs`
...
# Appendix B.07 — Current Code Architecture: `src/UI/SilentFoundryPanel.cs`
...
# Appendix B.08 — Current Code Architecture: `src/Main.UiTests.SilentFoundry.cs`
...
# Appendix C.09 — Catalog Census: `Assets/StreamingAssets/Data/foundry_treaty_consequences.json`
...
# Appendix C.10 — Catalog Census: `Assets/StreamingAssets/Data/foundry_accords.json`
...
# Appendix C.11 — Catalog Census: `Assets/StreamingAssets/Data/items.json`
...
# Appendix D.12 — Existing Focused Test Inventory: `Ashfall.Core.Tests/FoundryTreatyConsequenceExpansionTests.cs`
...
# Appendix D.13 — Existing Focused Test Inventory: `Ashfall.Core.Tests/Narrative/Plan95_103JournalTreatyIntegrationTests.cs`

### `Ashfall.Core.Tests/Narrative/Plan95_103JournalTreatyIntegrationTests.cs`

- Current test declarations: Fact=4, Theory=0, InlineData=0.
- File lines: 182; SHA-256: `f15c60bd77cc2e1b4ebdd4704c163b33c0d4db813bfcab6f0eebc99a265b228b`.
- The list below is an inventory, not a newly executed pass result. Focused tests must be rerun when the owning implementation changes.

```text
Plan95_JournalVoiceProseCatalog_LoadsAllSituationKeys_WithDistinctBiasVariants
Plan103_FoundryTreatyConsequences_LoadsAllFifteenPolicies_WithValidSignatoriesAndModifiers
CrossSystem_TreatyConsequenceOutcomesAndJournalVoices_ExhibitSystemicResonance
CrossSystem_DeterministicExecution_UnderRepeatedReloadsPasses
```


# Appendix D.14 — Existing Focused Test Inventory: `Ashfall.Core.Tests/SilentFoundryConsequenceTests.cs`
...
# Appendix E.15 — Supporting Code Evidence: `Assets/Ashfall.Core/Foundry/SilentFoundryConsequencePolicy.cs`
...
# Appendix E.17 — Supporting Code Evidence: `Assets/Ashfall.Core/Foundry/SilentFoundrySystem.TreatyLabor.cs`
...
# Appendix E.19 — Supporting Code Evidence: `src/Foundry/SilentFoundryHostSession.cs`
...
# Appendix G.20 — Supporting Regression Evidence: `Ashfall.Core.Tests/FoundryTreatyConsequenceExpansionTests.cs`
...
# Appendix G.21 — Supporting Regression Evidence: `Ashfall.Core.Tests/Narrative/Plan95_103JournalTreatyIntegrationTests.cs`
...
# Appendix G.22 — Supporting Regression Evidence: `Ashfall.Core.Tests/SilentFoundryConsequenceTests.cs`
...
| --- | --- | --- | --- | --- |
| policy parsing, validation and treaty/outcome lookup | SilentFoundryConsequencePolicyCatalog | treaty compliance, consequence records and durable foundry state | SilentFoundrySystem | Owner emits/reads a typed fact; no mirror state. |
| policy parsing, validation and treaty/outcome lookup | SilentFoundryConsequencePolicyCatalog | consequence ledger persistence | ExpansionHubSave | Owner emits/reads a typed fact; no mirror state. |
| policy parsing, validation and treaty/outcome lookup | SilentFoundryConsequencePolicyCatalog | stance/market surface application | SilentFoundryHostSession | Owner emits/reads a typed fact; no mirror state. |
| policy parsing, validation and treaty/outcome lookup | SilentFoundryConsequencePolicyCatalog | policy outcome presentation | Foundry panel/host UI | Owner emits/reads a typed fact; no mirror state. |
| policy parsing, validation and treaty/outcome lookup | SilentFoundryConsequencePolicyCatalog | policy, treaty and host consequence proof | Foundry focused tests | Owner emits/reads a typed fact; no mirror state. |
| treaty compliance, consequence records and durable foundry state | SilentFoundrySystem | policy parsing, validation and treaty/outcome lookup | SilentFoundryConsequencePolicyCatalog | Owner emits/reads a typed fact; no mirror state. |
| treaty compliance, consequence records and durable foundry state | SilentFoundrySystem | consequence ledger persistence | ExpansionHubSave | Owner emits/reads a typed fact; no mirror state. |
| treaty compliance, consequence records and durable foundry state | SilentFoundrySystem | stance/market surface application | SilentFoundryHostSession | Owner emits/reads a typed fact; no mirror state. |
| treaty compliance, consequence records and durable foundry state | SilentFoundrySystem | policy outcome presentation | Foundry panel/host UI | Owner emits/reads a typed fact; no mirror state. |
| treaty compliance, consequence records and durable foundry state | SilentFoundrySystem | policy, treaty and host consequence proof | Foundry focused tests | Owner emits/reads a typed fact; no mirror state. |
| consequence ledger persistence | ExpansionHubSave | policy parsing, validation and treaty/outcome lookup | SilentFoundryConsequencePolicyCatalog | Owner emits/reads a typed fact; no mirror state. |
| consequence ledger persistence | ExpansionHubSave | treaty compliance, consequence records and durable foundry state | SilentFoundrySystem | Owner emits/reads a typed fact; no mirror state. |
| consequence ledger persistence | ExpansionHubSave | stance/market surface application | SilentFoundryHostSession | Owner emits/reads a typed fact; no mirror state. |
| consequence ledger persistence | ExpansionHubSave | policy outcome presentation | Foundry panel/host UI | Owner emits/reads a typed fact; no mirror state. |
| consequence ledger persistence | ExpansionHubSave | policy, treaty and host consequence proof | Foundry focused tests | Owner emits/reads a typed fact; no mirror state. |
| stance/market surface application | SilentFoundryHostSession | policy parsing, validation and treaty/outcome lookup | SilentFoundryConsequencePolicyCatalog | Owner emits/reads a typed fact; no mirror state. |
| stance/market surface application | SilentFoundryHostSession | treaty compliance, consequence records and durable foundry state | SilentFoundrySystem | Owner emits/reads a typed fact; no mirror state. |
| stance/market surface application | SilentFoundryHostSession | consequence ledger persistence | ExpansionHubSave | Owner emits/reads a typed fact; no mirror state. |
| stance/market surface application | SilentFoundryHostSession | policy outcome presentation | Foundry panel/host UI | Owner emits/reads a typed fact; no mirror state. |
| stance/market surface application | SilentFoundryHostSession | policy, treaty and host consequence proof | Foundry focused tests | Owner emits/reads a typed fact; no mirror state. |
| policy outcome presentation | Foundry panel/host UI | policy parsing, validation and treaty/outcome lookup | SilentFoundryConsequencePolicyCatalog | Owner emits/reads a typed fact; no mirror state. |
| policy outcome presentation | Foundry panel/host UI | treaty compliance, consequence records and durable foundry state | SilentFoundrySystem | Owner emits/reads a typed fact; no mirror state. |
| policy outcome presentation | Foundry panel/host UI | consequence ledger persistence | ExpansionHubSave | Owner emits/reads a typed fact; no mirror state. |
| policy outcome presentation | Foundry panel/host UI | stance/market surface application | SilentFoundryHostSession | Owner emits/reads a typed fact; no mirror state. |
| policy outcome presentation | Foundry panel/host UI | policy, treaty and host consequence proof | Foundry focused tests | Owner emits/reads a typed fact; no mirror state. |
| policy, treaty and host consequence proof | Foundry focused tests | policy parsing, validation and treaty/outcome lookup | SilentFoundryConsequencePolicyCatalog | Owner emits/reads a typed fact; no mirror state. |
| policy, treaty and host consequence proof | Foundry focused tests | treaty compliance, consequence records and durable foundry state | SilentFoundrySystem | Owner emits/reads a typed fact; no mirror state. |
| policy, treaty and host consequence proof | Foundry focused tests | consequence ledger persistence | ExpansionHubSave | Owner emits/reads a typed fact; no mirror state. |
| policy, treaty and host consequence proof | Foundry focused tests | stance/market surface application | SilentFoundryHostSession | Owner emits/reads a typed fact; no mirror state. |
| policy, treaty and host consequence proof | Foundry focused tests | policy outcome presentation | Foundry panel/host UI | Owner emits/reads a typed fact; no mirror state. |
...
| --- | --- | --- | --- |
| R-01 | Replace the old 6→15/“breached” language with a 15-policy current census and exact `met`/`missed`/`violated` vocabulary. | Focused negative/edge test; host/runtime proof where player-visible. | BLOCKED if no named owner can accept the fact. |
| R-02 | Trace treaty assessment day/cycle marker → policy lookup → exactly-once record → faction stance and market modifier application. | Focused negative/edge test; host/runtime proof where player-visible. | BLOCKED if no named owner can accept the fact. |
| R-03 | Audit the current expansion-hub restore ordering so a record is marked applied before downstream effects are projected or re-applied. | Focused negative/edge test; host/runtime proof where player-visible. | BLOCKED if no named owner can accept the fact. |
| R-04 | Preserve authored magnitudes and reasons; do not tune treaty economy in this documentation package. | Focused negative/edge test; host/runtime proof where player-visible. | BLOCKED if no named owner can accept the fact. |
...
The subject is a data-driven treaty consequence with exactly-once application across Core policy, foundry state, host surfaces and the existing expansion save. The plan corrects stale outcome terminology and protects the single consequence ledger.

- **policy parsing, validation and treaty/outcome lookup** remains with `SilentFoundryConsequencePolicyCatalog` at `Assets/Ashfall.Core/Foundry/SilentFoundryConsequencePolicy.cs`. Owns static policy semantics and errors; it does not apply a modifier.
- **treaty compliance, consequence records and durable foundry state** remains with `SilentFoundrySystem` at `Assets/Ashfall.Core/Foundry/SilentFoundrySystem.cs; Assets/Ashfall.Core/Foundry/SilentFoundrySystem.TreatyLabor.cs`. Owns assessment/cycle and idempotency state.
- **consequence ledger persistence** remains with `ExpansionHubSave` at `Assets/Ashfall.Core/ExpansionHubSave.cs`. Existing envelope carries foundry and consequence state; no new section.
- **stance/market surface application** remains with `SilentFoundryHostSession` at `src/Foundry/SilentFoundryHostSession.cs`. Thin adapter; it writes existing faction/market owners, not policy state.
- **policy outcome presentation** remains with `Foundry panel/host UI` at `src/UI/SilentFoundryPanel.cs; src/Main.UiTests.SilentFoundry.cs`. Presentation and diagnostic surface; no policy authority.
- **policy, treaty and host consequence proof** remains with `Foundry focused tests` at `Ashfall.Core.Tests/FoundryTreatyConsequenceExpansionTests.cs; Ashfall.Core.Tests/Narrative/Plan95_103JournalTreatyIntegrationTests.cs; Ashfall.Core.Tests/SilentFoundryConsequenceTests.cs`. Focused evidence surface.
...
| --- | --- | --- | --- | --- | --- |
| S-01 | 103-01 load 15 policies | load the 15 policy rows and current accord ratification anchors | Policy rows are immutable; `SilentFoundryConsequenceState.applied` is the exactly-once ledger and `guildStanding` is the authoritative net policy standing. | An old `breached` token is introduced into current data. | SilentFoundryConsequencePolicyCatalog |
| S-02 | 103-02 met outcome | bind policy catalog to SilentFoundrySystem | A treaty/cycle marker is applied at most once; reloading the same save must not apply standing or market modifiers twice. | The same cycle applies a market modifier twice after reload. | SilentFoundryConsequencePolicyCatalog |
| S-03 | 103-03 missed outcome | assess treaty outcome on the current cycle day | Outcome values are finite and known; an unknown outcome is a validation error, not a fallback to success. | A missing good reference causes a partial host mutation. | SilentFoundryConsequencePolicyCatalog |
| S-04 | 103-04 violated outcome | look up exact treaty/outcome policy | Market modifiers are bounded authored deltas applied through the existing MarketSystem owner; they are not copied into a foundry-only price table. | A panel displays a consequence the ledger did not apply. | SilentFoundryConsequencePolicyCatalog |
| S-05 | 103-05 unknown outcome rejection | check the durable applied marker | Policy rows are immutable; `SilentFoundryConsequenceState.applied` is the exactly-once ledger and `guildStanding` is the authoritative net policy standing. | A new policy store duplicates ExpansionHubSave. | SilentFoundryConsequencePolicyCatalog |
| S-06 | 103-06 duplicate treaty/cycle | record the consequence and net standing once | A treaty/cycle marker is applied at most once; reloading the same save must not apply standing or market modifiers twice. | An old `breached` token is introduced into current data. | SilentFoundryConsequencePolicyCatalog |
| S-07 | 103-07 missing good reference | host applies faction stance and market modifiers | Outcome values are finite and known; an unknown outcome is a validation error, not a fallback to success. | The same cycle applies a market modifier twice after reload. | SilentFoundryConsequencePolicyCatalog |
| S-08 | 103-08 restore before apply | capture/restore the existing expansion hub envelope and refresh the panel | Market modifiers are bounded authored deltas applied through the existing MarketSystem owner; they are not copied into a foundry-only price table. | A missing good reference causes a partial host mutation. | SilentFoundryConsequencePolicyCatalog |
| S-09 | 103-09 host stance/market projection | load the 15 policy rows and current accord ratification anchors | Policy rows are immutable; `SilentFoundryConsequenceState.applied` is the exactly-once ledger and `guildStanding` is the authoritative net policy standing. | A panel displays a consequence the ledger did not apply. | SilentFoundryConsequencePolicyCatalog |
| S-10 | 103-10 legacy neutral consequence state | bind policy catalog to SilentFoundrySystem | A treaty/cycle marker is applied at most once; reloading the same save must not apply standing or market modifiers twice. | A new policy store duplicates ExpansionHubSave. | SilentFoundryConsequencePolicyCatalog |
...
| --- | --- | --- | --- | --- |
| T-01 | 103-TC-01 schema/policy key uniqueness | data | schema/policy key uniqueness; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
| T-02 | 103-TC-02 known outcome validation | unit | known outcome validation; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
| T-03 | 103-TC-03 treaty reference check | persistence | treaty reference check; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
| T-04 | 103-TC-04 market good reference check | determinism | market good reference check; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
| T-05 | 103-TC-05 bounded modifier range | host | bounded modifier range; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
| T-06 | 103-TC-06 met lookup | UI/accessibility | met lookup; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
| T-07 | 103-TC-07 missed lookup | cross-system | missed lookup; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
| T-08 | 103-TC-08 violated lookup | data | violated lookup; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
| T-09 | 103-TC-09 duplicate cycle marker | unit | duplicate cycle marker; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
| T-10 | 103-TC-10 missing policy no mutation | persistence | missing policy no mutation; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
| T-11 | 103-TC-11 guild standing restore | determinism | guild standing restore; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
| T-12 | 103-TC-12 host FactionStance application | host | host FactionStance application; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
| T-13 | 103-TC-13 host MarketSystem application | UI/accessibility | host MarketSystem application; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
| T-14 | 103-TC-14 no double journal/application | cross-system | no double journal/application; verify the current owner and its negative boundary without inventing a second authority. | SilentFoundryConsequencePolicyCatalog |
...
| --- | --- | --- |
| 56 | `Assets/Ashfall.Core/ExpansionHubSave.cs` | current reference count; inspect the caller before treating it as a live route |
| 49 | `Ashfall.Core.Tests/SilentFoundryConsequenceTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 25 | `Ashfall.Core.Tests/SilentFoundrySystemTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 23 | `Assets/Ashfall.Core/Foundry/SilentFoundrySystem.cs` | current reference count; inspect the caller before treating it as a live route |
| 23 | `src/Foundry/SilentFoundryHostSession.cs` | current reference count; inspect the caller before treating it as a live route |
| 19 | `Ashfall.Core.Tests/EconomyProbeTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 18 | `src/Host/HostCli.PanelTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 17 | `Ashfall.Core.Tests/ExpansionHubSaveV5Tests.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 16 | `Ashfall.Core.Tests/GapTestCoverageTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 16 | `src/Host/ExpansionHubSaveStore.cs` | current reference count; inspect the caller before treating it as a live route |
| 15 | `Ashfall.Core.Tests/FoundryTreatyConsequenceExpansionTests.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 13 | `Assets/Ashfall.Core/Foundry/SilentFoundryConsequencePolicy.cs` | current reference count; inspect the caller before treating it as a live route |
| 12 | `Ashfall.Core.Tests/Foundry/Plan213MetallurgyReconciliationTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 12 | `Ashfall.Core.Tests/Plan56Phase5Tests.cs` | current reference count; inspect the caller before treating it as a live route |
| 12 | `Assets/Ashfall.Core/Foundry/SilentFoundryHeadlessDemo.cs` | current reference count; inspect the caller before treating it as a live route |
| 11 | `Ashfall.Core.Tests/FlagshipEconomyScenarioTests.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 11 | `Assets/Ashfall.Core/Content/ContentUtilizationScanner.cs` | current reference count; inspect the caller before treating it as a live route |
| 10 | `Ashfall.Core.Tests/ExpansionHubSaveTests.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 9 | `Ashfall.Core.Tests/MarketAdapterProbeTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 8 | `Ashfall.Core.Tests/EconomySystemTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 8 | `Assets/Ashfall.Core/Foundry/SilentFoundrySystem.TreatyLabor.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 7 | `Ashfall.Core.Tests/Narrative/Plan95_103JournalTreatyIntegrationTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 7 | `Assets/Ashfall.Core/Content/ContentDeepChainGate.cs` | current reference count; inspect the caller before treating it as a live route |
...
# Appendix — Complete Catalog Audit: `Assets/StreamingAssets/Data/foundry_treaty_consequences.json`

### `Assets/StreamingAssets/Data/foundry_treaty_consequences.json` — complete current row audit

- Parse status: **valid JSON**.
- Bytes: 10615; characters: 10615.
- SHA-256: `8f9a0cffa4e257f13fa06c962c3bad29bf716f93a12e78b08e8da7f074861f16`.
- Row presence is not reachability. Each row below is an authored fact requiring a loader/consumer check.

Root keys: `schema_version`, `collection_id`, `policies`

#### `policies` — 15 current rows

- Row 001 `treaty_brine_pipe_and_iodine_exchange`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":-0.4,"good_id":"item_foundry_brine_pipe","reason":"Compliance restores the exchange lane; the foundry's pipe stock clears at normal terms again."},{"demand_delta":-…`
- Row 002 `treaty_brine_pipe_and_iodine_exchange`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":0.4,"good_id":"item_foundry_brine_pipe","reason":"The Office takes brine pipe where it can find it; the foundry's own stock sits unsold at softer terms."},{"demand_…`
- Row 003 `treaty_cluster_labour_schedule`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":-0.25,"good_id":"fuel","reason":"The Office and the Cutters release the works' fuel allotment again after compliance."}],"outcome":"met","reason":"Labour schedule u…`
- Row 004 `treaty_cluster_labour_schedule`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":0.25,"good_id":"fuel","reason":"Lockouts and unannounced strikes close the works' coal window; fuel demand rises as the Cluster scrambles for replacement heat."}],"…`
- Row 005 `treaty_road_iron_charter`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":-0.2,"good_id":"coal","reason":"The coal column runs to the marked lane again; coal terms ease back."},{"demand_delta":-0.3,"good_id":"item_foundry_ice_anchor","rea…`
- Row 006 `treaty_road_iron_charter`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":0.2,"good_id":"coal","reason":"The coal column is reprioritized to the road's own needs; coal for the Cluster becomes scarcer and dearer."},{"demand_delta":0.3,"goo…`
- Row 007 `treaty_saltworks_access`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":-0.2,"good_id":"clean_water","reason":"The saltworks draw is measured and serviced; clean-water demand eases while the access lane remains open."},{"demand_delta":-…`
- Row 008 `treaty_saltworks_access`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":0.35,"good_id":"clean_water","reason":"The measured draw is suspended pending inspection; replacement clean water becomes scarcer along the pipe walk."},{"demand_de…`
- Row 009 `treaty_coal_window`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":-0.25,"good_id":"coal","reason":"A cleared haul window prevents emergency coal buying and returns the column to ordinary terms."},{"demand_delta":-0.15,"good_id":"f…`
- Row 010 `treaty_coal_window`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":0.3,"good_id":"coal","reason":"The Foundry must source coal outside the marked window; short-haul scarcity raises the ordinary price."},{"demand_delta":0.15,"good_i…`
- Row 011 `treaty_membrane_repair`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":-0.2,"good_id":"item_foundry_brine_pipe","reason":"Accepted repair work clears the emergency replacement queue for brine-pipe components."},{"demand_delta":-0.15,"g…`
- Row 012 `treaty_membrane_repair`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":0.35,"good_id":"item_foundry_brine_pipe","reason":"Unaccepted repair work keeps replacement pipe stock in dispute and pushes demand into the open market."},{"demand…`
- Row 013 `treaty_crisis_mutual_aid`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":-0.2,"good_id":"clean_water","reason":"Logged emergency deliveries keep relief water moving without an unpriced panic premium."},{"demand_delta":-0.2,"good_id":"fue…`
- Row 014 `treaty_crisis_mutual_aid`: `{"faction_id":"faction_silent_foundry","market_modifiers":[{"demand_delta":0.4,"good_id":"clean_water","reason":"Relief water is withheld during the declared crisis; emergency stores become scarce until another column is found."},{"demand_…`
- Row 015 `treaty_the_incident_book`: `{"faction_id":"faction_silent_foundry","market_modifiers":[],"outcome":"met","reason":"Incident Book obligation met: the qualifying entry, copy receipt, and amendment trail are accepted before the next charter sitting.","standing_delta":2.…`
