...
- The current `military_faction_branch.json` contains 15 branches with entry morality bands, PoNR flags/triggers and two ending rows each. The current host can load and save the combined Weight of Choices envelope.
- The live route is catalog → `MilitaryBranchSystem` → `FactionBranchCoordinator` → `WeightOfChoicesSave` v2 → `FactionBranchHostSession`/Factions and Quests surfaces. Moral choice is an input gate; military alignment is a distinct faction-owned value.
- The plan’s depth is in making irreversible commitments, bounded alignment, durable flags, ending resolution and player-facing lockout semantics precise under reload and competing faction paths.

**Bounded outcome:** Retire the old 8→15 pure-data brief as a new authoring project. The current military catalog has 15 branches, the Core branch state/save codec and the combined FactionBranchCoordinator/host session are live, and historical DEC-288 integrated the set. The remaining plan is a precise branch-reachability, alignment, mutual-exclusion and migration audit—not another military manager or more rows.
...
- `military_faction_branch.json` is valid schema version 1 with 15 `branches`; the historical closeout identifies 15 PoNR flags and 30 endings.
- `MilitaryBranchSystem` owns timeline, committed branch, PoNR lock, military alignment and ending resolution; it reads `MoralChoiceSystem.CurrentBand` and `IFlagLedger` without owning either.
- `FactionBranchCoordinator` composes all three faction branch systems and enforces mutually exclusive commitment; `WeightOfChoicesSave` is current v2 with a frozen v1 migration.
- `FactionBranchHostSession` and `Main.FactionBranch` provide a live host/save route; current panel truthfulness and all player entry points still require focused audit.
...
- C7 Factions/war cluster: military branch commitment, moral gate, alignment and faction standing remain distinct.
...
- Replace the 8→15 target with a 15-row branch/flag/ending census and a current coverage matrix.
- Trace availability, commitment, PoNR, alignment, ending and cross-faction exclusivity through the combined coordinator.
- Audit WeightOfChoices v1→v2 migration, durable flag replay and future-version rejection.
- Preserve no-new-state/no-new-save boundaries and keep UI claims shared/integrator-owned.
...
| --- | --- | --- | --- |
| military branch definitions and ordered lookup | MilitaryBranchCatalog | `Assets/Ashfall.Core/Factions/MilitaryBranchCatalog.cs` | Immutable catalog after load. |
| military branch state, alignment, PoNR and ending | MilitaryBranchSystem | `Assets/Ashfall.Core/Factions/MilitaryBranchSystem.cs` | Sole military branch owner; reads moral/flag ports. |
| cross-faction exclusivity and active branch state | FactionBranchCoordinator | `Assets/Ashfall.Core/Factions/FactionBranchCoordinator.cs` | Owns combined branch lifecycle. |
| combined military/rebel/independent/PRPF persistence and migration | WeightOfChoicesSaveCodec | `Assets/Ashfall.Core/Factions/WeightOfChoicesSave.cs; Assets/Ashfall.Core/Factions/MilitaryBranchSave.cs` | Existing envelope; individual codec is a component, not a second live store. |
| host commands, lifecycle and panel route | FactionBranchHostSession/Main.FactionBranch | `src/Host/FactionBranchHostSession.cs; src/Main.FactionBranch.cs; src/UI/FactionsPanel.cs` | Host/UI composition only. |
| catalog, coordinator and integration proof | Military focused tests | `Ashfall.Core.Tests/MilitaryBranchExpansionTests.cs; Ashfall.Core.Tests/Factions/Plan121_122FactionBranchIntegrationTests.cs; Ashfall.Core.Tests/MilitaryBranchSystemTests.cs` | Focused evidence surface. |
...
Authored JSON / current persisted owner state
                ...
                ▼
┌──────────────────────────────────────────────────────────────┐
│ Military Faction Branches, Martial Duty Dilemmas and Combined Save Safety
│ Integration route: extend current seams; no parallel authority │
└──────────────────────────────────────────────────────────────┘
│ MilitaryBranchCatalog
│   military branch definitions and ordered lookup
│ MilitaryBranchSystem
│   military branch state, alignment, PoNR and ending
│ FactionBranchCoordinator
│   cross-faction exclusivity and active branch state
│ WeightOfChoicesSaveCodec
│   combined military/rebel/independent/PRPF persistence and migration
│ FactionBranchHostSession/Main.FactionBranch
│   host commands, lifecycle and panel route
                │
                ▼
...
1. **Preserve current state ownership.** MilitaryBranchCatalog owns military branch definitions and ordered lookup: Immutable catalog after load.
...
1. load 15 military rows and other branch catalogs
2. restore combined WeightOfChoicesSave and replay flags
3. query availability with current moral/faction state
4. commit through coordinator exclusivity
5. lock PoNR once
6. apply bounded military alignment through the existing faction seam
7. resolve first matching ending and project it
8. save combined envelope and refresh surfaces
...
- Catalog rows are immutable; `MilitaryBranchSystemState` owns timeline, branch record, alignment and durable flags.
- Commitment is first-commit-wins, PoNR is irreversible/idempotent, and ending resolution is first-resolution-wins after lock.
- The combined save always contains all branch sections and rejects a multi-faction committed state.
- Alignment is clamped to the current -200..200 scale and is not the player morality score.
...
- A second faction commitment is refused even if the Military row itself is available.
- An out-of-band moral entry range returns a named reason and does not mutate state.
- Unknown branch/ending IDs are diagnosed rather than silently mapped during validation.
- Restored flags are immediately visible to same-session gates and historical effects are not replayed.
...
- `military_faction_branch.json` is the sole Military branch authority.
- No branch copy belongs in a panel, save file or second coordinator catalog.
- New rows require current producers, moral ranges, PoNR flags, ending ranges and host command coverage.
...
- Use `WeightOfChoicesSave` v2 and its frozen v1 migration; no new military-only section.
- Missing v1 Independent state defaults fresh/uncommitted; future versions fail closed.
- The individual `MilitaryBranchSaveCodec` can be tested as a component but must not be wired as a second live store beside the combined envelope.
...
- Branch and ending selection is catalog-order/ID based with no random selection.
- Moral band parsing and alignment clamps are deterministic.
- Replay compares active faction kind, branch, PoNR day, alignment, ending and flags.
...
- Coordinator forwards commitment, PoNR, ending and state-change facts.
- Military alignment changes are emitted by the branch owner and consumed by current faction surfaces.
- Panels project lockout/commitment status and never write branch state.
...
- Military branches should explore duty, conscience, discipline and survival without glorifying real-world violence or copying real institutions.
- The interface must let the player understand the cost before an irreversible commitment.
- Endings derive from current moral/faction state, not a UI-authored reputation score.
...
| --- | --- | --- | --- |
| F-01 | Military and Rebel commitments coexist after restore. | MilitaryBranchCatalog | Focused negative test or static source gate; no broad-suite dependency. |
| F-02 | A panel marks PoNR/ending without the coordinator. | MilitaryBranchSystem | Focused negative test or static source gate; no broad-suite dependency. |
| F-03 | Alignment exceeds its clamp or changes the moral score. | FactionBranchCoordinator | Focused negative test or static source gate; no broad-suite dependency. |
| F-04 | A v1 save loses the active military branch or duplicates its ending. | WeightOfChoicesSaveCodec | Focused negative test or static source gate; no broad-suite dependency. |
| F-05 | A new military coordinator duplicates the combined owner. | FactionBranchHostSession/Main.FactionBranch | Focused negative test or static source gate; no broad-suite dependency. |
...
1. `bash scripts/run_test.sh Ashfall.Core.Tests/MilitaryBranchExpansionTests.cs`
2. `bash scripts/run_test.sh Ashfall.Core.Tests/Factions/Plan121_122FactionBranchIntegrationTests.cs`
3. `bash scripts/run_test.sh Ashfall.Core.Tests/MilitaryBranchSystemTests.cs`
...
| --- | --- | --- | --- |
| 0 — census/collision | Read 15 rows, coordinator, host and combined save. | Current branch set/owners are proven. | No production path until the owning implementation package is separately claimed. |
| 1 — command trace | Trace availability, exclusivity, PoNR, alignment and ending. | One live route is documented. | No production path until the owning implementation package is separately claimed. |
| 2 — migration/replay | Exercise v1/v2, flags, conflict and repeated resolution. | No loss or double effect. | No production path until the owning implementation package is separately claimed. |
| 3 — UI/precision pass | Review Factions/Quests projection and irreversible language. | Residual work is bounded and truthful. | No production path until the owning implementation package is separately claimed. |
...
| --- | --- | --- |
| Assets/StreamingAssets/Data/military_faction_branch.json | READ ONLY; MODIFY only for a proven branch/consumer gap | 15-row authority |
...
| src/UI/FactionsPanel.cs | READ ONLY; MODIFY only under a new UI claim | Presentation |
...
| Risk | Control |
| --- | --- |
| Creating a second military coordinator/save. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |
| Conflating alignment with morality. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |
| Changing exclusivity while fixing UI. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |
| Using an individual codec as a second host store. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |

# 22. Explicit Non-Goals

- No new military branches in this package.
...
- Future coordinator/save changes retain frozen v1/v2 fixtures and focused tests.
...
- 15 current branches, PoNR flags and endings are documented.
- Combined exclusivity, alignment, migration and host route are explicit.
- No parallel military state/save owner is proposed.
...
**Authority clause — concern-specific ownership.** Every mutable value named in this plan is governed by the owner shown for that concern in the matrix: military branch definitions and ordered lookup → MilitaryBranchCatalog; military branch state, alignment, PoNR and ending → MilitaryBranchSystem; cross-faction exclusivity and active branch state → FactionBranchCoordinator; combined military/rebel/independent/PRPF persistence and migration → WeightOfChoicesSaveCodec; host commands, lifecycle and panel route → FactionBranchHostSession/Main.FactionBranch; catalog, coordinator and integration proof → Military focused tests. A host object, panel, derived snapshot, generated document or test fixture is not an authority merely because it contains the same field name.

**Data clause.** A row in `Assets/StreamingAssets/Data/` is authored content, not reachability. The row must resolve to a loader, validator rule, runtime consumer, player or host command, and test surface. This applies to every catalog listed for Plan 122.

**Save clause.** Capture must be deep, restore must normalize only documented legacy absence, and a checksum must change when authoritative state changes. Plan 122 does not authorize a new save section when an existing owner can carry the fact.
...
**UI clause.** The interface reads the current owner projection, previews a real command and renders named refusals. It must not recompute state owned by MilitaryBranchCatalog or any other authority, hide uncertainty, or introduce a gameplay-only counter.
...
# Appendix B.02 — Current Code Architecture: `Assets/Ashfall.Core/Factions/MilitaryBranchCatalog.cs`

### `Assets/Ashfall.Core/Factions/MilitaryBranchCatalog.cs`

- Evidence status: **CURRENT FILE PRESENT**.
- Size: 84 lines / 3607 bytes.
- SHA-256: `a42f28b8febf5ba8f49f651e82095ce18982fed4b7acbfeb92efb2e9850d0112`.
...
# Appendix B.03 — Current Code Architecture: `Assets/Ashfall.Core/Factions/MilitaryBranchSystem.cs`
...
# Appendix B.04 — Current Code Architecture: `Assets/Ashfall.Core/Factions/MilitaryBranchState.cs`
...
# Appendix C.12 — Catalog Census: `Assets/StreamingAssets/Data/military_faction_branch.json`

### `Assets/StreamingAssets/Data/military_faction_branch.json`

- Parse: valid strict JSON.
- Schema version: `1`.
- Size: 12177 bytes / 12177 characters.
- SHA-256: `8510b25001cd38f1f007e4e9cb23c0a7856f0ae24668e89f688873ed241bf2a1`.
...
```text
branch_mil_1_loyal_soldier
branch_mil_2_defector
branch_mil_3_opportunist
branch_mil_4_martyr
branch_mil_5_tyrant
branch_mil_6_reformer
branch_mil_7_deserter
branch_mil_8_broken_chain
branch_mil_9_quartermaster
branch_mil_10_combat_medic
branch_mil_11_conscript_parent
branch_mil_12_intelligence_officer
branch_mil_13_peacekeeper
branch_mil_14_fugitive_deserter
branch_mil_15_dissident_officer
```
...
# Appendix D.15 — Existing Focused Test Inventory: `Ashfall.Core.Tests/MilitaryBranchExpansionTests.cs`

### `Ashfall.Core.Tests/MilitaryBranchExpansionTests.cs`

- Current test declarations: Fact=16, Theory=0, InlineData=0.
- File lines: 422; SHA-256: `f3228ed81cdea893af09aded2fd249aa4a47a79a98f040722740ccaaa7addd9a`.
...
```text
Catalog_LoadsSuccessfully_ContainsExactFifteenBranches
BaselineEightBranches_PreservedVerbatim_WithExactOrderAndData
SevenNewBranches_AllPresent_AndMatchExpectedIds
AllFifteenBranches_HaveThreeEndings_Totaling45Endings
AllBranchIds_AreUnique_AndFollowMilConvention
AllPonrFlags_AreUnique_AndFollowMilConvention
AllEndingIds_AreUnique_AcrossEntireCatalog
AllEntryBands_AreValid_AndOrdered
SevenNewBranches_EndingBands_CoverAllSevenBandsWithoutGaps
MilitaryBranchSystem_CanCommit_ToNewBranches_WithinValidBands
MilitaryBranchSystem_CommitOutsideBand_Throws
MilitaryBranchSystem_LocksPoNR_AndEmitsCorrectDurableFlag_ForNewBranches
MilitaryBranchSystem_ResolvesCorrectEnding_ForQuartermaster
MilitaryBranchSystem_ResolvesCorrectEnding_ForPeacekeeper
MilitaryBranchSystem_SaveAndRestore_PreservesBranchCommitAndPoNRLock
AllPonrTriggers_AreSingleSentence_AndIrreversible
```


# Appendix D.16 — Existing Focused Test Inventory: `Ashfall.Core.Tests/Factions/Plan121_122FactionBranchIntegrationTests.cs`

### `Ashfall.Core.Tests/Factions/Plan121_122FactionBranchIntegrationTests.cs`

- Current test declarations: Fact=6, Theory=0, InlineData=0.
- File lines: 237; SHA-256: `13b686f198d860a97ac06335e6b807822829da18daec531506e5faeae38c75e8`.
- The list below is an inventory, not a newly executed pass result. Focused tests must be rerun when the owning implementation changes.

```text
Catalogs_LoadSuccessfullyFromAuthJson
BranchIds_AreDisjointAndUniqueAcrossCatalogs
PonrFlags_AreDisjointAcrossCatalogs
EndingIds_AreDisjointAcrossCatalogs
IndependentAndMilitarySystems_OperateConcurrentlyWithoutInterference
Coordinator_CoordinatesBothCatalogsWithCleanExclusivity
```


# Appendix D.17 — Existing Focused Test Inventory: `Ashfall.Core.Tests/MilitaryBranchSystemTests.cs`

### `Ashfall.Core.Tests/MilitaryBranchSystemTests.cs`

- Current test declarations: Fact=16, Theory=0, InlineData=0.
- File lines: 275; SHA-256: `634c183a55f729aff3b7ac6497658671b2b0d8bee2de6a90be22a60a50d30c4d`.
- The list below is an inventory, not a newly executed pass result. Focused tests must be rerun when the owning implementation changes.

```text
Catalog_LoadsAllEightBranchesWithThreeEndingsEach
...
ResolveEnding_NeutralBand_ResolvesSurvivorKingEnding
...
# Appendix E.18 — Supporting Code Evidence: `Assets/Ashfall.Core/Factions/MilitaryBranchCatalog.cs`
...
# Appendix E.19 — Supporting Code Evidence: `Assets/Ashfall.Core/Factions/MilitaryBranchSystem.cs`
...
# Appendix E.20 — Supporting Code Evidence: `Assets/Ashfall.Core/Factions/MilitaryBranchState.cs`
...
# Appendix G.23 — Supporting Regression Evidence: `Ashfall.Core.Tests/MilitaryBranchExpansionTests.cs`
...
# Appendix G.24 — Supporting Regression Evidence: `Ashfall.Core.Tests/Factions/Plan121_122FactionBranchIntegrationTests.cs`
...
# Appendix G.25 — Supporting Regression Evidence: `Ashfall.Core.Tests/MilitaryBranchSystemTests.cs`
...
| --- | --- | --- | --- | --- |
| military branch definitions and ordered lookup | MilitaryBranchCatalog | military branch state, alignment, PoNR and ending | MilitaryBranchSystem | Owner emits/reads a typed fact; no mirror state. |
| military branch definitions and ordered lookup | MilitaryBranchCatalog | cross-faction exclusivity and active branch state | FactionBranchCoordinator | Owner emits/reads a typed fact; no mirror state. |
| military branch definitions and ordered lookup | MilitaryBranchCatalog | combined military/rebel/independent/PRPF persistence and migration | WeightOfChoicesSaveCodec | Owner emits/reads a typed fact; no mirror state. |
| military branch definitions and ordered lookup | MilitaryBranchCatalog | host commands, lifecycle and panel route | FactionBranchHostSession/Main.FactionBranch | Owner emits/reads a typed fact; no mirror state. |
| military branch definitions and ordered lookup | MilitaryBranchCatalog | catalog, coordinator and integration proof | Military focused tests | Owner emits/reads a typed fact; no mirror state. |
| military branch state, alignment, PoNR and ending | MilitaryBranchSystem | military branch definitions and ordered lookup | MilitaryBranchCatalog | Owner emits/reads a typed fact; no mirror state. |
| military branch state, alignment, PoNR and ending | MilitaryBranchSystem | cross-faction exclusivity and active branch state | FactionBranchCoordinator | Owner emits/reads a typed fact; no mirror state. |
| military branch state, alignment, PoNR and ending | MilitaryBranchSystem | combined military/rebel/independent/PRPF persistence and migration | WeightOfChoicesSaveCodec | Owner emits/reads a typed fact; no mirror state. |
| military branch state, alignment, PoNR and ending | MilitaryBranchSystem | host commands, lifecycle and panel route | FactionBranchHostSession/Main.FactionBranch | Owner emits/reads a typed fact; no mirror state. |
| military branch state, alignment, PoNR and ending | MilitaryBranchSystem | catalog, coordinator and integration proof | Military focused tests | Owner emits/reads a typed fact; no mirror state. |
| cross-faction exclusivity and active branch state | FactionBranchCoordinator | military branch definitions and ordered lookup | MilitaryBranchCatalog | Owner emits/reads a typed fact; no mirror state. |
| cross-faction exclusivity and active branch state | FactionBranchCoordinator | military branch state, alignment, PoNR and ending | MilitaryBranchSystem | Owner emits/reads a typed fact; no mirror state. |
| cross-faction exclusivity and active branch state | FactionBranchCoordinator | combined military/rebel/independent/PRPF persistence and migration | WeightOfChoicesSaveCodec | Owner emits/reads a typed fact; no mirror state. |
| cross-faction exclusivity and active branch state | FactionBranchCoordinator | host commands, lifecycle and panel route | FactionBranchHostSession/Main.FactionBranch | Owner emits/reads a typed fact; no mirror state. |
| cross-faction exclusivity and active branch state | FactionBranchCoordinator | catalog, coordinator and integration proof | Military focused tests | Owner emits/reads a typed fact; no mirror state. |
| combined military/rebel/independent/PRPF persistence and migration | WeightOfChoicesSaveCodec | military branch definitions and ordered lookup | MilitaryBranchCatalog | Owner emits/reads a typed fact; no mirror state. |
| combined military/rebel/independent/PRPF persistence and migration | WeightOfChoicesSaveCodec | military branch state, alignment, PoNR and ending | MilitaryBranchSystem | Owner emits/reads a typed fact; no mirror state. |
| combined military/rebel/independent/PRPF persistence and migration | WeightOfChoicesSaveCodec | cross-faction exclusivity and active branch state | FactionBranchCoordinator | Owner emits/reads a typed fact; no mirror state. |
| combined military/rebel/independent/PRPF persistence and migration | WeightOfChoicesSaveCodec | host commands, lifecycle and panel route | FactionBranchHostSession/Main.FactionBranch | Owner emits/reads a typed fact; no mirror state. |
| combined military/rebel/independent/PRPF persistence and migration | WeightOfChoicesSaveCodec | catalog, coordinator and integration proof | Military focused tests | Owner emits/reads a typed fact; no mirror state. |
| host commands, lifecycle and panel route | FactionBranchHostSession/Main.FactionBranch | military branch definitions and ordered lookup | MilitaryBranchCatalog | Owner emits/reads a typed fact; no mirror state. |
| host commands, lifecycle and panel route | FactionBranchHostSession/Main.FactionBranch | military branch state, alignment, PoNR and ending | MilitaryBranchSystem | Owner emits/reads a typed fact; no mirror state. |
| host commands, lifecycle and panel route | FactionBranchHostSession/Main.FactionBranch | cross-faction exclusivity and active branch state | FactionBranchCoordinator | Owner emits/reads a typed fact; no mirror state. |
| host commands, lifecycle and panel route | FactionBranchHostSession/Main.FactionBranch | combined military/rebel/independent/PRPF persistence and migration | WeightOfChoicesSaveCodec | Owner emits/reads a typed fact; no mirror state. |
| host commands, lifecycle and panel route | FactionBranchHostSession/Main.FactionBranch | catalog, coordinator and integration proof | Military focused tests | Owner emits/reads a typed fact; no mirror state. |
| catalog, coordinator and integration proof | Military focused tests | military branch definitions and ordered lookup | MilitaryBranchCatalog | Owner emits/reads a typed fact; no mirror state. |
| catalog, coordinator and integration proof | Military focused tests | military branch state, alignment, PoNR and ending | MilitaryBranchSystem | Owner emits/reads a typed fact; no mirror state. |
| catalog, coordinator and integration proof | Military focused tests | cross-faction exclusivity and active branch state | FactionBranchCoordinator | Owner emits/reads a typed fact; no mirror state. |
| catalog, coordinator and integration proof | Military focused tests | combined military/rebel/independent/PRPF persistence and migration | WeightOfChoicesSaveCodec | Owner emits/reads a typed fact; no mirror state. |
| catalog, coordinator and integration proof | Military focused tests | host commands, lifecycle and panel route | FactionBranchHostSession/Main.FactionBranch | Owner emits/reads a typed fact; no mirror state. |
...
| --- | --- | --- | --- |
| R-01 | Replace the 8→15 target with a 15-row branch/flag/ending census and a current coverage matrix. | Focused negative/edge test; host/runtime proof where player-visible. | BLOCKED if no named owner can accept the fact. |
| R-02 | Trace availability, commitment, PoNR, alignment, ending and cross-faction exclusivity through the combined coordinator. | Focused negative/edge test; host/runtime proof where player-visible. | BLOCKED if no named owner can accept the fact. |
| R-03 | Audit WeightOfChoices v1→v2 migration, durable flag replay and future-version rejection. | Focused negative/edge test; host/runtime proof where player-visible. | BLOCKED if no named owner can accept the fact. |
| R-04 | Preserve no-new-state/no-new-save boundaries and keep UI claims shared/integrator-owned. | Focused negative/edge test; host/runtime proof where player-visible. | BLOCKED if no named owner can accept the fact. |
...
The subject is an irreversible military character branch inside the combined Weight of Choices coordinator. The plan expands state, alignment, migration, host and UI truthfulness while keeping the single faction commitment authority.

- **military branch definitions and ordered lookup** remains with `MilitaryBranchCatalog` at `Assets/Ashfall.Core/Factions/MilitaryBranchCatalog.cs`. Immutable catalog after load.
- **military branch state, alignment, PoNR and ending** remains with `MilitaryBranchSystem` at `Assets/Ashfall.Core/Factions/MilitaryBranchSystem.cs`. Sole military branch owner; reads moral/flag ports.
- **cross-faction exclusivity and active branch state** remains with `FactionBranchCoordinator` at `Assets/Ashfall.Core/Factions/FactionBranchCoordinator.cs`. Owns combined branch lifecycle.
- **combined military/rebel/independent/PRPF persistence and migration** remains with `WeightOfChoicesSaveCodec` at `Assets/Ashfall.Core/Factions/WeightOfChoicesSave.cs; Assets/Ashfall.Core/Factions/MilitaryBranchSave.cs`. Existing envelope; individual codec is a component, not a second live store.
- **host commands, lifecycle and panel route** remains with `FactionBranchHostSession/Main.FactionBranch` at `src/Host/FactionBranchHostSession.cs; src/Main.FactionBranch.cs; src/UI/FactionsPanel.cs`. Host/UI composition only.
- **catalog, coordinator and integration proof** remains with `Military focused tests` at `Ashfall.Core.Tests/MilitaryBranchExpansionTests.cs; Ashfall.Core.Tests/Factions/Plan121_122FactionBranchIntegrationTests.cs; Ashfall.Core.Tests/MilitaryBranchSystemTests.cs`. Focused evidence surface.
...
| --- | --- | --- | --- | --- | --- |
| S-01 | 122-01 load 15 military branches | load 15 military rows and other branch catalogs | Catalog rows are immutable; `MilitaryBranchSystemState` owns timeline, branch record, alignment and durable flags. | Military and Rebel commitments coexist after restore. | MilitaryBranchCatalog |
| S-02 | 122-02 moral-band availability | restore combined WeightOfChoicesSave and replay flags | Commitment is first-commit-wins, PoNR is irreversible/idempotent, and ending resolution is first-resolution-wins after lock. | A panel marks PoNR/ending without the coordinator. | MilitaryBranchCatalog |
| S-03 | 122-03 out-of-band rejection | query availability with current moral/faction state | The combined save always contains all branch sections and rejects a multi-faction committed state. | Alignment exceeds its clamp or changes the moral score. | MilitaryBranchCatalog |
| S-04 | 122-04 first military commitment | commit through coordinator exclusivity | Alignment is clamped to the current -200..200 scale and is not the player morality score. | A v1 save loses the active military branch or duplicates its ending. | MilitaryBranchCatalog |
| S-05 | 122-05 Rebel exclusivity | lock PoNR once | Catalog rows are immutable; `MilitaryBranchSystemState` owns timeline, branch record, alignment and durable flags. | A new military coordinator duplicates the combined owner. | MilitaryBranchCatalog |
| S-06 | 122-06 Independent exclusivity | apply bounded military alignment through the existing faction seam | Commitment is first-commit-wins, PoNR is irreversible/idempotent, and ending resolution is first-resolution-wins after lock. | Military and Rebel commitments coexist after restore. | MilitaryBranchCatalog |
| S-07 | 122-07 PoNR lock once | resolve first matching ending and project it | The combined save always contains all branch sections and rejects a multi-faction committed state. | A panel marks PoNR/ending without the coordinator. | MilitaryBranchCatalog |
| S-08 | 122-08 alignment clamp | save combined envelope and refresh surfaces | Alignment is clamped to the current -200..200 scale and is not the player morality score. | Alignment exceeds its clamp or changes the moral score. | MilitaryBranchCatalog |
| S-09 | 122-09 ending once | load 15 military rows and other branch catalogs | Catalog rows are immutable; `MilitaryBranchSystemState` owns timeline, branch record, alignment and durable flags. | A v1 save loses the active military branch or duplicates its ending. | MilitaryBranchCatalog |
| S-10 | 122-10 v1 to v2 restore | restore combined WeightOfChoicesSave and replay flags | Commitment is first-commit-wins, PoNR is irreversible/idempotent, and ending resolution is first-resolution-wins after lock. | A new military coordinator duplicates the combined owner. | MilitaryBranchCatalog |
| S-11 | 122-11 conflicting save | query availability with current moral/faction state | The combined save always contains all branch sections and rejects a multi-faction committed state. | Military and Rebel commitments coexist after restore. | MilitaryBranchCatalog |
...
| --- | --- | --- | --- | --- |
| T-01 | 122-TC-01 catalog schema/duplicate IDs | data | catalog schema/duplicate IDs; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
| T-02 | 122-TC-02 15 branch census | unit | 15 branch census; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
| T-03 | 122-TC-03 flag uniqueness | persistence | flag uniqueness; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
| T-04 | 122-TC-04 ending range coverage | determinism | ending range coverage; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
| T-05 | 122-TC-05 entry-band parsing | host | entry-band parsing; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
| T-06 | 122-TC-06 first-commit-wins | UI/accessibility | first-commit-wins; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
| T-07 | 122-TC-07 cross-faction exclusivity | cross-system | cross-faction exclusivity; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
| T-08 | 122-TC-08 PoNR idempotence | data | PoNR idempotence; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
| T-09 | 122-TC-09 alignment clamp | unit | alignment clamp; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
| T-10 | 122-TC-10 ending idempotence | persistence | ending idempotence; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
| T-11 | 122-TC-11 durable flag replay | determinism | durable flag replay; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
| T-12 | 122-TC-12 WeightOfChoices v1 migration | host | WeightOfChoices v1 migration; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
| T-13 | 122-TC-13 future version rejection | UI/accessibility | future version rejection; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
| T-14 | 122-TC-14 host/UI projection | cross-system | host/UI projection; verify the current owner and its negative boundary without inventing a second authority. | MilitaryBranchCatalog |
...
| 23 | `Ashfall.Core.Tests/MilitaryBranchSystemTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 18 | `Ashfall.Core.Tests/MilitaryBranchExpansionTests.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 13 | `Assets/Ashfall.Core/Factions/MilitaryBranchSystem.cs` | current reference count; inspect the caller before treating it as a live route |
| 10 | `src/Host/FactionBranchHostSession.cs` | current reference count; inspect the caller before treating it as a live route |
| 8 | `Ashfall.Core.Tests/Factions/Plan121_122FactionBranchIntegrationTests.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 6 | `Ashfall.Core.Tests/FactionBranchCoordinatorTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 5 | `Assets/Ashfall.Core/Factions/MilitaryBranchCatalog.cs` | current reference count; inspect the caller before treating it as a live route |
| 5 | `Assets/Ashfall.Core/Factions/RebelBranchSystem.cs` | current reference count; inspect the caller before treating it as a live route |
| 4 | `Assets/Ashfall.Core/Factions/MilitaryBranchSave.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 3 | `Ashfall.Core.Tests/Factions/Plan123_127RebelVerdictIntegrationTests.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 2 | `Ashfall.Core.Tests/MilitaryBranchCatalogTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `Ashfall.Core.Tests/MusterWarfareTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `Assets/Ashfall.Core/Factions/IndependentBranchSystem.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `Assets/Ashfall.Core/Factions/MilitaryBranchIds.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `Assets/Ashfall.Core/Factions/MilitaryBranchState.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 1 | `Ashfall.Core.Tests/RebelBranchCatalogTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 1 | `Ashfall.Core.Tests/RebelBranchExpansionTests.cs` | current reference count; inspect the caller before treating it as a live route |
...
# Appendix — Complete Catalog Audit: `Assets/StreamingAssets/Data/military_faction_branch.json`

### `Assets/StreamingAssets/Data/military_faction_branch.json` — complete current row audit

- Parse status: **valid JSON**.
- Bytes: 12177; characters: 12177.
- SHA-256: `8510b25001cd38f1f007e4e9cb23c0a7856f0ae24668e89f688873ed241bf2a1`.
...
- Row 001 `branch_mil_1_loyal_soldier`: `{"display_name":"The Loyal Soldier","endings":[{"band_max":"very_positive","band_min":"positive","display_name":"The Benevolent Dictator","ending_id":"ending_mil_1a_benevolent_dictator"},{"band_max":"evil","band_min":"very_evil","display_n…`
- Row 002 `branch_mil_2_defector`: `{"display_name":"The Defector","endings":[{"band_max":"very_positive","band_min":"slightly_positive","display_name":"The Reformer","ending_id":"ending_mil_2a_reformer_of_rebels"},{"band_max":"evil","band_min":"very_evil","display_name":"Th…`
- Row 003 `branch_mil_3_opportunist`: `{"display_name":"The Opportunist","endings":[{"band_max":"very_evil","band_min":"very_evil","display_name":"The Tyrant","ending_id":"ending_mil_3a_tyrant"},{"band_max":"very_positive","band_min":"positive","display_name":"The Benevolent Wa…`
- Row 004 `branch_mil_4_martyr`: `{"display_name":"The Martyr","endings":[{"band_max":"very_positive","band_min":"very_positive","display_name":"The Saint of the Wasteland","ending_id":"ending_mil_4a_saint_of_wasteland"},{"band_max":"positive","band_min":"positive","displa…`
- Row 005 `branch_mil_5_tyrant`: `{"display_name":"The Tyrant","endings":[{"band_max":"very_evil","band_min":"very_evil","display_name":"The Tyrant King","ending_id":"ending_mil_5a_tyrant_king"},{"band_max":"evil","band_min":"slightly_evil","display_name":"The Benevolent W…`
- Row 006 `branch_mil_6_reformer`: `{"display_name":"The Reformer","endings":[{"band_max":"very_positive","band_min":"very_positive","display_name":"The Visionary","ending_id":"ending_mil_6a_visionary"},{"band_max":"positive","band_min":"positive","display_name":"The Reforme…`
- Row 007 `branch_mil_7_deserter`: `{"display_name":"The Deserter","endings":[{"band_max":"slightly_positive","band_min":"neutral","display_name":"The Lone Survivor","ending_id":"ending_mil_7a_lone_survivor"},{"band_max":"evil","band_min":"very_evil","display_name":"The Trai…`
- Row 008 `branch_mil_8_broken_chain`: `{"display_name":"The Broken Chain","endings":[{"band_max":"very_positive","band_min":"slightly_positive","display_name":"The New Leader","ending_id":"ending_mil_8a_new_leader"},{"band_max":"neutral","band_min":"slightly_evil","display_name…`
- Row 009 `branch_mil_9_quartermaster`: `{"display_name":"The Quartermaster","endings":[{"band_max":"very_positive","band_min":"positive","display_name":"The Keeper of Stores","ending_id":"ending_mil_9a_keeper_of_stores"},{"band_max":"slightly_positive","band_min":"slightly_evil"…`
- Row 010 `branch_mil_10_combat_medic`: `{"display_name":"The Combat Medic","endings":[{"band_max":"very_positive","band_min":"positive","display_name":"The Field Healer","ending_id":"ending_mil_10a_field_healer"},{"band_max":"slightly_positive","band_min":"slightly_evil","displa…`
- Row 011 `branch_mil_11_conscript_parent`: `{"display_name":"The Conscript Parent","endings":[{"band_max":"very_positive","band_min":"positive","display_name":"The Community Protector","ending_id":"ending_mil_11a_community_protector"},{"band_max":"slightly_positive","band_min":"slig…`
- Row 012 `branch_mil_12_intelligence_officer`: `{"display_name":"The Intelligence Officer","endings":[{"band_max":"very_positive","band_min":"positive","display_name":"The Public Witness","ending_id":"ending_mil_12a_public_witness"},{"band_max":"slightly_positive","band_min":"slightly_e…`
- Row 013 `branch_mil_13_peacekeeper`: `{"display_name":"The Peacekeeper","endings":[{"band_max":"very_positive","band_min":"positive","display_name":"The Community Shield","ending_id":"ending_mil_13a_community_shield"},{"band_max":"slightly_positive","band_min":"slightly_evil",…`
- Row 014 `branch_mil_14_fugitive_deserter`: `{"display_name":"The Fugitive Deserter","endings":[{"band_max":"very_positive","band_min":"positive","display_name":"The Integrated Civilian","ending_id":"ending_mil_14a_integrated_civilian"},{"band_max":"slightly_positive","band_min":"sli…`
- Row 015 `branch_mil_15_dissident_officer`: `{"display_name":"The Dissident Officer","endings":[{"band_max":"very_positive","band_min":"positive","display_name":"The Reform Commander","ending_id":"ending_mil_15a_reform_command"},{"band_max":"slightly_positive","band_min":"slightly_ev…`
...
# Appendix — Current Source Detail: `Assets/Ashfall.Core/Factions/MilitaryBranchSystem.cs`

### `Assets/Ashfall.Core/Factions/MilitaryBranchSystem.cs` — complete current file
...
00007: {
00008:     /// <summary>
00009:     /// Resolution engine for the Military faction slice of "The Weight of
00010:     /// Choices" branching system.
00011:     ///
00012:     /// Deliberately does NOT own morality or faction-standing math: it reads
00013:     /// MoralChoiceSystem.CurrentBand for the player's morality axis (morality
00014:     /// is a gate key here, never a "correct/incorrect" judgment — evil-leaning
00015:     /// and positive-leaning play lock out different, equally valid quest
00016:     /// pools and endings) and it can read player-relationship standing from a
00017:     /// caller-supplied FactionWarSystem-shaped source when that wiring exists.
00018:     /// What this system DOES own: which base branch (of the 8 Military
00019:     /// branches) the player has committed to, the branch's point-of-no-return
00020:     /// lock, the Military faction's own internal alignment (a value distinct
00021:     /// from the player's morality score, per design: "the player has the
00022:     /// chance to sway the faction to the positive side"), and morality-gated
00023:     /// ending resolution once the branch is locked.
00024:     ///
00025:     /// Zero engine dependencies; deterministic; runs on its own local day
00026:     /// counter (MilitaryBranchTimelineState), independent of the global
00027:     /// IClock and of Year of Ash's day 180-360 window.
00028:     /// </summary>
00029:     public sealed class MilitaryBranchSystem
00030:     {
00031:         public const string SystemId = "military_branch_system";
00032:
00033:         /// <summary>Faction alignment is clamped to the same -200..+200 range as
00034:         /// MoralChoiceSystem purely for player-facing scale consistency.</summary>
00035:         public const int MinAlignment = -200;
00036:         public const int MaxAlignment = 200;
00037:
00038:         private readonly MilitaryBranchCatalog _catalog;
00039:         private readonly IFlagLedger _flags;
00040:         private readonly ILog _log;
00041:         private MilitaryBranchSystemState _state;
00042:
00043:         public event Action<string>? OnBranchCommitted;
00044:         public event Action<string, int>? OnPonrLocked;
00045:         public event Action<string>? OnEndingResolved;
00046:         public event Action<int>? OnAlignmentChanged;
00047:
00048:         public MilitaryBranchSystem(MilitaryBranchCatalog catalog, IFlagLedger flags, MilitaryBranchSystemState? state = null, ILog? log = null)
00049:         {
00050:             _catalog = catalog ?? throw new ArgumentNullException(nameof(catalog));
00051:             _flags = flags ?? throw new ArgumentNullException(nameof(flags));
00052:             _log = log ?? NullLog.Instance;
00053:             _state = state ?? new MilitaryBranchSystemState();
00054:             if (_state.timeline == null) _state.timeline = new MilitaryBranchTimelineState();
00055:             if (_state.branch == null) _state.branch = new MilitaryBranchRecord();
00056:             if (_state.militaryAlignment == null)
00057:                 _state.militaryAlignment = new FactionAlignmentRecord { factionId = MilitaryBranchIds.FactionId, alignment = -80 };
00058:             if (_state.setFlags == null) _state.setFlags = new System.Collections.Generic.List<string>();
00059:         }
00060:
00061:         public MilitaryBranchSystemState State => _state;
00062:         public int CurrentDay => _state.timeline.currentDay;
00063:         public string? CommittedBranchId => string.IsNullOrEmpty(_state.branch.branchId) ? null : _state.branch.branchId;
00064:         public bool IsPonrLocked => _state.branch.ponrLocked;
00065:         public int MilitaryAlignment => _state.militaryAlignment.alignment;
00066:         public string? ResolvedEndingId => string.IsNullOrEmpty(_state.branch.resolvedEndingId) ? null : _state.branch.resolvedEndingId;
00067:
00068:         public void AdvanceDay(int day)
00069:         {
00070:             if (day < 0) throw new ArgumentOutOfRangeException(nameof(day));
00071:             if (day < _state.timeline.currentDay) return; // never rewind
00072:             _state.timeline.currentDay = day;
00073:         }
00074:
00075:         /// <summary>
00076:         /// Commits the player to a base branch. Soft-gated by the branch's
00077:         /// entry band range read from the catalog (a design "starting
00078:         /// morality range" is an entry gate, never a re-definition of the
00079:         /// morality tiers themselves) — attempting to commit outside that
00080:         /// range throws, since the design doc treats these as hard entry
00081:         /// requirements, not just flavor. Committing twice is a no-op that
00082:         /// returns the already-committed branch (first commit wins, matching
00083:         /// MoralChoiceSystem.Resolve's "one resolution per quest" idempotency
00084:         /// convention) rather than silently overwriting it.
00085:         /// </summary>
00086:         public string CommitBranch(string branchId, MoralChoiceSystem moralChoice)
00087:         {
00088:             if (string.IsNullOrEmpty(branchId)) throw new ArgumentNullException(nameof(branchId));
00089:             if (moralChoice == null) throw new ArgumentNullException(nameof(moralChoice));
00090:
00091:             if (_state.branch.committed)
00092:             {
00093:                 _log.Warn($"Military branch already committed to '{_state.branch.branchId}'; ignoring commit to '{branchId}'.");
00094:                 return _state.branch.branchId;
00095:             }
00096:
00097:             var def = _catalog.GetById(branchId);
00098:             if (def == null)
00099:                 throw new ArgumentException($"Unknown Military branch id '{branchId}'.", nameof(branchId));
00100:
00101:             var band = moralChoice.CurrentBand;
00102:             var min = ParseBand(def.entry_band_min);
00103:             var max = ParseBand(def.entry_band_max);
00104:             if (band < min || band > max)
00105:             {
00106:                 throw new InvalidOperationException(
00107:                     $"Branch '{branchId}' requires a morality band between {min} and {max}; " +
00108:                     $"current band is {band}. Morality is a gate, not a judgment — a different " +
00109:                     "branch is accessible at this band.");
00110:             }
00111:
00112:             _state.branch.branchId = branchId;
00113:             _state.branch.committed = true;
00114:             OnBranchCommitted?.Invoke(branchId);
00115:             return branchId;
00116:         }
00117:
00118:         /// <summary>
00119:         /// Fires the committed branch's point-of-no-return. Irreversible: once
00120:         /// locked, the flag is set in both the runtime IFlagLedger (for
00121:         /// same-session gating checks elsewhere) and the save-durable
00122:         /// setFlags list (since IFlagLedger itself is not persisted). Locking
00123:         /// twice is a no-op.
00124:         /// </summary>
00125:         public void LockPointOfNoReturn()
00126:         {
00127:             if (!_state.branch.committed)
00128:                 throw new InvalidOperationException("Cannot lock a point-of-no-return before a branch is committed.");
00129:             if (_state.branch.ponrLocked) return;
00130:
00131:             string flagId = MilitaryBranchIds.PonrFlagFor(_state.branch.branchId);
00132:             _state.branch.ponrLocked = true;
00133:             _state.branch.ponrLockedDay = _state.timeline.currentDay;
00134:             SetDurableFlag(flagId);
00135:             OnPonrLocked?.Invoke(flagId, _state.timeline.currentDay);
00136:         }
00137:
00138:         /// <summary>
00139:         /// Shifts the Military faction's OWN internal alignment (not the
00140:         /// player's MoralChoiceSystem score) toward good or evil as a
00141:         /// consequence of the player's in-faction choices. Clamped to
00142:         /// -200..+200, mirroring FactionStandingRecord's clamp-on-write shape.
00143:         /// </summary>
00144:         public void ShiftFactionAlignment(int delta)
00145:         {
00146:             int next = Math.Clamp(_state.militaryAlignment.alignment + delta, MinAlignment, MaxAlignment);
00147:             _state.militaryAlignment.alignment = next;
00148:             OnAlignmentChanged?.Invoke(next);
00149:         }
00150:
00151:         /// <summary>
00152:         /// Resolves the ending for the committed, PoNR-locked branch using the
00153:         /// player's current morality band. Each branch's ending table is
00154:         /// checked in catalog order; the first row whose band_min..band_max
00155:         /// contains the current band wins. Idempotent: resolving twice
00156:         /// returns the first-resolved ending, matching the "PoNR locks the
00157:         /// path" design intent — the ending is not re-rolled if morality
00158:         /// keeps drifting after the branch is already locked.
00159:         /// </summary>
00160:         public string ResolveEnding(MoralChoiceSystem moralChoice)
00161:         {
00162:             if (moralChoice == null) throw new ArgumentNullException(nameof(moralChoice));
00163:             if (!_state.branch.ponrLocked)
00164:                 throw new InvalidOperationException("Cannot resolve an ending before the point-of-no-return has locked the branch.");
00165:
00166:             if (!string.IsNullOrEmpty(_state.branch.resolvedEndingId))
00167:                 return _state.branch.resolvedEndingId;
00168:
00169:             var def = _catalog.GetById(_state.branch.branchId)
00170:                 ?? throw new InvalidOperationException($"Committed branch '{_state.branch.branchId}' is missing from the catalog.");
00171:
00172:             var band = moralChoice.CurrentBand;
00173:             foreach (var ending in def.endings)
00174:             {
00175:                 var min = ParseBand(ending.band_min);
00176:                 var max = ParseBand(ending.band_max);
00177:                 if (band >= min && band <= max)
00178:                 {
00179:                     _state.branch.resolvedEndingId = ending.ending_id;
00180:                     OnEndingResolved?.Invoke(ending.ending_id);
00181:                     return ending.ending_id;
00182:                 }
00183:             }
00184:
00185:             // No row matched (a gap in authored ranges) — fall back to the
00186:             // middle-listed ending rather than throwing, so a data gap never
00187:             // hard-crashes a playthrough's final resolution.
00188:             var fallback = def.endings.Count > 0 ? def.endings[def.endings.Count / 2].ending_id : string.Empty;
00189:             _state.branch.resolvedEndingId = fallback;
00190:             if (!string.IsNullOrEmpty(fallback)) OnEndingResolved?.Invoke(fallback);
00191:             return fallback;
00192:         }
00193:
00194:         /// <summary>
00195:         /// Full-party-wipe loss check. Deliberately a pure function over a
00196:         /// caller-supplied living-survivor count rather than an owned roster
00197:         /// — no SurvivorRosterSystem exists in Core yet (confirmed absent),
00198:         /// and building one is out of scope for the Military branch slice.
00199:         /// Mirrors EpilogueEvaluationContext's caller-supplied-count pattern.
00200:         /// </summary>
00201:         public static bool IsGameOver(int livingSurvivorCount) => livingSurvivorCount <= 0;
00202:
00203:         private void SetDurableFlag(string flagId)
00204:         {
00205:             _flags.Set(flagId);
00206:             if (!_state.setFlags.Contains(flagId))
00207:                 _state.setFlags.Add(flagId);
00208:         }
00209:
00210:         private static MoralPathBand ParseBand(string band) => band switch
00211:         {
00212:             "very_evil" => MoralPathBand.VeryEvil,
00213:             "evil" => MoralPathBand.Evil,
00214:             "slightly_evil" => MoralPathBand.SlightlyEvil,
00215:             "neutral" => MoralPathBand.Neutral,
00216:             "slightly_positive" => MoralPathBand.SlightlyPositive,
00217:             "positive" => MoralPathBand.Positive,
00218:             "very_positive" => MoralPathBand.VeryPositive,
00219:             _ => throw new ArgumentException($"Unknown morality band token '{band}'.", nameof(band))
00220:         };
00221:
00222:         public MilitaryBranchSystemState CaptureState() => Clone(_state);
00223:
00224:         public void RestoreState(MilitaryBranchSystemState state)
00225:         {
00226:             if (state == null) return;
00227:             if (!string.Equals(state.systemId, SystemId, StringComparison.Ordinal))
00228:             {
00229:                 throw new ArgumentException(
00230:                     $"State belongs to system '{state.systemId}', expected '{SystemId}'.", nameof(state));
00231:             }
00232:             if (state.schemaVersion > 1)
00233:             {
00234:                 throw new NotSupportedException(
00235:                     $"Future Military branch save schema {state.schemaVersion}; supported schema is 1.");
00236:             }
00237:             _state = Clone(state);
00238:
00239:             // Replay durable flags into the runtime ledger so same-session
00240:             // gating checks (IFlagLedger.IsSet) agree with the restored save
00241:             // immediately — IFlagLedger itself is not persisted.
00242:             foreach (var flagId in _state.setFlags)
00243:                 _flags.Set(flagId);
00244:         }
00245:
00246:         private static MilitaryBranchSystemState Clone(MilitaryBranchSystemState source)
00247:         {
00248:             return new MilitaryBranchSystemState
00249:             {
00250:                 systemId = source.systemId,
00251:                 schemaVersion = source.schemaVersion,
00252:                 timeline = new MilitaryBranchTimelineState { currentDay = source.timeline.currentDay },
00253:                 branch = new MilitaryBranchRecord
00254:                 {
00255:                     branchId = source.branch.branchId,
00256:                     committed = source.branch.committed,
00257:                     ponrLocked = source.branch.ponrLocked,
00258:                     ponrLockedDay = source.branch.ponrLockedDay,
00259:                     resolvedEndingId = source.branch.resolvedEndingId
00260:                 },
00261:                 militaryAlignment = new FactionAlignmentRecord
00262:                 {
00263:                     factionId = source.militaryAlignment.factionId,
00264:                     alignment = source.militaryAlignment.alignment
00265:                 },
00266:                 setFlags = new System.Collections.Generic.List<string>(source.setFlags ?? new System.Collections.Generic.List<string>())
00267:             };
00268:         }
00269:     }
00270: }
...
# Appendix — Current Source Detail: `Assets/Ashfall.Core/Factions/MilitaryBranchCatalog.cs`

### `Assets/Ashfall.Core/Factions/MilitaryBranchCatalog.cs` — complete current file
...
00008: {
00009:     /// <summary>One morality-gated ending row nested under a branch definition.</summary>
00010:     public sealed class MilitaryBranchEndingEntry
00011:     {
00012:         public string ending_id { get; set; } = string.Empty;
00013:         public string band_min { get; set; } = string.Empty;
00014:         public string band_max { get; set; } = string.Empty;
00015:         public string display_name { get; set; } = string.Empty;
00016:     }
00017:
00018:     /// <summary>One base branch row, matching military_faction_branch.json shape.</summary>
00019:     public sealed class MilitaryBranchEntry
00020:     {
00021:         public string id { get; set; } = string.Empty;
00022:         public string display_name { get; set; } = string.Empty;
00023:         public string ponr_flag { get; set; } = string.Empty;
00024:         public string ponr_trigger { get; set; } = string.Empty;
00025:         public string entry_band_min { get; set; } = string.Empty;
00026:         public string entry_band_max { get; set; } = string.Empty;
00027:         public List<MilitaryBranchEndingEntry> endings { get; set; } = new List<MilitaryBranchEndingEntry>();
00028:     }
00029:
00030:     /// <summary>Root shape of military_faction_branch.json.</summary>
00031:     public sealed class MilitaryBranchDataFile
00032:     {
00033:         public int schema_version { get; set; } = 1;
00034:         public string faction_id { get; set; } = string.Empty;
00035:         public List<MilitaryBranchEntry> branches { get; set; } = new List<MilitaryBranchEntry>();
00036:     }
00037:
00038:     /// <summary>Immutable-after-load catalog of Military branch/ending definitions.</summary>
00039:     public sealed class MilitaryBranchCatalog : IEnumerable<MilitaryBranchEntry>
00040:     {
00041:         private readonly Dictionary<string, MilitaryBranchEntry> _byId =
00042:             new Dictionary<string, MilitaryBranchEntry>(StringComparer.Ordinal);
00043:         private readonly List<MilitaryBranchEntry> _order = new List<MilitaryBranchEntry>();
00044:
00045:         public int Count => _order.Count;
00046:         public MilitaryBranchEntry this[int index] => _order[index];
00047:
00048:         public static MilitaryBranchCatalog Empty() => new MilitaryBranchCatalog();
00049:
00050:         public void Register(MilitaryBranchEntry entry)
00051:         {
00052:             if (entry == null || string.IsNullOrEmpty(entry.id) || _byId.ContainsKey(entry.id)) return;
00053:             _byId[entry.id] = entry;
00054:             _order.Add(entry);
00055:         }
00056:
00057:         public MilitaryBranchEntry? GetById(string id) =>
00058:             string.IsNullOrEmpty(id) ? null : (_byId.TryGetValue(id, out var e) ? e : null);
00059:
00060:         public bool Contains(string id) => GetById(id) != null;
00061:
00062:         public IEnumerator<MilitaryBranchEntry> GetEnumerator() => _order.GetEnumerator();
00063:         IEnumerator IEnumerable.GetEnumerator() => _order.GetEnumerator();
00064:
00065:         public static MilitaryBranchCatalog LoadAndRegister(string dataDir, IFileIO fileIO, IJsonSerializer json)
00066:         {
00067:             if (fileIO == null) throw new ArgumentNullException(nameof(fileIO));
00068:             if (json == null) throw new ArgumentNullException(nameof(json));
00069:
00070:             var catalog = new MilitaryBranchCatalog();
00071:             string path = fileIO.Combine(dataDir, "military_faction_branch.json");
00072:             if (!fileIO.FileExists(path)) return catalog;
00073:
00074:             string text = fileIO.ReadAllText(path);
00075:             var file = json.Deserialize<MilitaryBranchDataFile>(text);
00076:             if (file?.branches == null) return catalog;
00077:
00078:             foreach (var entry in file.branches)
00079:                 catalog.Register(entry);
00080:
00081:             return catalog;
00082:         }
00083:     }
00084: }
```
...
# Appendix — Current Source Detail: `Ashfall.Core.Tests/MilitaryBranchSystemTests.cs`

### `Ashfall.Core.Tests/MilitaryBranchSystemTests.cs` — complete current file

- Size: 275 lines / 11113 bytes.
- SHA-256: `634c183a55f729aff3b7ac6497658671b2b0d8bee2de6a90be22a60a50d30c4d`.
- This is read-only current evidence. It is not proposed replacement code.
...
00010: {
00011:     public class MilitaryBranchSystemTests
00012:     {
00013:         private static MilitaryBranchCatalog LoadCatalog()
00014:         {
...
00020:             return MilitaryBranchCatalog.LoadAndRegister(dir, new FileSystemIO(), new SystemTextJsonSerializer());
00021:         }
00022:
...
00025:
00026:         [Fact]
00027:         public void Catalog_LoadsAllEightBranchesWithThreeEndingsEach()
00028:         {
00029:             var catalog = LoadCatalog();
00030:             Assert.Equal(MilitaryBranchIds.BranchCount, catalog.Count);
00031:             foreach (var branchId in MilitaryBranchIds.AllBranches)
00032:             {
...
00044:             var system = new MilitaryBranchSystem(catalog, flags);
00045:             var moral = MakeMoralChoice(); // starts at Neutral band
00046:
00047:             string committed = system.CommitBranch(MilitaryBranchIds.BranchLoyalSoldier, moral);
00048:
00049:             Assert.Equal(MilitaryBranchIds.BranchLoyalSoldier, committed);
00050:             Assert.Equal(MilitaryBranchIds.BranchLoyalSoldier, system.CommittedBranchId);
00051:         }
00052:
00053:         [Fact]
...
00058:             var system = new MilitaryBranchSystem(catalog, flags);
00059:             var moral = MakeMoralChoice();
00060:
00061:             // MIL-4 Martyr requires a positive-or-better starting band; a fresh
00062:             // MoralChoiceSystem starts at Neutral, which is outside that range.
00063:             Assert.Throws<InvalidOperationException>(() =>
00064:                 system.CommitBranch(MilitaryBranchIds.BranchMartyr, moral));
00065:         }
00066:
00067:         [Fact]
...
00072:             var system = new MilitaryBranchSystem(catalog, flags);
00073:             var moral = MakeMoralChoice();
00074:
00075:             system.CommitBranch(MilitaryBranchIds.BranchLoyalSoldier, moral);
00076:             string second = system.CommitBranch(MilitaryBranchIds.BranchDeserter, moral);
00077:
00078:             Assert.Equal(MilitaryBranchIds.BranchLoyalSoldier, second);
00079:             Assert.Equal(MilitaryBranchIds.BranchLoyalSoldier, system.CommittedBranchId);
00080:         }
00081:
00082:         [Fact]
...
00087:             var system = new MilitaryBranchSystem(catalog, flags);
00088:
...
00097:             var system = new MilitaryBranchSystem(catalog, flags);
00098:             var moral = MakeMoralChoice();
00099:
00100:             system.CommitBranch(MilitaryBranchIds.BranchLoyalSoldier, moral);
00101:             system.AdvanceDay(65);
00102:             system.LockPointOfNoReturn();
00103:
00104:             Assert.True(system.IsPonrLocked);
00105:             Assert.True(flags.IsSet(MilitaryBranchIds.FlagPonrLoyalSoldier));
00106:             Assert.Contains(MilitaryBranchIds.FlagPonrLoyalSoldier, system.State.setFlags);
00107:             Assert.Equal(65, system.State.branch.ponrLockedDay);
00108:         }
00109:
00110:         [Fact]
...
00115:             var system = new MilitaryBranchSystem(catalog, flags);
00116:             var moral = MakeMoralChoice();
00117:
00118:             system.CommitBranch(MilitaryBranchIds.BranchLoyalSoldier, moral);
00119:             system.AdvanceDay(10);
00120:             system.LockPointOfNoReturn();
00121:             system.AdvanceDay(20);
00122:             system.LockPointOfNoReturn(); // should not move the lock day forward
00123:
...
00132:             var system = new MilitaryBranchSystem(catalog, flags);
00133:             var moral = MakeMoralChoice();
00134:
00135:             system.CommitBranch(MilitaryBranchIds.BranchLoyalSoldier, moral);
00136:
00137:             Assert.Throws<InvalidOperationException>(() => system.ResolveEnding(moral));
00138:         }
00139:
00140:         [Fact]
00141:         public void ResolveEnding_NeutralBand_ResolvesSurvivorKingEnding()
00142:         {
00143:             var catalog = LoadCatalog();
00144:             var flags = new InMemoryFlagLedger();
00145:             var system = new MilitaryBranchSystem(catalog, flags);
00146:             var moral = MakeMoralChoice(); // Neutral band, never resolved a quest
00147:
00148:             system.CommitBranch(MilitaryBranchIds.BranchLoyalSoldier, moral);
00149:             system.LockPointOfNoReturn();
00150:             string ending = system.ResolveEnding(moral);
00151:
00152:             Assert.Equal(MilitaryBranchIds.EndingLoyalSoldierC, ending);
...
00161:             var system = new MilitaryBranchSystem(catalog, flags);
00162:             var moral = MakeMoralChoice();
00163:
00164:             system.CommitBranch(MilitaryBranchIds.BranchLoyalSoldier, moral);
00165:             system.LockPointOfNoReturn();
00166:             string first = system.ResolveEnding(moral);
00167:
00168:             // Push morality to a wildly different band via a scripted quest choice.
00169:             var quest = new MoralChoiceQuestDefinition
00170:             {
00171:                 Id = "quest_moral_share_child",
00172:                 Choices = { new MoralChoiceOption { MoralDelta = 150, EmpathyDelta = 1 } }
00173:             };
00174:             moral.Resolve(quest, 0, "loc_test", 1);
00175:
00176:             string second = system.ResolveEnding(moral);
00177:             Assert.Equal(first, second);
00178:         }
00179:
00180:         [Fact]
00181:         public void ShiftFactionAlignment_ClampsToRange()
00182:         {
00183:             var catalog = LoadCatalog();
00184:             var flags = new InMemoryFlagLedger();
00185:             var system = new MilitaryBranchSystem(catalog, flags);
00186:
00187:             system.ShiftFactionAlignment(-500);
00188:             Assert.Equal(MilitaryBranchSystem.MinAlignment, system.MilitaryAlignment);
00189:
00190:             system.ShiftFactionAlignment(1000);
00191:             Assert.Equal(MilitaryBranchSystem.MaxAlignment, system.MilitaryAlignment);
00192:         }
00193:
00194:         [Fact]
00195:         public void IsGameOver_ZeroOrNegativeSurvivors_IsTrue()
00196:         {
00197:             Assert.True(MilitaryBranchSystem.IsGameOver(0));
00198:             Assert.True(MilitaryBranchSystem.IsGameOver(-1));
00199:             Assert.False(MilitaryBranchSystem.IsGameOver(1));
00200:         }
00201:
00202:         [Fact]
00203:         public void SaveRoundTrip_PreservesBranchTimelineAlignmentAndFlags()
00204:         {
00205:             var catalog = LoadCatalog();
00206:             var flagsA = new InMemoryFlagLedger();
00207:             var systemA = new MilitaryBranchSystem(catalog, flagsA);
00208:             var moral = MakeMoralChoice();
00209:
00210:             systemA.AdvanceDay(30);
00211:             systemA.CommitBranch(MilitaryBranchIds.BranchLoyalSoldier, moral);
00212:             systemA.AdvanceDay(65);
00213:             systemA.LockPointOfNoReturn();
00214:             systemA.ShiftFactionAlignment(40);
00215:             systemA.ResolveEnding(moral);
00216:
00217:             var save = MilitaryBranchSaveCodec.Capture(systemA);
00218:             var jsonSerializer = new SystemTextJsonSerializer();
00219:             string jsonText = MilitaryBranchSaveCodec.Encode(save, jsonSerializer);
00220:             Assert.Contains(MilitaryBranchIds.BranchLoyalSoldier, jsonText);
00221:
00222:             var loaded = MilitaryBranchSaveCodec.Decode(jsonText, jsonSerializer);
00223:
00224:             var flagsB = new InMemoryFlagLedger();
00225:             var systemB = new MilitaryBranchSystem(catalog, flagsB);
00226:             MilitaryBranchSaveCodec.Restore(loaded, systemB);
00227:
00228:             Assert.Equal(65, systemB.CurrentDay);
00229:             Assert.Equal(MilitaryBranchIds.BranchLoyalSoldier, systemB.CommittedBranchId);
00230:             Assert.True(systemB.IsPonrLocked);
00231:             Assert.Equal(-40, systemB.MilitaryAlignment); // -80 default + 40 shift
00232:             Assert.Equal(systemA.ResolvedEndingId, systemB.ResolvedEndingId);
00233:             Assert.True(flagsB.IsSet(MilitaryBranchIds.FlagPonrLoyalSoldier));
00234:         }
00235:
00236:         [Fact]
00237:         public void Decode_TamperedChecksum_Throws()
00238:         {
00239:             var catalog = LoadCatalog();
00240:             var flags = new InMemoryFlagLedger();
00241:             var system = new MilitaryBranchSystem(catalog, flags);
00242:             var moral = MakeMoralChoice();
00243:             system.CommitBranch(MilitaryBranchIds.BranchLoyalSoldier, moral);
00244:
00245:             var save = MilitaryBranchSaveCodec.Capture(system);
00246:             var jsonSerializer = new SystemTextJsonSerializer();
00247:             string jsonText = MilitaryBranchSaveCodec.Encode(save, jsonSerializer);
00248:
00249:             string tampered = jsonText.Replace(MilitaryBranchIds.BranchLoyalSoldier, MilitaryBranchIds.BranchDeserter);
00250:
00251:             Assert.Throws<InvalidOperationException>(() => MilitaryBranchSaveCodec.Decode(tampered, jsonSerializer));
00252:         }
00253:
00254:         [Fact]
00255:         public void Decode_NewerSaveVersion_Throws()
00256:         {
00257:             var save = new MilitaryBranchSave { saveVersion = 99 };
00258:             var jsonSerializer = new SystemTextJsonSerializer();
00259:             string jsonText = MilitaryBranchSaveCodec.Encode(save, jsonSerializer);
00260:
00261:             Assert.Throws<InvalidOperationException>(() => MilitaryBranchSaveCodec.Decode(jsonText, jsonSerializer));
00262:         }
00263:
00264:         [Fact]
00265:         public void RestoreState_WrongSystemId_Throws()
00266:         {
00267:             var catalog = LoadCatalog();
00268:             var flags = new InMemoryFlagLedger();
00269:             var system = new MilitaryBranchSystem(catalog, flags);
00270:             var badState = new MilitaryBranchSystemState { systemId = "not_the_right_system" };
00271:
00272:             Assert.Throws<ArgumentException>(() => system.RestoreState(badState));
00273:         }
00274:     }
00275: }
```
...
# Appendix — Current Source Detail: `Assets/Ashfall.Core/Factions/MilitaryBranchState.cs`

### `Assets/Ashfall.Core/Factions/MilitaryBranchState.cs` — complete current file
...
00007: {
00008:     /// <summary>
00009:     /// Self-contained day counter for the branching system's own clock. Runs
00010:     /// independently of the global IClock and of Year of Ash's 180-360 day
00011:     /// timeline, mirroring YearOfAshTimelineState's pattern: a local
00012:     /// currentDay field advanced only by this system's own AdvanceDay(int).
00013:     /// </summary>
00014:     [Serializable]
00015:     public class MilitaryBranchTimelineState
00016:     {
00017:         public int currentDay = 0;
00018:     }
00019:
00020:     /// <summary>
00021:     /// Faction-internal alignment: a value the FACTION itself holds, separate
00022:     /// from the player's own MoralChoiceSystem score and separate from
00023:     /// FactionWarSystem.standing (which is the player's relationship TO a
00024:     /// faction, not the faction's own moral character). Player choices made
00025:     /// while allied/serving nudge this; it uses the same -200..+200 range and
00026:     /// band semantics as MoralChoiceSystem purely for player-facing
00027:     /// consistency, not because the two scores are the same thing.
00028:     /// </summary>
00029:     [Serializable]
00030:     public class FactionAlignmentRecord
00031:     {
00032:         public string factionId = string.Empty;
00033:         public int alignment = -80; // Military starts evil-leaning per design, swayable by the player.
00034:     }
00035:
00036:     /// <summary>
00037:     /// Which base branch (if any) the player has committed to, and whether its
00038:     /// point-of-no-return has fired. A branch is "committed" the first time
00039:     /// SetBranch is called; it never silently changes afterward except
00040:     /// through an explicit, narratively-gated transition (not modeled in this
00041:     /// slice — Military-only branches do not yet cross into Rebel/Independent
00042:     /// territory).
00043:     /// </summary>
00044:     [Serializable]
00045:     public class MilitaryBranchRecord
00046:     {
00047:         public string branchId = string.Empty;
00048:         public bool committed = false;
00049:         public bool ponrLocked = false;
00050:         public int ponrLockedDay = -1;
00051:         public string resolvedEndingId = string.Empty;
00052:     }
00053:
00054:     [Serializable]
00055:     public class MilitaryBranchSystemState
00056:     {
00057:         public string systemId = MilitaryBranchSystem.SystemId;
00058:         public int schemaVersion = 1;
00059:
00060:         public MilitaryBranchTimelineState timeline = new MilitaryBranchTimelineState();
00061:         public MilitaryBranchRecord branch = new MilitaryBranchRecord();
00062:         public FactionAlignmentRecord militaryAlignment = new FactionAlignmentRecord
00063:         {
00064:             factionId = MilitaryBranchIds.FactionId,
00065:             alignment = -80
00066:         };
00067:
00068:         /// <summary>Flags set by branch/PoNR events. Distinct from the runtime IFlagLedger
00069:         /// (which is not persisted) — this list is the save-durable record.</summary>
00070:         public List<string> setFlags = new List<string>();
00071:     }
00072: }
```
