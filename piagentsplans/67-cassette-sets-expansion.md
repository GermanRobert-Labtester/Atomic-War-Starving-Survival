...
- The current catalog has 12 sets with parts, content, item IDs and metadata. CassettePlaybackSystem exposes LoadCatalog, AcquirePart, PlayPart, progress, completion, cache locations and CaptureState/RestoreState.
- VinylMoraleSystem is the current related morale/content owner; audio host/UI wiring must be verified rather than assumed.
- ContentUtilizationScanner and certification paths mention the catalog, but a certified consumer is not automatically a player-reachable command.

**Bounded outcome:** Retire the 4-to-12 target. cassette_sets.json currently has 12 sets, CassetteSetCatalogLoader parses them, and CassettePlaybackSystem owns acquire/play/progress/completion plus save state. The current content certification path references the catalog, but the plan must distinguish Core playback capability from a normal player acquisition/audio route.
...
- Replace the old row target with a 12-set/source/item/reference census.
- Trace current cassette item acquisition, playback command, audio event, VinylMorale effect, journal/codex and save route.
- No new cassette state or audio manifest authority.
...
- The current plan is a 12-set content and reachability audit, not a new audio framework.
...
| --- | --- | --- | --- |
| set/part definitions and item references | CassetteSetCatalogLoader | `Assets/Ashfall.Core/Audio/CassetteSetCatalogLoader.cs` | Static catalog owner. |
| part acquisition, play, progress, completion and persistence | CassettePlaybackSystem | `Assets/Ashfall.Core/Audio/CassettePlaybackSystem.cs` | Sole playback runtime owner. |
| current morale benefit owner | VinylMoraleSystem | `Assets/Ashfall.Core/VinylMoraleSystem.cs` | Separate effect owner. |
| catalog/consumer evidence | Content certification | `src/Main.ContentCertification.cs` | Diagnostic only. |
| normal player route to verify | Cassette/audio UI and host | `src/Main.ContentCertification.cs; src/Host/ContentCertificationHostSession.cs` | No fake route until current call sites are proven. |
| catalog/playback/save contract | Plan67CassetteSetsTests | `Ashfall.Core.Tests/Audio/Plan67CassetteSetsTests.cs` | Focused evidence. |
...
Authored JSON / current persisted owner state
                ...
                ▼
┌──────────────────────────────────────────────────────────────┐
│ Cassette Sets, Playback Progress and Audio-Discovery Reachability
│ Integration route: extend current seams; no parallel authority │
└──────────────────────────────────────────────────────────────┘
│ CassetteSetCatalogLoader
│   set/part definitions and item references
│ CassettePlaybackSystem
│   part acquisition, play, progress, completion and persistence
│ VinylMoraleSystem
│   current morale benefit owner
│ Content certification
│   catalog/consumer evidence
│ Cassette/audio UI and host
│   normal player route to verify
                │
                ▼
...
1. **Preserve current state ownership.** CassetteSetCatalogLoader owns set/part definitions and item references: Static catalog owner.
...
1. load 12 sets
2. validate part/item references
3. acquire a part through current item owner
4. play part through current command
5. apply VinylMorale effect through its owner
6. record journal/progress once
7. capture/restore playback state
...
- Set/part definitions are immutable catalog content.
- CassettePlaybackState owns collected/played/completed progress.
- Vinyl/morale effects remain with VinylMoraleSystem.
- No Plan-67 save section.
...
- A part cannot play without prior acquisition through current inventory/item authority.
- A set completes exactly once.
- Unknown item/part fails without state change.
- Audio asset absence cannot fabricate a successful narrative.
...
- cassette_sets.json is the cassette authority.
- Items, audio registry, Vinyl/morale and journal/codex remain separate.
- No duplicate set/part catalog.
...
- Use CassettePlaybackState/current audio save.
- No Plan-67 save section.
- Old empty playback state is valid.
...
- Part selection and completion order are catalog order or current seeded stream.
- No wall-clock playback completion.
- Same inventory/state/seed produces same progression.
...
- CassettePlaybackSystem emits tape played/set completed facts.
- Audio/UI consumes playback facts.
- Content certification does not grant progression.
...
- Tape prose is restrained, fictional and human.
- No copied scripts or real-world audio claims.
...
| --- | --- | --- | --- |
| F-01 | A catalog set is called playable without an item source. | CassetteSetCatalogLoader | Focused negative test or static source gate; no broad-suite dependency. |
| F-02 | A panel acquires/ plays directly. | CassettePlaybackSystem | Focused negative test or static source gate; no broad-suite dependency. |
| F-03 | A completed set fires morale twice. | VinylMoraleSystem | Focused negative test or static source gate; no broad-suite dependency. |
| F-04 | A missing audio asset produces a false transcript. | Content certification | Focused negative test or static source gate; no broad-suite dependency. |
| F-05 | A new playback save is created. | Cassette/audio UI and host | Focused negative test or static source gate; no broad-suite dependency. |
...
1. `bash scripts/run_test.sh Ashfall.Core.Tests/Audio/Plan67CassetteSetsTests.cs`
2. `bash scripts/run_test.sh Ashfall.Core.Tests/Verdict/Plan82_67VerdictCassetteIntegrationTests.cs`
...
| --- | --- | --- | --- |
| 0 | 12-set/item/source census. | Catalog and item references are explicit. | No production path until the owning implementation package is separately claimed. |
| 1 | Core/host/UI route trace. | Playback capability is not mislabeled as player reachability. | No production path until the owning implementation package is separately claimed. |
| 2 | Save/audio/Vinyl parity audit. | One owner per progression/effect. | No production path until the owning implementation package is separately claimed. |
| 3 | Accessibility/content polish. | Transcript and missing-audio states are truthful. | No production path until the owning implementation package is separately claimed. |
...
| --- | --- | --- |
| Assets/StreamingAssets/Data/cassette_sets.json | READ ONLY; MODIFY only for approved content/reference delta | 12 sets |
| Assets/Ashfall.Core/Audio/CassettePlaybackSystem.cs | READ ONLY | Playback owner |
| src/Main.ContentCertification.cs | READ ONLY | Current certification route |
...
| Risk | Control |
| --- | --- |
| Parallel cassette state. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |
| Catalog-only reachability claim. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |
| Duplicate morale effect. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |
| False transcript on missing audio. | Keep the phase gated; stop and report if the current owner cannot support the proposed seam. |

# 22. Explicit Non-Goals

- No new cassette sets.
- No new audio registry.
- No production/data/test/UI changes here.

# 23. Rollback and Recovery

- Revert planning artifact.
- Future audio route changes use current inventory/UI/save owners.
...
- 12 sets, playback/save/Vinyl boundaries and route uncertainty are explicit.
...
**Authority clause — concern-specific ownership.** Every mutable value named in this plan is governed by the owner shown for that concern in the matrix: set/part definitions and item references → CassetteSetCatalogLoader; part acquisition, play, progress, completion and persistence → CassettePlaybackSystem; current morale benefit owner → VinylMoraleSystem; catalog/consumer evidence → Content certification; normal player route to verify → Cassette/audio UI and host; catalog/playback/save contract → Plan67CassetteSetsTests. A host object, panel, derived snapshot, generated document or test fixture is not an authority merely because it contains the same field name.

**Data clause.** A row in `Assets/StreamingAssets/Data/` is authored content, not reachability. The row must resolve to a loader, validator rule, runtime consumer, player or host command, and test surface. This applies to every catalog listed for Plan 67.

**Save clause.** Capture must be deep, restore must normalize only documented legacy absence, and a checksum must change when authoritative state changes. Plan 67 does not authorize a new save section when an existing owner can carry the fact.
...
**UI clause.** The interface reads the current owner projection, previews a real command and renders named refusals. It must not recompute state owned by CassetteSetCatalogLoader or any other authority, hide uncertainty, or introduce a gameplay-only counter.
...
# Appendix B.02 — Current Code Architecture: `Assets/Ashfall.Core/Audio/CassetteSetCatalogLoader.cs`

### `Assets/Ashfall.Core/Audio/CassetteSetCatalogLoader.cs`

- Evidence status: **CURRENT FILE PRESENT**.
- Size: 72 lines / 2488 bytes.
- SHA-256: `758655ca39d0bf80e5dad0f062d497dd46c031f9617cc6e05a06f2c8b5258834`.
...
# Appendix B.05 — Current Code Architecture: `src/Host/ContentCertificationHostSession.cs`
...
# Appendix B.07 — Current Code Architecture: `src/Host/ContentCertificationHostSession.cs`
...
# Appendix C.08 — Catalog Census: `Assets/StreamingAssets/Data/cassette_sets.json`
...
# Appendix D.10 — Existing Focused Test Inventory: `Ashfall.Core.Tests/Audio/Plan67CassetteSetsTests.cs`

### `Ashfall.Core.Tests/Audio/Plan67CassetteSetsTests.cs`

- Current test declarations: Fact=4, Theory=0, InlineData=0.
- File lines: 79; SHA-256: `90019ef7e2151c080eb07fd547a102369eed7d37858076298c367b830d39c569`.
...
```text
CassetteSets_LoadsExactlyTwelveAuthoredSets
CassetteSets_EverySetHasValidMetadataAndPartCount
CassetteSets_AllFortyEightPartsHaveUniqueIdsAndTranscripts
CassetteSets_AllPartItemIdsExistInItemsJson
```


# Appendix D.11 — Existing Focused Test Inventory: `Ashfall.Core.Tests/Verdict/Plan82_67VerdictCassetteIntegrationTests.cs`
...
# Appendix E.12 — Supporting Code Evidence: `Assets/Ashfall.Core/Audio/CassetteSetCatalogLoader.cs`
...
# Appendix E.13 — Supporting Code Evidence: `Assets/Ashfall.Core/Audio/CassettePlaybackSystem.cs`
...
# Appendix E.14 — Supporting Code Evidence: `Assets/Ashfall.Core/VinylMoraleSystem.cs`
...
# Appendix E.15 — Supporting Code Evidence: `src/Host/ContentCertificationHostSession.cs`
...
# Appendix F.17 — Supporting Data Evidence: `Assets/StreamingAssets/Data/cassette_sets.json`
...
# Appendix G.19 — Supporting Regression Evidence: `Ashfall.Core.Tests/Audio/Plan67CassetteSetsTests.cs`
...
# Appendix G.20 — Supporting Regression Evidence: `Ashfall.Core.Tests/Verdict/Plan82_67VerdictCassetteIntegrationTests.cs`
...
| --- | --- | --- | --- | --- |
| set/part definitions and item references | CassetteSetCatalogLoader | part acquisition, play, progress, completion and persistence | CassettePlaybackSystem | Owner emits/reads a typed fact; no mirror state. |
| set/part definitions and item references | CassetteSetCatalogLoader | current morale benefit owner | VinylMoraleSystem | Owner emits/reads a typed fact; no mirror state. |
| set/part definitions and item references | CassetteSetCatalogLoader | catalog/consumer evidence | Content certification | Owner emits/reads a typed fact; no mirror state. |
| set/part definitions and item references | CassetteSetCatalogLoader | normal player route to verify | Cassette/audio UI and host | Owner emits/reads a typed fact; no mirror state. |
| set/part definitions and item references | CassetteSetCatalogLoader | catalog/playback/save contract | Plan67CassetteSetsTests | Owner emits/reads a typed fact; no mirror state. |
| part acquisition, play, progress, completion and persistence | CassettePlaybackSystem | set/part definitions and item references | CassetteSetCatalogLoader | Owner emits/reads a typed fact; no mirror state. |
| part acquisition, play, progress, completion and persistence | CassettePlaybackSystem | current morale benefit owner | VinylMoraleSystem | Owner emits/reads a typed fact; no mirror state. |
| part acquisition, play, progress, completion and persistence | CassettePlaybackSystem | catalog/consumer evidence | Content certification | Owner emits/reads a typed fact; no mirror state. |
| part acquisition, play, progress, completion and persistence | CassettePlaybackSystem | normal player route to verify | Cassette/audio UI and host | Owner emits/reads a typed fact; no mirror state. |
| part acquisition, play, progress, completion and persistence | CassettePlaybackSystem | catalog/playback/save contract | Plan67CassetteSetsTests | Owner emits/reads a typed fact; no mirror state. |
| current morale benefit owner | VinylMoraleSystem | set/part definitions and item references | CassetteSetCatalogLoader | Owner emits/reads a typed fact; no mirror state. |
| current morale benefit owner | VinylMoraleSystem | part acquisition, play, progress, completion and persistence | CassettePlaybackSystem | Owner emits/reads a typed fact; no mirror state. |
| current morale benefit owner | VinylMoraleSystem | catalog/consumer evidence | Content certification | Owner emits/reads a typed fact; no mirror state. |
| current morale benefit owner | VinylMoraleSystem | normal player route to verify | Cassette/audio UI and host | Owner emits/reads a typed fact; no mirror state. |
| current morale benefit owner | VinylMoraleSystem | catalog/playback/save contract | Plan67CassetteSetsTests | Owner emits/reads a typed fact; no mirror state. |
| catalog/consumer evidence | Content certification | set/part definitions and item references | CassetteSetCatalogLoader | Owner emits/reads a typed fact; no mirror state. |
| catalog/consumer evidence | Content certification | part acquisition, play, progress, completion and persistence | CassettePlaybackSystem | Owner emits/reads a typed fact; no mirror state. |
| catalog/consumer evidence | Content certification | current morale benefit owner | VinylMoraleSystem | Owner emits/reads a typed fact; no mirror state. |
| catalog/consumer evidence | Content certification | normal player route to verify | Cassette/audio UI and host | Owner emits/reads a typed fact; no mirror state. |
| catalog/consumer evidence | Content certification | catalog/playback/save contract | Plan67CassetteSetsTests | Owner emits/reads a typed fact; no mirror state. |
| normal player route to verify | Cassette/audio UI and host | set/part definitions and item references | CassetteSetCatalogLoader | Owner emits/reads a typed fact; no mirror state. |
| normal player route to verify | Cassette/audio UI and host | part acquisition, play, progress, completion and persistence | CassettePlaybackSystem | Owner emits/reads a typed fact; no mirror state. |
| normal player route to verify | Cassette/audio UI and host | current morale benefit owner | VinylMoraleSystem | Owner emits/reads a typed fact; no mirror state. |
| normal player route to verify | Cassette/audio UI and host | catalog/consumer evidence | Content certification | Owner emits/reads a typed fact; no mirror state. |
| normal player route to verify | Cassette/audio UI and host | catalog/playback/save contract | Plan67CassetteSetsTests | Owner emits/reads a typed fact; no mirror state. |
| catalog/playback/save contract | Plan67CassetteSetsTests | set/part definitions and item references | CassetteSetCatalogLoader | Owner emits/reads a typed fact; no mirror state. |
| catalog/playback/save contract | Plan67CassetteSetsTests | part acquisition, play, progress, completion and persistence | CassettePlaybackSystem | Owner emits/reads a typed fact; no mirror state. |
| catalog/playback/save contract | Plan67CassetteSetsTests | current morale benefit owner | VinylMoraleSystem | Owner emits/reads a typed fact; no mirror state. |
| catalog/playback/save contract | Plan67CassetteSetsTests | catalog/consumer evidence | Content certification | Owner emits/reads a typed fact; no mirror state. |
| catalog/playback/save contract | Plan67CassetteSetsTests | normal player route to verify | Cassette/audio UI and host | Owner emits/reads a typed fact; no mirror state. |
...
Current evidence and safe integration boundary for Plan 67: Cassette Sets, Playback Progress and Audio-Discovery Reachability.

- **set/part definitions and item references** remains with `CassetteSetCatalogLoader` at `Assets/Ashfall.Core/Audio/CassetteSetCatalogLoader.cs`. Static catalog owner.
- **part acquisition, play, progress, completion and persistence** remains with `CassettePlaybackSystem` at `Assets/Ashfall.Core/Audio/CassettePlaybackSystem.cs`. Sole playback runtime owner.
- **current morale benefit owner** remains with `VinylMoraleSystem` at `Assets/Ashfall.Core/VinylMoraleSystem.cs`. Separate effect owner.
- **catalog/consumer evidence** remains with `Content certification` at `src/Main.ContentCertification.cs`. Diagnostic only.
- **normal player route to verify** remains with `Cassette/audio UI and host` at `src/Main.ContentCertification.cs; src/Host/ContentCertificationHostSession.cs`. No fake route until current call sites are proven.
- **catalog/playback/save contract** remains with `Plan67CassetteSetsTests` at `Ashfall.Core.Tests/Audio/Plan67CassetteSetsTests.cs`. Focused evidence.
...
| --- | --- | --- | --- | --- | --- |
| S-01 | 67-01 12 sets load | load 12 sets | Set/part definitions are immutable catalog content. | A catalog set is called playable without an item source. | CassetteSetCatalogLoader |
| S-02 | 67-02 part item refs | validate part/item references | CassettePlaybackState owns collected/played/completed progress. | A panel acquires/ plays directly. | CassetteSetCatalogLoader |
| S-03 | 67-03 acquire part | acquire a part through current item owner | Vinyl/morale effects remain with VinylMoraleSystem. | A completed set fires morale twice. | CassetteSetCatalogLoader |
| S-04 | 67-04 play once | play part through current command | No Plan-67 save section. | A missing audio asset produces a false transcript. | CassetteSetCatalogLoader |
| S-05 | 67-05 set completion | apply VinylMorale effect through its owner | Set/part definitions are immutable catalog content. | A new playback save is created. | CassetteSetCatalogLoader |
| S-06 | 67-06 Vinyl effect | record journal/progress once | CassettePlaybackState owns collected/played/completed progress. | A catalog set is called playable without an item source. | CassetteSetCatalogLoader |
| S-07 | 67-07 missing audio | capture/restore playback state | Vinyl/morale effects remain with VinylMoraleSystem. | A panel acquires/ plays directly. | CassetteSetCatalogLoader |
| S-08 | 67-08 save replay | load 12 sets | No Plan-67 save section. | A completed set fires morale twice. | CassetteSetCatalogLoader |
| S-09 | 67-09 UI transcript | validate part/item references | Set/part definitions are immutable catalog content. | A missing audio asset produces a false transcript. | CassetteSetCatalogLoader |
...
| --- | --- | --- | --- | --- |
| T-01 | 67-TC-01 schema/count | data | schema/count; verify the named current owner and its negative boundary without inventing a second authority. | CassetteSetCatalogLoader |
| T-02 | 67-TC-02 reference resolution | unit | reference resolution; verify the named current owner and its negative boundary without inventing a second authority. | CassetteSetCatalogLoader |
| T-03 | 67-TC-03 acquisition gate | persistence | acquisition gate; verify the named current owner and its negative boundary without inventing a second authority. | CassetteSetCatalogLoader |
| T-04 | 67-TC-04 playback transition | determinism | playback transition; verify the named current owner and its negative boundary without inventing a second authority. | CassetteSetCatalogLoader |
| T-05 | 67-TC-05 completion once | host | completion once; verify the named current owner and its negative boundary without inventing a second authority. | CassetteSetCatalogLoader |
| T-06 | 67-TC-06 save round trip | UI/accessibility | save round trip; verify the named current owner and its negative boundary without inventing a second authority. | CassetteSetCatalogLoader |
| T-07 | 67-TC-07 effect parity | cross-system | effect parity; verify the named current owner and its negative boundary without inventing a second authority. | CassetteSetCatalogLoader |
| T-08 | 67-TC-08 UI truth | data | UI truth; verify the named current owner and its negative boundary without inventing a second authority. | CassetteSetCatalogLoader |
...
| --- | --- | --- |
| 11 | `Ashfall.Core.Tests/Verdict/Plan82_67VerdictCassetteIntegrationTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 10 | `src/Host/HostCli.ContentCertification.cs` | current reference count; inspect the caller before treating it as a live route |
| 7 | `src/Main.ContentCertification.cs` | current reference count; inspect the caller before treating it as a live route |
| 5 | `Ashfall.Core.Tests/Collectibles/CollectibleVinylIntegrationTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 5 | `Ashfall.Core.Tests/VinylAcquisitionIntegrationTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 4 | `Ashfall.Core.Tests/Audio/Plan67CassetteSetsTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 4 | `Ashfall.Core.Tests/VinylMoraleSystemTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 4 | `Assets/Ashfall.Core/Audio/CassettePlaybackSystem.cs` | current reference count; inspect the caller before treating it as a live route |
| 4 | `Assets/Ashfall.Core/Culture/CulturalArchiveVaultSystem.cs` | current reference count; inspect the caller before treating it as a live route |
| 4 | `src/Host/VinylMoraleHostSession.cs` | current reference count; inspect the caller before treating it as a live route |
| 3 | `Ashfall.Core.Tests/Content/Plan49ContentCertificationHostIntegrationTests.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 3 | `Assets/Ashfall.Core/VinylMoraleSystem.cs` | current reference count; inspect the caller before treating it as a live route |
| 3 | `src/Main.ShelterSocial.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `Ashfall.Core.Tests/Collectibles/CollectibleSaveMigrationTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `Ashfall.Core.Tests/Radio/RadioStationParityTests.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 2 | `Ashfall.Core.Tests/VinylRadioBridgeTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `Assets/Ashfall.Core/Collectibles/CollectibleEffectDispatcher.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `Assets/Ashfall.Core/Narrative/VinylRecordAcquisitionMap.cs` | current reference count; inspect the caller before treating it as a live route |
| 2 | `src/Host/ContentCertificationHostSession.cs` | current reference count; inspect the caller before treating it as a live route |
| 1 | `Ashfall.Core.Tests/Collectibles/CollectibleDispatcherHardeningTests.cs` | current reference count; inspect the caller before treating it as a live route |
| 1 | `Ashfall.Core.Tests/CulturalArchiveVaultTests.cs` | current reference count; inspect the caller before treating it as a live route |
...
| 1 | `Assets/Ashfall.Core/Audio/CassetteSetCatalogLoader.cs` | current reference count; inspect the caller before treating it as a live route |
| 1 | `src/Host/RadioHostSession.cs` | current reference count; inspect the caller before treating it as a live route |
...
# Appendix — Complete Catalog Audit: `Assets/StreamingAssets/Data/cassette_sets.json`

### `Assets/StreamingAssets/Data/cassette_sets.json` — complete current row audit

- Parse status: **valid JSON**.
- Bytes: 30666; characters: 30598.
- SHA-256: `b5073463b9fd938d6f59ebf87ce14aa7cad571e2fc1771aefdfcc8ff9e00c0b7`.
...
- Row 001 `checkpoint_kilo`: `{"completion_narrative":"narrative_cassette_checkpoint_kilo_complete","hidden_cache_items":["military_mre","ammo_556","field_surgical_kit"],"hidden_cache_location":"checkpoint_kilo_armory","parts":[{"description":"A corporal's voice, stead…`
- Row 002 `hospital_saint_maren`: `{"completion_narrative":"narrative_cassette_saint_maren_complete","hidden_cache_items":["antibiotics","morphine","surgical_suture","iodine_tablets"],"hidden_cache_location":"hospital_pharmacy","parts":[{"description":"A doctor's voice, cli…`
- Row 003 `family_bunker`: `{"completion_narrative":"narrative_cassette_family_bunker_complete","hidden_cache_items":["clean_water_jug","canned_food","childrens_books"],"hidden_cache_location":"family_bunker_backyard_shed","parts":[{"description":"A father's voice, w…`
- Row 004 `resistance_broadcasts`: `{"completion_narrative":"narrative_cassette_free_radio_complete","hidden_cache_items":["antibiotics","aa_batteries","seed_packets","water_filter"],"hidden_cache_location":"old_library_cache","parts":[{"description":"A young woman's voice, …`
- Row 005 `greenhouse_tapes`: `{"hidden_cache_items":["seed_packets","family_heirloom_seeds","item_seed_hardy_tuber"],"hidden_cache_location":"loc_seed_library_annex","parts":[{"description":"A woman's voice over the hum of grow-lights, mid-sentence, as if the recorder …`
- Row 006 `quarantine_tapes`: `{"completion_narrative":"narrative_cassette_quarantine_tapes_complete","hidden_cache_items":["surgical_mask","iodine_tablets","water_purification_tablets","medical_kit"],"hidden_cache_location":"loc_shelter_infirmary","parts":[{"descriptio…`
- Row 007 `field_hospital_7`: `{"completion_narrative":"narrative_cassette_field_hospital_7_complete","hidden_cache_items":["antibiotics","surgical_suture","item_medical_saline_salt","bandage"],"hidden_cache_location":"prewar_medical_cache","parts":[{"description":"A ti…`
- Row 008 `evacuation_train`: `{"hidden_cache_items":["dried_rations","thermal_blanket","handheld_radio","fuel_canister"],"hidden_cache_location":"loc_transit_authority_hq","parts":[{"description":"A man's voice, platform noise behind it: 'Departure board, Tuesday: plat…`
- Row 009 `station_14`: `{"completion_narrative":"narrative_cassette_station_14_complete","hidden_cache_items":["civil_defense_radio","battery","aa_batteries","radio_headset"],"hidden_cache_location":"loc_radio_relay_mast","parts":[{"description":"A woman's voice,…`
- Row 010 `fathers_tapes`: `{"hidden_cache_items":["childrens_books","wool_blanket","cigarette_lighter"],"hidden_cache_location":"suburban_house","parts":[{"description":"A man's voice, home-sounding, kitchen noise behind it: 'Saturday tape, for Mira. The bathroom ta…`
- Row 011 `dam_keeper_log`: `{"completion_narrative":"narrative_cassette_dam_keeper_log_complete","hidden_cache_items":["diesel_fuel","item_battery_electrolyte_concentrate","copper_wire_10m_of_10m"],"hidden_cache_location":"location_substation_omega","parts":[{"descri…`
- Row 012 `teachers_recordings`: `{"hidden_cache_items":["childrens_books","slate_and_chalk","pocket_notebook"],"hidden_cache_location":"loc_school_gymnasium","parts":[{"description":"A woman's voice, classroom-quiet: 'Geography, Unit Four, for the three of you keeping up …`
